...
**Root cause: the decompiler decodes Batman's property flag bits using stock UE3 bit meanings.** Batman has its own `CPF_*` bit layout - this tree already carries a BM2-to-stock remap table at `Core/Src/UnProp.cpp:101-186`, applied by `RemapBmPropertyFlags` on load. The bits genuinely collide: `BM2_CPF_EditConst` and stock `CPF_NonTransactional` are both `0x0000000400000000`, so a Batman `editconst` property decodes as `nontransactional` under stock meanings.

That means the decompile's keywords are recoverable rather than merely wrong: shown keyword -> stock bit -> true Batman meaning. It also means the remap table is load-bearing for the retail script package merge, since `RemapBmPropertyFlags` runs over every `UProperty` in `_Engine.upk`/`_BmGame.upk`. The table currently in the tree is BM2's and has never been verified against licensee 227 - and stock `CPF_DuplicateTransient` (`0x200000`) does not appear in it at all, which suggests BM4's layout is not simply BM2's. Deriving BM4's real table (from `BatmanAK.exe.c`, or from flag values observed in retail script packages) is a prerequisite for trusting anything loaded out of them.
