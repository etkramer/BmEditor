...
namespace CookerSync
{
	...
	partial class CookerSyncApp
	{
		...
		static void ShowUsage()
		{
			...
			WriteLine("                       Can be one of: Xbox360, Xenon, PS3 or PC [Default is Xbox360]");
			...
			WriteLine("  -t, -texformat     : Specifies the texture format extension to the CookedAndroid directory (DXT, PVRTC, ATITC)");
		...
		static int Main(string[] Args)
		{
			...
			if(Args.Length > 0)
			{
				...
				for( int ArgIndex = 1; ArgIndex < Args.Length; ArgIndex++ )
				{
					...
					else if( ( String.Compare( Args[ArgIndex], "-cm", true ) == 0 ) || ( String.Compare( Args[ArgIndex], "-codermode", true ) == 0 ) )
					{
						PlatSettings.CoderMode = true;
					}
					...
					else if( ( String.Compare( Args[ArgIndex], "-s", true ) == 0 ) || ( String.Compare( Args[ArgIndex], "-sleep", true ) == 0 ) )
					{
						...
						{
							WriteLine( "Error: No sleep delay specified (use -h to see usage)." );
							return ERR_CODE;
						}
					}
