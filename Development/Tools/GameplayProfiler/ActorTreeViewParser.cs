...
namespace GameplayProfiler
{
	class ActorTreeViewParser
	{
		...
		class NodePayload
		{
			...
			public NodePayload(TokenObject InObject, string InDescription, string InDescriptionGrouped)
			{
				Object = InObject;
			...
			/** Updates inclusive time for non-TokenObject based nodes, like e.g. level nodes. */
			public void AddToInclusiveTime( float ChildTime )
			{
				// Objects already handle their own inclusive time.
				if( Object == null )
				{
		 ...
		 */
		public static void Parse(TreeView PerActorTreeView, TreeView PerClassTreeView, ProfilerStream ProfilerStream, int FrameNumber, float TimeThreshold)
		{
			...
			ParseActorTreeView( PerActorTreeView, ProfilerStream, FrameNumber );
		 ...
		 */
		private static void ParseActorTreeView(TreeView PerActorTreeView, ProfilerStream ProfilerStream, int FrameNumber )
		{
			...
			foreach( var Actor in Frame.Actors )
			{
                ...
                if (!ActorNameToNodeMap.ContainsKey(Actor.GetActorPathName()))
                {
                    // Add actor node and children (recursive) and keep track of actor name to node mapping.
                    var ActorNode = AddToTree(Actor, LevelNode);
                    ActorNameToNodeMap.Add(Actor.GetActorPathName(), ActorNode);
                }
			}
		 ...
		 * Recursively adds
		 */
		private static TreeNode AddToTree( TokenObject Object, TreeNode RootNode )
		{
			TreeNode Node = new TreeNode("Object");
			Node.Tag = new NodePayload( Object, Object.GetObjectName(), Object.GetClassName() );
			RootNode.Nodes.Add(Node);
			foreach( var Child in Object.Children ) 
			{
				AddToTree( Child, Node );
			}
			return Node;
		}
