        ...
        static void IdleProc()
        {
            ...
            while( true )
            {
                ...
                try
                {
					...
					Log.WriteLine( "UPDS IDLE BUILDS CACHER", Log.LogType.Important, "Tick. (" + NumberOfFilesToCache.ToString() + " files to cache, and " + TaskExecutor.TaskCount.ToString() + " tasks)" );
