...
namespace UnrealProp
{
    public partial class UPMS_Service : ServiceBase
    {
		...
		{
			FolderInfo += Spaces.Substring( Spaces.Length - ( Depth * 4 ) );
			...
			if( Depth == 2 )
			{
				int Length = 48 - ( Path.Length + ( Depth * 4 ) );
				FolderInfo += Spaces.Substring( Spaces.Length - Length );
				...
				FolderInfo += LocalTotalSize.ToString( "0.00" ) + " GB (" + LocalTotalBuilds.ToString() + ")" + Environment.NewLine;
			}
			else
			{
				FolderInfo += Environment.NewLine;
		...
		protected void ExtractBuildRepositories( string BuildRepositoryPath )
		{
			...
			foreach( string Repository in Repositories )
			{
				...
				if( Directory.Exists( CleanRepository ) )
				{
					...
					Log.WriteLine( "UPMS", Log.LogType.Important, " ... added path: " + CleanRepository );
				}
			}
