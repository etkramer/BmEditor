                ...
                try
                {
					...
					foreach( string BuildPath in Owner.BuildRepositoryPaths )
					{
						...
						foreach( string PotentialBuild in FolderList )
						{
							...
							// Check for 'UnrealEngine3' or 'UE3' in the path
							...
							if( DataHelper.PlatformBuild_AddNew( ProjectName, Platform, DefineA, DefineB, Email, PublishFolder, PotentialBuild, TimeStamp ) )
							{
								Log.WriteLine( "UPMS BUILD DISCOVERER", Log.LogType.Important, "found and registered new build in repository: " + PotentialBuild );
