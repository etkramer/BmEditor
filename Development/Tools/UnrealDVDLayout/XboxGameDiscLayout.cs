...
namespace UnrealDVDLayout
{
	...
	public class Disc
	{
		...
		// Current incremental LBA
		private long CurrentLayer1StartLBA = UnrealDVDLayout.XboxSectorsPerLayer0;
		...
		public Disc( XboxGameDiscLayout Owner )
		{
			...
			CurrentLayer1StartLBA = UnrealDVDLayout.XboxSectorsPerLayer0;
		}
		...
		public long AddObject( Object Entry, int Layer )
		{
			...
			if( CurrentLayer0StartLBA > UnrealDVDLayout.XboxSectorsPerLayer0 )
			{
				return ( -1 );
			}
			else if( CurrentLayer1StartLBA > UnrealDVDLayout.XboxSectorsPerLayer0 + UnrealDVDLayout.XboxSectorsPerLayer1 )
			{
				return ( -1 );
			}
		...
		{
			long Layer0Free = UnrealDVDLayout.XboxSectorsPerLayer0 - CurrentLayer0StartLBA;
			long Layer1Free = ( UnrealDVDLayout.XboxSectorsPerLayer0 + UnrealDVDLayout.XboxSectorsPerLayer1 ) - CurrentLayer1StartLBA;
			float MBFree = ( Layer0Free + Layer1Free ) / 512.0f;

			float GBUsed = ( CurrentLayer0StartLBA + ( CurrentLayer1StartLBA - UnrealDVDLayout.XboxSectorsPerLayer0 ) ) / ( 512.0f * 1024.0f );
		...
		public string GetPerfSummary()
		{
			...
			long BytesUsed = ( CurrentLayer0StartLBA + ( CurrentLayer1StartLBA - UnrealDVDLayout.XboxSectorsPerLayer0 ) ) * 2048;
		...
		{
			long Capacity = ( UnrealDVDLayout.XboxSectorsPerLayer0 + UnrealDVDLayout.XboxSectorsPerLayer1 ) * 2048;
	...
	partial class UnrealDVDLayout
	{
		...
		private void CreateXGDFile( string LayoutID )
		{
			...
			if( LayoutID != null )
			{
				...
				Log( UnrealDVDLayout.VerbosityLevel.Informative, "[PERFCOUNTER] " + Options.GameName + LayoutID + "_" + XboxDiscLayout.GetPerfSummary(), Color.Blue );
		...
		public void SaveXGD()
		{
			...
			if( GenericSaveFileDialog.ShowDialog() == DialogResult.OK )
			{
				...
				if( UnrealControls.XmlHandler.WriteXml<XboxGameDiscLayout>( XboxDiscLayout, GenericSaveFileDialog.FileName, "http://www.w3.org/2001/XMLSchema-instance" ) )
				{
		...
		public void SaveXGDISO()
		{
			...
			if( GenericSaveFileDialog.ShowDialog() == DialogResult.OK )
			{
				...
				if( UnrealControls.XmlHandler.WriteXml<XboxGameDiscLayout>( XboxDiscLayout, TempXGDName, "http://www.w3.org/2001/XMLSchema-instance" ) )
				{
		...
		public void LaunchEmulation()
		{
			...
			if( UnrealControls.XmlHandler.WriteXml<XboxGameDiscLayout>( XboxDiscLayout, TempXGDName, "http://www.w3.org/2001/XMLSchema-instance" ) )
			{
				...
				if( XEDKPath != null )
				{
					Log( VerbosityLevel.Informative, "Spawning xbEmulate ...", Color.Blue );
					string CWD = Path.GetDirectoryName( TempXGDName );
					string FileName = Path.GetFileName( TempXGDName );
					SpawnProcess( Path.Combine( XEDKPath, "bin\\win32\\xbEmulate.exe" ), CWD, "/media", FileName );
				}
				else
				{
					Error( "Unable to find XEDK environment variable!" );
				}
			}
		}
