...
namespace UnrealDVDLayout
{
	...
	public class psproject
	{
		...
		{
			long Capacity = UnrealDVDLayout.PS3SectorsPerLayer * 2048;
	...
	public partial class UnrealDVDLayout
	{
		...
		private void CreateGP3File( string LayoutID )
		{
			if( GP3TemplateName.Length > 0 )
			{
				// Create the gp3 file by reading in the template
				PS3DiscLayout = UnrealControls.XmlHandler.ReadXml<psproject>( GP3TemplateName );
				...
				else
				{
					...
					if( LayoutID != null )
					{
						Log( UnrealDVDLayout.VerbosityLevel.Informative, "[PERFCOUNTER] PS3DVDCapacity " + PS3DiscLayout.GetCapacity(), Color.Blue );
					}
				}
			}
		}
		...
		public void SaveGP3()
		{
			...
			else
			{
				...
				if( GenericSaveFileDialog.ShowDialog() == DialogResult.OK )
				{
					...
					if( UnrealControls.XmlHandler.WriteXml<psproject>( PS3DiscLayout, GenericSaveFileDialog.FileName, "" ) )
					{
		...
		public void SaveGP3ISO()
		{
			...
			else
			{
				...
				if( GenericSaveFileDialog.ShowDialog() == DialogResult.OK )
				{
					...
					if( UnrealControls.XmlHandler.WriteXml<psproject>( PS3DiscLayout, TempGP3Name, "" ) )
					{
