...
namespace UnrealFrontend.Pipeline
{
	...
	public class Sync : Pipeline.CommandletStep
	{
		...
		internal static bool UpdateMobileCommandlineFile(Profile InProfile)
		{
			...
			// get which texture format's cooked directories to update (or single emtpy string for non-Android)
			string[] TextureFormats = { "" };
			if (CurPlat.Type == ConsoleInterface.PlatformType.Android)
			{
				TextureFormats = GetTextureFormatsToCookAndSync(InProfile);
			}

			foreach (string Format in TextureFormats)
			{
				// compute where to write it
				string DestinationDirectory = "..\\" + InProfile.SelectedGameName + "\\Cooked" + PlatformName;

				// append texture format if we have one
				if (Format != "")
				{
					DestinationDirectory += "_" + Format;
				}
