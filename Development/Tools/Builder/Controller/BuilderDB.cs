...
namespace Controller
{
    ...
    public partial class BuilderDB
    {
		...
		public Main.BranchDefinition GetBranchDefinition( string Branch )
		{
			...
			string Query = "SELECT Branch, Server, BranchAdmin, DirectXVersion, XDKVersion, PS3SDKVersion, PFXSubjectName FROM BranchConfig WHERE ( Branch = '" + Branch + "' )";
			...
			if( DataReader != null )
			{
				try
				{
					if( DataReader.Read() )
					{
						...
						BranchDef.BranchAdmin = DataReader.GetString( 2 );
						BranchDef.DirectXVersion = DataReader.GetString( 3 );
						BranchDef.XDKVersion = DataReader.GetString( 4 );
						BranchDef.PS3SDKVersion = DataReader.GetString( 5 );
						BranchDef.PFXSubjectName = DataReader.GetString( 6 );
					}
				}
				catch( Exception Ex )
				{
		...
		public void Trigger( string BuildDescription, string User )
		{
			string Query = "SELECT ID FROM Commands WHERE ( Description = '" + BuildDescription + "' )";
			int CommandID = ReadInt( Query );
