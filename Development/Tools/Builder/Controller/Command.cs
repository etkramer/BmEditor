    ...
    public partial class Command
    {
        ...
        private void SCC_Sync()
        {
            try
            {
				...
				int HeadChangelist = SCC.GetLatestChangelist( Builder, "//" + Parent.MachineName + "/" + Builder.BranchDef.Branch + "/..." );
				if( HeadChangelist > 0 )
				{
					Parent.DB.SetIntForBranch( Builder.BranchDef.Branch, "HeadChangelist", HeadChangelist );
				}
		...
		{
			List<string> Executables = Config.GetExecutableNames( Builder.ServerInfo.BranchVersion );
			foreach( string Executable in Executables )
			{
				SCC.CheckoutFileSpec( Builder, Executable );
			}

			List<string> SymbolFiles = Config.GetSymbolFileNames( Builder.ServerInfo.BranchVersion );
			...
			if( CheckoutConfig )
			{
				string CfgFileName = Config.GetCfgName( Builder.ServerInfo.BranchVersion );
				SCC.CheckoutFileSpec( Builder, CfgFileName );
			}

			List<string> FilesToClean = Config.GetFilesToClean( Builder.ServerInfo.BranchVersion );
		...
		private void SCC_CheckoutCat()
		{
			try
			{
				...
				else
				{
					...
					List<string> CatNames = Config.GetCatNames( Builder.ServerInfo.BranchVersion );
		...
		private void BuildUBT()
		{
			try
			{
				...
				// Currently, UBT is always built with .NET Framework version 3.5
        ...
        private void Unreal_Build( bool bAllowXGE )
        {
            try
            {
				...
				string CommandLine = Config.GetUBTCommandLine( Builder.ServerInfo.BranchVersion, Builder.IsPrimaryBuild, bAllowXGE, Builder.AllowPCH, Builder.SpecifyOutput );
        ...
        private void Shader_Build()
        {
            try
            {
                ...
                string Executable = Config.GetComName( Builder.ServerInfo.BranchVersion );
				if( Builder.CommandLine.Length > 0 )
				{
                ...
                string ShaderPlatform = Config.GetShaderPlatform( Builder.ServerInfo.BranchVersion );
        ...
        private void PreHeat()
        {
            try
            {
                ...
                else
                {
                    ...
                    string CookedFolder = Config.GetCookedFolderName( Builder.ServerInfo.BranchVersion );
					...
					string ConfigFolder = Config.GetCookedConfigFolderName( Builder.ServerInfo.BranchVersion );
        ...
        private void PreHeatDLC()
        {
            try
            {
                ...
                else
                {
					...
					string[] CookedFolders = Config.GetDLCCookedFolderNames( Builder.ModName, Builder.ServerInfo.BranchVersion );
        ...
        private void Cook()
        {
            try
            {
				...
				string Executable = Config.GetComName( Builder.ServerInfo.BranchVersion );
				string CommandLine = "CookPackages -Platform=" + Config.GetCookedFolderPlatform( Builder.ServerInfo.BranchVersion );
		...
		private void Publish( COMMANDS Command, string Tagset, string CommandLine, bool AddToReport, string AdditionalOptions )
        {
            try
            {
                ...
                else
                {
					...
					string CookerSyncCommandLine = Builder.LabelInfo.Game + " -p " + Config.GetCookedFolderPlatform( Builder.ServerInfo.BranchVersion );
        ...
        {
			Publish( COMMANDS.PublishLayout, "CompleteBuild", Builder.CommandLine, true, " -notoc" );
        }
		...
		private void GenerateTOC()
		{
			...
			{
				ErrorLevel = COMMANDS.GenerateTOC;
        ...
        private void CreateDVDLayout()
        {
            try
            {
                ...
                else
                {
					...
					string CommandLine = Builder.LabelInfo.Game + " " + Config.GetCookedFolderPlatform( Builder.ServerInfo.BranchVersion );
        ...
        private void PatchScript()
        {
            try
            {
                ...
                else
                {
					...
					string Executable = Config.GetComName( Builder.ServerInfo.BranchVersion );
					string CommandLine = "patchscript " + Parms[0] + " -Platform=" + Config.GetCookedFolderPlatform( Builder.ServerInfo.BranchVersion ) + " " + Builder.GetScriptConfiguration() + " ..\\..\\" + Config.GetCookedFolderName( Builder.ServerInfo.BranchVersion ) + " ..\\..\\" + Config.GetOriginalCookedFolderName() + " " + CommonCommandLine;
		...
		private void TagReferencedAssets( bool bUseCookedData )
		{
			try
			{
				...
				if( bUseCookedData )
				{
					CommandLine = "TagCookedReferencedAssets -Platform=" + Config.GetCookedFolderPlatform( Builder.ServerInfo.BranchVersion ) + " ";
				}
				CommandLine += CommonCommandLine;
		...
		private void MineCookedPackages()
		{
			try
			{
				...
				string Executable = Config.GetComName( Builder.ServerInfo.BranchVersion );
				string CommandLine = "MineCookedPackages " + CommonCommandLine + Builder.GetOptionalCommandletConfig() + Builder.GetDataBaseConnectionInfo() + " ../../" + Config.GetCookedFolderName( Builder.ServerInfo.BranchVersion ) + "/*.xxx";
...
// 				}
		...
		private void ZDPP()
		{
			try
			{
				...
				GameConfig Config = Builder.CreateGameConfig();
				List<string> ExeNames = Config.GetExecutableNames( Builder.ServerInfo.BranchVersion );
        ...
        private void CheckSigned()
        {
            try
            {
				...
				if( Parent.PendingCommandLines.Count > 0 )
				{
					Builder.CommandLine = Parent.PendingCommandLines.Dequeue();
				}
		...
		private void SignCat()
		{
			...
			{
				GameConfig Config = Builder.CreateGameConfig();
				List<string> CatFiles = Config.GetCatNames( Builder.ServerInfo.BranchVersion );
		...
		private void SignBinary()
		{
			...
			{
				GameConfig Config = Builder.CreateGameConfig();
				List<string> ExeFiles = Config.GetExecutableNames( Builder.ServerInfo.BranchVersion );
		...
		private void CleanupSymbolServer()
		{
			try
			{
				...
				DateTime PurgeDate = DateTime.Now - new TimeSpan( 90, 0, 0, 0, 0 );
...
// 							DeleteProcess.WaitForExit();
        ...
        public MODES Execute( COMMANDS CommandID )
        {
            ...
            switch( CommandID )
            {
				...
				case COMMANDS.MS10Build:
					MS_Build( COMMANDS.MS10Build );
					return ( MODES.Monitor );
				...
				case COMMANDS.MSVC10Clean:
					MSVC_Clean( COMMANDS.MSVC10Clean, Builder.MSVC10Application );
					return ( MODES.Monitor );

				case COMMANDS.MSVC10Build:
					MSVC_Build( COMMANDS.MSVC10Build, Builder.MSVC10Application );
					return ( MODES.Monitor );

				case COMMANDS.MSVC10Deploy:
					MS_Deploy( COMMANDS.MSVC10Deploy );
					return ( MODES.Monitor );
