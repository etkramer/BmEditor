...
namespace Controller
{
    public class P4
    {
		 ...
		 */
		private string ConvertFileSpec( Main.PerforceServerInfo ServerInfo, string FileSpec, string NewFileSpecType )
		{
			string NewFileSpec = FileSpec;

			P4RecordSet Records = RunSimpleCommand( ServerInfo, P4ExceptionLevels.ExceptionOnBothErrorsAndWarnings, COMMANDS.SCC_GetDepotFileSpec, "where", FileSpec );
			if( Records != null && Records.Records.Length > 0 )
			{
		...
		public bool SyncBuildFiles( Main.PerforceServerInfo ServerInfo, string SourceControlBranch, string FileSpec )
		{
			...
			if( GetClientRoot( ServerInfo ) )
			{
				...
				string DepotFileSpec = ConvertFileSpec( ServerInfo, ClientFileSpec, "depotFile" );
        ...
        public void CheckConsistency( ScriptParser Builder, string FileSpec )
        {
			...
			string DepotFileSpec = ConvertFileSpec( Builder.ServerInfo, ClientFileSpec, "depotFile" );
		...
		public void CreateNewLabel( ScriptParser Builder )
		{
            try
            {
				...
				string ClientFileSpec = "//" + Parent.MachineName + "/" + Builder.BranchDef.Branch + "/...";
				Files[0] = ConvertFileSpec( Builder.ServerInfo, ClientFileSpec, "depotFile" );
		 ...
		 */
        public List<string> AutoResolveFiles( ScriptParser Builder )
        {
		 ...
		 * Get a list of files that require a resolve, or are locked
		 */
		public void BackOutHeadRevision( ScriptParser Builder, string DepotPath )
        {
			bool bFilesLocked = false;
				...
				if( Pending != null )
				{
					...
					if( OpenedFiles != null && OpenedFiles.Records.Length > 0 )
					{
						...
						foreach( P4Record Record in OpenedFiles.Records )
						{
							...
							string LocalFileSpec = ConvertFileSpec( Builder.ServerInfo, ClientFileSpec, "path" );
        ...
        public void GetChangesSinceLastBuild( ScriptParser Builder )
        {
			...
			if( Builder.LastGoodBuild == 0 || Parent.JobID != 0 )
			{
				return;
			}
		 ...
		 */
		public List<string> GetCheckedOutFiles( ScriptParser Builder )
		{
			List<string> CheckedOutFiles = new List<string>();
			int TrimIndex = Environment.CurrentDirectory.Length + 1;

			P4RecordSet Results = RunSimpleCommand( Builder, P4ExceptionLevels.ExceptionOnBothErrorsAndWarnings, COMMANDS.SCC_SignUnsubmitted, "describe", Pending.Number.ToString() );
			if( Results != null && Results.Records.Length > 0 )
			{
				foreach( string DepotFile in Results.Records[0].ArrayFields["depotFile"] )
				{
					string LocalFileSpec = ConvertFileSpec( Builder.ServerInfo, DepotFile, "path" );
