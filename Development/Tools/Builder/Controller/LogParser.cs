...
namespace Controller
{
    class LogParser
    {
        ...
        public LogParser( Main InParent, ScriptParser InBuilder )
        {
            Parent = InParent;
            Builder = InBuilder;
		...
		public string Parse( bool ReportEntireLog, bool CheckCookingSuccess, bool CheckCookerSyncSuccess, ref COMMANDS ErrorLevel )
        {
            ...
            while( Line != null )
            {
				...
				else if( Builder.ErrorMode == ScriptParser.ErrorModeType.IgnoreErrors && Line.IndexOf( "DBGHELP: " ) >= 0 )
				{
					// We've crashed - check for errors and fail properly
					Builder.ErrorMode = ScriptParser.ErrorModeType.SuppressErrors;
				...
				// Check for errors
						 ...
						 || Line.IndexOf( "System.NullReferenceException: Invalid pointer" ) >= 0
					...
					// || Line.IndexOf( "System.Runtime.InteropServices.COMException" ) >= 0
				...
				else if( Line.Contains( "recommended you revert any checked out Engine packages" ) )
				{
					// Disable the checkin of engine packages
					Builder.bCommandletsSuccessful = false;
				}
