...
namespace Controller
{
	...
	public partial class Main : Form
	{
		...
		public string LabelRoot = Properties.Settings.Default.LabelRoot;
		...
		public class PerforceServerInfo
		{
			...
			{
				BranchVersion = DB.GetIntForBranch( SourceControlBranch, "Version" );
		...
		// The installed version of MSVC10
		public string MSVC10Version { get; set; }
		...
		private List<COMMANDS> PendingCommands = new List<COMMANDS>();
		public Queue<string> PendingCommandLines = new Queue<string>();
		...
		private void GetMSVCVersion()
		{
			try
			{
				...
				string MSVC10Exe = Environment.GetEnvironmentVariable( "VS100COMNTOOLS" );
				if( MSVC10Exe != null && MSVC10Exe.Length > 0 )
				{
					FileVersionInfo VI = FileVersionInfo.GetVersionInfo( MSVC10Exe + "/../IDE/Devenv.exe" );
					MSVC10Version = VI.FileDescription + " version " + VI.ProductVersion;
					Log( "Compiling with: " + MSVC10Version, Color.Blue );
				}
				else
				{
					Log( "Could not find VS100COMNTOOLS environment variable", Color.Blue );
				}
			}
			catch
			{
		...
		public void SetMSVC10EnvVars()
		{
			...
			Environment.SetEnvironmentVariable( "INCLUDE", ProgramFilesFolder + "\\Microsoft Visual Studio 10.0\\VC\\ATLMFC\\INCLUDE;" + ProgramFilesFolder + "\\Microsoft Visual Studio 10.0\\VC\\INCLUDE;C:\\Program Files\\Microsoft SDKs\\Windows\\v6.0A\\include;" );
			Environment.SetEnvironmentVariable( "LIB", ProgramFilesFolder + "\\Microsoft Visual Studio 10.0\\VC\\ATLMFC\\LIB;" + ProgramFilesFolder + "\\Microsoft Visual Studio 10.0\\VC\\LIB;C:\\Program Files\\Microsoft SDKs\\Windows\\v6.0A\\lib;" );
		...
		private void SetIPhoneEnvVar()
		{
			...
			if( Builder.BranchDef.Branch == "UnrealEngine3-ProjectSword" || Builder.BranchDef.Branch == "UnrealEngine3-InfinityBlade1" )
			{
		...
		private void TrackFolderSize()
		{
			try
			{
				Log( "Tracking folder size of '" + Builder.CommandLine + "'", Color.Black );
		...
		private void ClearCommands()
		{
			...
			SetNextCommand( COMMANDS.SCC_Revert );
		}

		private void AddCommandToEnd( COMMANDS NextCommand )
		{
			PendingCommands.Add( NextCommand );
		}

		private void AddCommandToEnd( COMMANDS NextCommand, string CommandLine )
		{
			PendingCommands.Add( NextCommand );
			PendingCommandLines.Enqueue( CommandLine );
		}

		private void SetNextCommand( COMMANDS NextCommand )
		{
		...
		private MODES HandleError()
		{
			...
			if( Status.Contains( "The specified timestamp server could not be reached" ) )
			{
				...
				Builder.LineCount++;
				SetNextCommand( CurrentCommand.LastExecutedCommand );
				return ( MODES.Init );
			}
				...
				|| Status.Contains( ": unexpected error with pch, try rebuilding the pch" ) )
			{
				if( CompileRetryCount < 2 )
				{
					...
					Builder.LineCount++;
					SetNextCommand( COMMANDS.UnrealBuild );
					SetNextCommand( COMMANDS.BuildUBT );
					SetNextCommand( COMMANDS.Clean );
		...
		private bool RunBuild()
		{
			...
			case MODES.Init:
				...
				if( NextCommand == COMMANDS.MSVC9Build || NextCommand == COMMANDS.MSVC9Clean || NextCommand == COMMANDS.MSVC9Deploy || NextCommand == COMMANDS.MS9Build )
				{
					SetMSVC9EnvVars();
				}
				else if( NextCommand == COMMANDS.MSVC10Build || NextCommand == COMMANDS.MSVC10Clean || NextCommand == COMMANDS.MSVC10Deploy || NextCommand == COMMANDS.MS10Build )
				{
					SetMSVC10EnvVars();
				}
				...
				case COMMANDS.RestartSteamServer:
					...
					SetNextCommand( COMMANDS.StartSteamServer );
					break;
				...
				case COMMANDS.SteamDRM:
					...
					SetNextCommand( COMMANDS.FixupSteamDRM );
					break;
				...
				case COMMANDS.MSVC9Full:
					...
					SetNextCommand( COMMANDS.MSVC9Build );
					break;

				case COMMANDS.MSVC10Full:
					CurrentCommand = new Command( this, SCC, Builder );
					Mode = CurrentCommand.Execute( COMMANDS.MSVC10Clean );

					SetNextCommand( COMMANDS.MSVC10Build );
					break;
				...
				case COMMANDS.GCCFull:
					...
					SetNextCommand( COMMANDS.GCCBuild );
					break;
				...
				case COMMANDS.ShaderFull:
					...
					SetNextCommand( COMMANDS.ShaderBuild );
					break;
				...
				case COMMANDS.SCC_SignUnsubmitted:
					...
					foreach( string CheckedOutFile in CheckedOutFiles )
					{
						...
						{
							AddCommandToEnd( COMMANDS.CheckSigned, CheckedOutFile );
						}
					}

					AddCommandToEnd( COMMANDS.SCC_Submit );
					break;
				...
				case COMMANDS.UpdateSourceServer:
					if( Builder.LabelInfo.Platform.ToLower() != "ps3" && Builder.LabelInfo.Platform.ToLower() != "sonyps3" )
					{
						...
						AddCommandToEnd( COMMANDS.UpdateSymbolServer );
					}
					else
					{
				...
				case COMMANDS.AddDllToSymbolServer:
					if( Builder.LabelInfo.Platform.ToLower() != "ps3" && Builder.LabelInfo.Platform.ToLower() != "sonyps3" )
					{
						...
						AddCommandToEnd( COMMANDS.UpdateSymbolServerTick );
					}
					else
					{
				...
				case COMMANDS.Conform:
					...
					{
						AddCommandToEnd( COMMANDS.Conform );
					}
