...
public partial class RiftTextureSize : BasePage
{
		...
		using( SqlCommand Command = new SqlCommand( Query, Connection ) )
		{
			...
			if( Reader.HasRows )
			{
				...
				foreach( DataRow Row in Table.Rows )
				{
					...
					if( !PackageTotals.Keys.Contains( PackageName ) )
					{
		...
		using( SqlCommand Command = new SqlCommand( Query, Connection ) )
		{
			...
			if( Reader.HasRows )
			{
				...
				foreach( DataRow Row in Table.Rows )
				{
					...
					if( PackageTotals.Keys.Contains( XAxis ) )
					{
	...
	private void FillSeries( string Item, Dictionary<string, float> PackageTotals )
	{
		foreach( string Package in PackageTotals.Keys )
		{
			...
			RiftTextureSizeChart.Series[Item].Points.Add( Point );
		}
	}

	protected void Page_Load( object sender, EventArgs e )
	{
		using( SqlConnection Connection = new SqlConnection( ConfigurationManager.ConnectionStrings["DevDbConnectionString"].ConnectionString ) )
		{
			Connection.Open();
		...
		RiftTextureSizeChart.ChartAreas["ChartArea1"].Position.X = 0.0f;
		RiftTextureSizeChart.ChartAreas["ChartArea1"].Position.Y = 0.0f;
		RiftTextureSizeChart.ChartAreas["ChartArea1"].Position.Width = 100.0f;
		RiftTextureSizeChart.ChartAreas["ChartArea1"].Position.Height = 100.0f;
	}

	private Dictionary<string, float> GetValues( string SeriesName )
	{
		Dictionary<string, float> Values = new Dictionary<string, float>();
		foreach( DataPoint Point in RiftTextureSizeChart.Series[SeriesName].Points )
		{
	...
	protected void Button_SaveAsCSV_Click( object sender, EventArgs e )
	{
		...
		Response.AddHeader( "Content-Disposition", "attachment; filename=\"RiftTextureSize.csv\"" );
		Response.Flush();
		Response.End();
	}
}
