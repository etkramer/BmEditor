...
namespace ConsoleInterface
{
	...
	List<TOCInfo^>^ Platform::GenerateTOC(String^ TagSetName, TOCSettings^ BuildSettings, String^ Language)
	{
		...
		{
			if (BuildSettings->CoderMode == false)
			{
				TOCFilename = String::Format(L"{0}TOC.txt", mType.ToString());
			}
			else
			{
				TOCFilename = String::Format(L"{0}TOC_CC.txt", mType.ToString());
			}
		}
		else
		{
			...
			{
				TOCFilename = String::Format(L"{0}TOC_CC_{1}.txt", mType.ToString(), Language);
			}
		}
	...
	List<TOCInfo^>^ Platform::TocBuild(String^ TOCName, TOCSettings^ BuildSettings, array<Tag^>^ SyncTags, String^ Language)
	{
		...
		if(mSharedSettings != nullptr && mSharedSettings->FileGroups != nullptr)
		{
			...
			{
				ProcessFileGroups(TOC, mSharedSettings->FileGroups, BuildSettings, SyncTags, NotPlatforms, NotLanguages, NotGames, Language, TexExtension);
			}
		}
		...
		if(BuildSettings->GameOptions != nullptr && BuildSettings->GameOptions->FileGroups != nullptr)
		{
			for each (String^ TexExtension in TextureExtensions)
			{
				ProcessFileGroups(TOC, BuildSettings->GameOptions->FileGroups, BuildSettings, SyncTags, NotPlatforms, NotLanguages, NotGames, Language, TexExtension);
			}
		}
	...
	bool Platform::IsMatchingPlatform( FileGroup^ Group )
	{
		...
		String^ GroupPlatform = Group->Platform->ToLower();
		String^ CurrentPlatform = mType.ToString()->ToLower();
		...
		if( GroupPlatform->Equals( L"pc*" ) )
		{
			...
			if( mType == PlatformType::PC || mType == PlatformType::PCConsole || mType == PlatformType::PCServer )
			{
				return( true );
			}
		}
		...
		if( GroupPlatform->Equals( L"console" ) )
		{
			...
			if( mType != PlatformType::PC && mType != PlatformType::PCServer )
			{
				return( true );
			}
		}
		...
		if( GroupPlatform->Equals( L"mobile" ) )
		{
			...
			if( ((DWORD)mType & (DWORD)PlatformType::Mobile) != 0 )
			{
				return( true );
			}
		}

		// Check for the alternate 360 name
		if( mType == PlatformType::Xbox360 && GroupPlatform->Equals( L"xenon" ) )
		{
			return( true );
		}

		// Check for exact platform match
		if( GroupPlatform->Equals( CurrentPlatform ) )
		{
			return( true );
		}
	...
	void Platform::ProcessFileGroups(List<TOCInfo^> ^TOC, array<FileGroup^> ^FileGroups, TOCSettings ^BuildSettings, array<Tag^> ^SyncTags, List<String^> ^NotPlatforms, List<String^> ^NotLanguages, List<String^> ^NotGames, String ^Language, String ^TexExtension)
	{
			...
			if (IsMatchingPlatform( Group ) )
			{
				...
				if(bTagsMatch)
				{
					for each (FileSet ^CurSet in Group->Files)
					{
						...
						TocIncludeFiles(TOC, BuildSettings, Path, Wildcard, CurSet, Group, NotPlatforms, NotLanguages, NotGames, Language, TexExtension);
					}
				}
			}
		}
	...
	void Platform::TocAddFile(List<TOCInfo^> ^TOC, TOCSettings ^BuildSettings, FileInfo ^SrcFile, String^ CurrentDir, FileGroup^ Group)
	{
		...
		for each( TOCInfo^ TOCEntry in TOC )
		{
			...
			{
				// due to multiple CookedAndroid directories, this is an annoying, but not needed, warning on Android
				if (mType != PlatformType::Android)
				{
	...
	void Platform::TocIncludeFiles(List<TOCInfo^> ^TOC, TOCSettings ^BuildSettings, String ^SourceFolder, String ^SourcePattern, FileSet ^CurSet, FileGroup ^Group, List<String^> ^NotPlatforms, List<String^> ^NotLanguages, List<String^> ^NotGames, String ^Language, String ^TexExtension)
	{
		if (SourceFolder->Contains("%CODERMODE%"))
		{
			if (BuildSettings->CoderMode == TRUE)
			{
				SourceFolder = SourceFolder->Replace("%CODERMODE%", L"_Coder");
			}
			else
			{
				SourceFolder = SourceFolder->Replace("%CODERMODE%", L"");
			}
		}
		...
		FixString(SourceFolder, BuildSettings, Language, TexExtension);
		FixString(SourcePattern, BuildSettings, Language, TexExtension);
				...
				for each(FileFilter ^Filter in CurSet->FileFilters)
				{
					...
					for (INT FileDirPass = 0; FileDirPass < 2; FileDirPass++)
					{
						...
						else
						{
							...
							case 7:		// "%TEXEXTENSION%"
								if (TexExtension && TexExtension != "")
								{
									Fmt = Filter->Name->Replace( L"%TEXEXTENSION%", L"_{0}" );
									ExpandedFilters->Add( String::Format( Fmt, TexExtension ) );
								}
								else
								{
	...
	void Platform::FixString(String ^%Str, TOCSettings ^BuildSettings, String ^Language, String ^TexExtension)
	{
		if(Str != nullptr)
		{
			...
			if (TexExtension && TexExtension != "")
			{
				String ^Fmt = Str->Replace(L"%TEXEXTENSION%", L"_{0}");
				Str = String::Format( Fmt, TexExtension );
			}
			else
			{
				Str = Str->Replace(L"%TEXEXTENSION%", L"");
			}
		...
		try
		{
			...
			if(BuildSettings->bRebootBeforeCopy)
			{
				...
				do
				{
					...
					for(int i = 0; i < Targets->Count; ++i)
					{
						if(Targets[i]->State == TargetState::Rebooting || Targets[i]->State == TargetState::NotRunning)
						{
	...
	{
		String ^TMName = gcnew String( mConsoleSupport->GetTargetManagerNameForConsole( Handle ) );
		int TargetPlatformType = ( int )mConsoleSupport->GetPlatformType( Handle );
	...
	int Platform::EnumerateAvailableTargets()
	{
		...
		try
		{
			...
			NumTargets = mConsoleSupport->EnumerateAvailableTargets();
