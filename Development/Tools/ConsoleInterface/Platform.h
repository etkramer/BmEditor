...
namespace ConsoleInterface
{
	...
	public enum class PlatformType
	{
		...
		NGP			=	0x00000100,
		Android		=	0x00000200,
		All = PC | PS3 | Xbox360 | PCServer | IPhone | PCConsole | Android | NGP,
		Mobile = IPhone | Android | NGP,
	};
	...
	private:
		...
		void ProcessFileGroups(List<TOCInfo^>^ TOC, array<FileGroup^>^ FileGroups, TOCSettings^ BuildSettings, array<Tag^>^ SyncTags, List<String^>^ NotPlatforms, List<String^>^ NotLanguages, List<String^>^ NotGames, String^ Language, String ^TexExtension);
		void TocAddFile(List<TOCInfo^>^ TOC, TOCSettings^ BuildSettings, FileInfo^ SrcFile, String^ RootFolder, FileGroup^ Group);
		void TocIncludeFiles(List<TOCInfo^>^ TOC, TOCSettings^ BuildSettings, String^ SourceFolder, String^ SourcePattern, FileSet^ CurSet, FileGroup^ Group, List<String^>^ NotPlatforms, List<String^>^ NotLanguages, List<String^>^ NotGames, String^ Language, String ^TexExtension);
		bool IsMatchingPlatform( FileGroup^ Group );
		void FixString(String^% Str, TOCSettings^ BuildSettings, String^ Language, String ^TexExtension);
