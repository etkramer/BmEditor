...
namespace Lightmass
{
	...
	void FLandscapeStaticLightingMesh::GetStaticLightingVertex(INT VertexIndex, FStaticLightingVertex& OutVertex) const
	{
		...
		//WorldPos = WorldTangentX = WorldTangentY = WorldTangentZ = FVector4();
		...
		//FVector2D((FLOAT)X / (FLOAT)(ComponentSizeQuads), (FLOAT)Y / (FLOAT)(ComponentSizeQuads));

		OutVertex.TextureCoordinates[LightmapUVIndex].X = (FLOAT)(X * StaticLightingResolution + 0.5f) * UVFactor; //SizeX;
		OutVertex.TextureCoordinates[LightmapUVIndex].Y = (FLOAT)(Y * StaticLightingResolution + 0.5f) * UVFactor; //SizeY;
	}
