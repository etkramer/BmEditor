...
namespace UnSetup
{
	public partial class Utils
	{
		...
		public class GameManifestOptions
		{
            ...
            // Whether to add the previewer shortcut
            [XmlElementAttribute]
            public bool AddPreviewShortcut { get; set; }

            // Whether to add the iPhonePackager shortcut
            [XmlElementAttribute]
            public bool IsIPhoneBuild { get; set; }
		...
		public string InstallerRoot = "UDK";
		...
		public Utils( string Language, string ParentLanguage )
		{
			...
			ReadDefaultManifestOptions( Environment.CurrentDirectory + "\\Binaries\\" );
			...
			ReadDefaultManifestOptions( Application.StartupPath );
		...
		public void ReadDefaultManifestOptions( string InstallFolder )
		{
			string ManifestsName = "UnSetup.Manifests.xml";
			Manifest = ReadXml<ManifestOptions>( Path.Combine( InstallFolder, ManifestsName ) );
		}

		public void ReadManifestOptions()
		{
#if DEBUG
			...
			Console.WriteLine( "Loaded: " + ManifestsName + " (" + Manifest.MainFilesToExclude.Length.ToString() + " filespecs to exclude)" );
		}
		...
		{
			InstallInfoData = ReadXml<InstallInfo>( Path.Combine( InstallFolder, "InstallInfo.xml" ) );
		}
		...
		private void GetEULAs()
		{
			...
			if( DirInfo.Exists )
			{
				foreach( FileInfo EULAFile in DirInfo.GetFiles( "EULA." + Manifest.RootName + ".???.rtf" ) )
				{
					string DestName = Path.Combine( "Binaries\\InstallData\\", EULAFile.Name.Replace( "." + Manifest.RootName, "" ) );
		...
		public void CreateManifest()
		{
			...
			// Write out the current manifest options file to be used from now on
			Manifest.bManifestCreated = true;
			WriteXml<ManifestOptions>( "Binaries\\UnSetup.Manifests.xml", Manifest );
			Console.WriteLine( " ... manifest options saved." );
		...
		public int GameCreateManifest()
		{
			...
			DirectoryInfo DirInfo = new DirectoryInfo( "UDKGame/CookedPC" );
			if( !DirInfo.Exists )
			{
				Console.WriteLine( " ... failed. No data has been cooked." );
				return ( 3 );
			}
			...
			foreach( string EULAFile in EULAFiles )
			{
				...
				{
					Console.WriteLine( " ... failed. EULA file is corrupt." );
					return ( 3 );
				}
			}
#endif
		...
		// UDKNET35
		private List<byte> UDKStartDotNetSignature = new List<byte> { 0x55, 0x44, 0x4B, 0x4E, 0x45, 0x54, 0x33, 0x35 };
		// UDKRedis
		...
		public int PackageFiles()
		{
			...
			if( Packaged != null )
			{
				...
				PackageFile( Packaged, null, "Binaries\\Win32\\UnSetupNativeWrapper.exe", 0 );
				PackageFile( Packaged, UDKStartUnSetupSignature, "Binaries\\UnSetup.exe", 1 );
				PackageFile( Packaged, UDKStartDotNetSignature, "Binaries\\InstallData\\dotnetfx35setup.exe", 2 );
		...
		public void PackageRedist( bool IsRedistGamePackage )
		{
			...
			{
				// Construct a package from all the required elements
				PackageFile( Packaged, null, "Binaries\\Win32\\UnSetupNativeRedistWrapper.exe", 0 );
				PackageFile( Packaged, UDKStartUnSetupSignature, "Binaries\\UnSetup.exe", 1 );
				PackageFile( Packaged, UDKStartDotNetSignature, "Binaries\\InstallData\\dotnetfx35setup.exe", 2 );
		...
		public int ProcessBuildCommandLine( string[] Arguments )
		{
			...
			{
				if( Argument.ToLower().StartsWith( "-module=" ) )
				{
		...
		{
			MessageBox.Show( "UnSetup - Copyright 2009 Epic Games, Inc. All Rights reserved" + Environment.NewLine +
							"" + Environment.NewLine +
