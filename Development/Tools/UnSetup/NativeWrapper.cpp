...
static const wchar_t* UDKNET35 = L"UDKNET35";
...
bool CheckDotNet35( void )
{
	...
	if( Error == ERROR_SUCCESS )
	{
		...
		wcscat_s( Result, MAX_PATH, L"v3.5\\csc.exe" );
		DWORD VersionSize = GetFileVersionInfoSize( Result, &Handle );
		if( VersionSize > 0 )
		{
			Block = malloc( VersionSize );
			if( Block )
			{
				if( GetFileVersionInfo( Result, 0, VersionSize, Block ) != 0 )
				{
					if( VerQueryValue( Block, L"\\", &VersionData, &VersionDataSize ) != 0 )
					{
						VS_FIXEDFILEINFO* FileInfo = ( VS_FIXEDFILEINFO* )VersionData;

						if( FileInfo->dwProductVersionMS == 0x00030005 )
						{
							if( ( FileInfo->dwProductVersionLS & 0xffff0000 ) == 0x78090000 )
							{
				...
				{
					OutputDebugStringW( L"GetFileVersionInfoe failed - presuming no .NET\r\n" );
				}
			}
			else
			{
				OutputDebugStringW( L"malloc of file version info size failed - presuming no .NET\r\n" );
			}
		}
		else
		{
			OutputDebugStringW( L"GetFileVersionInfoSize failed - presuming no .NET\r\n" );
		}
	}
	else
	{
		OutputDebugStringW( L"Failed to find .NET registry key (SHRegGetValue) - presuming no .NET\r\n" );
	}
...
BOOL ExtractUnSetup( wchar_t* ModuleName, wchar_t* WorkFolder )
{
	...
	bSuccess = ExtractSection( ModuleName, DestinationPath, UDKNativ, UDKNET35, NULL );
...
BOOL ExtractDotNetFx( wchar_t* ModuleName, wchar_t* WorkFolder )
{
	...
	// Always create the required folders
	swprintf_s( DestinationPath, MAX_PATH, L"%s\\dotnetfx35setup.exe", WorkFolder );

	bSuccess = ExtractSection( ModuleName, DestinationPath, UDKNET35, UDKRedis, UDKMagic );
...
/**
 * Check to make sure the OS version is at least Windows XP SP2
 */
...
INT WINAPI WinMain( HINSTANCE HInstance, HINSTANCE, char* Argument, INT )
{
...
#if _DEBUG
	wcscpy_s( ModuleName, MAX_PATH, L"C:\\Users\\john.scott\\Desktop\\UT_PC_[2009-10-09_02.02]_[UDK=1]_Install\\UnrealEngine3\\UDKInstall-2009-10.exe" );
#endif
	...
	if( !bRunAgainAfterReboot && !bProgressOnly )
	{
		...
		if( !OSVersionValid() )
		{
			MessageBox( NULL, L"You must have at least Windows XP Service Pack 2 installed. Installer will now exit.", L"Installation Failed", MB_OK | MB_ICONERROR );
			return( 2 );	
		}
	...
	OutputDebugStringW( L"Checking for .NET35\r\n" );
	...
	if( !bSkipNETCheck )
	{
		...
		if( ( !CheckDotNet35() && !CheckDotNet4() ) || bForceNETInstall )
		{
			if( ExtractDotNetFx( ModuleName, WorkFolder ) )
			{
				...
				// Launch dotnetfx35setup.exe from the temp folder
				swprintf_s( CommandLine, MAX_COMMAND_LINE, L"%s\\dotnetfx35setup.exe", WorkFolder );
				DWORD ExitCode = Launch( CommandLine, true );

				if( ExitCode == 0 )
				{
					// Restart the machine
					if( MessageBox( NULL, L"Reboot now?", L"Reboot Required", MB_OKCANCEL | MB_ICONQUESTION ) == IDOK )
					{
				...
				// A reboot was canceled, so remove the automatic restart
				RemoveRestart();
			}
			else
			{
	...
	if( ExtractUnSetup( ModuleName, WorkFolder ) )
	{
		if( swprintf_s( CommandLine, MAX_COMMAND_LINE, L"%s\\Binaries\\UnSetup.exe \"/module=%s\" /SplashHandle=%d /InstallGuid=%s", WorkFolder, ModuleName, ( DWORD )Splash, ( wchar_t* )GuidString ) > 0 )
		{
			...
			Launch( CommandLine, false );

			// Wait for the splash screen to be closed
			while( Ticking )
			{
				PumpMessageLoop();
				Sleep( 100 );
