...
void mdct_init( mdct_lookup* lookup, int n )
{
	...
	float* T = ( float* )_ogg_malloc( sizeof( *T ) * ( n + n / 4 ) );

	int i;
	int n2 = n >> 1;
	int log2n = lookup->log2n = rint( logf( ( float ) n ) / logf( 2.0f ) );
	lookup->n = n;
	lookup->trig = T;
	...
	for( i = 0; i < n / 4; i++ )
	{
		T[i * 2] = cosf( ( M_PI / n ) * ( 4 * i ) );
		T[i * 2 + 1] = -sinf( ( M_PI / n ) * ( 4 * i ) );
		T[n2 + i * 2] = cosf( ( M_PI / ( 2 * n ) ) * ( 2 * i + 1 ) );
		T[n2 + i * 2 + 1] = sinf( ( M_PI / ( 2 * n ) ) * ( 2 * i + 1 ) );
	}

	for( i = 0; i < n / 8; i++ )
	{
		T[n + i * 2] = cosf( ( M_PI / n ) * ( 4 * i + 2 ) ) * 0.5f;
		T[n + i * 2 + 1] = -sinf( ( M_PI / n ) * ( 4 * i + 2 ) ) * 0.5f;
	}
...
#ifdef __SSE__												/* SSE Optimize */
	{
		...
		for( i = 0; i < n4; i += 8 )
		{
			...
			XMM2 = _mm_mul_ps( XMM2, PCS_RNRN2 );
			XMM3 = _mm_mul_ps( XMM3, PCS_RNRN2 );
		...
		for( i = 0, j = n2 - 4; i < n8; i += 4, j -= 4 )
		{
			...
			XMM2 = _mm_shuffle_ps( XMM2, XMM2, _MM_SHUFFLE( 2, 3, 0, 1 ) );
			XMM0 = _mm_mul_ps( XMM0, PCS_RRNN2 );
		...
		for( ; i < n4; i += 4, j -= 4 )
		{
			...
			XMM0 = _mm_shuffle_ps( XMM0, XMM0, _MM_SHUFFLE( 2, 3, 0, 1 ) );
			XMM2 = _mm_shuffle_ps( XMM2, XMM2, _MM_SHUFFLE( 2, 3, 0, 1 ) );
			XMM0 = _mm_mul_ps( XMM0, PCS_NNRR2 );
			XMM2 = _mm_mul_ps( XMM2, PCS_NNRR2 );
			XMM1 = _mm_mul_ps( XMM1, PCS_RNNR2 );
			XMM3 = _mm_mul_ps( XMM3, PCS_RNNR2 );
			_mm_store_ps( T + i * 4, XMM0 );
			_mm_store_ps( T + i * 4 + 4, XMM1 );
		...
		for( i = 0; i < n4; i += 4 )
		{
			...
			XMM1 = _mm_mul_ps( XMM1, PCS_RNRN2 );
			XMM4 = _mm_mul_ps( XMM4, PCS_RNRN2 );
			_mm_store_ps( T, XMM1 );
			_mm_store_ps( T + 4, XMM4 );
			_mm_store_ps( T + 8, XMM0 );
			_mm_store_ps( T + 12, XMM3 );
			XMM1 = _mm_mul_ps( XMM1, PCS_RRRR2 );
			XMM4 = _mm_mul_ps( XMM4, PCS_RRRR2 );
			XMM0 = _mm_mul_ps( XMM0, PCS_RRRR2 );
			XMM3 = _mm_mul_ps( XMM3, PCS_RRRR2 );
...
STIN void mdct_butterfly_8( float* x )
{
	...
	XMM1 = _mm_mul_ps( XMM1, PCS_NRRN2 );
	XMM3 = _mm_mul_ps( XMM3, PCS_NNRR2 );
...
{
#ifdef __SSE__												/* SSE Optimize */
	static const __m128 PFV0 = { cPI2_8,  cPI2_8, 1.0f, -1.0f };
	static const __m128 PFV1 = { cPI2_8, -cPI2_8, 0.0f,  0.0f };
	static const __m128 PFV2 = { cPI2_8,  cPI2_8, 1.0f,  1.0f };
	static const __m128 PFV3 = {-cPI2_8,  cPI2_8, 0.0f,  0.0f };
...
STIN void mdct_butterfly_32( float* x )
{
	...
	static const __m128 PFV2 = {-cPI1_8, -cPI3_8,   -1.0f,    1.0f };
	static const __m128 PFV3 = {-cPI3_8,  cPI1_8,    0.0f,    0.0f };
	static const __m128 PFV4 = { cPI3_8,  cPI3_8,  cPI2_8,  cPI2_8 };
	static const __m128 PFV5 = {-cPI1_8,  cPI1_8, -cPI2_8,  cPI2_8 };
	static const __m128 PFV6 = { cPI1_8,  cPI3_8,    1.0f,    1.0f };
	static const __m128 PFV7 = {-cPI3_8,  cPI1_8,    0.0f,    0.0f };
	__m128	XMM0, XMM1, XMM2, XMM3, XMM4, XMM5, XMM6, XMM7;
...
STIN void mdct_butterfly_first( float* T, float* x, int points )
{
#ifdef __SSE__												/* SSE Optimize */
	float* X1 = x + points - 8;
	float* X2 = x + ( points >> 1 ) - 8;
...
#else														/* SSE Optimize */
STIN void mdct_butterfly_generic( float* T,
#endif														/* SSE Optimize */
								  float* x, int points, int trigint )
{
#ifdef __SSE__												/* SSE Optimize */
	float* T;
	float* x1 = x + points - 8;
...
#ifdef __SSE__												/* SSE Optimize */
STIN void mdct_butterflies_backward( mdct_lookup* init, float* x, int points, float* x0, float* x1 )
{
...
STIN void mdct_butterflies( mdct_lookup* init, float* x, int points )
{
...
#ifdef __SSE__												/* SSE Optimize */
		if( l->trig_bitreverse )
		{
			_ogg_free(l->trig_bitreverse);
		}
		if( l->trig_forward )
		{
			_ogg_free( l->trig_forward );
		}
		if( l->trig_backward )
		{
			_ogg_free( l->trig_backward );
		}
		if( l->trig_butterfly_first )
		{
			_ogg_free( l->trig_butterfly_first );
		}
		if( l->trig_butterfly_generic8 )
		{
			_ogg_free( l->trig_butterfly_generic8 );
		}
		if( l->trig_butterfly_generic16 )
		{
			_ogg_free( l->trig_butterfly_generic16 );
		}
		if( l->trig_butterfly_generic32 )
		{
			_ogg_free( l->trig_butterfly_generic32 );
		}
		if( l->trig_butterfly_generic64 )
		{
			_ogg_free( l->trig_butterfly_generic64 );
		}
#endif														/* SSE Optimize */
		memset( l, 0, sizeof( *l ) );
	}
...
STIN void mdct_bitreverse( mdct_lookup* init, float* x )
{
	int n = init->n;
	int* bit = init->bitrev;
#ifdef __SSE__												/* SSE Optimize */
...
void mdct_backward( mdct_lookup* init, float* in, float* out )
{
	...
	/* rotate + window */
	{
		...
		do
		{
			...
			XMM0 = _mm_mul_ps( XMM0, PCS_RRRR2 );
			XMM1 = _mm_mul_ps( XMM1, PCS_RRRR2 );
			XMM2 = _mm_mul_ps( XMM2, PCS_RRRR2 );
			XMM3 = _mm_mul_ps( XMM3, PCS_RRRR2 );
...
#ifdef __SSE__												/* SSE Optimize */
void mdct_forward( mdct_lookup* init, float* in, float* out, float* out1 )
{
	int n = init->n;
	int n2 = n >> 1;
	int n4 = n >> 2;
	...
	float* w = ( float* )_ogg_alloca( n * sizeof( *w ) ); 
	float* w2 = w + n2;
