...
{
	vc->user_comments = ( char** )_ogg_realloc( vc->user_comments, ( vc->comments + 2 ) * sizeof( *vc->user_comments ) );
	vc->comment_lengths = ( int* )_ogg_realloc( vc->comment_lengths, ( vc->comments + 2 ) * sizeof( *vc->comment_lengths ) );
...
#else
	strncpy( vc->user_comments[vc->comments], comment, vc->comment_lengths[vc->comments] + 1 );
#endif
	vc->comments++;
...
{
	/* +2 for = and \0 */
	int size = ( int )( strlen( tag ) + strlen( contents ) + 2 );
	char* comment = alloca( size ); 
...
#else
	strncpy( comment, tag, size );
	strncat( comment, "=", size );
	strncat( comment, contents, size );
#endif
...
#else
	strncpy( fulltag, tag, taglen + 1 );
	strncat( fulltag, "=", taglen + 1 );
#endif
	
	for( i = 0; i < vc->comments; i++ )
	{
#ifdef _WIN32
		if( !_strnicmp( vc->user_comments[i], fulltag, taglen ) )
#else
		if( !strncasecmp( vc->user_comments[i], fulltag, taglen ) )
#endif
		{
			if( count == found )
			{
...
void vorbis_info_init( vorbis_info* vi )
{
	...
	vi->codec_setup = ( void* )_ogg_calloc( 1, sizeof( codec_setup_info ) );
}
...
void vorbis_info_clear( vorbis_info* vi )
{
	...
	if( ci )
	{
		...
		for( i = 0; i < ci->maps; i++ ) 
		{
			/* this may be cleaning up an aborted unpack, in which case the below type cannot be trusted */
			if( ci->map_param[i] )
			{
...
static int _vorbis_unpack_comment( vorbis_comment* vc, oggpack_buffer* opb )
{
	...
	if( vendorlen < 0 || vendorlen > MAX_VENDOR_NAME - 1 )
	{
		goto err_out;
	}
...
{
	char temp[] = "LANCER: aoTuV b5 [20061024] (based on Xiph.Org's libVorbis)";
	int bytes = ( int )strlen( temp );
	if( bytes > MAX_VENDOR_NAME - 1 )
	{
		return( -1 );
	}

	/* preamble */  
	oggpack_write( opb, 3, 8 );
	_v_writestring( opb, "vorbis", 6 );
