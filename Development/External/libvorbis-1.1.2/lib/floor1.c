...
{
	long i, j;
	vorbis_info_floor1* info =  ((vorbis_look_floor1*)(look))->vi;
	long n = ((vorbis_look_floor1*)(look))->n;
	long posts = ((vorbis_look_floor1*)(look))->posts;
	long nonzero = 0;
	...
	else
	{
		for( i = 0; i < posts - 1; i++ )
		{
			nonzero += accumulate_fit( logmask, logmdct, ((vorbis_look_floor1*)(look))->sorted_index[i],	((vorbis_look_floor1*)(look))->sorted_index[i + 1], fits + i, n, info, tflr, tmask, tcres );
		}
	}
#else														/* SSE Optimize */
	if( posts == 0 )
	...
	else
	{
		for( i = 0; i < posts - 1; i++ )
		{
			nonzero += accumulate_fit( logmask, logmdct, ((vorbis_look_floor1*)(look))->sorted_index[i], ((vorbis_look_floor1*)(look))->sorted_index[i + 1], fits + i, n, info );
		}
	}
#endif														/* SSE Optimize */
	...
	if( nonzero )
	{
		...
		for( i = 2; i < posts; i++ )
		{
			int sortpos = ((vorbis_look_floor1*)(look))->reverse_index[i];
			...
			if( memo[ln] != hn )
			{
				...
				int lsortpos = ((vorbis_look_floor1*)(look))->reverse_index[ln];
				int hsortpos = ((vorbis_look_floor1*)(look))->reverse_index[hn];
				memo[ln] = hn;
		...
		for( i = 2; i < posts; i++ )
		{
			int ln = ((vorbis_look_floor1*)(look))->loneighbor[i - 2];
			int hn = ((vorbis_look_floor1*)(look))->hineighbor[i - 2];
...
int floor1_encode( oggpack_buffer* opb, vorbis_block* vb, vorbis_look_floor* look, int* post, int* ilogmask )
{
	long i, j;
	vorbis_info_floor1* info = ((vorbis_look_floor1*)(look))->vi;
	long posts = ((vorbis_look_floor1*)(look))->posts;
	...
	if( post )
	{
		...
		for( i = 2; i < posts; i++ )
		{
			...
			{
				int headroom = ( ((vorbis_look_floor1*)(look))->quant_q-predicted < predicted ? ((vorbis_look_floor1*)(look))->quant_q - predicted : predicted );
		...
		/* beginning/end post */
		((vorbis_look_floor1*)(look))->frames++;
		((vorbis_look_floor1*)(look))->postbits += ilog( ((vorbis_look_floor1*)(look))->quant_q - 1 ) * 2;
		oggpack_write( opb, out[0], ilog( ((vorbis_look_floor1*)(look))->quant_q - 1 ) );
		oggpack_write( opb, out[1], ilog( ((vorbis_look_floor1*)(look))->quant_q - 1 ) );
		...
		for( i = 0, j = 2; i < info->partitions; i++ )
		{
			...
			if( csubbits )
			{
				...
				/* write it */
				((vorbis_look_floor1*)(look))->phrasebits += vorbis_book_encode( books + info->class_book[class], cval, opb );
			}
			...
			for( k = 0; k < cdim; k++ )
			{
				...
				if( book >= 0 )
				{
					...
					{
						((vorbis_look_floor1*)(look))->postbits += vorbis_book_encode( books + book, out[j + k], opb );
					}
				}
			}
	    ...
	    
		{
			...
			for( j = 1; j < ((vorbis_look_floor1*)(look))->posts; j++ )
			{
				int current = ((vorbis_look_floor1*)(look))->forward_index[j];
