...
vorbis_look_residue* res0_look( vorbis_dsp_state* vd, vorbis_info_residue* vr )
{
	...
	look->partvals = rint( powf( ( float )look->parts, ( float )dim ) );
...
STIN int local_book_besterror_dim2x2( codebook* book, float* a, oggpack_buffer* opb, int* ia )
{
	...
	XMM1 = _mm_loadl_pi( PFV_0, ( __m64* )( book->valuelist + ctrl0 * 2 ) );
...
static int _encodepart( oggpack_buffer* opb, float* vec, int n, codebook* book, long* acc )
{
	int i, bits = 0;
	int dim = book->dim;
#ifdef __SSE__												/* SSE Optimize */
	...
	if( dim <= 8 )
	{
		if( book->c->thresh_tree->quantthresh[0] == -( float )( book->c->thresh_tree->threshvals >> 1 ) + 0.5f )
		{
#if	defined(__SSE2__)
			for( i = 0; i < n; i += 16 )
			{
		...
		{
			/* for control of round */
			__m128 PM = _mm_set1_ps( 1.0f / ( ( float )( book->c->thresh_tree->quantthresh[1] - book->c->thresh_tree->quantthresh[0] ) - ( float )1.0e-04 ) );	
...
static long** _01class( vorbis_block* vb, vorbis_look_residue* vl, float** in, int ch )
{
	long i, j, k;
	...
	for( i = 0; i < partvals; i++ )
	{
		...
		for( j = 0; j < ch; j++ )
		{
			...
			{
				if( fabsf( in[j][offset + k] ) > max )
				{
					max = fabsf( in[j][offset + k] );
				}
				ent += fabsf( rint( in[j][offset + k] ) );
			}
			ent *= scale;

			for( k = 0; k < possible_partitions - 1; k++ )
			{
				if( max <= info->classmetric1[k] && ( info->classmetric2[k] < 0 || ( int )ent < info->classmetric2[k] ) )
				{
					break;
				}
			}
...
static long** _2class( vorbis_block* vb, vorbis_look_residue* vl, float** in, int ch )
{
	long i, j, k, l;
	vorbis_look_residue0* look = ( vorbis_look_residue0* )vl;
	vorbis_info_residue0* info = look->info;
	...
	for( i = 0, l = info->begin / ch; i < partvals; i++ )
	{
		...
		{
			__m128 PMAGMAX = _mm_setzero_ps();
			__m128 PANGMAX = _mm_setzero_ps();
			float* pin0 = in[0];
			float* pin1 = in[1];
			...
			PANGMAX = _mm_set1_ps( angmax );
			{
				...
				else
				{
					...
					{
						if( magmax <= info->classmetric1[8] && angmax <= info->classmetric2[8] )
						{
							j = 8;
						}
						else
...
static int _01forward( oggpack_buffer* opb, vorbis_block* vb, vorbis_look_residue* vl, float** in, int ch, long** partword, int ( *encode )( oggpack_buffer*, float*, int, codebook*, long * ) )
{
	long i, j, k, s;
	vorbis_look_residue0* look = ( vorbis_look_residue0* )vl;
	vorbis_info_residue0* info = look->info;
...
int res1_forward( oggpack_buffer* opb, vorbis_block* vb, vorbis_look_residue* vl, float** in, float** out, int* nonzero, int ch, long** partword )
{
...
int res2_forward( oggpack_buffer* opb, vorbis_block* vb, vorbis_look_residue* vl, float** in, float** out, int* nonzero, int ch, long** partword )
{
			...
			for( i = 0, l = 0; i < partvals; l++ )
			{
				...
				for( k = 0; k < partitions_per_word && i < partvals; k++, i++ )
				{
					if( info->secondstages[partword[l][k]] & ( 1 << s ) )
					{
						...
						if( stagebook )
						{
							if( vorbis_book_decodevv_add( stagebook, in, i * samples_per_partition + info->begin, ch, &vb->opb, samples_per_partition ) == -1 )
							{
								goto eopbreak;
							}
						}
