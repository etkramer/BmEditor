...
// FxTextChunkInfo
//------------------------------------------------------------------------------
class FxTextChunkInfo
{
public:
	void SetChunkStartTime(FxReal chunkStart) { _chunkStart = chunkStart; }
	void SetChunkEndTime(FxReal chunkEnd) { _chunkEnd = chunkEnd; }
	void SetChunkText(const FxWString& chunkText) { _chunkText = chunkText.StripWhitespace(); }

	FX_INLINE FxReal GetChunkStartTime() const { return _chunkStart; }
	FX_INLINE FxReal GetChunkEndTime()   const { return _chunkEnd; }

	FX_INLINE const FxWString& GetChunkText() const { return _chunkText; }

protected:
	FxReal _chunkStart; // The starting time of the chunk, in seconds.
	FxReal _chunkEnd;   // The ending time of the chunk, in seconds.
	
	FxWString _chunkText;	// The complete text of the chunk.
};
...
public:
	...
	void Parse(const FxArray<FxWordTimeInfo>& wordList);
	...
	// normal.
	FxSize GetNumTextChunks() const;
	// Returns the text chunk at index.
	const FxTextChunkInfo& GetTextChunk( const FxSize index ) const;
...
protected:
	...
	// main text.
	void ProcessTextChunks(const FxWString& text);
	...
	FxBool ReplaceWordsWithMarkers(const FxWString& text, const FxArray<FxWordTimeInfo>& wordList);
...
// Finds the next chunk marker from index. Returns FxInvalidIndex if none.
	FxSize FindNextChunkMarker(const FxWString& text, FxSize index);
	...
	FxWString _noChunkText;  // The text with chunks stripped.
	...
	FxArray<FxUnicodeTextTagInfo> _tagInfos; // The tagged information from the text.
        ...
        FxArray<FxTextChunkInfo> _textChunks;    // The text chunks the user marked in the text.
};
