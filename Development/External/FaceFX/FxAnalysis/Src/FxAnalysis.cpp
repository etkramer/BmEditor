...
// Splits the diphthongs into component vowels, when appropriate.
void FX_CALL ___SplitDiphthongs(FxAnalysisResults& analysisResults);
							...
							FxAnalysisResults&     analysisResults )
{
	...
	___SplitDiphthongs(analysisResults);
...
void ___SplitDiphthongs(FxAnalysisResults& analysisResults)
{
	for(FxSize i = 0; i < analysisResults.phonemeTimes.Length(); ++i )
	{
		FxArray<FxPhoneTimeInfo>& phonemeTimes = analysisResults.phonemeTimes;
		FxPhoneme firstPhoneme = PHON_SIL;
		FxPhoneme secondPhoneme = PHON_SIL;
		FxPhoneme current = static_cast<FxPhoneme>(phonemeTimes[i].phoneEnum);
		FxReal duration = phonemeTimes[i].phoneEnd - phonemeTimes[i].phoneStart;
		switch( current )
		{
		case PHON_EY:
		...
		case PHON_OW:
			firstPhoneme = PHON_AO;
			secondPhoneme = PHON_UW;
			break;
		default:
			break;
		};
		if( firstPhoneme != PHON_SIL && secondPhoneme != PHON_SIL && duration > 0.05 )
		{
			FxReal start = phonemeTimes[i].phoneStart;
			FxReal midway = start + duration * 0.5f;
			phonemeTimes[i].phoneEnum = secondPhoneme;
			phonemeTimes[i].phoneStart = midway;
			phonemeTimes.Insert(FxPhoneTimeInfo(firstPhoneme, start, midway), i);
		}
