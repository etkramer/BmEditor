...
void FxUnicodeTextTagParser::StripPunctuation(const FxWString text)
{
	...
	for( FxSize i = 0; i < len; ++i )
	{
		...
		if( !inTag )
		{
			...
			if( iswalnum(text[i]) || text[i] == '\'' || text[i] == ' ' )
			{
				_noPuncText += text[i];
			}
			else
			{
				// Add an additional space so that the words on either side of an ellipsis or other punctuation don't get combined.
				_noPuncText += FxWString(" ");
			}
		}
		else
		{
