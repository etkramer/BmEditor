...
public:
  ...
  /// Removes the element at index 'in place' (without reallocating any
  /// container memory).
  /// \param index The index of the element to remove.
  ...
  /// Removes the element at the iterator 'in place' (without reallocating any
  /// container memory).
  /// \param iter The iterator to remove.
  void RemoveIteratorInPlace(Iterator iter);
	...
	void Prefetch() const;
  /// Trims the container memory to only use what is actually needed (for
  /// example, after calls to RemoveInPlace). This performs a copy and swap.
  void Trim();
