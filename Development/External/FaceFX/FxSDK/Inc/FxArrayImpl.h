...
// Removes the element at index 'in place'.
template<typename Elem>
void FxArray<Elem>::RemoveInPlace(FxSize index)
{
  FxAssert(index < _container._usedCount);
  // Swap the last item with the item to be removed.
  Elem* curr = _container._v + index;
  Elem* swap = _container._v + _container._usedCount - 1;
  (*curr) = (*swap);
...
// Removes the element at the iterator 'in place'.
template<typename Elem>
void FxArray<Elem>::RemoveIteratorInPlace(Iterator iter)
{
  RemoveInPlace(static_cast<FxSize>(iter - _container._v));
}
...
template<typename Elem>
void FxArray<Elem>::Trim()
{
  FxArray<Elem> temp(*this);
  Swap(temp);
}
