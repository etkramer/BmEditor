...
public:
  ...
  FxBool RemoveAnimByIndex(FxSize index);

  /// Trims the animation array so that it contains only the amount of memory
  /// it really needs (for example, after calls to RemoveAnimByIndex).
  FX_INLINE void Trim() { _anims.Trim(); }
