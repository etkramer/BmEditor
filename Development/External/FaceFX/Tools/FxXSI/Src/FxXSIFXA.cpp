...
XSIPLUGINCALLBACK CStatus FaceFXOpenActor_Init( const CRef& in_context )
{
	...
	args.Add( L"ActorFilePath");

	return CStatus::OK;
}

XSIPLUGINCALLBACK CStatus FaceFXOpenActor_Execute( CRef& in_context )
{
	//Temp
...
XSIPLUGINCALLBACK CStatus FaceFXSaveActor_Init( const CRef& in_context )
{
	Context ctx(in_context);
	Command cmd(ctx.GetSource());
...
XSIPLUGINCALLBACK CStatus FaceFXSaveActor_Execute( CRef& in_context )
{
	//Temp
	OC3Ent::Face::FxXSIData::xsiInterface.Startup();
	Application app;
...
XSIPLUGINCALLBACK CStatus FaceFXNewActor_Init( const CRef& in_context )
{
	Context ctx(in_context);
	Command cmd(ctx.GetSource());
...
XSIPLUGINCALLBACK CStatus FaceFXNewActor_Execute( CRef& in_context )
{
	//Temp
	OC3Ent::Face::FxXSIData::xsiInterface.Startup();
	Application app;
