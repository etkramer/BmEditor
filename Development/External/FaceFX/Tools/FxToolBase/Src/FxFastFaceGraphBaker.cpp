				...
				for( FxReal time = startTime; time < endTime; time += timeStep )
				{
					...
					for( FxSize i = 0; i < numNodes; ++i )
					{
						...
						if( lastDerivEqualsZero && !thisDerivEqualsZero)
						{
							...
							// Offset by FX_REAL_RELAXED_EPSILON so that if we are baking a curve with a
							// discontinuity (like a series of steps), the key is inserted on the correct
							// side of the step.							
							FxAnimKey key(time-halfStep -  FxReal(FX_REAL_RELAXED_EPSILON), values1FrameAgo[i]);
						...
						else if (!lastDerivEqualsZero && thisDerivEqualsZero)
						{
							...
							// discontinuity (like a series of steps), the key is inserted on the correct
							// side of the step.							
							FxAnimKey key(time-timeStep-halfStep + FxReal(FX_REAL_RELAXED_EPSILON), values1FrameAgo[i]);
							firstDerivativeKeys[i].PushBack(key);
						}
						else if (
