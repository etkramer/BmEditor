...
void FxMaxMainPanel::BeginEditParams( Interface* ip, IUtil* iu )
{
	...
	if( _pMaxInterface && _pUtilityInterface )
	{
		...
		if(OC3Ent::Face::FxMaxData::maxInterface.IsNoSave()) 
		{
			::EnableWindow(	_hSaveActorButton, false);
		}
...
// Utility function used to calculate the horizontal extent for a list box based on string length.
unsigned int GetTextLength(HWND hwnd, LPCTSTR lpszText)
{
    HDC hdc         = ::GetDC(hwnd);
    ...
    if( ::GetTextExtentPoint32(hdc, lpszText, min(8192, (int) _tcslen(lpszText)), &size) )
    {
...
void FxMaxMainPanel::
_fillListBox( HWND hListBoxToFill, const OC3Ent::Face::FxArray<OC3Ent::Face::FxString>& listBoxStrings )
{
	if( hListBoxToFill )
	{
		...
		for( OC3Ent::Face::FxSize i = 0; i < listBoxStrings.Length(); ++i )
		{
            ...
            const unsigned int extent = static_cast<unsigned int>(
				::SendMessage(	hListBoxToFill, 
								LB_GETHORIZONTALEXTENT, 
								0, 
								0));
            const unsigned int textLength = ::GetTextLength(hListBoxToFill, _T(listBoxStrings[i].GetData()));
            if (extent < textLength)
            {
                ::SendMessage(	hListBoxToFill, 
								LB_SETHORIZONTALEXTENT, 
								textLength, 
