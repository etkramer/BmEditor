...
void FxWidget::OnPreSaveActor( FxWidget* FxUnused(sender) )
{
}
...
void FxWidget::OnPostSaveActor( FxWidget* FxUnused(sender) )
{
}
...
void FxWidgetMediator::OnPreSaveActor( FxWidget* sender )
{
	// Propagate.
	for( FxSize i = 0; i < _widgets.Length(); ++i )
	{
		if( sender != _widgets[i] )
		{
			if( _widgets[i] )
			{
				_widgets[i]->OnPreSaveActor(sender);
			}
		}
	}
...
void FxWidgetMediator::OnPostSaveActor( FxWidget* sender )
{
	// Propagate.
	for( FxSize i = 0; i < _widgets.Length(); ++i )
	{
		if( sender != _widgets[i] )
		{
			if( _widgets[i] )
			{
				_widgets[i]->OnPostSaveActor(sender);
			}
		}
	}
}
