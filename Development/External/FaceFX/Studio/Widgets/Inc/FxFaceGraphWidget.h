...
//------------------------------------------------------------------------------
	...
	// Called when the fg_enableunrealnodes console variable changes.
	static void OnEnableUnrealNodesChanged(FxConsoleVariable& cvar);
...
//------------------------------------------------------------------------------
	...
	void DrawLink( wxDC* pDC, wxPoint fromPoint, wxPoint toPoint, 
				   ...
				   wxColour linkColour = *wxBLACK, FxBool touchesSelected = FxFalse );
...
//------------------------------------------------------------------------------
	...
	static void ConstructAddableNodeInfo();
	// Modify a node type's addable flag dynamically.
	static void ModifyNodeInfoAddableFlag(const FxName& nodeType, FxBool isAddable);
...
//------------------------------------------------------------------------------
	...
	struct AddableNodeInfo
	{
		const FxClass* pNodeClass;
		FxBool isAddable;
	};
	// The list of FxFaceGraphNode derived node types that can be added into the
	// face graph by the user.
	static FxArray<AddableNodeInfo> _addableNodeInfo;
