...
class FxWorkspaceInstanceWidget;
...
// Widget messages.
//------------------------------------------------------------------------------
	virtual void OnPreSaveActor(FxWidget* sender);
	virtual void OnPostSaveActor(FxWidget* sender);
...
//------------------------------------------------------------------------------
	...
	void OnToolbarWorkspaceOptionChange(wxCommandEvent& event);
...
//------------------------------------------------------------------------------
	...
	void AddInstanceWindow(FxWorkspaceInstanceWidget* pInstance);
	void RemoveInstanceWindow(FxWorkspaceInstanceWidget* pInstance);
...
//------------------------------------------------------------------------------
	...
	virtual void OnWorkspaceOptionChanged(FxBool autoKey, FxBool autoSelectCurve);
...
//------------------------------------------------------------------------------
	...
	FxButton* _toolbarAutoselectCurvesToggle;
	...
	// The list of active workspace instance windows.
	FxArray<FxWorkspaceInstanceWidget*> _activeInstances;

	// The cached workspace instance windows.
	struct FxWorkspaceInstanceData
	{
		FxName workspaceName;
		wxRect screenRect;
	};
	FxArray<FxWorkspaceInstanceData> _cachedInstances;
	FxName _cachedWorkspaceName;
};
...
public:
	FxWorkspaceInstanceWidget(wxWindow* parent = NULL, FxWidgetMediator* mediator = NULL, FxWorkspaceWidget* workspaceWidget = NULL);
	~FxWorkspaceInstanceWidget();
	...
	// Public methods
	//------------------------------------------------------------------------------
	void SetWorkspace(const FxWorkspace* workspace);
	void SetAnimController(FxAnimController* animController);

	const FxName& GetWorkspaceName() { return _currentWorkspace.GetName(); }
