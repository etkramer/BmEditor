...
FxCommandError FxVM::_lastCommandResult = CE_Success;
...
FxCommandError FxVM::ExecuteCommand( const FxString& command )
{
	...
	FxCheckLargeNumber(cmd);
	FxArray<FxString> commandParts = _extractCommand(cmd);
#else
	FxArray<FxString> commandParts = _extractCommand(command);
#endif
	if( 0 == commandParts.Length() )
	{
		return _setResult(CE_Success);
	}
	// Find the command.
	...
	if( !pCommandDesc )
	{
		...
		return _setResult(CE_InvalidCommand);
	}
	else
	{
		...
		if( !pCommand )
		{
			...
			return _setResult(CE_InternalError);
		}
		else
		{
			...
			if( !argListValid )
			{
				...
				delete pCommand;
				pCommand = NULL;
				return _setResult(CE_ArgumentFailure);
			}
			// Execute the command.
			...
			return _setResult(commandReturnCode);
		}
	}
}
...
FxCommandError FxVM::LastCommandResult( void )
{
...
FxCommandError FxVM::_setResult(FxCommandError result)
{
...
FxArray<FxString> FxVM::_extractCommand( const FxString& command )
{
	...
	FxSize commandLength = command.Length();
	for( FxSize i = 0; i < commandLength; ++i )
	{
		...
		else if( (command[i] == ' ' || command[i] == ';') && !inQuote )
		{
			...
			{
				const FxChar* c = command.GetData()+i+1;
				while( *c == ' ' && i < commandLength )
				{
					++c; ++i;
				}
			}
