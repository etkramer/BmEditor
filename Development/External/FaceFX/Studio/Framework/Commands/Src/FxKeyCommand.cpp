...
{
	// First off, if the user has passed a curveIndex, check its validity.
	FxSize tryCurveIndex = FxInvalidIndex;
	if( argList.GetArgument("-curveIndex", tryCurveIndex) )
	{
		if( !FxSessionProxy::IsCurveIndexValid(tryCurveIndex) )
		{
			FxVM::DisplayError("There is no curve that corresponds with the given curveIndex.");
			return CE_Failure;
		}
	}
