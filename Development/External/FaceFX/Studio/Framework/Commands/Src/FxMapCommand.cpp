...
FxCommandError FxMapCommand::Execute( const FxCommandArgumentList& argList )
{
	...
	{
		if( FxSessionProxy::GetPhonemeMap(_cachedMap) )
		{
			_commandMode = MakeDefault;
			return Redo();
		}
		else
		{
			FxVM::DisplayError("Could not cache phoneme mapping.");
			return CE_Failure;
		}
	}
	else if( argList.GetArgument("-create") )
	{
	...
	case MakeDefault:
		{
			...
			FxBool retval = FxSessionProxy::SetPhonemeMap(_cachedMap);
			if( _noRefresh )
			{
...
FxCommandError FxMapCommand::Redo( void )
{
	...
	case MakeDefault:
		{
			FxSessionProxy::BeginMappingBatch();
			FxBool retval = FxSessionProxy::MakeDefaultMapping();
			if( _noRefresh )
			{
				_noRefresh = FxFalse;
				FxSessionProxy::EndMappingBatchNoRefresh();
