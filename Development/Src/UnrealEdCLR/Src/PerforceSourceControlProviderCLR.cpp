...
private:
	...
	void AttemptReconnectAndRefreshClientSpecList()
	{
		...
		FPerforceNET Connection(ServerName, UserName);
...
FPerforceNET::FPerforceNET(String^ InServerName, String^ InUserName)
{
	//copy the connection information
	SourceControlProvider = NULL;
	String^ EmptyClientSpecName;

	EstablishConnection( InServerName, InUserName, EmptyClientSpecName );
}
...
FPerforceNET::FPerforceNET(const FPerforceSourceControlProvider* InSourceControlProvider)
{
	...
	EstablishConnection(ServerName, UserName, ClientSpecName);
}
 ...
 */
UBOOL FPerforceNET::EnsureValidConnection(FString& InOutServerName, FString& InOutUserName, FString& InOutClientSpecName)
{
	...
	while (!bEstablishedConnection)
	{
		...
		if (!bEstablishedConnection)
		{
			...
			if (GetPerforceLogin(NewServerName, NewUserName, NewClientSpecName) == PerforceConstants::LoginCanceled)
			{
				break;
			}
		}
 ...
 */
void FPerforceNET::EstablishConnection(String^ InServerName, String^ InUserName, String^ InClientSpecName)
{
...
FString FPerforceNET::UpdateBranchName( void )
{
	...
	if( bBranchNameIsEmpty )
	{
		...
		if( !bBranchNameGood )
		{
			debugf( TEXT( "ERROR: No branch could be determined through P4" ) );
		}
		else
		{
...
PerforceConstants::LoginResults FPerforceNET::GetPerforceLogin (String^% InOutServerName, String^% InOutUserName, String^% InOutClientSpecName)
{
	...
	TArray<FString> ErrorMessages;
	FPerforceNET Connection(InOutServerName, InOutUserName);
...
void FPerforceSourceControlProvider::LoadSettings()
{
	...
	if (!bDisabled)
	{
		...
		UBOOL bSuccessfulInitialConnection = FPerforceNET::EnsureValidConnection(PortName, UserName, ClientSpecName);
