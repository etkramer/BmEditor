		...
		else if ( String::Compare(Args->PropertyName, "Viewmode")==0 )
		{
			...
			if (Layer->Index >= 0)
			{
				if (LandscapeEditSystem->CurrentToolTarget.Landscape)
				{
					...
					for (INT Idx = 1; Idx-1 < LandscapeEditSystem->CurrentToolTarget.Landscape->LayerInfos.Num(); Idx++)
					{
						if (Idx-1 == Layer->Index)
						{
						...
						else
						{
							...
							INT& DebugColorChannel = LandscapeEditSystem->CurrentToolTarget.Landscape->LayerInfos(Idx-1).DebugColorChannel;
							if (Layer->ViewmodeR && (DebugColorChannel & 1))
							{
								bChanged = TRUE;
								DebugColorChannel -= 1;
							}
							if (Layer->ViewmodeG && (DebugColorChannel & 2))
							{
								bChanged = TRUE;
								DebugColorChannel -= 2;
							}
							if (Layer->ViewmodeB && (DebugColorChannel & 4))
							{
								bChanged = TRUE;
								DebugColorChannel -= 4;
							}
