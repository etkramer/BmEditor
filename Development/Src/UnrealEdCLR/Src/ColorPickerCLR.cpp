...
Color MColorPickerPanel::GetBrushColor(const FLOAT InAlphaPercent, const FLOAT InRed, const FLOAT InGreen, const FLOAT InBlue)
{
	float ClampedRed = Clamp<FLOAT>(appPow(InRed,1.0f / 2.2f)*255.0, 0, 255.0);
	float ClampedGreen = Clamp<FLOAT>(appPow(InGreen,1.0f / 2.2f)*255.0, 0, 255.0);
	float ClampedBlue = Clamp<FLOAT>(appPow(InBlue,1.0f / 2.2f)*255.0, 0, 255.0);
	float ClampedAlpha = Clamp<FLOAT>(InAlphaPercent*255.0, 0, 255.0);
...
void MColorPickerPanel::PushColorToDataPtrs(const UBOOL bStopUpdateBasedOnPerf)
{
	...
	if (bStopUpdateBasedOnPerf && ColorStruct.bSendEventsOnlyOnMouseUp)
	{
		bUpdateWhenChanged = FALSE;
	}
	...
	if (bUpdateWhenChanged)
	{
		...
		for (INT ObjectIndex = 0; ObjectIndex < ColorStruct.ParentObjects.Num(); ++ObjectIndex)
		{
			...
			if (ColorStruct.PropertyWindow && ColorStruct.PropertyNode)
			{
				...
				UBOOL bChangesTopology = FALSE;

				ColorStruct.PropertyWindow->NotifyPreChange(PropertyControl, PropertyThatChanged, ParentObject);
...
System::Windows::Media::RadialGradientBrush^ MColorPickerPanel::GetHDRColorGradient(const float InAlpha, const float InRed, const float InGreen, const float InBlue, bool bRightHalf)
{
	...
	GradientBrush->GradientStops->Add( gcnew GradientStop( GetBrushColor(InAlpha, InRed, InGreen, InBlue), 0.0 ) );
	...
	for (INT i = 0; i < ValuesOverOne->Count; ++i)
	{
		...
		GradientBrush->GradientStops->Add( gcnew GradientStop( GetBrushColor(InAlpha, InRed*OneOverValue, InGreen*OneOverValue, InBlue*OneOverValue), Percent ) );
	}
...
void MColorPickerPanel::UpdateOriginalColorPreview()
{
	//Setup color previews
	System::Windows::Shapes::Rectangle^ OldColorRect = safe_cast< System::Windows::Shapes::Rectangle^ >( LogicalTreeHelper::FindLogicalNode( this, "OldColorRectNoAlpha" ) );
	OldColorRect->Fill = gcnew SolidColorBrush( GetBrushColor(1.0f, Red, Green, Blue) );

	System::Windows::Shapes::Rectangle^ OldColorRectNoAlpha = safe_cast< System::Windows::Shapes::Rectangle^ >( LogicalTreeHelper::FindLogicalNode( this, "OldColorRect" ) );
	OldColorRectNoAlpha->Fill = gcnew SolidColorBrush( GetBrushColor(Alpha, Red, Green, Blue) );
}
...
void MColorPickerPanel::UpdateSimpleSliders()
{
	...
	SaturationSliderBackground->GradientStops->Add( gcnew GradientStop( GetBrushColor(1, NoSaturationColor.R, NoSaturationColor.G, NoSaturationColor.B), 0.0 ) );
	SaturationSliderBackground->GradientStops->Add( gcnew GradientStop( GetBrushColor(1, FullSaturationColor.R, FullSaturationColor.G, FullSaturationColor.B), 1.0 ) );
	...
	BrightnessSliderBackground->GradientStops->Add( gcnew GradientStop( GetBrushColor(1, 0, 0, 0), 0.0 ) );
	BrightnessSliderBackground->GradientStops->Add( gcnew GradientStop( GetBrushColor(1, FullbrightColor.R, FullbrightColor.G, FullbrightColor.B), 1.0 ) );
