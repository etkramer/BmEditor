...
bool MGameAssetJournalBase::CreateJournalEntryFromString( String^ Text, List<String^>^ AllClassNames, [Out] MGameAssetJournalEntry^% OutJournalEntry )
{
	...
	if( EntryVersionNumber <= GADDefs::JournalEntryVersionNumber )
	{
		...
		if( EntryBranchName == BranchName &&
			EntryGameName == GameName )
		{
		...
		MySqlConnection.reset(
			gcnew SqlConnection(
				String::Format(
					...
					"Integrated Security=True;" +				// Use Windows credentials?
...
bool MGameAssetJournalClient::QueryJournalEntries( [Out] MGameAssetJournalEntryList^% OutJournalEntries )
{
	// Load the name for the current branch from the configuration file
	String^ BranchName = QueryBranchName();
	if( BranchName->Length == 0 )
	...
	try
	{
		...
		while( MySqlReader->Read() )
		{
			...
			if( CreateJournalEntryFromString( Text, %AllClassNames, NewJournalEntry ) )
			{
...
bool MGameAssetJournalFile::QueryJournalEntries( [Out] MGameAssetJournalEntryList^% OutJournalEntries )
{
	OutJournalEntries = gcnew MGameAssetJournalEntryList();
	...
	if( JournalStreamReader )
	{
		...
		while( JournalStreamReader->Peek() >= 0 )
		{
			...
			// Parse the text
			MGameAssetJournalEntry^ NewJournalEntry = nullptr;
			if( CreateJournalEntryFromString( JournalEntryString, %AllClassNames, NewJournalEntry ) )
			{
...
{
	return ( bIsJournalServerAvailable == FALSE && bInOfflineMode == FALSE );
}
...
bool FGameAssetDatabase::CreateCollection( System::String^ InCollectionName, EGADCollection::Type InType )
{
	...
	bool Result = false;
	if( CollectionTagType == ESystemTagType::LocalCollection )
	{
		// If this is a local collection type, locally create the tag and do not update the database.
		LocalCreateTag( MakeSystemTag( CollectionTagType, InCollectionName ) );
		Result = true;
	}
	else
	{
...
bool FGameAssetDatabase::DestroyCollection( System::String^ InCollectionName, EGADCollection::Type InType )
{
	...
	if( CollectionTagType == ESystemTagType::LocalCollection )
	{
		...
		LocalDestroyTag( MakeSystemTag( CollectionTagType, InCollectionName ) );
		return true;
	}
	else
	{
		return DestroyTag( MakeSystemTag( CollectionTagType, InCollectionName ) );
	}
}
...
bool FGameAssetDatabase::AddAssetsToCollection( System::String^ InCollectionName, EGADCollection::Type InType, Generic::ICollection<System::String^>^ InAssetFullNames )
{
	...
	if( CollectionTagType == ESystemTagType::LocalCollection )
	{
		...
		Result = AddAssetsToLocalCollection( MakeSystemTag( CollectionTagType, InCollectionName ), InAssetFullNames );
	}
	else
	{
		Result = AddTagToAssets( InAssetFullNames, MakeSystemTag( CollectionTagType, InCollectionName ) );
	}
	return Result;
}

/** Adds assets to a local collection. */
bool FGameAssetDatabase::AddAssetsToLocalCollection( System::String^InTag, Generic::ICollection<System::String^>^ InAssetFullNames )
{
	bool Result = false;
	if( GetSystemTagType( InTag ) == ESystemTagType::LocalCollection )
	{
		for each (System::String^ AssetName in InAssetFullNames)
		{
			...
			// be loaded so we don't know if it really exists on disk.)
			bool bIsAuthoritative = false;
			AddTagMapping( AssetName, InTag, bIsAuthoritative );
		}
		Result = true;
	}
...
bool FGameAssetDatabase::RemoveAssetsFromCollection( System::String^ InCollectionName, EGADCollection::Type InType, Generic::ICollection<System::String^>^ InAssetFullNames )
{
	...
	if( CollectionTagType == ESystemTagType::LocalCollection )
	{
		Result = RemoveAssetsFromLocalCollection( MakeSystemTag( CollectionTagType, InCollectionName ), InAssetFullNames );
	}
	else
	{
		Result = RemoveTagFromAssets( InAssetFullNames, MakeSystemTag( CollectionTagType, InCollectionName ) );
	}

	return Result;
}

/** Removes assets from a local collection. */
bool FGameAssetDatabase::RemoveAssetsFromLocalCollection( System::String^ InTag, Generic::ICollection<System::String^>^ InAssetFullNames )
{
	bool Result = false;
	if( GetSystemTagType( InTag ) == ESystemTagType::LocalCollection )
	{
		for each (System::String^ AssetName in InAssetFullNames)
		{
			RemoveTagMapping( AssetName, InTag );
		}
		Result = true;
	}
...
bool FGameAssetDatabase::IsCollectionTag( String^ Tag, EGADCollection::Type InType )
{
	...
	return IsSystemTag(Tag) && ( GetSystemTagType( Tag ) == CollectionTagType );
}
...
bool FGameAssetDatabase::LoadJournalData( String^ RestrictUserName, DateTime^ IgnoreServerEntriesAsOldAs, DateTime^ DeleteServerEntriesOlderThan, bool bIsAuthoritative, List< int >^& OutDatabaseIndicesToDelete )
{
	...
	MGameAssetJournalEntryList^ OfflineJournalEntries = nullptr;
	{
		...
		if( !OfflineJournalFile->QueryJournalEntries( OfflineJournalEntries ) )
		{
			// Couldn't query journal entries.  Reason is stored in ErrorMessageText
			return false;
		}
	...
	if( !bInOfflineMode )
	{
		...
		// Query all of the records we're interested in
		if( !JournalClient->QueryJournalEntries( ServerJournalEntries ) )
		{
			// Couldn't query journal entries.  Reason is stored in ErrorMessageText
			return false;
		}
