...
static bool				GWasRightClick = false;		// This remembers if mouse left button was pressed with Cmd or without it.
...
-(void)mouseDown:(NSEvent*)Event
{
	...
	else
	{
		...
		[self addKeyDownEvent: (int)MOUSEBUTTON_Left withCharacter: (wchar_t)0xFFFF repeated: ( [Event clickCount] > 1 )];
	}
 ...
 */
void MacGetApplicationDirectory( char *AppDir, int MaxLen )
{
 ...
 */
void MacGetDocumentDirectory( char *DocDir, int MaxLen )
{
	// use the API to retrieve where the application is stored
