...
#if XBOX
INT GFxDepthBufferCount = 0;
#endif
void FGFxRenderResources::InitDynamicRHI()
{
#if XBOX
	if (GFxDepthBufferCount == 0)
	{
		//always use the scene depth surface for the first requested surface
		DepthSurface = GSceneRenderTargets.GetSceneDepthSurface();
	}
	else
	{
...
#if XBOX
	GFxDepthBufferCount--;
	//if this wasn't the first requested depth surface, decrement the stats
	if (GFxDepthBufferCount != 0)
	{
...
// Number of saved off render targets for the game thread
UINT GFx_SaveCurrentStackCount = 0;
// Copy of the above value for the render thread
UINT GFx_SaveCurrentStackCount_RenderThread = 0;
...
/** Clamp the width and height to the size of the current render target */
void GFx_ClampWidthAndHeight(UINT& inw, UINT& inh)
{
	inw = (inw > GSceneRenderTargets.GetBufferSizeX()) ? GSceneRenderTargets.GetBufferSizeX() : inw;
	inh = (inh > GSceneRenderTargets.GetBufferSizeY()) ? GSceneRenderTargets.GetBufferSizeY() : inh;
}

/**
 * Render Thread function to update the render thread copy of the current stack count
 *
 * @param InStackCount The new stack count from the game thread
 */
void GFx_SetCurrentStackCount_RenderThread(UINT InStackCount)
{
	FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread = InStackCount;
}

/** Return the proper scale value for FrameRects */
inline UINT GFx_GetClampedFrameRectScalar()
{
	UINT ReturnValue = 20;
#if XBOX
	if (FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread >= 2)
	{
		ReturnValue = 40;
	}
#endif
...
{
#if XBOX
	//PIX Event - Draw UI Background
	SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,*(FString::Printf(TEXT("GFx DrawUIBkgClr: 0x%08x"), Renderer->GetCurRenderTarget())));
#endif
...
/** Render thread wrapper for PIX events*/
void FGFxRenderer::ScopedEventCallback_RenderThread(const char* pEventName)
{
#if XBOX
	SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,*(FString::Printf(TEXT("GFx: %s"), ANSI_TO_TCHAR(pEventName))));
#endif
}

/** Game thread wrapper for PIX events*/
void FGFxRenderer::ScopedEventCallback(const char* pEventName)
{
#if XBOX
	GFX_ENQUEUE_UNIQUE_RENDER_COMMAND_TWOPARAMETER
		(GFxScopedEventCallbackRT,
		FGFxRenderer*, Renderer, this,
		const char*, InEventName, pEventName,
	{
		Renderer->ScopedEventCallback_RenderThread(InEventName);
	});
#endif
}

/** Render thread callback to allow custom saving of the contents of the current render target */
void FGFxRenderer::SaveCurrentRenderTargetContents_RenderThread()
{
#if XBOX && !USE_NULL_RHI
	SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,
		*(FString::Printf(TEXT("GFx SaveCurr: 0x%08x (0x%08x"), 
		CurRenderTarget ? (IDirect3DSurface9*)(CurRenderTarget->Resource.ColorBuffer) : 0, 
		CurRenderTarget ? (IDirect3DTexture9*)(CurRenderTarget->Resource.ColorTexture) : 0)));

	if (CurRenderTarget)
	{
		//if we are still allowing new render targets
		if (FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread < 2)
		{
			//0th render target is the full size color buffer
			if (FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread == 0)
			{
				// Saving off the color buffer (back buffer)
				CurRenderTarget->Resource.ColorBuffer.ResolveTargetTexture2D = GSceneRenderTargets.GetSceneColorARGBTexture();
			}
			//1st render target is the half size translucency buffer
			else if (FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread == 1)
			{
				// Saving off the light attenuation buffer (back buffer)
				CurRenderTarget->Resource.ColorBuffer.ResolveTargetTexture2D = GSceneRenderTargets.GetLightAttenuationTexture();
			}
			//now that the resolve target is set, copy from the surface
			RHICopyToResolveTarget(CurRenderTarget->Resource.ColorBuffer, FALSE, FResolveParams());
		}
		else
		{
			debugf(TEXT("SaveCurrentRenderTargetContents: Invalid StackCount %d"), FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread);
		}
	}
#endif
}

/** Game thread callback to allow custom saving of the contents of the current render target */
void FGFxRenderer::SaveCurrentRenderTargetContents()
{
#if XBOX
	GFX_ENQUEUE_UNIQUE_RENDER_COMMAND_ONEPARAMETER
		(GFxSaveCurrentRTContents,
		FGFxRenderer*, Renderer, this,
	{
		Renderer->SaveCurrentRenderTargetContents_RenderThread();
	}
	);

	// Increment the stack count and update it on the render thread
	FGFxRendererImpl::GFx_SaveCurrentStackCount++;
	GFX_ENQUEUE_UNIQUE_RENDER_COMMAND_ONEPARAMETER
		(GFxSetStackCount,
		UINT, InStackCount, FGFxRendererImpl::GFx_SaveCurrentStackCount,
	{
		FGFxRendererImpl::GFx_SetCurrentStackCount_RenderThread(InStackCount);
	}
	);
	//@MERGE. make this a constant?
	if (FGFxRendererImpl::GFx_SaveCurrentStackCount >= 2)
	{
		// Disable GFx in this case filters...
		SetForceDisabledFilters(true);
	}
#endif
}

/** Render thread callback to allow the previous contents of the rendertarget to be restored */
void FGFxRenderer::RestoreCurrentRenderTargetContents_RenderThread()
{
#if XBOX && !USE_NULL_RHI
	SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,
		*(FString::Printf(TEXT("GFx RestoreCurr: 0x%08x (0x%08x"), 
		CurRenderTarget ? (IDirect3DSurface9*)(CurRenderTarget->Resource.ColorBuffer) : 0, 
		CurRenderTarget ? (IDirect3DTexture9*)(CurRenderTarget->Resource.ColorTexture) : 0)));

	if (CurRenderTarget)
	{
		//if this was a backed up render target, restore the previous surface via the saved texture
		if (FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread <= 2)
		{
			RHICopyFromResolveTargetFast(CurRenderTarget->Resource.ColorBuffer);
			CurRenderTarget->Resource.ColorBuffer.ResolveTargetTexture2D = NULL;

			//ensure that stencil is on (turned it off in resolve)
...
/** Game thread callback to allow the previous contents of the rendertarget to be restored */
void FGFxRenderer::RestoreCurrentRenderTargetContents()
{
#if XBOX
	GFX_ENQUEUE_UNIQUE_RENDER_COMMAND_ONEPARAMETER
		(GFxRestoreCurrentRTContents,
		FGFxRenderer*, Renderer, this,
	{
		Renderer->RestoreCurrentRenderTargetContents_RenderThread();
	});

	// Decrement the stack count and update the render thread version of it
	FGFxRendererImpl::GFx_SaveCurrentStackCount--;
	GFX_ENQUEUE_UNIQUE_RENDER_COMMAND_ONEPARAMETER
		(GFxSetStackCount,
		UINT, InStackCount, FGFxRendererImpl::GFx_SaveCurrentStackCount,
	...
	if (FGFxRendererImpl::GFx_SaveCurrentStackCount < 2)
	{
		// Enable GFx filters again...
		SetForceDisabledFilters(false);
	}
#endif
}
...
bool FGFxRenderer::GetRenderCaps(RenderCaps* Caps)
{
	...
	// Filters
#if 0 //We no longer use prepass on XBOX
...
void FGFxRenderer::SetDisplayRenderTarget_RenderThread(GRenderTarget* InRT, bool bsetstate)
{
	...
	{
#if XBOX
		//PIX Event - Display Render Target
		SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,*(FString::Printf(TEXT("GFx DisplayRT: 0x%08x"), CurRenderTarget)));
#endif
...
void FGFxRenderer::PushRenderTarget_RenderThread(const GRectF& FrameRect, GRenderTarget* InRT)
{
	FGFxRenderTarget* RT = (FGFxRenderTarget*)InRT;
#if XBOX && !USE_NULL_RHI
	//PIX Event - Push Render Target
	SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,*(FString::Printf(TEXT("GFx PushRT: 0x%08x (0x%08x"), 
		(IDirect3DSurface9*)(RT->Resource.ColorBuffer), 
		(IDirect3DSurface9*)(CurRenderTarget->Resource.ColorBuffer))));
#endif
	if (RT->Texture)
		RT->Texture->AddRef();

	FLOAT SavedInvGamma = CurRenderTarget->Resource.InverseGamma;
	...
	CurRenderTarget->Resource.InverseGamma = SavedInvGamma;
	...
	//ensure the desired target size will fit within the 
	check(CurRenderTarget->TargetWidth <= UInt(CurRenderTarget->Texture->RenderTarget->Resource.AllocatedSizeX));
	check(CurRenderTarget->TargetHeight <= UInt(CurRenderTarget->Texture->RenderTarget->Resource.AllocatedSizeY));
	...
	ViewRect.xAxis.ViewLengthMax = CurRenderTarget->Texture->RenderTarget->Resource.AllocatedSizeX;
	// y-axis
	...
	ViewRect.yAxis.ViewLengthMax = CurRenderTarget->Texture->RenderTarget->Resource.AllocatedSizeY;
	...
	//@TODO. This function is missing the perspFOV parameter
	// The 'invertY' is an optional parameter, so the FOV is always 1.0 ((FLOAT)true)
	MakeViewAndPersp3D(FrameRect, matView, matPersp, 
//		NEED TO PUT perpFOV here!!!
		true /*invert Y */);
	ViewMatrix = matView;
	ProjMatrix = matPersp;
...
{
#if XBOX
	//PIX Event - Pop Render Target
	SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,*(FString::Printf(TEXT("GFx  PopRT: 0x%08x (%2d)"), CurRenderTarget, FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread)));
#endif

#if XBOX && !USE_NULL_RHI
	UBOOL bClearTarget = FALSE;
	//if we are popping out of a filtered render target
	if (FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread <= 2)
	{
		if (FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread < 2)
		{
			//0th render target comes from the full size color buffer
			if (FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread == 0)
			{
				CurRenderTarget->Resource.ColorBuffer.ResolveTargetTexture2D = GSceneRenderTargets.GetSceneColorARGBTexture();
				bClearTarget = TRUE;
			}
			//1st render target comes from the translucency buffer
			else if (FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread == 1)
			{
				CurRenderTarget->Resource.ColorBuffer.ResolveTargetTexture2D = GSceneRenderTargets.GetLightAttenuationTexture();
				bClearTarget = TRUE;
		...
		//Resolve from the saved off texture
	...
	{
		debugf(NAME_Warning, TEXT("PopRT> Skipping resolve due to StackCount %d"), FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread);
	}
#else
	RHICopyToResolveTarget(CurRenderTarget->Resource.ColorBuffer, TRUE, FResolveParams());
#endif

	//release references to depth buffers
	if (CurRenderTarget->StencilBuffer)
	{
		CurRenderTarget->StencilBuffer = NULL;
	}
	...
	//release references to the color buffers
	RTState rts = RenderTargetStack.Back();
	RenderTargetStack.PopBack();

#if XBOX && !USE_NULL_RHI
	//PIX Event - Render Target Transition
	SCOPED_DRAW_EVENT(InnerEvent)(DEC_SCENE_ITEMS,*(FString::Printf(TEXT("  0x%08x --> 0x%08x"), 
		(IDirect3DSurface9*)(CurRenderTarget->Resource.ColorBuffer), 
		(IDirect3DSurface9*)(rts.pRT->Resource.ColorBuffer))));
#endif
...
#if XBOX
	// Set the render target so that subsequent calls to save off the buffer (which occur
	// when there are nested filters) save off the proper buffer!
	RHISetRenderTarget(CurRenderTarget->Resource.ColorBuffer, CurRenderTarget->Resource.DepthBuffer);
...
void        FGFxRenderer::CheckRenderTarget_RenderThread()
{
...
#if XBOX && !USE_NULL_RHI
	//PIX Event - Check Render Target
	SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,*(FString::Printf(TEXT("GFx CheckRT: 0x%08x (%2d)"), CurRenderTarget, FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread)));
	SCOPED_DRAW_EVENT(Event2)(DEC_SCENE_ITEMS,*(FString::Printf(TEXT("    ColorSurf: 0x%08x"), 
		(IDirect3DSurface9*)(GSceneRenderTargets.GetRenderTargetSurface(SceneColor)))));
#endif
	...
	if (CurRenderTarget->Texture)
	{
		//if we're not masking currently and we have a valid stencil buffer
		UBOOL bClearStencil = ((StencilCounter==0) && CurRenderTarget->StencilBuffer) ? TRUE : FALSE;
#if XBOX
		//clear viewport enough so blur won't read out-of-bounds pixels
		const INT ClearSafetyBorder = 8;
		INT ClearWidth  = Min(CurRenderTarget->Texture->RenderTarget->Resource.SizeX + ClearSafetyBorder, CurRenderTarget->Texture->RenderTarget->Resource.AllocatedSizeX);
		INT ClearHeight = Min(CurRenderTarget->Texture->RenderTarget->Resource.SizeY + ClearSafetyBorder, CurRenderTarget->Texture->RenderTarget->Resource.AllocatedSizeY);
		RHISetViewport(0,0,0, ClearWidth, ClearHeight, 1);
		check(FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread <= 2);
		
		//clear with the expanded border
		RHIClear(TRUE, FLinearColor(0,0,0,0), FALSE, 0, bClearStencil, 0);
#endif
	...
	ApplyUIBlendMode_RenderThread(bAlphaComposite, BlendMode );
 	...
 	FGFxPixelShaderInterface* pShaderInterface = GetUIPixelShaderInterface_RenderThread(GFx_PS_TextTexture);
  	pShaderInterface->SetParameterInverseGamma(pShaderInterface->GetNativeShader()->GetPixelShader(), CurRenderTarget->Resource.InverseGamma);
...
FGFxTexture*  FGFxRenderer::PushTempRenderTarget_RenderThread(FGFxTexture* StubTexture, const GRectF& FrameRect, UInt inw, UInt inh, bool wantStencil)
{
	...
	UInt MaxTempRTSizeX = GSceneRenderTargets.GetBufferSizeX();
	UInt MaxTempRTSizeY = GSceneRenderTargets.GetBufferSizeY();
	FLOAT MaxTempRTXOverY = (FLOAT)MaxTempRTSizeX / (FLOAT)MaxTempRTSizeY;

#if XBOX
	//use a quarter resolution (transparency) buffer for anything past the 0th saved off buffer
	if (FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread > 1)
	{
		MaxTempRTSizeX /= 2;
		MaxTempRTSizeY /= 2;
	}
	//PIX Event - Push Temp Render Target Save Stack
	SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,*(FString::Printf(TEXT("GFx PushTempRT: SaveStack %2d"), FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread)));
#endif

	//Clamp width & height to valid screen size
	FGFxRendererImpl::GFx_ClampWidthAndHeight(inw, inh);
	...
	if (inw > MaxTempRTSizeX || inh > MaxTempRTSizeY)
	{
		FLOAT DesiredRatio = (FLOAT)inw / (FLOAT)inh;
		if (DesiredRatio > MaxTempRTXOverY)
		{
			inh = (UInt)ceilf(MaxTempRTSizeX*inh/Float(inw));
			inw = MaxTempRTSizeX;
		}
		else
		{
			inw = (UInt)ceilf(MaxTempRTSizeY*inw/Float(inh));
			inh = MaxTempRTSizeY;
		}
	}

#if XBOX
	//to get here, we must have called Begin Filters which saved off the current contents of the render target
	if (FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread > 0)
	{
		UINT RenderTargetIndex = FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread - 1;
		//if this scratch buffer was created already, just use it.  The backing store is already correct
		if (RenderTargetIndex < TempRenderTargets.GetSize())
		{
			pRT = TempRenderTargets[RenderTargetIndex];

			//inflate these sizes for depth comparisons
			w = inw;
			h = inh;
			//the size in the target is arbitrary since AllocatedSize is the real size
			pRT->Texture->RenderTarget->Resource.SizeX = inw;
			pRT->Texture->RenderTarget->Resource.SizeY = inh;
	...
	{
		checkf(0, TEXT("GFx PushTempRT: Invalid SaveStackCount!"));
	}
	...
	//make sure the size is large enough.  Keep in braces to prevent PS3 compile issue
	{
		UInt iw = 0, ih = 0;
		while (w < inw) w = TempRTSizes[iw++];
		while (h < inh) h = TempRTSizes[ih++];
	...
	//Keep in braces to prevent PS3 compile issue
	{
...
dostencil:
	...
	// Re-init the render target resource.
	// This allows it to update its ColorTexture to reflect swapping RT textures between the LightAttenuationMemoryBuffer
	// and the PreviousFrameBackBufferMemoryBuffer on Xbox360.
#if XBOX
	pRT->Resource.InitDynamicRHI();
#endif

#if XBOX
	//PIX Event- Push Temp Render Target - Target Pointer
	SCOPED_DRAW_EVENT(EventStack)(DEC_SCENE_ITEMS,*(FString::Printf(TEXT("GFx PushTempRT: 0x%08x"), pRT)));
#endif
	...
	UINT SizeScalar = FGFxRendererImpl::GFx_GetClampedFrameRectScalar();
	//don't render anything less than zero, ever.
	FLOAT MinX = Max(FrameRect.Left, GetVisibleStageOffsetX());
 	FLOAT MinY = Max(FrameRect.Top, 0.0f);
	GRectF ClampedFrameRect(MinX, MinY, MinX + inw*SizeScalar, MinY + inh*SizeScalar);
	PushRenderTarget_RenderThread(ClampedFrameRect, pRT);

#if XBOX
	// Force the buffer to be set so that subsequent 'Save current' calls will
	// properly save off the 'current' render target.
	CheckRenderTarget_RenderThread();
#endif
...
/**
 *	Render thread implementation for setting the UI Viewport
 *
 *	@param	ViewportParams		The viewport parameters to set
 *	@param	NewViewMatrix		The new viewport matrix to use
 */
void FGFxRenderer::SetUIViewport_RenderThread(FGFxViewportUserParams& ViewportParams, GRenderer::Matrix& NewViewMatrix)
{ 
	ViewRect = ViewportParams;
...
void FGFxRenderer::SetUIViewport( FGFxViewportUserParams& InViewportParams)
{
	...
	GFX_ENQUEUE_UNIQUE_RENDER_COMMAND_THREEPARAMETER( 
		...
		FGFxRenderer*, Renderer, this,
	{ 
		Renderer->SetUIViewport_RenderThread(ViewportParams, NewViewMatrix);
	}
	);
}
...
void FGFxRenderer::InitUIBlendStackAndMiscRenderState_RenderingThread(FMiscRenderStateInitParams& Params)
{
	...
	*Params.bAlphaComposite = (Params.ViewFlags & GViewport::View_AlphaComposite) != 0;
	...
	ApplyUIBlendMode_RenderThread( *Params.bAlphaComposite, BlendMode );
...
/**
 * Render thread begin display function for debugging and PIX Events
 */
void FGFxRenderer::BeginDisplay_RenderThread(FMiscRenderStateInitParams& Params, FGFxRenderer* Renderer)
{
#if XBOX && !USE_NULL_RHI
	//PIX Event - Begin Display
	SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,*(FString::Printf(TEXT("GFx BeginDisplay"))));
	SCOPED_DRAW_EVENT(InnerEvent)(DEC_SCENE_ITEMS,*(FString::Printf(TEXT("0x%08x (0x%08x)"), Renderer->CurRenderTarget, 
		(Renderer->CurRenderTarget ? (IDirect3DSurface9*)(Renderer->CurRenderTarget->Resource.ColorBuffer) : 0))));
#endif
...
void FGFxRenderer::BeginDisplay(GColor BackgroundColor, const GViewport& Viewport, Float x0, Float x1, Float y0, Float y1)
{
	...
	(
		...
		FGFxRenderer*, Renderer, this,
	{
		Renderer->BeginDisplay_RenderThread(Params, Renderer);
	}
	)
...
{
#if XBOX
	//PIX Event - End Display
	SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,*(FString::Printf(TEXT("GFx EndDisplay: 0x%08x"), CurRenderTarget)));
#endif
...
void FGFxRenderer::ApplyUIBlendMode_RenderThread(UBOOL bAlphaComposite, const GRenderer::BlendType NewMode, UBOOL bSourceAc)
{
	if (bSourceAc)
	{
		switch(NewMode)
...
void FGFxRenderer::PushBlendMode(BlendType Mode)
{
		...
		FBlendModeParams, Params, InitParams,
	{
		...
		if( (Params.NewBlendMode > Blend_Layer) && (BlendMode != Params.NewBlendMode))
		{
			...
			Renderer->ApplyUIBlendMode_RenderThread( *Params.bAlphaComposite, BlendMode );
		}
	}
	)
}
...
void FGFxRenderer::DrawDistanceFieldBitmaps( BitmapDesc* BitmapList, int ListSize, int StartIndex, int Count, const GTexture* Ti, const Matrix& M, const DistanceFieldParams& DFParams, CacheProvider* Cache /*= 0*/ ) 
{
	...
	FGFxDrawBitmapsParams Params =
	{
		Store, StartIndex, Count, (FGFxTexture*)Ti, M, DFParams
	};
	
	// The distance field font edge sharpness coefficient; selected for aesthetic value via trial and error.
	// Note: We override the distance field font value here because changing it in the DistanceFieldParams constructor
	// requires a recompile of all the GFx libraries, even though the number is not used anywhere in GFx code!
	Params.DFParams.Width = 12.0f;
...
/**
 * Render thread callback for rendering color matrix filter
 * bIsLastFilter - TRUE if this is the final filter applied to a movie
 */
// Render background color quad
void FGFxRenderer::DrawColorMatrixRect_RenderThread(GTexture* InTexture, const GRectF& SrcRect, const GRectF& DestRect, const Float* CMatrix, bool bIsLastFilter)
{
#if XBOX
	//PIX Event - Draw Color Matrix
	SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,*(FString::Printf(TEXT("GFx ClrMatRect%s: 0x%08x"), bIsLastFilter ? TEXT("<!>") : TEXT(""), CurRenderTarget)));
#endif
	
	FLOAT TempInvGamma = CurRenderTarget->Resource.InverseGamma;
	CurRenderTarget->Resource.InverseGamma = 1.0f;

	CheckRenderTarget_RenderThread();

	if (bIsLastFilter == TRUE)
	{
		// Restore the contents of the target buffer
		RestoreCurrentRenderTargetContents_RenderThread();
	}

	FGFxBoundShaderState           BoundShaderState;
	FGFxEnumeratedBoundShaderState Shaders;

	GRectF ClampedInSrcRect;
	GRectF ClampedInDstRect;

	// Clamp to the size of the backing buffer
	GetClampedRects(SrcRect, DestRect, ClampedInSrcRect, ClampedInDstRect);
	...
	ApplyUITransform_RenderThread(ViewportMatrix, CurrentMatrix, VertexShader);
	ApplyUIBlendMode_RenderThread(TRUE, BlendMode, TRUE);
	...
	float TexScaleX = 1.0f/Texture->RenderTarget->Resource.AllocatedSizeX;
	float TexScaleY = 1.0f/Texture->RenderTarget->Resource.AllocatedSizeY;
	Float bufWidthf = ClampedInSrcRect.Width();
	Float bufHeightf = ClampedInSrcRect.Height();

	if (Texture->RenderTarget->AdjustBounds(&bufWidthf, &bufHeightf))
	{
		TexScaleX *= (bufWidthf/ClampedInSrcRect.Width());
		TexScaleY *= (bufHeightf/ClampedInSrcRect.Height());
	}
#if XBOX
	//if this is a half size buffer, compensate for the texture coordinates we are using
	if (FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread==1)
	{
		TexScaleX *= .5f;
		TexScaleY *= .5f;
	...
	verts[0].SetVertex2D(ClampedInDstRect.Left, ClampedInDstRect.Top, ClampedInSrcRect.Left * TexScaleX, ClampedInSrcRect.Top * TexScaleY, 0);
	verts[1].SetVertex2D(ClampedInDstRect.Right, ClampedInDstRect.Top, ClampedInSrcRect.Right * TexScaleX, ClampedInSrcRect.Top * TexScaleY, 0);
	verts[2].SetVertex2D(ClampedInDstRect.Left, ClampedInDstRect.Bottom, ClampedInSrcRect.Left * TexScaleX, ClampedInSrcRect.Bottom * TexScaleY, 0);
	verts[3].SetVertex2D(ClampedInDstRect.Right, ClampedInDstRect.Bottom, ClampedInSrcRect.Right * TexScaleX, ClampedInSrcRect.Bottom * TexScaleY, 0);

	RHIDrawPrimitiveUP(PT_TriangleStrip, 2, verts, sizeof(FGFxVertex_Glyph));

	CurRenderTarget->Resource.InverseGamma = TempInvGamma;
}
...
/**
 * Render thread callback for rendering blur filter
 * bIsLastFilter - TRUE if this is the final filter applied to a movie
 */
void  FGFxRenderer::DrawBlurRect_RenderThread(GTexture* psrcin, const GRectF& insrcrect, const GRectF& indestrect, const BlurFilterParams& params, bool bIsLastFilter)
{
#if XBOX
	//PIX Event - Draw Blur Rect
	SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,*(FString::Printf(TEXT("GFx BlurRect%s: 0x%08x"), bIsLastFilter ? TEXT("<!>") : TEXT(""), CurRenderTarget)));
#endif
	...
	// Clamp to the size of the backing buffer
	GetClampedRects(insrcrect, indestrect, ClampedInSrcRect, ClampedInDstRect);
	...
	Float bufWidthf0 = ClampedInSrcRect.Width();
	Float bufHeightf0 = ClampedInSrcRect.Height();
...
#if XBOX
	// Clamping to a max of 4 for performance reasons...
	static UINT MaxPasses = 4;
#else
	static UINT MaxPasses = 8;
#endif
	//force blur to ONLY 1 pass (which will resolve to the color buffer for now)
	n = n > MaxPasses ? MaxPasses : n;

	// Temporary viewport for rendering passes [0..n-1]
	Matrix TempViewportMatrix = GMatrix2D::Identity;
	// Flip the image in Y
	TempViewportMatrix.M_[1][1] = -1.0f;

	for (UInt i = 0; i < n; i++)
	{
		Float srcXScale = (1.0f/psrc->RenderTarget->Resource.AllocatedSizeX);
		Float srcYScale = (1.0f/psrc->RenderTarget->Resource.AllocatedSizeY);

		Float  bufWidthf = bufWidthf0;
		Float  bufHeightf = bufHeightf0;
		if (psrc->RenderTarget->AdjustBounds(&bufWidthf, &bufHeightf))
		{
			srcXScale *= (bufWidthf/ClampedInSrcRect.Width());
			srcYScale *= (bufHeightf/ClampedInSrcRect.Height());
		}
		...
		if (i != n - 1)
		{
			// Fixup the half-pixel offset when rendering...
			TempViewportMatrix.M_[0][2] = -srcXScale;
			TempViewportMatrix.M_[1][2] = srcYScale;
			ApplyUITransform_RenderThread(TempViewportMatrix, GMatrix2D::Identity, VertexShader);
			DestRect = GRectF(-1,-1,1,1);
		}
		else
		{
			ApplyUITransform_RenderThread(ViewportMatrix, CurrentMatrix, VertexShader);
			DestRect = ClampedInDstRect;
		}
		...
		SrcRect = GRectF(ClampedInSrcRect.Left * srcXScale,
				 ClampedInSrcRect.Top * srcYScale,
				 ClampedInSrcRect.Right * srcXScale,
				 ClampedInSrcRect.Bottom * srcYScale);

		if (i < n - 1)
		{
			ApplyUIBlendMode_RenderThread(TRUE, Blend_Normal, TRUE);
		}
		else
		{
			ApplyUIBlendMode_RenderThread(TRUE, Blend_Normal, TRUE);
...
/**
 * Game thread callback for rendering blur filter
 * bIsLastFilter - TRUE if this is the final filter applied to a movie
 */
void FGFxRenderer::DrawBlurRect(GTexture* Texture, const GRectF& SrcRect, const GRectF& DestRect, const BlurFilterParams& InParams, bool bIsLastFilter)
{
	...
	Params.bIsLastFilter = bIsLastFilter;
	...
	{
		pRenderer->DrawBlurRect_RenderThread(Params.Texture, Params.SrcRect, Params.DestRect, Params.Params, Params.bIsLastFilter);
	}
	)
...
/**
 * Game thread callback for rendering color matrix filter
 * bIsLastFilter - TRUE if this is the final filter applied to a movie
 */
void FGFxRenderer::DrawColorMatrixRect(GTexture* Texture, const GRectF& SrcRect, const GRectF& DestRect, const Float* CMatrix, bool bIsLastFilter)
{
	...
	GFX_ENQUEUE_UNIQUE_RENDER_COMMAND_TWOPARAMETER
		...
		FGFxRenderer* const, pRenderer, this,
	{
		pRenderer->DrawColorMatrixRect_RenderThread(Params.Texture, Params.SrcRect, Params.DestRect, Params.CMatrix, Params.bIsLastFilter);
	}
	)
...
/**
 *	Retrieve the offset for the visible stage on the X-axis
 *
 *	@return	FLOAT		The visible stage offset along the X-axis
 */
FLOAT FGFxRenderer::GetVisibleStageOffsetX()
{
	// HACK - need to properly use stage values for the movie being played!!!
	// For now, this assumes that all 'movies' are authored for 1280.
	return ((1280.0f - GSceneRenderTargets.GetBufferSizeX()) / 2.0f) * 20.0f;
}

/** 
 *	Return clamped source and destination rectangles
 *
 *	@param	InSrcRect			The unclamped source rect
 *	@param	InDestRect			The unclamped dest rect
 *	@param	OutClampedSrcRect	The clamped source rect
 *	@param	OutClampedDestRect	The clamped dest rect
 */
void FGFxRenderer::GetClampedRects(const GRectF& InSrcRect, const GRectF& InDestRect, GRectF& OutClampedSrcRect, GRectF& OutClampedDestRect)
{
	UINT ClampedWidth = InSrcRect.Width();
	UINT ClampedHeight = InSrcRect.Height();

	// Clamp the width and height to the max buffer size
	FGFxRendererImpl::GFx_ClampWidthAndHeight(ClampedWidth, ClampedHeight);

	// Set up the clamped source rect
	OutClampedSrcRect.Left = InSrcRect.Left;
	OutClampedSrcRect.Top = InSrcRect.Top;
	OutClampedSrcRect.Right = InSrcRect.Left + ClampedWidth;
	OutClampedSrcRect.Bottom = InSrcRect.Top + ClampedHeight;

	UINT SizeScalar = FGFxRendererImpl::GFx_GetClampedFrameRectScalar();

	// find proportion based on scaling
	FLOAT ScaleWidth  = InDestRect.Width() / (SizeScalar * InSrcRect.Width());
	FLOAT ScaleHeight = InDestRect.Height() / (SizeScalar * InSrcRect.Height());


	// never render anything less than zero (left side of the screen)
	FLOAT MinX = Max(InDestRect.Left, GetVisibleStageOffsetX());
	FLOAT MinY = Max(InDestRect.Top, 0.0f);

	// find the destination taking into account scaling
	OutClampedDestRect.Left = MinX;
	OutClampedDestRect.Top = MinY;
	OutClampedDestRect.Right = MinX + ClampedWidth * SizeScalar * ScaleWidth;
	OutClampedDestRect.Bottom = MinY + ClampedHeight*SizeScalar*ScaleHeight;
}
...
{
#if XBOX
	//PIX Event - Begin Submit Mask
	SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,TEXT("GFx BeginSubmitMask"));
#endif
	CheckRenderTarget_RenderThread();
...
{
#if XBOX
	//PIX Event - End Submit Mask
	SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,TEXT("GFx EndSubmitMask"));
#endif
	// Assuming stencil buffer is available and nested stenciling is supported
...
#if XBOX
	//PIX Event - Texture Update
	SCOPED_DRAW_EVENT(Event)(DEC_SCENE_ITEMS,*(FString::Printf(TEXT("GFx TextureUpdate: 0x%08x"), Tex2d)));
#endif
...
{
	AllocatedSizeX = SizeX;
	AllocatedSizeY = SizeY;
	...
	{
#if XBOX
		check(FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread > 0);
		//0th target is the full size ARGB buffer
		if (FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread == 1)
		{
			ColorTexture = GSceneRenderTargets.GetLightAttenuationTexture();
			ColorBuffer = GSceneRenderTargets.GetLightAttenuationSurface();
			AllocatedSizeX = GSceneRenderTargets.GetBufferSizeX();
			AllocatedSizeY = GSceneRenderTargets.GetBufferSizeY();
		}
		//1st target is the half size translucency buffer
		else if (FGFxRendererImpl::GFx_SaveCurrentStackCount_RenderThread >= 2)
		{
			ColorTexture = GSceneRenderTargets.GetTranslucencyBufferTexture();
			ColorBuffer = GSceneRenderTargets.GetTranslucencyBufferSurface();
			AllocatedSizeX = GSceneRenderTargets.GetTranslucencyBufferSizeX();
			AllocatedSizeY = GSceneRenderTargets.GetTranslucencyBufferSizeY();
		}
#else
