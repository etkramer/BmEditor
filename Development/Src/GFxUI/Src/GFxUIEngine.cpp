...
void FGFxEngine::InsertMovieIntoList(FGFxMovie* Movie, TArray<FGFxMovie*>* List)
{
	...
	if( Movie->pUMovie )
	{
		...
		for (INT iter=0; iter < List->Num(); iter++)
		{
			const UGFxMoviePlayer* MoviePlayer = (*List)(iter)->pUMovie;
			if (MoviePlayer && (MoviePlayer->Priority > Movie->pUMovie->Priority))
			{
	...
	//If we failed to find a place to add the movie, add it to the end
	if (!bInsertedMovie)
	{
		List->AddItem(Movie);
	}
...
void FGFxEngine::CloseScene(FGFxMovie* pMovie, UBOOL fDelete)
{
	...
	else if (fDelete && TextureMovies.FindItem(pMovie, index))
	{
...
void FGFxEngine::SetRenderViewport(FViewport* pinViewport)
{
	...
	//Now reevaluate the sizes all movies, as the vieweport has changed.
	ReevaluateSizes();
}
...
void FGFxEngine::RenderUI(UBOOL bRenderToSceneColor, INT DPG, FRenderTarget* OverrideRenderTarget)
{
	...
	// allow caller to optionally specify a render target
	FRenderTarget* RenderTarget = OverrideRenderTarget;
	if (RenderTarget == NULL)
	{
		RenderTarget = HudViewport;
	}

  	ENQUEUE_UNIQUE_RENDER_COMMAND_THREEPARAMETER(
		FSetGFxRenderTargets,
        FRenderTarget*, DrawRenderTarget, RenderTarget,
        ...
        UBOOL, bRenderToSceneColor, bRenderToSceneColor,
	{
        ...
        GSceneRenderTargets.Allocate(DrawRenderTarget->GetSizeX(), DrawRenderTarget->GetSizeY());
        ...
        else
        {
            ...
            pRenderer->SetRenderTarget(DrawRenderTarget);
            rt.Owner = DrawRenderTarget;
            rt.OwnerDepth = &SceneDepthTarget;
        }
	...
	for(INT i = 0; i < DPGOpenMovies[DPG].Num(); ++i)
	{
		...
		if (movie.fVisible && bDisplayHud || (movie.pUMovie && movie.pUMovie->bDisplayWithHudOff))
		{
			...
			if(DwTriovizImpl_SetViewportForGFxUIRendering(movie.pView, HudViewport->GetSizeX(), HudViewport->GetSizeY()))
#endif
			{
...
FGFxMovie*  FGFxEngine::LoadMovie(const TCHAR* ppath, UBOOL fInitFirstFrame)
{
	...
	if (!pImeManager)
	{
		...
		// We do not pass a log into the IME manager as a work around for certain log spews causing a crash.
		pImeManager->Init(NULL);
...
void  FGFxEngine::StartScene(FGFxMovie* Movie, UTextureRenderTarget2D* RenderTexture, UBOOL fVisible, UBOOL fUpdate)
{
	...
	if (Movie->pUMovie && Movie->pUMovie->bPauseGameWhileActive)
	{
...
void FGFxEngine::SetMovieSize(FGFxMovie* Movie)
{
	...
	//If we want to use the player specifically AND we are NOT only showing one viewport atm (main menu, etc)...
	if (LP != NULL && 
		!(LP->Origin.X == 0 && LP->Origin.Y == 0 && LP->Size.X == 0 && LP->Size.Y == 0) )
	{
