...
UBOOL GDwTriovizPostProcessVideo = FALSE; //Turn this on to activate FMV post processing
UBOOL GDwTriovizPostProcessVideoAlphaBlendHack = FALSE;
#endif
...
#if DWTRIOVIZSDK || DWTRIOVIZSDK_PC_VIDEO
...
void FFullScreenMovieGFx::Tick(FLOAT DeltaTime)
{
	...
	else
	{
		...
		Renderer->SetRenderTarget((DwTriovizImpl_NeedsFinalize() || GDwTriovizPostProcessVideo) ? (FRenderTarget *)&GDwSceneRenderTargetSceneColorProxy : Viewport);		
#else
	...
	{
#if DWTRIOVIZSDK || DWTRIOVIZSDK_PC_VIDEO
		if (DwTriovizImpl_NeedsFinalize() || GDwTriovizPostProcessVideo)
		{
			#if XBOX
			...
			if (GDwTriovizPostProcessVideo)
			{
				GDwTriovizPostProcessVideoAlphaBlendHack = TRUE;
			}				
		}
#endif
		MovieView->Display();
		...
		if(g_DwUseFramePacking && !GDwTriovizPostProcessVideo)
		{
			static FDwSceneRenderTargetBackbufferStereoProxy BackbufferEyeRight;
			BackbufferEyeRight.SetStereo(1);

			rt.Owner = &BackbufferEyeRight;

			Renderer->SetRenderTarget(&BackbufferEyeRight);
			GGFxEngine->HudRenderTarget->InitRenderTarget_RenderThread(rt);
			Renderer->SetDisplayRenderTarget_RenderThread(GGFxEngine->HudRenderTarget, bSetState);
		...
		if (DwTriovizImpl_NeedsFinalize() || GDwTriovizPostProcessVideo)
		{
			...
			const FTexture2DRHIRef& SceneColorTexture = GSceneRenderTargets.GetSceneColorLDRTexture();
#else
			GSceneRenderTargets.FinishRenderingSceneColor();
			const FTexture2DRHIRef& SceneColorTexture = GSceneRenderTargets.GetSceneColorTexture();
#endif
			GSceneRenderTargets.BeginRenderingBackBuffer();

			if (GDwTriovizPostProcessVideo)
			{
				GDwTriovizPostProcessVideoAlphaBlendHack = FALSE;
				DwTriovizImpl_GfxRender(SceneColorTexture, Viewport->GetSizeX(), Viewport->GetSizeY());
			}
			else
			{
				DwTriovizImpl_FinalizeGfxVideo(SceneColorTexture, Viewport->GetSizeX(), Viewport->GetSizeY());
			}
		}
#endif
	}
