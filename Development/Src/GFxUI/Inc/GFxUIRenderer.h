...
public:
	...
	*/
	...
	/**
	 *	Apply the given UI blend mode on the render thread
	 *
	 *	bAlphaComposite		TRUE if Alpha Compositing is enabled
	 *	NewMode				The new blend mode to apply
	 *	bSourceAc			TRUE if the source if Alpha Compositing
	 */
	...
	*/
	 ...
	 */
	void SetUIViewport_RenderThread(FGFxViewportUserParams& ViewportParams, GRenderer::Matrix& NewViewMatrix);
	 ...
	 */
    void        DrawBlurRect_RenderThread(GTexture* psrcin, const GRectF& insrcrect, const GRectF& indestrect, const BlurFilterParams& params, bool bIsLastFilter);
	/**
	 * Game thread callback for rendering blur filter
	 * bIsLastFilter - TRUE if this is the final filter applied to a movie
	 */
    void        DrawBlurRect(GTexture* psrcin, const GRectF& insrcrect, const GRectF& indestrect, const BlurFilterParams& params, bool bIsLastFilter);

	/**
	 * Render thread callback for rendering color matrix filter
	 * bIsLastFilter - TRUE if this is the final filter applied to a movie
	 */
    void        DrawColorMatrixRect_RenderThread(GTexture* Texture, const GRectF& SrcRect, const GRectF& DestRect, const Float* CMatrix, bool bIsLastFilter);
	/**
	 * Game thread callback for rendering color matrix filter
	 * bIsLastFilter - TRUE if this is the final filter applied to a movie
	 */
    void        DrawColorMatrixRect(GTexture* psrcin, const GRectF& insrcrect, const GRectF& destrect, const Float *matrix, bool bIsLastFilter);
	 ...
	 */
	void GetClampedRects(const GRectF& InSrcRect, const GRectF& InDestRect, GRectF& OutClampedSrcRect, GRectF& OutClampedDestRect);
...
private:
	 ...
	 */
	void BeginDisplay_RenderThread(FMiscRenderStateInitParams& Params, FGFxRenderer* Renderer);
