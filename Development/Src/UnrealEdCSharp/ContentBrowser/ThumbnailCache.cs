...
namespace UnrealEdCSharp.ContentBrowser
{
	...
	class ThumbnailCache
	{
		...
		private BitmapSource CreateThumbnailForAssetType(String TypeString, bool bIsArchetype, Color ThumbColor)
		{
			...
			// Create a glow effect around the text.  This is mainly for aesthetic purposes but also makes the font easier to read on dark backgrounds
			OuterGlowBitmapEffect GlowEffect = new OuterGlowBitmapEffect();
			// These are arbitrary values that look nice
			GlowEffect.GlowSize = 20;
			GlowEffect.GlowColor = ThumbColor;
			GlowEffect.Opacity = 0.8f;

			// Add a subtle highlight that makes the letters appear embossed.
			DropShadowBitmapEffect Emboss = new DropShadowBitmapEffect();
			Emboss.Color = Colors.White;
			Emboss.ShadowDepth = 1;
			Emboss.Softness = 0;
			Emboss.Opacity = 0.5f;


			// Draw the blended thumbnail background
			DrawContext.DrawImage(Bitmap, new Rect(0, 0, BitmapWidth, BitmapHeight));
			DrawContext.PushEffect( GlowEffect, null );
			{
				DrawContext.PushEffect( Emboss, null );
				{
