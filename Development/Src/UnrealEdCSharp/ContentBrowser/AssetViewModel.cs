...
namespace ContentBrowser
{
	...
	public class AssetViewModel : ObservableCollection<AssetItem>
	{
		...
		public int FindAssetIndex( String InAssetFullName )
		{
			...
			if ( DotIndex != -1 )
			{
				String ObjectPathOnly = FullyQualifiedPath.Substring( 0, DotIndex );	// Everything before the first dot
				String ObjectName = FullyQualifiedPath.Substring( DotIndex + 1 );	// Everything after the first dot
				...
				for ( int AssetIndex = 0; AssetIndex < this.Count; ++AssetIndex )
				{
					...
					if( CurAssetItem.AssetType.Equals( AssetClassName, StringComparison.OrdinalIgnoreCase ) &&
						CurAssetItem.Name.Equals( ObjectName, StringComparison.OrdinalIgnoreCase ) &&
						CurAssetItem.PathOnly.Equals( ObjectPathOnly, StringComparison.OrdinalIgnoreCase ) )
					{
						// Found it!
						return AssetIndex;
					}
