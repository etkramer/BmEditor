...
{
	FString ConvertedPath(Path);
	ConvertedPath = ConvertedPath.Replace(TEXT("/"), TEXT("\\"));
	const TCHAR* CheckPath = *ConvertedPath;

	// Does Path refer to a drive letter or BNC path?
	if( appStricmp(CheckPath,TEXT(""))==0 )
	{
		return 1;
	}
	else if( appToUpper(CheckPath[0])!=appToLower(CheckPath[0]) && CheckPath[1]==':' && CheckPath[2]==0 )
	{
		return 1;
	}
	else if( appStricmp(CheckPath,TEXT("\\"))==0 )
	{
		return 1;
	}
	else if( appStricmp(CheckPath,TEXT("\\\\"))==0 )
	...
	else if( CheckPath[0]=='\\' && CheckPath[1]=='\\' && !appStrchr(CheckPath+2,'\\') )
	{
		return 1;
	}
	else if( CheckPath[0]=='\\' && CheckPath[1]=='\\' && appStrchr(CheckPath+2,'\\') && !appStrchr(appStrchr(CheckPath+2,'\\')+1,'\\') )
	{
		return 1;
	}
	else
