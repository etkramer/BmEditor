...
*/
TMap<UPackage*, UBOOL> UObject::StartupPackageToWarnState;
...
#error Please set SUPPORT_NAME_FLAGS to 1 in UnBuild.h if you want to support saving packages
	...
	if( !GIsUCC												&&	// Don't prompt about saving startup packages when running UCC
		...
		FString(Filename).EndsWith(TEXT(".upk"))			&& // Maps, even startup maps, are ok
		IsStartupPackage( *FFilename(Filename).GetBaseFilename(), GEngineIni ) &&
		(!StartupPackageToWarnState.Find(InOuter) || (*(StartupPackageToWarnState.Find(InOuter)) == FALSE))
	  )
	{		
		// Prompt to save startup packages
		INT PopupResult = appMsgf(AMT_YesNo, *FString::Printf(LocalizeSecure(LocalizeUnrealEd("Prompt_AboutToEditStartupPackage"), *(InOuter->GetName()))));

		switch( PopupResult )
		{
		case ART_No:
			StartupPackageToWarnState.Set( InOuter, FALSE );
			Error->Logf( NAME_Warning, LocalizeSecure(LocalizeError(TEXT("CannotSaveStartupPackage"),TEXT("Core")), Filename) );
			return FALSE;
			break;

		case ART_Yes:
			StartupPackageToWarnState.Set( InOuter, TRUE );
			break;
		}
	}
