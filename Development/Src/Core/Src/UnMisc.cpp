...
UBOOL appCreateBitmap( const TCHAR* Pattern, INT Width, INT Height, FColor* Data, FFileManager* FileManager )
{
	...
	{
		extern void DwTriovizImpl_SavePNG(TCHAR * File, INT Width, INT Height, FColor* Data);
		DwTriovizImpl_SavePNG(File, Width, Height, Data);
		return TRUE;
	}
#endif
...
#if ANDROID
	// fix up cooked directory name for texture format
	FixedPath = FixedPath.Replace(TEXT("CookedAndroid"), appAndroidGetCookedDirectoryName());
#endif

#if CONSOLE
	if (GUseCoderMode)
	{
		if (FixedPath.InStr(TEXT("\\Cooked"), FALSE, TRUE) != INDEX_NONE)
		{
			FixedPath += TEXT("_Coder");
		}
	}
	else
#endif
...
const TArray<FString>& appGetKnownLanguageExtensions()
{
	...
	{
#if CONSOLE
		// Hard code the languages on console for now.  GConfig is null the first time the consoles need the known languages
		KnownLanguageExtensions.AddItem( TEXT("INT") );
		KnownLanguageExtensions.AddItem( TEXT("JPN") );
		KnownLanguageExtensions.AddItem( TEXT("DEU") );
		KnownLanguageExtensions.AddItem( TEXT("FRA") );
		KnownLanguageExtensions.AddItem( TEXT("ESM") );
		KnownLanguageExtensions.AddItem( TEXT("ESN") );
		KnownLanguageExtensions.AddItem( TEXT("ITA") );
		KnownLanguageExtensions.AddItem( TEXT("KOR") );
		KnownLanguageExtensions.AddItem( TEXT("CHN") );
		KnownLanguageExtensions.AddItem( TEXT("RUS") );
		KnownLanguageExtensions.AddItem( TEXT("POL") );
		KnownLanguageExtensions.AddItem( TEXT("HUN") );
		KnownLanguageExtensions.AddItem( TEXT("CZE") );
		KnownLanguageExtensions.AddItem( TEXT("SLO") );
		KnownLanguageExtensions.AddItem( TEXT("PTB") );
#else
...
void appInit( const TCHAR* InCmdLine, FOutputDevice* InLog, FOutputDeviceConsole* InLogConsole, FOutputDeviceError* InError, FFeedbackContext* InWarn, FFileManager* InFileManager, FCallbackEventObserver* InCallbackEventDevice, FCallbackQueryDevice* InCallbackQueryDevice, FConfigCacheIni*(*ConfigFactory)() )
{
	...
	// use the normal PC defaults for all windows platforms (dedicated server, etc)
	// NOTE the PC_ prefix on the 2 lines below
	appCreateIniNames( GCompatIni, GDefaultCompatIni, TEXT("DEFCOMPATINI="), TEXT("COMPATINI="), *CompatIniSuffix, PC_DEFAULT_INI_PREFIX, PC_INI_PREFIX );
	appCreateIniNames( GLightmassIni, GDefaultLightmassIni, TEXT("DEFLIGHTMASSINI="), TEXT("LIGHTMASSINI="), TEXT("Lightmass.ini"), PC_DEFAULT_INI_PREFIX, PC_INI_PREFIX );
#endif
	...
	// See if this is a CoderMode run...
	if( ParseParam(appCmdLine(),TEXT("CODERMODE")) == TRUE )
	{
		GUseCoderMode = TRUE;
	}
	...
	if (GUseCoderMode)
	{
		...
		{
			GLog->Logf(NAME_Warning, TEXT("CODERMODE -- Forcing GUseSeekFreeLoading to FALSE!"));
			GLog->Logf(NAME_Warning, TEXT("CODERMODE -- Shader compression will be disabled, resulting in more memory usage!"));
	...
	if ( bEnableSteam )
	{
		...
		debugf(TEXT("STEAMWORKS initialized %d"), GSteamworksInitialized);
	}
	else
	{
		debugf(TEXT("No STEAMWORKS because bEnableSteam==FALSE"));
	}
#endif
...
UE3::EPlatformType appPlatformStringToType(const FString& PlatformStr)
{
	...
	else if (PlatformStr == TEXT("pc") || PlatformStr == TEXT("win32") || PlatformStr == TEXT("windows"))
	{
...
UBOOL appGetCookedContentPath(UE3::EPlatformType InPlatform, FString& OutPath)
{
	...
	// Android needs special handling due to multiple texture format directories
	if (InPlatform == UE3::PLATFORM_Android)
	{
#if ANDROID
		// if we are running on android, then we query the current device's format
		CookedDirName = appAndroidGetCookedDirectoryName();
#else
		// otherwise, we look on the command line
		FString TextureFormat;
		if (!Parse(appCmdLine(), TEXT("-TexFormat="), TextureFormat))
		{
			warnf(TEXT("When cooking for Android, you MUST specify a texture format with -TexFormat=[dxt | pvrtc | atitc]"));
			return FALSE;
		}
		// append the texture format
		CookedDirName = TEXT("CookedAndroid_");
		CookedDirName += TextureFormat;
#endif
	}
	else
	{
...
#if PLATFORM_MACOSX
		// On Macs we use data cooked for PC
		CookedDirName += TEXT("PC");
#else
