...
#if BATMAN
	// https://github.com/EliotVU/Unreal-Library/blob/f7e45802bf69b89ce979af7c2dd8011f8ac6f4a8/src/Core/Classes/Props/UProperty.cs#L128
	if (Ar.IsLoading() && Ar.LicenseeVer() >= VER_BATMAN2)
	{
...
const TCHAR* UArrayProperty::ImportText( const TCHAR* Buffer, BYTE* Data, INT PortFlags, UObject* Parent, FOutputDevice* ErrorText ) const
{
	...
	if ( !ValidateImportFlags(PortFlags,ErrorText) || Buffer == NULL || *Buffer++ != TCHAR('(') )
	{
		return NULL;
	}
...
void UStructProperty_ExportTextItem(class UScriptStruct* InStruct, FString& ValueStr, const BYTE* PropertyValue, const BYTE* DefaultValue, UObject* Parent, INT PortFlags)
{
	INT Count=0;
...
{
	UStructProperty_ExportTextItem(Struct, ValueStr, PropertyValue, DefaultValue, Parent, PortFlags);
} 
