...
void UProperty::Serialize( FArchive& Ar )
{
	...
	if (Ar.LicenseeVer() >= VER_BATMAN2)
	{
		...
		if (Ar.IsSaving())
		{
			RemapBmPropertyFlags(SerializedPropertyFlags, FALSE);
		}

		Ar << SerializedPropertyFlags;

		if (Ar.IsLoading())
		{
			PropertyFlags = SerializedPropertyFlags;
			RemapBmPropertyFlags(PropertyFlags, TRUE);
		}
	}
	else
	{
