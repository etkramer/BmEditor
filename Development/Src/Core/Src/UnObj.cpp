...
{
	FStateFrame** Found = Map.Find(Object);
	return Found ? *Found : NULL;
}
void FObjectStateFrameManager::Set(const UObject* Object, FStateFrame* Frame)
{
	if (Frame == NULL)
	{
		Map.Remove(Object);
	...
	{
		Map.Set(Object, Frame);
	}
}
void FObjectStateFrameManager::Remove(const UObject* Object)
{
	Map.Remove(Object);
}

INT FObjectNetIndexManager::Get(const UObject* Object)
{
	const INT* Found = Map.Find(Object);
	return Found ? *Found : (INT)INDEX_NONE;
}
void FObjectNetIndexManager::Set(const UObject* Object, INT Value)
{
	if (Value == INDEX_NONE)
	{
		Map.Remove(Object);
...
void FObjectNetIndexManager::Remove(const UObject* Object)
{
	Map.Remove(Object);
}
...
,	ObjectArchetype	( NULL													)
{
	...
	*(const TCHAR  **)&Outer        = InPackageName;
	GObjAutoRegisterNext.Set(this, GAutoRegister);
	GAutoRegister                   = this;
...
{
	ULinkerLoad* OldLinker = GetLinker();
	PTRINT OldLinkerIndex = GetLinkerIndex();
	...
	if( OldLinker )
	{
		...
		check(OldLinker->ExportMap(OldLinkerIndex)._Object!=NULL);
		check(OldLinker->ExportMap(OldLinkerIndex)._Object==this);
		OldLinker->ExportMap(OldLinkerIndex)._Object = NULL;
	}
...
void UObject::SerializeNetIndex(FArchive& Ar)
{
	...
	{
		INT InNetIndex = GetNetIndex();
		Ar << InNetIndex;
		if (Ar.IsLoading())
		{
			ULinkerLoad* Linker = GetLinker();
			PTRINT LinkerIdx = GetLinkerIndex();
#if SUPPORTS_SCRIPTPATCH_CREATION
			//@script patcher
			if (GIsScriptPatcherActive || Linker == NULL || Linker->LinkerRoot == NULL || (Linker->LinkerRoot->PackageFlags & PKG_Cooked))
#else
			if (Linker == NULL || Linker->LinkerRoot == NULL || (Linker->LinkerRoot->PackageFlags & PKG_Cooked))
#endif
			{
			...
			else if (Linker != NULL && LinkerIdx != INDEX_NONE)
			{
				SetNetIndex((INT)LinkerIdx);
			}
		}
	}
}
	...
	if( !Ar.IsTransacting() )
	{
		...
		if( HasAnyFlags(RF_HasStack) )
		{
			if( !StateFrame )
			{
				...
				FObjectStateFrameManager::Set(this, StateFrame);
			}
...
void UObject::InitExecution()
{
	if ( !HasAnyFlags(RF_ClassDefaultObject) )
	{
		...
		FStateFrame* OldSF = GetStateFrame();
		if( OldSF )
		{
			delete OldSF;
		}
		FObjectStateFrameManager::Set(this, new FStateFrame( this ));
		SetFlags( RF_HasStack );
	}
}
...
UObject* UObject::StaticFindObject( UClass* ObjectClass, UObject* InObjectPackage, const TCHAR* OrigInName, UBOOL ExactClass )
{
	...
	MatchingObject = StaticFindObjectFast( ObjectClass, ObjectPackage, ObjectName, ExactClass, InObjectPackage==ANY_PACKAGE );

	// Attempt to dynamically load the object in CODERMODE
	if (!MatchingObject && GUseCoderMode && ObjectClass
	// don't bother if we're attempting to find a package - packages can't be loaded via SLO	
	&& appStrchr(OrigInName, TEXT('.')) != NULL )
	{
		// add LOAD_Quiet so that we don't get tons of warnings about unable to find objects when testing for existence
		// (i.e. Failed to find object 'Engine.TheWorld' when checking whether the package contains a map)
		MatchingObject = UObject::StaticLoadObject(ObjectClass, NULL, OrigInName, NULL,  LOAD_NoWarn|LOAD_Quiet, NULL);
	}
...
void UObject::ProcessRegistrants()
{
	...
	while( GAutoRegister )
	{
		UObject* Cur = GAutoRegister;
		UObject** NextPtr = GObjAutoRegisterNext.Find(Cur);
		GAutoRegister = NextPtr ? *NextPtr : NULL;
		GObjAutoRegisterNext.Remove(Cur);
		ObjRegistrants.AddItem( Cur );
	}
...
static void PrivateRecursiveDumpFlags(UStruct* Struct, BYTE* Data, FOutputDevice& Ar)
{
	...
	{
		if ( It->GetOwnerClass()->GetPropertiesSize() != sizeof(UObject::StaticClass()) )
		{
...
void UObject::BeginLoad()
{
	if( ++GObjBeginLoadCount == 1 )
	{
		...
		FlushAsyncLoading( NAME_None, TRUE );
...
)
{
	...
	else
	{
		...
		LinkerIndex = (INT)Obj->GetLinkerIndex();
		InFlags		|= Obj->GetMaskedFlags(RF_Keep);
		Index		= Obj->Index;
		OldNetIndex = Obj->GetNetIndex();
	...
	FObjectNetIndexManager::Remove(Obj);
...
UObject* UObject::StaticDuplicateObjectEx( FObjectDuplicationParameters& Parameters )
{
	...
	checkf( (Parameters.DestClass->GetPropertiesSize() >= Parameters.SourceObject->GetClass()->GetPropertiesSize()) || (GUglyHackFlags & HACK_AllowDifferingClassSizesWhenDuplicatingObjects),
