...
void appRequestExit( UBOOL Force )
{
	...
	if( Force )
	{
		...
		// Dangerous because config code isn't flushed, global desutructors aren't called, etc.
...
void appPlatformInit()
{
	...
	debugf( NAME_Init, TEXT("Memory total: Physical=%.1fGB Pagefile=%.1fGB Virtual=%.1fGB"), FLOAT(M.ullTotalPhys/1024.0/1024.0/1024.0), FLOAT(M.ullTotalPageFile/1024.0/1024.0/1024.0), FLOAT(M.ullTotalVirtual/1024.0/1024.0/1024.0) );
		...
		while( PeekMessage(&Msg,NULL,0,0,PM_REMOVE) )
		{
				 ...
				 ImmIsUIMessage(NULL, Msg.message, Msg.wParam, Msg.lParam)))
			{
				...
				if (GameViewportClient && GameViewportClient->ScaleformInteraction)
				{
					...
					{
						GFxIMEWin32Event ev(GFxIMEWin32Event::IME_PreProcessKeyboard,
 ...
 */
void FMallocTBB::GetAllocationInfo( FMemoryAllocationStats& MemStats )
{
#if !CONSOLE
