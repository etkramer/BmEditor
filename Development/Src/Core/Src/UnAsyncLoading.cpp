 ...
 * @param	bTriggerTextureStreaming	If TRUE, triggers a full texture streaming update next tick (if any packages were flushed)
 */
void UObject::FlushAsyncLoading(FName ExcludeType/*=NAME_None*/, UBOOL bTriggerTextureStreaming/*=FALSE*/)
{
	if( GObjAsyncPackages.Num() )
	{
		...
		{
			// Trigger a full texture streaming update next tick.
			(*GTriggerTextureStreaming)();
		}
...
void FAsyncIOSystemBase::FulfillCompressedRead( const FAsyncIORequest& IORequest, const FAsyncIOHandle& FileHandle )
{
	...
	InternalRead( FileHandle, IORequest.Offset + sizeof(HeaderData), ChunkInfoSize, CompressionChunks );
	RETURN_IF_EXIT_REQUESTED;
	...
	CompressedBuffer[0]	= appMalloc( MaxCompressedSize + Padding );
	CompressedBuffer[1] = appMalloc( MaxCompressedSize + Padding );
