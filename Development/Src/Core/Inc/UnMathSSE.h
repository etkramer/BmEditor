...
#include <xmmintrin.h>
	// Define an additional intrinsic that isn't in the Mac header, but is trivial to define here
	#define move_mask(Vec) _mm_movemask_ps(Vec)
#endif
...
#if __GNUC__
	*((DWORD*)Ptr) = (DWORD) _mm_cvtsi64_si32(NewVec);
#else
 ...
 */
#define VectorAnyGreaterThan( Vec1, Vec2 )		move_mask( _mm_cmpgt_ps(Vec1, Vec2) )
...
inline UBOOL VectorContainsNaNOrInfinite(const VectorRegister& Vec)
{
	...
	UBOOL IsNotNAN = move_mask(_mm_cmpeq_ps(Vec, Vec)); // Test for the fact that NAN != NAN
