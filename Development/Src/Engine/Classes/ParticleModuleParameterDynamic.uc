...
cpptext
{
	 ...
	 */
	inline FLOAT GetParameterValue(INT ParamIndex, FBaseParticle& Particle, FParticleEmitterInstance* Owner, class FRandomStream* InRandomStream)
	{
		...
		if ((DynParams.bScaleVelocityByParamValue == TRUE) || (DynParams.ValueMethod == EDPV_UserSet))
		{
			DistributionValue = (DynParams.bUseEmitterTime ? 
				DynParams.ParamValue.GetValue(Owner->EmitterTime, Owner->Component, (DynParams.bSpawnTimeOnly == FALSE) ? NULL : InRandomStream) :
				DynParams.ParamValue.GetValue(Particle.RelativeTime, Owner->Component, (DynParams.bSpawnTimeOnly == FALSE) ? NULL : InRandomStream));
		}
