...
void FFullScreenMovieBink::GameThreadPlayMovie(EMovieMode InMovieMode, const TCHAR* InMovieFilename, INT InStartFrame, INT InStartOfRenderingMovieFrame, INT InEndOfRenderingMovieFrame)
{
	...
	// Remember the name of the movie we're playing so kismet can use it
	...
	FString FullPath = appGameDir() + TEXT("Movies\\") + GameThreadMovieName + TEXT(".bik");
	if ( GFileManager->FileSize(*FullPath) < 0 )
	{
		debugf(NAME_DevMovie,  TEXT( "Attempting to play a movie that doesn't exist \"%s\"; aborting" ), *GameThreadMovieName );
		return;
	}
...
// max frame rate to run on consoles
const INT MAX_FRAME_RATE=30;
const INT MAX_FRAME_RATE_DIVISOR=1;
...
UBOOL FFullScreenMovieBink::OpenStreamedMovie(const TCHAR* MoviePath)
{
	...
	Filename = *GFileManager->GetPlatformFilepath(*Filename);
#endif

	DWORD BinkOpenFlags = BINKSNDTRACK | BINKNOFRAMEBUFFERS;
#if CONSOLE
	// always run at MAX_FRAME_RATE on consoles
	BinkSetFrameRate(MAX_FRAME_RATE,MAX_FRAME_RATE_DIVISOR);
	BinkOpenFlags |= BINKFRAMERATE;
#endif
 ...
 */
UBOOL FFullScreenMovieBink::PlayMovie(EMovieMode InMovieMode, const TCHAR* MovieFilename, void* Buffer, DWORD BufferSize, INT StartFrame, INT bIsSkippable, INT InStartOfRenderingMovieFrame, INT InEndOfRenderingMovieFrame)
{
	check(!Bink);
	check(BinkRender);
	...
	FString SubtitlePath = appGameDir() + TEXT("Movies\\") + BaseMovieName + TEXT(".txt");
	SubtitleStorage->Load(SubtitlePath);

	// use the region name from the startup movies if available
	FString FullPath = appGameDir() + TEXT("Movies\\") + BaseMovieName + TEXT(".bik");
...
{
	check(Font && Font->HasAllFlags(RF_RootSet));
...
#else
	// Bink does not downmix XAudio2 into stereo, so continue to use DirectSound in that case
	UXAudio2Device::InitHardware();
	if( UXAudio2Device::NumSpeakers > 2 )
	{
		BinkSoundUseXAudio2(UXAudio2Device::XAudio2);
	}
	else
...
{
	INT FrontLeft = SoundTrack_FrontLeft;
	INT FrontCenter = SoundTrack_DefaultCenter;
	FString LangExt = appGetLanguageExt();	

	// Ideally, we should append _LOC to any movies that need loc
	if( appStristr( MovieName, TEXT( "EpicLogo" ) ) != NULL || 
		appStristr( MovieName, TEXT( "MGSLogo" ) ) != NULL || 
		appStristr( MovieName, TEXT( "ESRB" ) ) != NULL || 
		appStristr( MovieName, TEXT( "LoadingMovie" ) ) != NULL )
	{
		// Regular INT 5.1 audio in channels 0 to 5, or simple stereo
	}
	else
	{
		if( LangExt == TEXT( "FRA" ) )
	...
	BinkSoundTracks[SoundTrack_FrontLeft] = FrontLeft;
	BinkSoundTracks[SoundTrack_FrontRight] = FrontLeft + 1;
	BinkSoundTracks[SoundTrack_BackLeft] = SoundTrack_BackLeft;
	BinkSoundTracks[SoundTrack_BackRight] = SoundTrack_BackRight;
	BinkSoundTracks[SoundTrack_LowFrequency] = SoundTrack_LowFrequency;
	BinkSoundTracks[SoundTrack_DefaultCenter] = FrontCenter;

    const UINT NumBinkSoundTracks = ARRAY_COUNT(BinkSoundTracks);
    BinkSetSoundTrack(NumBinkSoundTracks, BinkSoundTracks);
...
void FBinkMovieAudio::HandleXAudio2Volumes(BINK* Bink)
{
	...
	// set mix bins for each track
	UINT AudioBins[2];
	if ( Bink->NumTracks >= 6 )
	{
		for (UINT Index = 0; Index < NumBinkSoundTracks; ++Index)
		{
			VolumeArray[Index] = BinkFoleyVolume;
			UINT SoundTrackID = BinkSoundTracks[Index];
			switch(SoundTrackID)
			{
			case SoundTrack_FrontLeft:
				AudioBins[0] = SPEAKER_FrontLeft;
				break;

			case SoundTrack_FrontRight:
				AudioBins[0] = SPEAKER_FrontRight;
				break;

			case SoundTrack_BackLeft:
				AudioBins[0] = SPEAKER_LeftSurround;
				break;

			case SoundTrack_BackRight:
				AudioBins[0] = SPEAKER_RightSurround;
				break;

			case SoundTrack_LowFrequency:
				AudioBins[0] = SPEAKER_LowFrequency;
				break;

			default:
				VolumeArray[Index] = BinkVoiceVolume;
				AudioBins[0] = SPEAKER_FrontCenter;
				break;
			}

			BinkSetSpeakerVolumes( Bink, SoundTrackID, AudioBins, VolumeArray, 1 );
		}
	}
	else if ( Bink->NumTracks == 3 )
	{
		BinkSetVolume(Bink, 0, BinkFoleyVolume);
		AudioBins[0] = SPEAKER_FrontLeft;
		AudioBins[1] = SPEAKER_FrontRight;
		BinkSetMixBins(Bink, 0, AudioBins, 2);

		BinkSetVolume(Bink, 1, BinkFoleyVolume);
		AudioBins[0] = SPEAKER_LeftBack;
		AudioBins[1] = SPEAKER_RightBack;
		BinkSetMixBins(Bink, 1, AudioBins, 2);

		BinkSetVolume(Bink, 2, BinkFoleyVolume);
		AudioBins[0] = SPEAKER_FrontCenter;
		AudioBins[1] = SPEAKER_LowFrequency;
		BinkSetMixBins(Bink, 2, AudioBins, 2);
	}
	else if ( Bink->NumTracks > 0 )
	{
		BinkSetVolume(Bink, 0, BinkFoleyVolume);
		AudioBins[0] = SPEAKER_FrontLeft;
		AudioBins[1] = SPEAKER_FrontRight;
...
void FBinkMovieAudio::HandleMultiStreamVolumes(BINK* Bink)
{
	...
	if ( Bink->NumTracks >= 6 && UPS3AudioDevice::NumSpeakers >= 6)
	{
		...
		for (UINT Index = 0; Index < NumBinkSoundTracks; ++Index)
		{
			...
			{
			case SoundTrack_FrontLeft:
				BinkSetVolume(Bink, SoundTrackID, BinkFoleyVolume);
				VolumeArray[CELL_MS_SPEAKER_FL] = MAX_VOLUME;
				BinkSetMixBinVolumes(Bink, SoundTrackID, 0, VolumeArray, Bink->NumTracks);
				VolumeArray[CELL_MS_SPEAKER_FL] = 0;
				break;

			case SoundTrack_FrontRight:
				BinkSetVolume(Bink, SoundTrackID, BinkFoleyVolume);
				VolumeArray[CELL_MS_SPEAKER_FR] = MAX_VOLUME;
				BinkSetMixBinVolumes(Bink, SoundTrackID, 0, VolumeArray, Bink->NumTracks);
				VolumeArray[CELL_MS_SPEAKER_FR] = 0;
				break;

			case SoundTrack_BackLeft:
				BinkSetVolume(Bink, SoundTrackID, BinkFoleyVolume);
				VolumeArray[CELL_MS_SPEAKER_RL] = MAX_VOLUME;
				BinkSetMixBinVolumes(Bink, SoundTrackID, 0, VolumeArray, Bink->NumTracks);
				VolumeArray[CELL_MS_SPEAKER_RL] = 0;
				break;

			case SoundTrack_BackRight:
				BinkSetVolume(Bink, SoundTrackID, BinkFoleyVolume);
				VolumeArray[CELL_MS_SPEAKER_RR] = MAX_VOLUME;
				BinkSetMixBinVolumes(Bink, SoundTrackID, 0, VolumeArray, Bink->NumTracks);
				VolumeArray[CELL_MS_SPEAKER_RR] = 0;
				break;

			case SoundTrack_LowFrequency:
				BinkSetVolume(Bink, SoundTrackID, BinkFoleyVolume);
				VolumeArray[CELL_MS_SPEAKER_LFE] = MAX_VOLUME;
				BinkSetMixBinVolumes(Bink, SoundTrackID, 0, VolumeArray, Bink->NumTracks);
				VolumeArray[CELL_MS_SPEAKER_LFE] = 0;
				break;

			default:
				BinkSetVolume(Bink, SoundTrackID, BinkVoiceVolume);
				VolumeArray[CELL_MS_SPEAKER_FC] = MAX_VOLUME;
				BinkSetMixBinVolumes(Bink, SoundTrackID, 0, VolumeArray, Bink->NumTracks);
				VolumeArray[CELL_MS_SPEAKER_FC] = 0;
				break;
			}
		}
	}
...
void FBinkMovieAudio::HandleDirectSoundVolumes(BINK* Bink)
{
	...
	// set volume levels for each track

	// 5.1 is well supported in Windows, so we just need to pan the channels to give stereo separation
	// (this will in effect mix 5.1 down to stereo for l/r and mono for center/lfe
	if ( Bink->NumTracks >= 6 )
	{
		for (UINT Index = 0; Index < NumBinkSoundTracks; ++Index)
		{
			...
			case SoundTrack_FrontLeft:
				BinkSetVolume(Bink, SoundTrackID, BinkFoleyVolume);
				BinkSetPan(Bink, SoundTrackID, 0);
				break;

			case SoundTrack_FrontRight:
				BinkSetVolume(Bink, SoundTrackID, BinkFoleyVolume);
				BinkSetPan(Bink, SoundTrackID, 65535);
				break;
			...
			case SoundTrack_LowFrequency:
				BinkSetVolume(Bink, SoundTrackID, BinkFoleyVolume);
				BinkSetPan(Bink, SoundTrackID, 32768);
				break;
	...
	else
	{
		...
		{
			BinkSetVolume(Bink, TrackIndex, BinkFoleyVolume);
			BinkSetPan(Bink, TrackIndex, 32768);
		}
	}
}
#endif
 ...
 */
void FBinkMovieRenderClient::RenderFrame(BINK* Bink, const TCHAR* Subtitle, const TCHAR* InPauseText, UBOOL bLoadingMovie, const TArray<FFullScreenMovieOverlay>& TextOverlays)
{
	...
	if( GEngine && GEngine->Client )
	{
		...
		if( InPauseText != NULL || bLoadingMovie || bNeedsNormalSubtitle || TextOverlays.Num())
		{
			...
			{
				FIntRect Parms( 
					appTrunc(Viewport->GetSizeX()*0.1f), 
					appTrunc(Viewport->GetSizeY()*0.1f), 
					appTrunc(Viewport->GetSizeX()*0.9f), 
					appTrunc(Viewport->GetSizeY()*0.9f)
					);
				FSubtitleManager::GetSubtitleManager()->DisplaySubtitleWordWrapped( &Canvas, Subtitle, Parms, FLinearColor::White );
			}
			...
			for (INT OverlayIndex = 0; OverlayIndex < TextOverlays.Num(); OverlayIndex++)
			{
				...
				const INT PixelsWrapWidth = appTrunc( ( FLOAT )Viewport->GetSizeX() * Overlay.WrapWidth );
				...
				{
					// @todo: Doesn't support scaled text with Overlay.ScaleX and Overlay.ScaleY!
					INT Center = Viewport->GetSizeX() / 2;
					DrawStringCentered( &Canvas, Center - 1, PixelsY - 1, *Overlay.Text, Overlay.Font, FLinearColor::Black );
					DrawStringCentered( &Canvas, Center - 1, PixelsY + 1, *Overlay.Text, Overlay.Font, FLinearColor::Black );
					DrawStringCentered( &Canvas, Center + 1, PixelsY + 1, *Overlay.Text, Overlay.Font, FLinearColor::Black );
					DrawStringCentered( &Canvas, Center + 1, PixelsY - 1, *Overlay.Text, Overlay.Font, FLinearColor::Black );
					DrawStringCentered( &Canvas, Center, PixelsY, *Overlay.Text, Overlay.Font, FLinearColor::White );
				}
				else if( Overlay.bIsWrapped )
				{
					// @todo: Doesn't support scaled text with Overlay.ScaleX and Overlay.ScaleY!
					FLOAT XL = PixelsWrapWidth;
					FLOAT YL = 0;
					DrawStringWrapped( &Canvas, TRUE, PixelsX - 1, PixelsY - 1, XL, YL, Overlay.Font, *Overlay.Text, FLinearColor::Black );
					XL = PixelsWrapWidth;
					YL = 0;
					DrawStringWrapped( &Canvas, TRUE, PixelsX - 1, PixelsY + 1, XL, YL, Overlay.Font, *Overlay.Text, FLinearColor::Black );
					XL = PixelsWrapWidth;
					...
					XL = PixelsWrapWidth;
					YL = 0;
					DrawStringWrapped( &Canvas, TRUE, PixelsX, PixelsY, XL, YL, Overlay.Font, *Overlay.Text, FLinearColor::White );
				}
				else
				{
					// Cheat the viewport height for the purposes of font scale so that we get a higher resolution font
					// for scaled text
					const FLOAT ForcedViewportHeight = Viewport->GetSizeY() * CorrectedScaleY;
					CorrectedScaleX /= CorrectedScaleY;
					CorrectedScaleY = 1.0f;

					INT Center = Viewport->GetSizeX() / 2;
					DrawString( &Canvas, PixelsX - 1, PixelsY - 1, *Overlay.Text, Overlay.Font, FLinearColor::Black, CorrectedScaleX, CorrectedScaleY, 0.0f, &ForcedViewportHeight );
					DrawString( &Canvas, PixelsX - 1, PixelsY + 1, *Overlay.Text, Overlay.Font, FLinearColor::Black, CorrectedScaleX, CorrectedScaleY, 0.0f, &ForcedViewportHeight );
					DrawString( &Canvas, PixelsX + 1, PixelsY + 1, *Overlay.Text, Overlay.Font, FLinearColor::Black, CorrectedScaleX, CorrectedScaleY, 0.0f, &ForcedViewportHeight );
					DrawString( &Canvas, PixelsX + 1, PixelsY - 1, *Overlay.Text, Overlay.Font, FLinearColor::Black, CorrectedScaleX, CorrectedScaleY, 0.0f, &ForcedViewportHeight );
					DrawString( &Canvas, PixelsX, PixelsY, *Overlay.Text, Overlay.Font, FLinearColor::White, CorrectedScaleX, CorrectedScaleY, 0.0f, &ForcedViewportHeight );
				}
			}
			...
			if( InPauseText != NULL )
			{
				FIntRect Parms( 
					...
					appTrunc(Viewport->GetSizeY()*0.1f), 
					appTrunc(Viewport->GetSizeX()*0.9f), 
					appTrunc(Viewport->GetSizeY()*0.5f)
					);

				FSubtitleManager::GetSubtitleManager()->DisplaySubtitleWordWrapped( &Canvas, InPauseText, Parms, FLinearColor::White );
			}
