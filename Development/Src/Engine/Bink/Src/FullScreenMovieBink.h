...
private:
	 ...
	 */
	UBOOL PlayMovie(EMovieMode InMovieMode, const TCHAR* MovieFilename, void* Buffer=NULL, DWORD BufferSize=0, INT StartFrame=0, INT bIsSkippable=FALSE, INT InStartOfRenderingMovieFrame=-1, INT InEndOfRenderingMovieFrame=-1);
...
{
public:
	enum EBinkSoundTrack 
	{
		SoundTrack_FrontLeft     = 0, // Mono channel for front left
		SoundTrack_FrontRight    = 1, // Mono channel for front right
		SoundTrack_BackLeft      = 2, // Mono channel for rear left
		SoundTrack_BackRight     = 3, // Mono channel for rear right
		SoundTrack_LowFrequency  = 4, // Mono channel for sub-woofer
		SoundTrack_DefaultCenter = 5, // Mono channel for center channel (alternate center channel tracks follow)
	};
...
public:
	 ...
	 */
	void RenderFrame(BINK* Bink, const TCHAR* Subtitle, const TCHAR* InPauseText, UBOOL bLoadingMovie, const TArray<FFullScreenMovieOverlay>& TextOverlays);
