...
#endif
	ApexAsset(const char *fname,const void *mem,physx::PxU32 dlen,const char *originalName,UApexAsset *ue3Object);
		...
		if ( iface )
		{
			...
			if ( handle.getArraySize(NumSubmeshes,0) == ::NxParameterized::ERROR_NONE )
			{
				...
				for (physx::PxI32 i = 0; i < NumSubmeshes; i++)
				{
					...
					physx::string::sprintf_s(scratch,512,"submeshes[%d].vertexBuffer.vertexFormat.bufferFormats",i);
					...
					{
						physx::string::sprintf_s(scratch,512,"submeshes[%d].vertexBuffer.vertexFormat.bufferFormats[%d].semantic",i,j);
						iface = NxParameterized::findParam(*ApexRenderMeshAssetAuthoring, scratch, handle);
						...
						if (iface)
						{
								...
								&& bufSemantic >= physx::NxRenderVertexSemantic::TEXCOORD0 )
							{
								...
								physx::string::sprintf_s(scratch,512,"submeshes[%d].vertexBuffer.buffers[%d].data",i,j);
								iface = NxParameterized::findParam(*ApexRenderMeshAssetAuthoring, scratch, handle);
	...
	virtual UBOOL ApplyTransformation(const physx::PxMat44 &transformation,physx::PxF32 scale,UBOOL applyToGraphics,UBOOL applyToPhysics,UBOOL /*clockWise*/)
  	{
		...
		if ( MType == AAT_DESTRUCTIBLE && MApexAsset )
		{
			...
			if( ensure(ApexDestructibleAssetAuthoring) )
			{
				...
				physx::string::strncpy_s(ApexAssetName, 1024, ApexDestructibleAsset->getName(), sizeof(ApexAssetName));
			...
			NxClothingAssetAuthoring *ApexClothingAssetAuthoring = static_cast<NxClothingAssetAuthoring*>(ApexAuthoringFromAsset(MApexAsset, apexSDK));
   			...
   			if(ApexClothingAssetAuthoring)
   			{
    			...
    			if(ApexClothingAssetAuthoring)
    			{
    				...
    				// cache off asset name...
    				char ApexAssetName[1024] = {0};
					physx::string::strncpy_s(ApexAssetName,1024, ApexClothingAsset->getName(), sizeof(ApexAssetName));
			...
			if(ApexClothingAssetAuthoring)
			{
				...
				for (physx::PxU32 i=0; i<256; i++)
				{
					...
					if ( ApexRenderMeshAssetAuthoring )
					{
						...
						{
							char ibName[512];
							physx::string::sprintf_s(ibName,512,"submeshes[%d].indexBuffer",i);
						...
						// reverse normals
						{
							...
							physx::string::sprintf_s(vbName,512,"submeshes[%d].vertexBuffer.vertexFormat.bufferFormats",i);
							...
							for ( physx::PxI32 j=0; j<numFormats; j++ )
							{
								...
								physx::string::sprintf_s(vbName,512,"submeshes[%d].vertexBuffer.vertexFormat.bufferFormats[%d].semantic",i,j);
								iface = NxParameterized::findParam( *ApexRenderMeshAssetAuthoring, vbName, handle );
								...
								if ( physx::NxRenderVertexSemantic::NORMAL == semantic )
								{
									...
									physx::string::sprintf_s(vbName,512,"submeshes[%d].vertexBuffer.vertexFormat.bufferFormats[%d].format",i,j);
									iface = NxParameterized::findParam( *ApexRenderMeshAssetAuthoring, vbName, handle );
									...
									physx::string::sprintf_s(vbName,512,"submeshes[%d].vertexBuffer.buffers[%d].data",i,j);
									iface = NxParameterized::findParam( *ApexRenderMeshAssetAuthoring, vbName, handle );
									...
									if ( handle.getArraySize(size) == NxParameterized::ERROR_NONE )
									{
										...
										physx::PxMat33 mirrorTransform = -physx::PxMat33::getIdentity();
	...
	virtual UBOOL GetDesc(char *dest,physx::PxU32 slen)
	{
		...
		if ( MApexAsset )
		{
			...
			case AAT_APEX_EMITTER:		// asset used to construct emitters.
				type = "Shape Emitter";
				break;
			case AAT_CLOTHING_MATERIAL:	// asset used to represent clothing material library data.
				type = "Clothing Material";
				break;
			...
			physx::string::sprintf_s(dest,slen,"%s", type );
		...
		{
			physx::string::sprintf_s(dest,slen,"NO ASSET");
		}
		return ret;
	}
...
#define MODULE_COUNT 11
			...
			cmd = GetCommand(cmd,command);
			for (physx::PxU32 sno=0; sno<mApexScenes.size(); sno++)
			{
				...
				physx::PxF32 visScale = 1.0f;

				UBOOL debugOffApex  = strcmp(command,"APEX_CLEAR_ALL") == 0;
				for (physx::PxU32 m=0; m<=MODULE_COUNT; m++)
				{
					...
					if ( debugRenderParams )
					{
						...
						if ( results )
						{
							for (physx::PxU32 i=0; i<count; i++)
							{
								const NxParameterized::ParamResult result = results[i];

								if ( strcmp(result.mName,"VISUALIZATION_ENABLE") == 0 ||
									 strcmp(result.mName,"VISUALIZATION_SCALE") == 0  ||
									 strcmp(result.mName,"VISUALIZE_ACTOR") == 0  ||
									 strcmp(result.mName,"VISUALIZE_EMITTER_ACTOR") == 0  ||
									 strcmp(result.mName,"VISUALIZE_DESTRUCTIBLE_ACTOR") == 0  ||
									 strcmp(result.mName,"VISUALIZE_IOFX_ACTOR") == 0  ||
									 strcmp(result.mName,"VISUALIZE_GROUND_EMITTER_ACTOR") == 0  ||
									 strcmp(result.mName,"VISUALIZE_IMPACT_EMITTER_ACTOR") == 0  ||
									 strcmp(result.mName,"VISUALIZE_BASIC_IOS_ACTOR") == 0  ||
									 strcmp(result.mName,"VISUALIZE_FLUID_IOS_ACTOR") == 0  ||
									 strcmp(result.mName,"VISUALIZE_CLOTHING_ACTOR") == 0 )
								{
									continue;
								}
								if ( debugOffApex )
								{
									switch ( result.mDataType )
									{
										case NxParameterized::TYPE_BOOL:
											{
												NxParameterized::setParamBool(*debugRenderParams,result.mLongName,false);
								...
								{
									if ( strcmp(result.mName,command) == 0 )
									{
										physx::PxF32 vis=0;
										switch ( result.mDataType )
										{
										case NxParameterized::TYPE_BOOL:
											{
												bool b;
												NxParameterized::getParamBool(*debugRenderParams,result.mLongName,b);
												vis = b ? 1.0f : 0.0f;
											}
											break;
										case NxParameterized::TYPE_F32:
										...
										{
											case NxParameterized::TYPE_BOOL:
												NxParameterized::setParamBool(*debugRenderParams,result.mLongName, vis > 0 );
												break;
											case NxParameterized::TYPE_F32:
												NxParameterized::setParamF32(*debugRenderParams,result.mLongName,vis);
												break;
											case NxParameterized::TYPE_REF:
												// do nothing
												break;
							...
							NxParameterized::releaseParamList(count,results,MApexSDK->getParameterizedTraits());
						}
					}
				}
				...
				NxParameterized::setParamF32( *debugRenderParams, 	"VISUALIZATION_SCALE", 	visScale );
	 ...
	 * Returns the Apex Asset from memory
  	...
  	**/
  	virtual FIApexAsset* GetApexAssetFromMemory(const char *fname,const void *data,physx::PxU32 dlen,const char *originalName,UApexAsset *obj)
  	{
		...
		if ( aa )
		{
        	aa->LoadFromMemory(data,dlen,fname,originalName,obj);
        	ret = static_cast< FIApexAsset *>(aa);
		}
		else
		{
        	ApexAsset *aa = new ApexAsset(fname,data,dlen,originalName,obj);
        	ret = static_cast< FIApexAsset *>(aa);
	...
	virtual ApexAssetFormat GetApexAssetFormat(const void *Buffer,physx::PxU32 dlen,physx::PxU32 &numObj)
	{
		...
		if (headerVersion >= 10 && headerVersion <= 48 && stringLen < 256 )
		{
			...
			if ( slen == stringLen && isAsciiText((const unsigned char *)scratch) )
			{
				...
				else if ( strcmp(scratch,NX_CLOTHING_MATERIAL_AUTHORING_TYPE_NAME) == 0 )
				{
					ret = AAF_CLOTHING_MATERIAL_LEGACY;
				}
				...
				if ( err == NxParameterized::Serializer::ERROR_NONE )
				{
					if ( numObj == 1 )
					{
						...
						if ( stype == ::NxParameterized::Serializer::NST_XML )
						{
							...
							physx::string::strncpy_s(scratch,1024,(const char *)Buffer,clen);
							scratch[1023] = 0;
...
**/
ApexAsset::ApexAsset(const char *fname,const void *data,physx::PxU32 dlen,const char *originalName,UApexAsset *obj)
{
#if WITH_APEX_PARTICLES
	mEdited = FALSE;
	mApexEditInterface = NULL;
	...
	memcpy(MName,fname,slen+1);
  	...
  	MType = AAT_LAST;
  	MApexAsset = NULL;
	MTransformation = physx::PxMat44::getIdentity();
	...
	switch ( type )
	{
		...
		case AAT_FLUID_IOS:
  		...
  		case AAT_IOFX:
  			...
  			break;
  		case AAT_CLOTHING_MATERIAL:	// asset used to represent clothing material library data.
  			assetName = NX_CLOTHING_MATERIAL_AUTHORING_TYPE_NAME;
  			break;
  		case AAT_RENDER_MESH:
...
UBOOL ApexAsset::LoadFromMemory(const void *data,physx::PxU32 dlen,const char *assetName,const char *originalName,UApexAsset *obj)
{
	...
	if ( originalName == NULL || strlen(originalName) == 0 )
	{
...
#if 0
					if( GIsEditor && !GIsGame && bIsUpdated && obj )
					{
						// APEX asset was upgraded in memory to the latest version, need a resave.
	...
	if ( MApexAsset )
	{
		...
		else if ( strcmp(typeName,NX_CLOTHING_MATERIAL_AUTHORING_TYPE_NAME) == 0 )
		{
			MType = AAT_CLOTHING_MATERIAL;
		}
