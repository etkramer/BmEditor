...
enum ESceneRenderTargetTypes
{
	...
	// Render target for SeparateTranslucency (R:depth, needs to be normalized with SeparateTranslucency.a)
	...
	PreviousFrameBackBuffer,
	// A copy of the current frame's back buffer.
	CurrentFrameBackBuffer,

	// optional ones at the bottom

#if WITH_GFx
	// A8R8G8B8 version of the SceneColor texture memory
	SceneColorARGB,
#endif
...
// 0:off 1:experimental (for BokehDOF translucency)
#define ENABLE_SEPARATE_TRANSLUCENCY_DEPTH		0
...
public:
	 ...
	 **/
	void SetBackBuffer( FSurfaceRHIParamRef InBackBuffer );
...
#if WITH_GFx
	const FTexture2DRHIRef& GetSceneColorARGBTexture() const { return RenderTargets[SceneColorARGB].Texture; }
#endif
	...
	*/
...
#if WITH_GFx
	const FSurfaceRHIRef& GetSceneColorARGBSurface() const						{ return RenderTargets[SceneColorARGB].Surface; }
#endif
	...
	{
		return GSystemSettings.RenderThreadSettings.bAllowHardwareShadowFiltering && GSupportsFetch4;
	}
	...
	UBOOL IsPreviousFrameBackBufferValid(UINT CurrentFrameNumber) const { return (PreviousFrameBackBufferFrameIndex + 1) == CurrentFrameNumber; }
...
protected:
	...
	FSceneRenderTargets(): 
		...
		PreviousFrameBackBufferFrameIndex(MININT)
		{}
...
private:
	...
	FSharedMemoryResourceRHIRef PreviousFrameBackBufferMemoryBuffer;
	...
	*/
	...
	/** The index of the frame that is stored in PreviousFrameBackBuffer. */
	INT PreviousFrameBackBufferFrameIndex;
	...
	// These are the texture references for the render target textures that are currently aliased by PreviousFrameBackBuffer.
	FTexture2DRHIRef LightAttenuation0_OverlappingPreviousFrameBackBuffer;
	FTexture2DRHIRef FogFrontfacesIntegralAccumulation_OverlappingPreviousFrameBackBuffer;
	FTexture2DRHIRef ShadowDepthZ_OverlappingPreviousFrameBackBuffer;
	FTexture2DRHIRef FilterColor1_OverlappingPreviousFrameBackBuffer;
	FTexture2DRHIRef AOInput_OverlappingPreviousFrameBackBuffer;
	FTexture2DRHIRef AOOutput_OverlappingPreviousFrameBackBuffer;
};
