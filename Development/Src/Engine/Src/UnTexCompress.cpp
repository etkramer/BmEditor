...
struct FNVCompression
{
	 ...
	 */
	void	Setup( void* SourceData, EPixelFormat PixelFormat, INT SizeX, INT SizeY, UBOOL SRGB, UBOOL bIsNormalMap, UBOOL bUseCUDAAcceleration, UBOOL bSupportDXT1a, INT QualityLevel );
 ...
 * @param	QualityLevel			A value from nvtt::Quality that represents the quality of the lightmap encoding
 */
void FNVCompression::Setup( void* SourceData, EPixelFormat PixelFormat, INT SizeX, INT SizeY, UBOOL SRGB, UBOOL bIsNormalMap,  UBOOL bUseCUDAAcceleration, UBOOL bSupportDXT1a, INT QualityLevel )
{
...
{
	FNVCompression Compression;
	Compression.Setup( SourceData, PixelFormat, SizeX, SizeY, SRGB, bIsNormalMap, bUseCUDAAcceleration, bSupportDXT1a, QualityLevel );
...
,	SizeY( InSizeY )
{
	Compression = new FNVCompression();
	Compression->Setup( SourceData, PixelFormat, SizeX, SizeY, SRGB, bIsNormalMap, GUseCUDAAcceleration, bSupportDXT1a, QualityLevel );
}
...
void FAsyncPVRTCCompressor::CompressTextures()
{
	...
	{
		GWarn->StatusUpdatef( -1, -1, TEXT("Compressing textures to PVRTC") );
...
void FAsyncPVRTCCompressor::FinishTasks()
{
	...
	do
	{
		...
		if (NumNowCompleted > NumCompleted)
		{
			...
			GWarn->StatusUpdatef( NumCompleted, NumTasks, *FString::Printf( TEXT("Compressing textures to PVRTC: %d of %d"), NumCompleted, NumTasks ) );
		}
		appSleep(0.1f);
