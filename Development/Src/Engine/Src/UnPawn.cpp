...
UBOOL APawn::SetDesiredRotation(FRotator TargetDesiredRotation,UBOOL InLockDesiredRotation,UBOOL InUnlockWhenReached,FLOAT InterpolationTime, UBOOL bResetRotationRate)
{
	...
	{
		DesiredRotation = TargetDesiredRotation;
		DesiredRotation = DesiredRotation.Clamp();

		bLockDesiredRotation = InLockDesiredRotation;

		bDesiredRotationSet = (bLockDesiredRotation || TargetDesiredRotation != Rotation);
...
void APawn::CheckDesiredRotation()
{
	...
	if ( bDesiredRotationSet && (Rotation == DesiredRotation) )
	{
		// if auto unlocks is wanted,
		if ( bUnlockWhenReached==TRUE )
		{
...
FVector APawn::AdjustDestination( AActor* GoalActor, FVector Dest )
{
	...
	if(GoalActor != NULL)
	{
		FVector Extent = GoalActor->GetCylinderExtent();
		// drop to the floor, then come up to mid level for our size
		FLOAT MyHeight = (CylinderComponent != NULL) ? CylinderComponent->CollisionHeight : 0.f;
		return -FVector(0.f,0.f,Extent.Z-MyHeight);
	}
