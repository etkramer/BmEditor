...
void AEmitterPool::TickSpecial(FLOAT DeltaTime)
{
	...
	while (i < RelativePSCs.Num())
	{
		...
		else
		{
			...
			if(RelativePSCs(i).bInheritBaseScale)
			{
				const FVector CurrentScale = RelativePSCs(i).PSC->Scale * RelativePSCs(i).PSC->Scale3D;
				const FVector NewScale = RelativePSCs(i).Base->DrawScale * RelativePSCs(i).Base->DrawScale3D;
				if(!CurrentScale.Equals(NewScale))
				{
					RelativePSCs(i).PSC->Scale = RelativePSCs(i).Base->DrawScale;
					RelativePSCs(i).PSC->Scale3D = RelativePSCs(i).Base->DrawScale3D;
					bNeedsUpdateFromScale=TRUE;
				}
			}

			if (RelativePSCs(i).PSC->Translation != NewTranslation || RelativePSCs(i).PSC->Rotation != NewRotation || bNeedsUpdateFromScale)
			{
		...
		Info->PeerHostMigration.HostMigrationProgress != HostMigration_Failed)
	{
		...
		{	
				...
				NetOwner->BestNextHostPeers.Num() > 0)
			{
				...
				UBOOL bAllPeersLostConnectionToHost = NetOwner->ConnectedPeers.Num() > 0;
...
void UWorld::Tick( ELevelTick TickType, FLOAT DeltaSeconds )
{
	...
	if(ThisFramePawnSpawns.Num() > 1 && GIsGame && !GIsServer)
	{
