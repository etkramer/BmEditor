...
public:
	...
	INT CompileProperty(EMaterialShaderPlatform MatPlatform,EMaterialProperty Property,FMaterialCompiler* Compiler) const
	{
...
UBOOL FDecalState::QuadToClippedScreenSpaceAABB(const FSceneView* SceneView, FVector2D& OutMin, FVector2D& OutMax, const FMatrix& FrustumVertexTransform) const
{
	...
	const FVector2D MinView(SceneView->X, SceneView->Y);
	const FVector2D MaxView(SceneView->X + SceneView->SizeX, SceneView->Y + SceneView->SizeY);
