 ...
 */
void UAudioDevice::ApplyClassAdjusters()
{
	if( CurrentMode )
	{
 ...
 */
void UAudioComponent::HandleInteriorVolumes( UAudioDevice* AudioDevice, AWorldInfo* WorldInfo )
{
	// Get the settings of the ambient sound
	FInteriorSettings Ambient;
	INT ReverbVolumeIndex;
	if ((GIsEditor && !GIsGame) || (CurrentLocation - LastLocation).SizeSquared() > KINDA_SMALL_NUMBER)
	{
		ReverbVolumeIndex = WorldInfo->GetAudioSettings(CurrentLocation, NULL, &Ambient);
...
void UAudioComponent::UpdateWaveInstances( UAudioDevice* AudioDevice, TArray<FWaveInstance*> &InWaveInstances, const TArray<FListener>& InListeners, FLOAT DeltaTime )
{
	...
	FVector ModifiedLocation = WorldInfo->RemapLocationThroughPortals( CurrentLocation, Listener->Location );
	...
	for( INT i = 1; i < InListeners.Num(); i++ )
	{
		...
		ModifiedLocation = WorldInfo->RemapLocationThroughPortals( CurrentLocation, TestListener->Location );
	...
	{
		HandleInteriorVolumes( AudioDevice, WorldInfo );
	}
...
void AAmbientSoundSimple::CheckForErrors( void )
{
	...
	{
		GWarn->MapCheck_Add( MCTYPE_WARNING, this, *FString::Printf( TEXT( "%s::%s : Ambient sound's AmbientProperties has a NULL Wave" ), *GetName(), *OwnerName ), MCACTION_NONE, TEXT( "NoSounds" ) );
	}
