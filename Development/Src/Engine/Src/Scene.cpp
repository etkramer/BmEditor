...
void appInitShowFlags()
{
	...
	EShowFlags SHOW_DefaultGame_Part5 = SHOW_LightShafts;
	...
	EShowFlags SHOW_DefaultEditor_Part5 = SHOW_LightShafts;
		...
		if (Proxy != NULL && Component != NULL)
		{
			...
			if (Component->MotionBlurInfoIndex != INDEX_NONE)
			{
				...
				else
				{
					...
					{
						//Relinquish the claim to this cache line (rather than forcing the current owner out of the cache line
						Component->MotionBlurInfoIndex = INDEX_NONE;
