...
void FHeightFogShaderParameters::Set(const FVertexFactory* VertexFactory, const FMaterialRenderProxy* MaterialRenderProxy, const FSceneView* View, const UBOOL bAllowGlobalFog, ShaderRHIParamRef Shader) const
{
	...
	if ( bAllowGlobalFog && ( Material->AllowsFog() && !(VertexFactory->IsDecalFactory() && VertexFactory->IsGPUSkinned()) ))
	{
		...
		{
			SetShaderValue(Shader,FogMinHeightParameter,ViewInfo->FogMinHeight);
			SetShaderValue(Shader,FogMaxHeightParameter,ViewInfo->FogMaxHeight);
			SetShaderValue(Shader,FogInScatteringParameter,ViewInfo->FogInScattering);
			SetShaderValue(Shader,FogDistanceScaleParameter,ViewInfo->FogDistanceScale);
			SetShaderValue(Shader,FogExtinctionDistanceParameter,ViewInfo->FogExtinctionDistance);
			SetShaderValue(Shader,FogStartDistanceParameter,ViewInfo->FogStartDistance);
		}
	}
	else
	{
		...
		SetShaderValue(Shader,FogMinHeightParameter,DefaultFogMinHeight);
		SetShaderValue(Shader,FogMaxHeightParameter,DefaultFogMaxHeight);
		SetShaderValue(Shader,FogInScatteringParameter,DefaultFogInScattering);
		SetShaderValue(Shader,FogDistanceScaleParameter,DefaultFogDistanceScale);
		SetShaderValue(Shader,FogExtinctionDistanceParameter,DefaultFogExtinctionDistance);
		SetShaderValue(Shader,FogStartDistanceParameter,DefaultFogStartDistance);
	}
}
...
*/
void FHeightFogShaderParameters::SetVertexShader(const FVertexFactory* VertexFactory, const FMaterialRenderProxy* MaterialRenderProxy,const FSceneView* View,const UBOOL bAllowGlobalFog,FShader* VertexShader) const
{
	Set(VertexFactory, MaterialRenderProxy,View,bAllowGlobalFog,VertexShader->GetVertexShader());
}
...
*/
void FHeightFogShaderParameters::SetDomainShader(const FVertexFactory* VertexFactory, const FMaterialRenderProxy* MaterialRenderProxy,const FSceneView* View,FShader* DomainShader) const
{
	const UBOOL bAllowGlobalFog = FALSE;
	Set(VertexFactory, MaterialRenderProxy,View,bAllowGlobalFog,DomainShader->GetDomainShader());
}
#endif
	...
	for(INT ViewIndex = 0;ViewIndex < Views.Num();ViewIndex++)
	{
		...
		if(ShouldRenderFog(View.Family->ShowFlags))
		{
			...
			if (Scene->ExponentialFogs.Num() > 0)
			{
				...
				const FLOAT CollapsedFogParameter = FogInfo.FogDensity * appPow(2.0f, -FogInfo.FogHeightFalloff * (View.ViewOrigin.Z - FogInfo.FogHeight));
...
void SetFogShaders(FScene* Scene,const FViewInfo& View)
{
	...
	else if (Scene->Fogs.Num() == 1)
	{
		...
		VertexShader->SetParameters(View);
		OneLayerHeightFogPixelShader->SetParameters(View, Scene->Fogs.Num(), FALSE);
	}
	...
	else
	{
		...
		VertexShader->SetParameters(View);
		FourLayerHeightFogPixelShader->SetParameters(View, Scene->Fogs.Num(), FALSE);
	}
}
