...
void UMaterialInstance::GetUsedTextures(TArray<UTexture*> &OutTextures, EMaterialShaderPlatform Platform, UBOOL bAllPlatforms)
{
	...
	for (INT PlatformIndex = (bAllPlatforms ? 0 : Platform); (bAllPlatforms ? PlatformIndex < MSP_MAX : PlatformIndex == Platform); PlatformIndex++)
	{
		...
		if ( bHasStaticPermutationResource && StaticPermutationResources[Platform]->ShaderMap )
		{
...
UBOOL UMaterialInstance::GetTerrainLayerWeightParameterValue(FName ParameterName, INT& OutWeightmapIndex, FGuid &OutExpressionGuid)
{
	...
	FGuid* ExpressionId = NULL;
	for (INT ValueIndex = 0;ValueIndex < StaticParameters[MSP_SM3]->TerrainLayerWeightParameters.Num();ValueIndex++)
	{
		if (StaticParameters[MSP_SM3]->TerrainLayerWeightParameters(ValueIndex).ParameterName == ParameterName)
		{
			WeightmapIndex = StaticParameters[MSP_SM3]->TerrainLayerWeightParameters(ValueIndex).WeightmapIndex;
			ExpressionId = &StaticParameters[MSP_SM3]->TerrainLayerWeightParameters(ValueIndex).ExpressionGUID;
			break;
		}
	}
	if (WeightmapIndex >= 0)
