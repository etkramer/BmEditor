...
public:
	...
	{
		Super::SetMesh(VertexShaderParameters, PixelShaderParameters, VertexShader, PixelShader, Mesh, ElementData);
		if (PixelShaderParameters)
		{
			UBOOL bShowMipLevels = FALSE;
#if !FINAL_RELEASE
...
#endif
		{
			PixelShader->SetMesh(PrimitiveSceneInfo,Mesh,View,bReceiveDynamicShadows,TranslucentPreShadowInfo,bOverrideDynamicShadowsOnTranslucency,bBackFace);
...
#endif
		{
			PixelShader->SetParameters(MaterialRenderProxy,VertexFactory,View,bReceiveDynamicShadows,bUseTranslucencyLightAttenuation);
