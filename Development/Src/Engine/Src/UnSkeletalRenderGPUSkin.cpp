...
void ResetInfluences (FVertexInfluence* Buffer, const FSkeletalMeshVertexBuffer& InDefaultGPUSkin, UINT NumVertices)
{
	...
	{
		const FGPUSkinVertexBase* BaseVert = InDefaultGPUSkin.GetVertexPtr(VertIndex);
		appMemcpy(Buffer[VertIndex].Weights.InfluenceWeights, BaseVert->InfluenceWeights, sizeof(BYTE)*4);
		appMemcpy(Buffer[VertIndex].Bones.InfluenceBones, BaseVert->InfluenceBones, sizeof(BYTE)*4);
	}
}
...
#else
				FBoneSkinning& BoneMat = ChunkMatrices(BoneIdx);
				SET_BONE_DATA(BoneMat, ReferenceToLocalMatrices(Chunk.BoneMap(BoneIdx)).ToMatrix());
#endif
			}
		}
	}
...
void FSkeletalMeshObjectGPUSkin::FSkeletalMeshObjectLOD::UpdateMorphVertexBuffer( const TArray<FActiveMorph>& ActiveMorphs )
{
	...
	static FMorphGPUSkinVertex ZeroVertexArray16[16] = {	ZeroVertex, ZeroVertex, ZeroVertex, ZeroVertex,
															ZeroVertex, ZeroVertex, ZeroVertex, ZeroVertex,
															ZeroVertex, ZeroVertex, ZeroVertex, ZeroVertex,
															ZeroVertex, ZeroVertex, ZeroVertex, ZeroVertex};
...
#define DEBUG_MORPH_TANGENT 0
	...
	if( IsValidRef(MorphVertexBuffer.VertexBufferRHI) )
	{
...

		{
			...
			// zero all deltas (NOTE: DeltaTangentZ is FPackedNormal, so we can't just appMemzero)
			// start from 16 batches to 4, and 1
			UINT VertIndex =0;
			for (; VertIndex + 15 < LodModel.NumVertices; VertIndex += 16)
			{
				appMemcpy(&Buffer[VertIndex], &ZeroVertexArray16, sizeof(FMorphGPUSkinVertex)*16);
			}
			if ( VertIndex < LodModel.NumVertices )
			{
				appMemcpy(&Buffer[VertIndex], &ZeroVertexArray16, sizeof(FMorphGPUSkinVertex)*(LodModel.NumVertices-VertIndex));
			}
		}
										...
										const FSkeletalMeshObjectGPUSkin::FVertexFactoryBuffers& VertexBuffers)
{
		...
		VertexBuffers.AltVertexWeightsBuffer->IsInitialized())
	{
		...
		VertexBuffers.InstancedWeightsBuffer->IsInitialized())
	{
