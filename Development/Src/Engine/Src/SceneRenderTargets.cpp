...
{
	PreviousFrameBackBufferFrameIndex = CurrentFrameNumber;
	Exchange(RenderTargets[CurrentFrameBackBuffer].Texture,RenderTargets[PreviousFrameBackBuffer].Texture);

#if XBOX && !USE_NULL_RHI
	Exchange(LightAttenuationMemoryBuffer,PreviousFrameBackBufferMemoryBuffer);
	Exchange(RenderTargets[LightAttenuation0].Texture,LightAttenuation0_OverlappingPreviousFrameBackBuffer);
	Exchange(RenderTargets[ShadowDepthZ].Texture,ShadowDepthZ_OverlappingPreviousFrameBackBuffer);
	Exchange(RenderTargets[FilterColor1].Texture,FilterColor1_OverlappingPreviousFrameBackBuffer);
	Exchange(RenderTargets[AOInput].Texture,AOInput_OverlappingPreviousFrameBackBuffer);
	Exchange(RenderTargets[AOOutput].Texture,AOOutput_OverlappingPreviousFrameBackBuffer);
#endif
}
	...
	if(BufferSizeX > 0 && BufferSizeY > 0)
	{
		...
		if( GUsingMobileRHI )
		{
			...
			if ( GMobileUsePostProcess )
			{
				...
				RenderTargets[FilterColor1].Surface = RHICreateTargetableSurface(FilterBufferSizeX, FilterBufferSizeY, PF_A8R8G8B8, RenderTargets[FilterColor1].Texture, TargetSurfCreate_Dedicated, TEXT("FilterColor1"));
...

				{
					...
					RenderTargets[LUTBlend].Surface = RHICreateTargetableSurface(Width, Height, FSceneRenderTargetFormats::LUTFormat, RenderTargets[LUTBlend].Texture, TargetSurfCreate_Dedicated, TEXT("LUTBlend"));
				}
		...
		// Create an ARGB texture for resolving to... 
#if WITH_GFx
		RenderTargets[SceneColorARGB].Texture = RHICreateSharedTexture2D(BufferSizeX,BufferSizeY,PF_A8R8G8B8,1,SceneColorMemoryBuffer,TexCreate_ResolveTargetable);
		RenderTargets[SceneColorARGB].Surface = RHICreateTargetableSurface(
			BufferSizeX,BufferSizeY,PF_A8R8G8B8,RenderTargets[SceneColorARGB].Texture,TargetSurfCreate_Dedicated|MultiSampleFlag,TEXT("DefaultColorARGB"));
#endif
...
#if WITH_GFx
		RenderTargets[SceneColorARGB].Texture = NULL;
		RenderTargets[SceneColorARGB].Surface = NULL;
#endif
...
#if XBOX && !USE_NULL_RHI	
		const SIZE_T LightAttenuationSize = XeCalculateTextureBytes(BufferSizeX,BufferSizeY,1,FSceneRenderTargetFormats::LightAttenuationBufferFormat);
		const SIZE_T BackBufferSize = XeCalculateTextureBytes(BufferSizeX,BufferSizeY,1,FSceneRenderTargetFormats::BackBufferFormat);
		...
		LightAttenuationMemoryBuffer = RHICreateSharedMemory(Max(BackBufferSize,LightAttenuationSize));
#endif
...
#if !PS3
		// Create a texture to store the resolved light attenuation values, and a render-targetable surface to hold the unresolved light attenuation values.
		RenderTargets[LightAttenuation0].Texture = RHICreateTexture2D(BufferSizeX,BufferSizeY,FSceneRenderTargetFormats::LightAttenuationBufferFormat,1,TexCreate_ResolveTargetable,NULL);
		RenderTargets[LightAttenuation0].Surface = RHICreateTargetableSurface(
			BufferSizeX,BufferSizeY,FSceneRenderTargetFormats::LightAttenuationBufferFormat,RenderTargets[LightAttenuation0].Texture,MultiSampleFlag,TEXT("LightAttenuation0"));
#endif
...

		{
			...
			UINT Height = Extend;
#if XBOX && !USE_NULL_RHI
			RenderTargets[LUTBlend].TextureArray = RHICreateTexture2DArray(Extend, Extend, Extend, FSceneRenderTargetFormats::LUTFormat, 1, TexCreate_ResolveTargetable, NULL);

			// * 2 : SourceX needs to be 32 pixel aligned as this is a X360 Resolve() requirement
			RenderTargets[LUTBlend].Surface = RHICreateTargetableSurface(Width * 2, Height, FSceneRenderTargetFormats::LUTFormat, 0, MultiSampleFlag, TEXT("LUTBlend"));
#else
			// Create a texture to store the resolved light attenuation values, and a render-targetable surface to hold the unresolved light attenuation values.
			RenderTargets[LUTBlend].Texture = RHICreateTexture2D(Width, Height, FSceneRenderTargetFormats::LUTFormat, 1, TexCreate_ResolveTargetable, NULL);
		...
		RenderTargets[FilterColor2].Texture = RHICreateTexture2D(FilterBufferSizeX,FilterBufferSizeY,FSceneRenderTargetFormats::FilterBufferFormat,1,TexCreate_ResolveTargetable,NULL);
		...
		RenderTargets[FilterColor1].Texture = RHICreateTexture2D(FilterBufferSizeX,FilterBufferSizeY,FSceneRenderTargetFormats::FilterBufferFormat,1,TexCreate_ResolveTargetable,NULL);
		RenderTargets[FilterColor1].Surface = RHICreateTargetableSurface(
			FilterBufferSizeX,FilterBufferSizeY,FSceneRenderTargetFormats::FilterBufferFormat,RenderTargets[FilterColor1].Texture,TargetSurfCreate_Dedicated|MultiSampleFlag,TEXT("FilterColor1"));
		...
		// TODO: Can we overlap with another buffer (e.g. light attenuation buffer)?
		{
#if XBOX && !USE_NULL_RHI
			extern SIZE_T XeCalculateTextureBytes(DWORD SizeX,DWORD SizeY,DWORD SizeZ,BYTE Format);
			SIZE_T BufferSize	= XeCalculateTextureBytes(TranslucencyBufferSizeX, TranslucencyBufferSizeY, 1, FSceneRenderTargetFormats::TranslucencyBufferTextureFormat);
			...
			RenderTargets[HalfResPostProcess].Texture = RHICreateSharedTexture2D(TranslucencyBufferSizeX,TranslucencyBufferSizeY,PF_A16B16G16R16,1,TranslucencyMemoryBuffer,TexCreate_ResolveTargetable);
			...
			RenderTargets[HalfResPostProcess].Surface = RHICreateTargetableSurface(TranslucencyBufferSizeX,TranslucencyBufferSizeY,FSceneRenderTargetFormats::TranslucencyBufferSurfaceFormat,RenderTargets[HalfResPostProcess].Texture,TargetSurfCreate_Dedicated,TEXT("HalfResPostProcess"));
#elif PS3
...
#else
			RenderTargets[TranslucencyBuffer].Texture = RHICreateTexture2D(TranslucencyBufferSizeX,TranslucencyBufferSizeY,FSceneRenderTargetFormats::TranslucencyBufferTextureFormat,1,TexCreate_ResolveTargetable,NULL);
			RenderTargets[HalfResPostProcess].Texture = RHICreateTexture2D(TranslucencyBufferSizeX,TranslucencyBufferSizeY,FSceneRenderTargetFormats::TranslucencyBufferTextureFormat,1,TexCreate_ResolveTargetable,NULL);
...
void FSceneRenderTargets::UpdateTemporalAAAllocation(UBOOL bNeedsAllocation)
{
	...
	if(BufferSizeX > 0 && BufferSizeY > 0)
	{
		const UBOOL bIsAllocated = IsValidRef(GetRenderTargetTexture(PreviousFrameBackBuffer));
	
		if(!bIsAllocated && bNeedsAllocation)
		{
			#if XBOX && !USE_NULL_RHI
				...
				const SIZE_T LightAttenuationSize = XeCalculateTextureBytes(BufferSizeX,BufferSizeY,1,FSceneRenderTargetFormats::LightAttenuationBufferFormat);
				const SIZE_T BackBufferSize = XeCalculateTextureBytes(BufferSizeX,BufferSizeY,1,FSceneRenderTargetFormats::BackBufferFormat);
				PreviousFrameBackBufferMemoryBuffer = RHICreateSharedMemory(Max(BackBufferSize,LightAttenuationSize));

				// Allocate the PreviousFrameBackBufferMemoryBuffer part of the double-buffered resolve target textures that are overlapped with CurrentFrameBackBuffer.
				LightAttenuation0_OverlappingPreviousFrameBackBuffer = RHICreateSharedTexture2D(BufferSizeX,BufferSizeY,FSceneRenderTargetFormats::LightAttenuationBufferFormat,1,PreviousFrameBackBufferMemoryBuffer,TexCreate_ResolveTargetable);
				FilterColor1_OverlappingPreviousFrameBackBuffer = RHICreateSharedTexture2D(FilterBufferSizeX,FilterBufferSizeY,PF_A16B16G16R16,1,PreviousFrameBackBufferMemoryBuffer,TexCreate_ResolveTargetable);
				if(GSystemSettings.RenderThreadSettings.bAllowAmbientOcclusion 
				&& AOBufferSizeX > 0
				&& AOBufferSizeY > 0)
				{
					AOInput_OverlappingPreviousFrameBackBuffer = RHICreateSharedTexture2D(AOBufferSizeX,AOBufferSizeY,FSceneRenderTargetFormats::AOInputFormat,1,PreviousFrameBackBufferMemoryBuffer,TexCreate_ResolveTargetable);
					AOOutput_OverlappingPreviousFrameBackBuffer = RHICreateSharedTexture2D(AOBufferSizeX,AOBufferSizeY,FSceneRenderTargetFormats::AOOutputFormat,1,PreviousFrameBackBufferMemoryBuffer,TexCreate_ResolveTargetable);
				}
				if(GSystemSettings.RenderThreadSettings.bAllowDynamicShadows)
				{
					const FIntPoint ObjectShadowBufferResolution = GetShadowDepthTextureResolution(FALSE);
					ShadowDepthZ_OverlappingPreviousFrameBackBuffer = RHICreateSharedTexture2D(ObjectShadowBufferResolution.X,ObjectShadowBufferResolution.Y,PF_ShadowDepth,1,PreviousFrameBackBufferMemoryBuffer,TexCreate_ResolveTargetable);
				}
				if(GSystemSettings.RenderThreadSettings.bAllowFogVolumes)
				{
					FogFrontfacesIntegralAccumulation_OverlappingPreviousFrameBackBuffer = RHICreateSharedTexture2D(FogAccumulationBufferSizeX,FogAccumulationBufferSizeY,PF_A8R8G8B8,1,PreviousFrameBackBufferMemoryBuffer,TexCreate_ResolveTargetable);
				}
			#endif

			// Create the double-buffered PreviousFrameBackBuffer.
			// CurrentFrameBackBuffer is used to hold the current frame's pre-TAA backbuffer while TAA is being applied to it.
			// After TAA is applied, it is swapped with PreviousFrameBackBuffer.
			#if XBOX && !USE_NULL_RHI
				RenderTargets[PreviousFrameBackBuffer].Texture = RHICreateSharedTexture2D(BufferSizeX,BufferSizeY,FSceneRenderTargetFormats::BackBufferFormat,1,PreviousFrameBackBufferMemoryBuffer,TexCreate_ResolveTargetable);
				RenderTargets[CurrentFrameBackBuffer].Texture = RHICreateSharedTexture2D(BufferSizeX,BufferSizeY,FSceneRenderTargetFormats::BackBufferFormat,1,LightAttenuationMemoryBuffer,TexCreate_ResolveTargetable);
			#else
				RenderTargets[PreviousFrameBackBuffer].Texture = RHICreateTexture2D(BufferSizeX,BufferSizeY,FSceneRenderTargetFormats::BackBufferFormat,1,TexCreate_ResolveTargetable,NULL);
				RenderTargets[CurrentFrameBackBuffer].Texture = RHICreateTexture2D(BufferSizeX,BufferSizeY,FSceneRenderTargetFormats::BackBufferFormat,1,TexCreate_ResolveTargetable,NULL);
			#endif
		}
		else if(bIsAllocated && !bNeedsAllocation)
		{
			#if XBOX && !USE_NULL_RHI
				PreviousFrameBackBufferMemoryBuffer.SafeRelease();

				LightAttenuation0_OverlappingPreviousFrameBackBuffer.SafeRelease();
				FilterColor1_OverlappingPreviousFrameBackBuffer.SafeRelease();
				AOInput_OverlappingPreviousFrameBackBuffer.SafeRelease();
				AOOutput_OverlappingPreviousFrameBackBuffer.SafeRelease();
				ShadowDepthZ_OverlappingPreviousFrameBackBuffer.SafeRelease();
				FogFrontfacesIntegralAccumulation_OverlappingPreviousFrameBackBuffer.SafeRelease();
			#endif

			RenderTargets[PreviousFrameBackBuffer].Texture.SafeRelease();
			RenderTargets[CurrentFrameBackBuffer].Texture.SafeRelease();
		}
	}
}
...
FString FSceneRenderTargets::GetRenderTargetInfo(ESceneRenderTargetTypes EnumIndex, FIntPoint &OutExtent) const
{
	...
	if(Index < MAX_SCENE_RENDERTARGETS)
	{
		...
		switch(Index)
		{
			...
			case PreviousFrameBackBuffer:
			case CurrentFrameBackBuffer:
...
#define RTENUMNAME(x) case x: RenderTargetName = TEXT(#x); break;
	switch(RTEnum)
	{
		...
		RTENUMNAME(PreviousFrameBackBuffer)
		RTENUMNAME(CurrentFrameBackBuffer)
...
void FSceneTextureShaderParameters::Set(const FSceneView* View,FShader* PixelShader, ESamplerFilter ColorFilter/*=SF_Point*/, ESceneDepthUsage DepthUsage/*=SceneDepthUsage_Normal*/) const
{
	...
	SetCustom(View, PixelShader, ColorFilter, *DesiredSceneColorTexture, DepthUsage);
}
