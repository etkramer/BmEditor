...
void UActorComponent::CheckForErrors()
{
	...
	{
		GWarn->MapCheck_Add(MCTYPE_WARNING, Owner, *FString::Printf(TEXT("%s::%s is obsolete and must be removed! (Class is deprecated)"), *GetName(), *Owner->GetName()), MCACTION_DELETE);
	}

	if (Owner != NULL && GetClass()->ClassFlags & CLASS_Abstract)
	{
		GWarn->MapCheck_Add(MCTYPE_WARNING, Owner, *FString::Printf(TEXT("%s::%s is obsolete and must be removed! (Class is abstract)"), *GetName(), *Owner->GetName()), MCACTION_DELETE);
	}
}
#endif
...
void UActorComponent::ConditionalTick(FLOAT DeltaTime)
{
	if(bAttached)
	{
		GAMEPLAY_PROFILER_TRACK_COMPONENT(this);
		Tick(DeltaTime);
	}
}
...
FVector FWindSourceSceneProxy::GetWindSkew(const FVector& Location,FLOAT CurrentWorldTime) const
{
	...
	const FLOAT LocalPhase = (Phase + CurrentWorldTime) * Frequency - LocalDistance / Speed;
