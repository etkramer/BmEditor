	...
	if( GIsGame && !GIsCooking && !GIsUCC )
	{
		if ( IsStatic() )
		{
			...
			for(INT Idx=0;Idx<Obstacles.Num();++Idx)
			{
				...
				if(!Obstacle->GetBoundingShape(BoundingShape))
				{
					continue;
				}
				...
				GetIntersectingPolys(Ctr,Extent,Polys,TRUE,TRUE);
							...
							INT OverrideHeuristicCost/*=-1*/)
{
	...
	for( INT EdgeIdx = 0; EdgeIdx < NumEdges; ++EdgeIdx )
	{
		...
		CurrentPoly->UpdateDynamicObstaclesForEdge(PathSessionID,EdgeIdx);
		...
		{
			if (CurrentNeighborEdge->EdgeType == NAVEDGE_BackRefDummy)
			{
				// completely ignore backrefs so they don't clutter up the debug space
				continue;
			}

			if( CurrentNeighborEdge->IsInSameGroupAs(PredecessorEdge) )
			{
				continue;
			}
		}
...
/**
 * returns TRUE if hte passed segment (start,end) leaves the poly supplied
 * @param Poly - poly to check against
 * @param start - start point of segment
 * @param end - end point of segment
 * @return TRUE if the segmetn leaves the poly
 */
...
UBOOL FNavMeshEdgeBase::SupportsMoveToEdge(const FNavMeshPathParams& PathParams, FNavMeshEdgeBase* NextEdge, FNavMeshPolyBase* PolyToMoveThru)
{
	...
	// find dirs perpindicular to the edges which point into the poly
...
extern UBOOL InsertSorted(ANavigationPoint* NodeForInsertion, ANavigationPoint*& OpenList);
...
UBOOL UNavigationHandle::InsertSorted( PathCardinalType NodeForInsertion, PathOpenList& OpenList )
{
	...
	INT LoopCounter = 0;
#endif
	for(;CurrentNode != NULL;CurrentNode = CurrentNode->NextOpenOrdered)
	{
#if !PS3 && !FINAL_RELEASE
												   ...
												   FLOAT ArrivalDistance)
{
	...
	if(Dist2dSq < BumpThresh*BumpThresh )
	{
		...
		FVector NewPos(0.f);
		FLOAT DistToNextSq = GetFlattenedDistanceBetweenVectsSq(NextMovePoint,CurHandlePos,OtherPoly->GetPolyNormal());
	...
	else // then we were not really close to the next move point, so move to a compensated (bumped) destination as usual
	{
		...
		out_MovePosition = out_MovePosition + (Ground_movePos - CurHandlePos).SafeNormal() * BumpAmt;
		checkSlow(!out_MovePosition.ContainsNaN());
	}


	// finally, see if the move destintion we have chosen is very close to the edge even after bump offsets.. if so move it away from the edge a bit so we don't thrash along the line of the edge
	if( BumpThresh > 0.f )
	{
		FVector Closest(0.f);
...
{	
	...
	FLOAT EffectiveRadius = HandleExtent.X * 1.5/*MT-> a little extra buffer here to bump dudes away from boundaries by a little1.414f*/;
	...
	for(INT Idx=0;Idx<=PathCache.Num();++Idx)
	{
		...
		if( Idx < PathCache.Num() )
		{
			...
			MovePoints(Idx) = CurEdge->GetEdgeDestination(CachedPathParams,EffectiveRadius,LastPoint,LastPoint,this,TRUE);
		}
		else
		{
			MovePoints(Idx) = *FinalDestination;
		}
		LastPoint = MovePoints(Idx);
		...
		for(INT InflectionCheckIdx=CurrentInflectionIdx+1;InflectionCheckIdx <= Min<INT>(Idx,PathCache.Num()-1);++InflectionCheckIdx)
		{
			...
			SegmentDistToSegmentSafe(InnerEdge->GetVertLocation(0),InnerEdge->GetVertLocation(1),MovePoints(Idx),CurrentInflectionPoint,ClosestPt,NotUsed);
			if( ClosestPt.Equals(InnerEdge->GetVertLocation(0),0.01f) || ClosestPt.Equals(InnerEdge->GetVertLocation(1),0.01f) )
			{
				...
				for(INT InnerIdx=CurrentInflectionIdx+1;InnerIdx<Idx;++InnerIdx)
				{
					FNavMeshEdgeBase* CurrentInnerEdge = PathCache(InnerIdx);

					FVector NotUsed(0.f);
					SegmentDistToSegmentSafe(CurrentInnerEdge->GetVertLocation(0),CurrentInnerEdge->GetVertLocation(1),MovePoints(Idx),CurrentInflectionPoint,ClosestPt,NotUsed);
					MovePoints(InnerIdx) = ClosestPt;
				}

				CurrentInflectionPoint = MovePoints(InflectionCheckIdx);
	...
	for(INT Idx=CurrentInflectionIdx+1;Idx < PathCache.Num();++Idx)
	{
		...
		FVector NotUsed(0.f), ClosestPt(0.f);
		SegmentDistToSegmentSafe(CurrentInnerEdge->GetVertLocation(0),CurrentInnerEdge->GetVertLocation(1),CurrentInflectionPoint,FinalDestPt,ClosestPt,NotUsed);
		MovePoints(Idx) = ClosestPt;
	}

	

	// second pass, adjust for corners
	LastPoint = CurHandlePos;
	for(INT Idx=0;Idx<PathCache.Num();++Idx)
	{
		FNavMeshEdgeBase* CurEdge = PathCache(Idx);


		MovePoints(Idx) = CurEdge->GetEdgeDestination(CachedPathParams,EffectiveRadius,MovePoints(Idx),LastPoint,this,FALSE);
		LastPoint = MovePoints(Idx);
#if DEBUG_DRAW_NAVMESH_PATH
		if(Idx > 0)
		{
...
UBOOL UNavigationHandle::ComputeValidFinalDestination(FVector& out_ComputedPosition)
{
	...
	while(bColliding && Tries++ < MAX_TRIES && HitPoly != NULL)
	{
		...
		FLOAT OffsetDist = SearchExtent.X;
		
		// compensate for off-axis normals
		FLOAT DotP[4] = {PolyNorm | FVector(1.f,0.f,0.f),//north
						PolyNorm | FVector(-1.f,0.f,0.f),//south
	 					PolyNorm | FVector(0.f,-1.f,0.f),//west	
	 					PolyNorm | FVector(0.f,1.f,0.f)};//east
		FLOAT DotPBest = 0.f;
...
#define CAPTURE_GNM_NAV_MESH_PERF 0
	...
	UBOOL bInAtLeastOnePoly = FALSE; // bool that indicates we've been in at least one poly so far
	UBOOL bConstrainedToCurrentEdge = FALSE;
	INT PathIdx = 0;
	INT ClipIdx = PathCache.Num() - 1;
	for (;PathIdx<PathCache.Num();++PathIdx)
	{
		...
		else
		{
			...
			if (PathIdx >= PathCache.Num()-1 && ClipIdx == PathCache.Num()-1) 
			{
	...
	if(PathIdx > 0)
	{
		bConstrainedToCurrentEdge = (ClipIdx <= PathIdx);
		PathCache_RemoveIndex(0,Min(ClipIdx,PathIdx));
	}
...
UBOOL UNavigationHandle::HandleNotOnPath( FLOAT ArrivalDistance, FVector& out_Dest )
{
	...
	if(CurrentEdge != NULL && PathCache.Num() > 0)
	{
		...
		else if( DistFromEdge < Extent.X*1.5f )
		{
...
UBOOL UNavigationHandle::GetValidatedAnchorPosition(FVector& out_NewAnchorLoc, FVector OverrideStartLoc/*=FVector()*/)
{
	...
	GetValidPositionsForBoxEx(StartLoc,CachedPathParams.SearchExtent.X*6.0f,CachedPathParams.SearchExtent,FALSE,Points,1,0.f,CachedPathParams.SearchExtent,&IsValidAnchorPos);
 ...
 */
UBOOL UNavigationHandle::PolyIntersectsMesh( TArray<FVector>& InPoly, APylon*& out_Pylon, struct FNavMeshPolyBase*& out_Poly, TArray<FNavMeshPolyBase*>* ExclusionPolys/*=NULL*/, UBOOL bIgnoreImportedMeshes/*=FALSE*/, TArray<APylon*>* IgnorePylons/*=NULL*/)
{
	FPylonOctreeType* Octree = FNavMeshWorld::GetPylonOctree();
	...
	for(FPylonOctreeType::TConstElementBoxIterator<> OctreeIt(*(Octree),QueryBox); OctreeIt.HasPendingElements(); OctreeIt.Advance())
	{
		...
		if (CurPylon->IsValid() && (IgnorePylons == NULL || !IgnorePylons->ContainsItem(CurPylon)))
		{
			...
			if(CurPylon->NavMeshPtr->IntersectsPoly(InPoly,Poly,ExclusionPolys,WORLD_SPACE,MinWalkZ))
			{
				out_Poly = Poly;
				out_Pylon = CurPylon;
				return TRUE;
...
UBOOL UNavigationHandle::BoxIntersectsMesh( const FVector& Center, const FVector& Extent, APylon*& out_Pylon, struct FNavMeshPolyBase*& out_Poly)
{
	FPylonOctreeType* Octree = FNavMeshWorld::GetPylonOctree();
	...
	for(FPylonOctreeType::TConstElementBoxIterator<> OctreeIt(*(Octree),QueryBox); OctreeIt.HasPendingElements(); OctreeIt.Advance())
	{
		...
		if(CurPylon->IsValid())
		{
			if(CurPylon->NavMeshPtr->IntersectsPolyBounds(Center,Extent,Poly,WORLD_SPACE))
			{
				out_Poly = Poly;
				out_Pylon = CurPylon;
				return TRUE;
...
UBOOL UNavigationHandle::GetAllPolysFromPos( const FVector& Pos, const FVector& Extent, TArray<struct FNavMeshPolyBase*>& out_PolyList, UBOOL bIgnoreDynamic, UBOOL bReturnBothDynamicAndStatic/*=FALSE*/, TArray<APylon*>* PylonsToConsider/*=NULL*/ )
{
	static TArray<APylon*> Pylons;
	Pylons.Reset();
	...
	{
		APylon* CurPylon = (*PylonsToConsider)(PylonIdx);
		CurPylon->GetIntersectingPolys( Pos, Extent, out_PolyList, bIgnoreDynamic, bReturnBothDynamicAndStatic);
	}
											...
											const TArray<APylon*>* PylonsToCheck/*=NULL*/)
{
	...
	for(INT PyIdx=0;PyIdx< PylonsToCheck->Num();++PyIdx)
	{
		...
		if (Pylon != NULL)
		{
			...
			{
				Pylon->ObstacleMesh->GetIntersectingPolys(Pos,Extent,out_PolyList);
			}

			if( Pylon->DynamicObstacleMesh != NULL )
			{
				Pylon->DynamicObstacleMesh->GetIntersectingPolys(Pos,Extent,out_PolyList);
			}
		}
	}
}
												...
												ValidBoxPositionFunc FitnessFunction/*=NULL*/)
{
	...
	TArray<APylon*> PylonsWeCareAbout;
...
void UNavigationHandle::GetIntersectingPylons(const FVector& Loc, const FVector& Extent, TArray<APylon*>& out_Pylons)
{
	FPylonOctreeType* Octree = FNavMeshWorld::GetPylonOctree();
	if(Octree == NULL)
	{
												  ...
												  )
{
	...
	if(PylonsToCheck != NULL)
	{
		...
		for(INT PylonIdx=0;PylonIdx<PylonsToCheck->Num();++PylonIdx)
		{
			...
			bInAtLeastOnePoly = (bInAtLeastOnePoly || CurMesh->GetPolyFromPoint(Pt,-1.0f,WORLD_SPACE));
			CurObsMesh = (*PylonsToCheck)(PylonIdx)->ObstacleMesh;
			if( !CurObsMesh->PointCheck(CurMesh,Hit,Pt,Extent,0,out_HitPoly) )
			{
				return FALSE;
			}
		}
	...
	UBOOL bHit = !Pylon->ObstacleMesh->PointCheck( Pylon->NavMeshPtr,Hit,Pt,Extent,0,out_HitPoly);
										  ...
										  const TArray<APylon*>* PylonsToCheck/*=NULL*/)
{
	...
	for( INT PylonIdx=0; PylonIdx < PylonsToCheck->Num(); ++PylonIdx )
	{
		...
		if (!Pylon->NavMeshPtr->PointCheck( Pylon->NavMeshPtr,Hit,Pt,Extent,0,out_HitPoly))
		{
			return FALSE;
		}
	}
												...
												DWORD TraceFlags/*=0*/)
{
	...
	if(CastDist > UCONST_LINECHECK_GRANULARITY)
	{		
		...
		FLOAT StartHeightOffset = 0.f;
		if( GetPylonAndPolyFromPos(Start,-1.f,StartPylon,StartPoly) )
		{
	...
	if (!StartPylon->ObstacleMesh->LineCheck(StartPylon->NavMeshPtr,Hit,TruncatedEnd,Start,Extent,0,out_HitPoly) ||
		...
		(!bIgnoreNormalMesh && !StartPylon->NavMeshPtr->LineCheck(StartPylon->NavMeshPtr,Hit,TruncatedEnd,Start,Extent,0,out_HitPoly))
		)
	{
#if DEBUG_DRAW_NAVMESH_PATH
	...
	if(StartPylon != EndPylon)
	{
		if(!EndPylon->ObstacleMesh->LineCheck(EndPylon->NavMeshPtr, Hit,TruncatedEnd,Start,Extent,0,out_HitPoly) || 
			// also make sure we don't collide with the walkable mesh either (make sure we're not punching through the ground)
			(!bIgnoreNormalMesh && !EndPylon->NavMeshPtr->LineCheck(EndPylon->NavMeshPtr, Hit,TruncatedEnd,Start,Extent,0,out_HitPoly))
			)
		{
#if DEBUG_DRAW_NAVMESH_PATH
	...
	if(bNeedsRecurse)
	{
		...
		return PointReachableLineCheck(InOuter,Hit,TruncatedEnd,End,Extent,bIgnoreNormalMesh,out_HitPoly,bComparePolyNormalZs);
	}
	else
	{
		...
		if(bComparePolyNormalZs && StartPoly != NULL && EndPoly != NULL)
		{
			...
			const FLOAT DefaultMinZ = AScout::GetGameSpecificDefaultScoutObject()->WalkableFloorZ;
...
UBOOL UNavigationHandle::PointReachable( FVector Point, FVector OverrideStartPoint/*=FVector(EC_EventParm)*/, UBOOL bAllowHitsInEndCollisionBox/*=TRUE*/ )
{
	...
	else
	{
		...
		if( bAllowHitsInEndCollisionBox )
		{
			...
			Poly->AdjustPositionToDesiredHeightAbovePoly(CheckLoc,Max<FLOAT>(CachedPathParams.SearchExtent.X,CachedPathParams.SearchExtent.Z));
...
FLOAT UNavigationHandle::CalculatePathDistance(FVector FinalDest)
{
	...
	if(PathCache.Num() > 0)
	{
		...
		FVector Pos(0.f);
		ComputeOptimalEdgePosition(0,Pos,0.f,FALSE,&Points);
...
*/
UBOOL AEnvironmentVolume::GetBoundingShape(TArray<FVector>& out_PolyShape)
{
...
#define MAX_STACK 10
UBOOL NavMeshWalkingObstacleCheck( APawn* Pawn, UNavigationHandle* Handle, FCheckResult& Hit,const FVector& Start,const FVector& End, const FVector& Extent, const FVector& Floor, TArray<FNavMeshPolyBase*>& PolysToCheckAgainst, INT StackCount=0)
{
	FNavMeshPolyBase* HitPoly=NULL;
	FVector HitLoc(0.f);
	FLOAT HitTime;
	...
	if( !bTrajectoryClear )
	{
		...
		if(Handle->CurrentEdge != NULL && HitPoly != NULL)
		{
			...
			FVector IsectPt(0.f);
			const FLOAT Close_Thresh = (Extent.X*0.22f);
			...
			{ 
				...
				if(ProjToEdge_T > -Edge_T_Tolerance && ProjToEdge_T < 1.f+Edge_T_Tolerance)
				{
					...
					{
						bTrajectoryClear=NavMeshWalkingObstacleCheck(Pawn,Handle,Hit,NewStart,End,Extent,Floor,PolysToCheckAgainst,StackCount+1);
					}
					else
					{
		...
		Pawn->CollisionComponent ))
	{
		...
		if( Pawn->bCollideWorld || Pawn->bBlockActors )
		{
			...
			if( Hit.Actor )
			{
				...
				{
					Pawn->NotifyBumpLevel(Hit.Location, Hit.Normal);
				}
				// Notify first bumped actor unless it's the level or the actor's base.
				else if( !Pawn->IsBasedOn(Hit.Actor) )
				{
					// Notify both actors of the bump.
					Hit.Actor->NotifyBump(Pawn, Pawn->CollisionComponent, Hit.Normal);
					Pawn->NotifyBump(Hit.Actor, Hit.Component, Hit.Normal);
				}
				bTrajectoryClear=FALSE;
			}

			if( bNeedTouchCheck || (!Pawn->bBlockActors && !Pawn->bCollideWorld && Pawn->bCollideActors) )
			{
				for( FCheckResult* Test=FirstHit; Test && Test->Time<Hit.Time; Test=Test->GetNext() )
				{
					if ( (!Pawn->IsBasedOn(Test->Actor)) &&
						(!Pawn->IsBlockedBy(Test->Actor,Test->Component)) && Pawn != Test->Actor)
					{
						Pawn->BeginTouch(Test->Actor, Test->Component, Test->Location, Test->Normal, Test->SourceComponent);
					}
				}
			}
...
void APawn::physNavMeshWalking(FLOAT deltaTime)
{
	...
	UNavigationHandle::GetAllObstaclePolysFromPos(Mid, Extent+CacheExtent, ObstaclePolysToConsider,&PylonsToConsider);
...
#define DEBUG_THIS 0			
		...
		// TEST destination
		FCheckResult Hit(1.f);
		if(!NavMeshWalkingObstacleCheck(this,Handle,Hit,Working_location,ProjectedDesired,Extent,Floor,ObstaclePolysToConsider))
		{
			...
			if( (NewDelta | Delta) >= 0.f )
			{
				...
				if (!NavMeshWalkingObstacleCheck(this,Handle,Hit,Working_location,Working_location+NewDelta,Extent,Floor,ObstaclePolysToConsider))
				{
