	...
	if ( DecalMaterial ) 
	{
		...
		for ( INT ReceiverIndex = 0 ; ReceiverIndex < StaticReceivers.Num() ; ++ReceiverIndex )
		{
			...
			if ( Receiver && Receiver->IsAttached() && Receiver->GetScene() == GetScene() )
			{
				...
				if( Receiver->IsA(UTerrainComponent::StaticClass()) ) 
				{
					FDecalState DecalState;
...
void UDecalComponent::CaptureDecalState(FDecalState* DecalState) const
{
	...
	if( !Cast<UDecalMaterial>(DecalState->DecalMaterial) 
		&& !DecalState->DecalMaterial->CheckMaterialUsage(MATUSAGE_Decals) )
	{
		warnf( TEXT("Decal (%s) had a material with invalid usage and DefaultDecalMaterial was used!"), *DecalState->DecalMaterial->GetFullName() );
		DecalState->DecalMaterial = GEngine->DefaultDecalMaterial;
	}
	...
	if( !DecalState->DecalMaterial->GetMaterial() || DecalState->DecalMaterial->GetMaterial()->bUsedAsSpecialEngineMaterial)
	{
