	...
	:	FMeshDrawingPolicy(InVertexFactory,InMaterialRenderProxy)
{
	...
	VertexShader = MaterialResource->GetShader<TFogIntegralVertexShader<DensityFunctionPolicy> >(InVertexFactory->GetType());
	PixelShader = MaterialResource->GetShader<TFogIntegralPixelShader<DensityFunctionPolicy> >(InVertexFactory->GetType());
}
...
FFogVolumeApplyDrawingPolicy::FFogVolumeApplyDrawingPolicy(
	...
	:	FMeshDrawingPolicy(InVertexFactory,InMaterialRenderProxy,bInOverrideWithShaderComplexity,bInOverrideWithShaderComplexity)
{
	const FMaterial* MaterialResource = InMaterialRenderProxy->GetMaterial();
	...
	if (bInOverrideWithShaderComplexity)
	{
		...
		NumIntegralInstructions = DensitySceneInfo->GetNumIntegralShaderInstructions(MaterialResource, InVertexFactory);
	}
#endif
}
	...
	)
{
	...
	DrawingPolicy.SetMeshRenderState(View,PrimitiveSceneInfo,Mesh,bBackFace,FFogVolumeApplyDrawingPolicy::ElementDataType());
