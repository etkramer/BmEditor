	...
	, PNGColorType(0)
{
	...
	if ( this->IsPNG() )
	{
		...
		PNGReadGuard PNGGuard( &PNGReadStruct, &PNGInfo );
		{
			...
			WidthFromHeader = PNGInfo->width;
			HeightFromHeader = PNGInfo->height;
			PNGColorType = PNGInfo->color_type;
		}
	}
}
...
void FPNGHelper::Uncompress()
{
	if( !RawData.Num() )
	{
		...
		PNGHeaderWidth  = info_ptr->width;
		PNGHeaderHeight = info_ptr->height;
