			...
			if( !StreamingLevel->bHasLoadRequestPending )
			{
				...
				if( bNeedToLoad && bAllowLevelLoadRequests  )
				{
					...
					if( GIsEditor || !GEngine->bUseBackgroundLevelStreaming )
					{
						...
						UObject::FlushAsyncLoading( NAME_None, TRUE );
...
void UWorld::FlushLevelStreaming( FSceneViewFamily* ViewFamily, UBOOL bOnlyFlushVisibility, FName ExcludeType)
{
	...
	if( !bOnlyFlushVisibility )
	{
		...
		UObject::FlushAsyncLoading(ExcludeType, TRUE);
	}
...
{
	debugf( NAME_NetComeGo, TEXT("UWorld: New peer connection %s %s %s"), *GetName(), appTimestamp(), *Connection->LowLevelGetRemoteAddress() );
}
...
#if WITH_STEAMWORKS
			// NOTE: If you update this code, update the code for this in UnPenLev.cpp too!
			// This code was added, so clients can re-auth with Steam midgame (i.e. post seamless travel), instead of just at login
			case NMT_AuthServer:
			{
		...
		switch (MessageType)
		{
			case NMT_Hello:
			{
...
#if UDK
				FGuid RemoteModGUID;
				FNetControlMessage<NMT_Hello>::Receive(Bunch, PlatformType, RemoteMinVer, RemoteVer, RemoteModGUID);
#else
				FNetControlMessage<NMT_Hello>::Receive(Bunch, PlatformType, RemoteMinVer, RemoteVer);
#endif // UDK
				...
				else
				{
					...
					{
						// by default, don't wait for OnlineSubsystem to auth user.
						Connection->bPendingAuth = FALSE;

						// this will be set when WELCOME is okay to send
						Connection->bWelcomeReady = FALSE;

						// Client isn't using Steam (jumped right to NMT_Hello), so we boot him with a polite message.
						if (!Connection->bSupportsAuth)
						{
							FString ErrCode(TEXT("Engine.Errors.SteamClientRequired"));
							FNetControlMessage<NMT_Failure>::Send(Connection, ErrCode);
							Connection->FlushNet();
							Connection->Close();
							break;
						}

						FString InvalidAuthErrCode(TEXT("Engine.Errors.SteamAuthFailed"));

						// make sure we got all of the auth blobs.
						const INT NumAuthBlobs = Connection->TempAuthBlobs.Num();

						// figure out length of the blob as bytes
						FString FullAuthString;
						for (INT SubBlob = 0; SubBlob < NumAuthBlobs; SubBlob++)
						{
							// no string should be empty
							if (Connection->TempAuthBlobs(SubBlob).Len() == 0)
							{
								FNetControlMessage<NMT_Failure>::Send(Connection, InvalidAuthErrCode);
								Connection->FlushNet();
								Connection->Close();
								break;
							}

							// put all the strings togehter
							FullAuthString += Connection->TempAuthBlobs(SubBlob);
						}

						// toss temp strings
						Connection->TempAuthBlobs.Empty();

						const INT BlobLen = FullAuthString.Len() / 2;
						if (BlobLen > 0)
						{
							// if there was a blob, send it to the onlinesubsystem.
							BYTE AuthBlob[2048];
							appStringToBlob(FullAuthString, AuthBlob, ARRAY_COUNT(AuthBlob));

							UBOOL bAuthIsGood = TRUE;

							// send the auth request to Steam, will return the Client's SteamID
							extern UBOOL appSteamAuthenticate(DWORD IP, BYTE* AuthBlob, DWORD BlobLen, QWORD *Uid);
							bAuthIsGood = appSteamAuthenticate(Connection->GetAddrAsInt(), AuthBlob, BlobLen, &Connection->UniquePlayerIdForAuth.Uid);

							// kick player if basic auth fails
							if (!bAuthIsGood)
							{
								FNetControlMessage<NMT_Failure>::Send(Connection, InvalidAuthErrCode);
								Connection->FlushNet();
							...
							{
								// mark that now we must wait for a steam callback before sending the WELCOME message
								Connection->bPendingAuth = TRUE;
							}
						}
						else
						{
...
#if WITH_STEAMWORKS
					// If Steam needs to re-auth a client, it reuses this control channel message;
					//	so don't call any client login code again if the player has already been welcomed
					if (!Connection->bWelcomed)
					{
#endif
#if UDK
						...
						{
							FString InvalidAuthErrCode(TEXT("Engine.Errors.ServerHostingDifferentGame"));
							FNetControlMessage<NMT_Failure>::Send(Connection, InvalidAuthErrCode);
							Connection->FlushNet();
							Connection->Close();
							break;
			...
			case NMT_AuthHello:
			{
				Connection->bSupportsAuth = TRUE;

				QWORD ServerID = 0;
				BYTE bServerAntiCheatSecured = (BYTE) FALSE;

				// If the server didn't initialize Steam, we need to tell this client via a bogus server ID of zero.
				//  But don't drop the player; he will just revert to non-Steamworks connection.
				if (GSteamworksInitialized)
				{
					extern QWORD appSteamGameServer_GetSteamID();
					extern INT appSteamGameServer_BSecure();
					ServerID = appSteamGameServer_GetSteamID();
					bServerAntiCheatSecured = (BYTE) appSteamGameServer_BSecure();
				}

				FNetControlMessage<NMT_AuthServer>::Send(Connection, ServerID, bServerAntiCheatSecured);
				Connection->FlushNet();
				break;
			}
			...
			case NMT_AuthBlob:
			{
				FString BlobChunk;
				BYTE Current, Num;
				FNetControlMessage<NMT_AuthBlob>::Receive(Bunch, Current, Num, BlobChunk);

				// Clients shouldn't get here normally if Steam's not initialized, but make sure we drop them in case an such a packet arrives.
				if (!GSteamworksInitialized)
				{
					Num = 0;  // just pretend there's no data so we drop them.
				}

				// preallocate space for all the strings
				if ((Num > 0) && (Num <= 8))  // prevent spamming.
				{
					if (Connection->TempAuthBlobs.Num() == 0)
					{
						Connection->TempAuthBlobs.AddZeroed(Num);
					}
				}

				// they all need to match
				if ((Num == 0) || (Connection->TempAuthBlobs.Num() != Num) || (Current < 0) || (Current >= Num) || (Connection->TempAuthBlobs(Current).Len() != 0))
				{
					FString InvalidAuthErrCode(TEXT("Engine.Errors.SteamAuthFailed"));
					FNetControlMessage<NMT_Failure>::Send(Connection, InvalidAuthErrCode);
					Connection->FlushNet();
				...
				Connection->TempAuthBlobs(Current) = BlobChunk;
				break;
			}
#endif
			...
			case NMT_Login:
			{
				...
				FNetControlMessage<NMT_Login>::Receive(Bunch, Connection->ClientResponse, Connection->RequestURL);
#if WITH_GAMESPY
...
#if WITH_STEAMWORKS
				// if the connection is waiting on a OnlineSubsystem auth, don't welcome yet
				else if (GSteamworksInitialized && Connection->bPendingAuth)
				{
					debugf(NAME_DevNet, TEXT("Waiting on OnlineSubsystem auth..."));
			...
			case NMT_Join:
			{
				...
				FNetControlMessage<NMT_Join>::Receive(Bunch, UniqueId);
				if (Connection->Actor == NULL)
				{
					...
					*/
					...
					Connection->Actor = SpawnPlayActor(Connection, ROLE_AutonomousProxy, FURL(NULL, *Connection->RequestURL, TRAVEL_Absolute), UniqueId, Error);
					if (Connection->Actor == NULL)
					{
						// Failed to connect.
			...
			case NMT_JoinSplit:
			{
				...
				else
				{
					...
					APlayerController* PC = SpawnPlayActor(ChildConn, ROLE_AutonomousProxy, URL, UniqueId, Error, BYTE(Connection->Children.Num()));
					if (PC == NULL)
					{
						// Failed to connect.
					...
					else
					{
						...
						debugf(NAME_DevNet, TEXT("JOINSPLIT: Succeeded: %s playerid=0x%016I64X"), 
							*ChildConn->Actor->PlayerReplicationInfo->PlayerName, ChildConn->Actor->PlayerReplicationInfo->UniqueId.Uid);
					}
				}
				break;
			}
...
/**
 * Process a OnlineSubsystem callback for the given user/message
 */
UBOOL UWorld::HandleOnlineSubsystemAuth(const FUniqueNetId &NetId, const UBOOL bAccept)
{
#if WITH_STEAMWORKS
	debugf(NAME_DevNet, TEXT("Got OnlineSubsystem Auth callback %uI64 %d"), NetId.Uid, (INT) bAccept);

	// find the matching connection
	if (NetDriver && GSteamworksInitialized)
	{
		// Handle case of listen server, where the ID to auth is local and not on a net connection
		if (GetNetMode() == NM_ListenServer)
		{
			for (FLocalPlayerIterator It(GEngine); It; ++It)
			{
				if (It->Actor != NULL && It->Actor->PlayerReplicationInfo != NULL &&
					It->Actor->PlayerReplicationInfo->UniqueId == NetId)
				{
					if (!bAccept)
						debugf(TEXT("Listen host failed Steam authentication!"));
		...
		for (INT ConnectionIndex = 0; ConnectionIndex < NetDriver->ClientConnections.Num(); ConnectionIndex++)
		{
			UNetConnection* Connection = NetDriver->ClientConnections(ConnectionIndex);

			// does the client have the same ID
			if (Connection->UniquePlayerIdForAuth == NetId && Connection->bPendingAuth)
			{
				Connection->bPendingAuth = FALSE;

				// handle successes (accept or Deny that was stalled we allow)
				if (bAccept)
				{
					debugf(NAME_DevNet, TEXT("OnlineSubsystem auth accepted connection %s"), *Connection->LowLevelDescribe());
					// now that we have the accepted callback, we can welcome the player
					if (Connection->bWelcomeReady)
					{
						WelcomePlayer(Connection);
						Connection->bWelcomeReady = FALSE;  // don't welcome twice!
					}
				}
				// handle failures
				else
				{
					FString ErrCode(TEXT("Engine.Errors.SteamAuthFailed"));
					// kill the client connection, goodbye!
					FNetControlMessage<NMT_Failure>::Send(Connection, ErrCode);
					Connection->FlushNet();
					Connection->Close();
				}

				return TRUE;  // ID was found (even if we rejected it!).
			}
		}
	}
#endif
...
void UWorld::MountPersistentFaceFXAnimSetOnActor(AActor* InActor)
{
	if (PersistentFaceFXAnimSet != NULL)
	{
		...
		if (InPawn && InPawn->Mesh && InPawn->Mesh->SkeletalMesh && InPawn->Mesh->SkeletalMesh->FaceFXAsset)
		{
			InPawn->Mesh->SkeletalMesh->FaceFXAsset->MountFaceFXAnimSet(PersistentFaceFXAnimSet);
		}

		if (InSkelMeshActor && InSkelMeshActor->SkeletalMeshComponent && InSkelMeshActor->SkeletalMeshComponent->SkeletalMesh && InSkelMeshActor->SkeletalMeshComponent->SkeletalMesh->FaceFXAsset)
		{
...
UBOOL AWorldInfo::BeginHostMigration()
{
	...
	if (bAllowHostMigration &&
		...
		GWorld->PeerNetDriver->ClientConnections.Num() > 0 &&
		eventCanBeginHostMigration())
	{
		// Only start once since there will be multiple disconnect messages
		if (PeerHostMigration.HostMigrationProgress == HostMigration_None)
		{
