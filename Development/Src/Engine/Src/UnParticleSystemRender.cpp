...
void FinishOneFrameParticleStats()
{
	if (GTrackParticleRenderingStatsForOneFrame)
	{
		...
		const FString WritePath = appGameDir() + TEXT("Screenshots") + PATH_SEPARATOR + PathFromScreenshots;
...
FParticleVertexFactory* FParticleVertexFactoryPool::GetParticleVertexFactory(EParticleVertexFactoryType InType)
{
	...
	VertexFactoriesInUse.Set(VertexFactory, TRUE);
...
{
	SCOPE_CYCLE_COUNTER(STAT_ParticlePoolTime);
	if (VertexFactoriesInUse.Find(InVertexFactory) != NULL)
	{
		// Set it to false to indicate it is not in use
		VertexFactoriesInUse.Set(InVertexFactory, FALSE);
		VertexFactoriesAvailable[InVertexFactory->GetVertexFactoryType()].AddItem(InVertexFactory);
		return TRUE;
	}
	warnf(NAME_Warning, TEXT("ParticleVertexFactory NOT from pool!"));
	check(0);
	return FALSE;
}
...
void FParticleVertexFactoryPool::ClearPool()
{
	...
	for (TMap<FParticleVertexFactory*,UBOOL>::TIterator KillIt(VertexFactoriesInUse); KillIt; ++KillIt)
	{
		if (KillIt.Value() == FALSE)
		{
			FParticleVertexFactory* VertexFactory = KillIt.Key();
			VertexFactoriesInUse.RemoveKey(VertexFactory);
		}
	}
	VertexFactoriesInUse.Compact();
}
...
void FParticleVertexFactoryPool::FreePool()
{
	ClearPool();
	{
		...
		for (TMap<FParticleVertexFactory*,UBOOL>::TIterator KillIt(VertexFactoriesInUse); KillIt; ++KillIt)
		{
			if (KillIt.Value() == TRUE)
			{
				// Should never hit this... but just in case, delete and VFs that are hanging around
				FParticleVertexFactory* VertexFactory = KillIt.Key();
				// Has already been released by the device cleanup...
				delete VertexFactory;
			}
		}
		VertexFactoriesInUse.Empty();
	}
}
...
void FParticleVertexFactoryPool::DumpInfo(FOutputDevice& Ar)
{
	...
	TotalMemory = 0;
	Ar.Logf(TEXT("ACTIVE,%d"), VertexFactoriesInUse.Num());
	if (VertexFactoriesInUse.Num() > 0)
	{
		INT ActiveCounts[PVFT_MAX];
		appMemzero(&ActiveCounts[0], sizeof(INT) * PVFT_MAX);
		for (TMap<FParticleVertexFactory*,UBOOL>::TIterator DumpIt(VertexFactoriesInUse); DumpIt; ++DumpIt)
		{
			if (DumpIt.Value() == TRUE)
			{
				FParticleVertexFactory* VertexFactory = DumpIt.Key();
...
INT FDynamicSortableSpriteEmitterDataBase::Render(FParticleSystemSceneProxy* Proxy, FPrimitiveDrawInterface* PDI,const FSceneView* View,UINT DPGIndex)
{
	...
	if (SourceData->EmitterRenderMode == ERM_Normal)
	{
		...
		if (SourceData->ParticleRenderData)
		{
			...
			Mesh.DynamicIndexData = SourceData->ParticleRenderIndices;
			Mesh.ParticleType = PET_PresuppliedMemory;
			Mesh.DynamicIndexStride = 2;
			Mesh.NumPrimitives = appTrunc(ParticleCount * 2);
		}
		// otherwise, set up the mesh to let render thread fill out the vertices
		else
		{
			Mesh.DynamicVertexData = this;
			Mesh.DynamicIndexData = ParticleOrder;
...
UBOOL FDynamicSpriteEmitterData::CreateRenderThreadResources(const FParticleSystemSceneProxy* InOwnerProxy)
{
	...
	{
		// If using point sprites (currently only supported on ES2 platforms)
		if (GUsingMobileRHI && (InOwnerProxy->UsesPointSprites() == TRUE))
		{
			VertexFactory = GParticleVertexFactoryPool.GetParticleVertexFactory(PVFT_PointSprite);
		}
		else
		{
...
UBOOL FDynamicSubUVEmitterData::GetVertexAndIndexData(void* VertexData, void* FillIndexData, FParticleOrder* ParticleOrder)
{
	...
	for (INT i = 0; i < ParticleCount; i++)
	{
		...
		if (Source.bDirectUV)
		{
			...
			FillVertex->Offsets[0] = U;
			FillVertex->Offsets[1] = V;
			FillVertex->Offsets[2] = U;
			FillVertex->Offsets[3] = V;
#else	//#if PARTICLES_USE_INDEXED_SPRITES
			U = baseU * (PayloadData->ImageHVInterp_UVOffset.X + 0);
			V = baseV * (PayloadData->ImageHVInterp_UVOffset.Y + 0);
			FillVertex->Tex_U	= U;
		...
		{
#if PARTICLES_USE_INDEXED_SPRITES
			FillVertex->Offsets[0] = baseU * (appTruncFloat(PayloadData->ImageHVInterp_UVOffset.X) + 0.0f);
			FillVertex->Offsets[1] = baseV * (appTruncFloat(PayloadData->ImageHVInterp_UVOffset.Y) + 0.0f);
			FillVertex->Offsets[2] = baseU * (appTruncFloat(PayloadData->Image2HV_UV2Offset.X) + 0.0f);
			FillVertex->Offsets[3] = baseV * (appTruncFloat(PayloadData->Image2HV_UV2Offset.Y) + 0.0f);
#else	//#if PARTICLES_USE_INDEXED_SPRITES
					...
					for (INT ValidIndex = 0; ValidIndex < ValidElementIndices.Num(); ValidIndex++)
					{
							...
							MeshDataPayload != NULL)
						{
							...
							if (bShouldUpdateMBTransforms)
							{
								...
								NewMBInfo.PreviousLocalToWorld = Mesh.LocalToWorld;
...
void FDynamicMeshEmitterData::RenderDebug(FPrimitiveDrawInterface* PDI, const FSceneView* View, UINT DPGIndex, UBOOL bCrosses)
{
	...
	FParticleInstancedMeshInstance* SrcInstance = (FParticleInstancedMeshInstance*)PhysXParticleBuf;
...
void FDynamicMeshEmitterData::RenderNxFluidNonInstanced(FParticleSystemSceneProxy* Proxy, FPrimitiveDrawInterface* PDI, const FSceneView* View, UINT DPGIndex)
{
	...
	if (MaterialResource[bSelected])
	{
		MIRes->Parent = (FMaterialRenderProxy*)(MaterialResource[bSelected]);
	}
	else
	...
	if (PhysXParticleBuf)
	{
		FParticleInstancedMeshInstance* SrcInstance = PhysXParticleBuf;
		...
		for (INT i = ParticleCount - 1; i >= 0; i--)
		{
			...
			MIRes->Param_MeshEmitterVertexColor = SrcInstance->Color;
			SrcInstance++;
...
void FDynamicMeshEmitterData::RenderNxFluidInstanced(FParticleSystemSceneProxy* Proxy, FPrimitiveDrawInterface* PDI, const FSceneView* View, UINT DPGIndex)
{
	...
	if( !InstanceBuffer->bFilled )
	{
		check(PhysXParticleBuf);
		if( PhysXParticleBuf )
		{
			FParticleInstancedMeshInstance* SrcInstance = PhysXParticleBuf;
			...
			FParticleInstancedMeshInstance* DstInstance = 
				static_cast<FParticleInstancedMeshInstance*>( InstanceBuffer->CreateAndLockInstances(ParticleCount) );

			appMemcpy(DstInstance,SrcInstance,ParticleCount*sizeof(FParticleInstancedMeshInstance));

			InstanceBuffer->bFilled = TRUE;
			InstanceBuffer->UnlockInstances();
...
INT FDynamicBeam2EmitterData::Render(FParticleSystemSceneProxy* Proxy, FPrimitiveDrawInterface* PDI,const FSceneView* View,UINT DPGIndex)
{
	...
	if (Data.OutTriangleCount > 0)
	{
		if (!bMaterialIgnored || View->Family->ShowFlags & SHOW_Wireframe)
		{
			...
			Mesh.DynamicIndexData		= Data.IndexData;
...
INT FDynamicTrail2EmitterData::Render(FParticleSystemSceneProxy* Proxy, FPrimitiveDrawInterface* PDI,const FSceneView* View,UINT DPGIndex)
{
	...
	if (Source.bUseLocalSpace == TRUE)
	{
		Mesh.LocalToWorld = SceneProxy->GetLocalToWorld();
		Mesh.WorldToLocal = SceneProxy->GetWorldToLocal();
	}
	else
	{
		Mesh.LocalToWorld = FMatrix::Identity;
	...
	Mesh.FirstIndex				= 0;
	Mesh.NumPrimitives			= Data.OutTriangleCount;
	Mesh.MinVertexIndex			= 0;
	Mesh.MaxVertexIndex			= Source.VertexCount - 1;
	Mesh.UseDynamicData			= TRUE;
	Mesh.ReverseCulling			= Proxy->GetLocalToWorldDeterminant() < 0.0f ? TRUE : FALSE;
...
INT FDynamicTrailsEmitterData::Render(FParticleSystemSceneProxy* Proxy, FPrimitiveDrawInterface* PDI, const FSceneView* View, UINT DPGIndex)
{
	...
	if (bRenderGeometry == TRUE)
	{
		...
		Mesh.DynamicIndexData		= Data.IndexData;
		Mesh.DynamicIndexStride		= SourcePointer->IndexStride;
		Mesh.LCI					= NULL;
// 		if (Source.bUseLocalSpace == TRUE)
// 		{
// 			Mesh.LocalToWorld = Proxy->GetLocalToWorld();
...
// 		}
		...
		Mesh.NumPrimitives			= Data.OutTriangleCount;
		Mesh.MinVertexIndex			= 0;
		Mesh.MaxVertexIndex			= SourcePointer->VertexCount - 1;
		Mesh.UseDynamicData			= TRUE;
		Mesh.ReverseCulling			= Proxy->GetLocalToWorldDeterminant() < 0.0f ? TRUE : FALSE;
		Mesh.CastShadow				= Proxy->GetCastShadow();
		Mesh.DepthPriorityGroup		= (ESceneDepthPriorityGroup)DPGIndex;
		...
		RenderedPrimitiveCount = Mesh.NumPrimitives;
	}
 ...
 *	@param	FrameNumber		The frame number of this pre-render
 */
void FDynamicTrailsEmitterData::PreRenderView(FParticleSystemSceneProxy* Proxy, const FSceneViewFamily* ViewFamily, const TBitArray<FDefaultBitArrayAllocator>& VisibilityMap, INT FrameNumber)
{
	if (bValid == FALSE)
	{
		return;
...
FPrimitiveSceneProxy(Component, Component->Template ? Component->Template->GetFName() : NAME_None)
	...
	, bUsePointSprites(Component->Template ? Component->Template->bUseMobilePointSprites : FALSE)
...
void FParticleSystemSceneProxy::GetObjectPositionAndScale(const FSceneView& View, FVector& ObjectPostProjectionPosition, FVector& ObjectNDCPosition, FVector4& ObjectMacroUVScales) const
{
	...
	if (DynamicData->SystemRadiusForMacroUVs > 0.0f)
	{
		...
		const FVector4 RightPostProjectionPosition = View.ViewProjectionMatrix.TransformFVector(DynamicData->SystemPositionForMacroUVs + DynamicData->SystemRadiusForMacroUVs * View.ViewMatrix.GetColumn(0));
		const FVector4 UpPostProjectionPosition = View.ViewProjectionMatrix.TransformFVector(DynamicData->SystemPositionForMacroUVs + DynamicData->SystemRadiusForMacroUVs * View.ViewMatrix.GetColumn(1));
