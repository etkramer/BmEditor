...
UBOOL UTexture2D::GetResourceMemSettings(INT FirstMipIdx, INT& OutSizeX, INT& OutSizeY, INT& OutNumMips, DWORD& OutTexCreateFlags)
{
	...
	OutTexCreateFlags	= (SRGB && (GEngine == NULL || !GEngine->bEmulateMobileRendering)) ? TexCreate_SRGB : 0;
	if ( MipTailBaseIdx == -1 )
	{
...
void UTexture2D::Serialize(FArchive& Ar)
{
	...
	{
		warnf(TEXT("Batman Texture2D: %d and %d"), Ar.Tell(), Ar.TotalSize());
	}
...
INT UTexture2D::Get360Size( INT MipCount ) const
{
	...
	DWORD TexCreateFlags = (SRGB && (GEngine == NULL || !GEngine->bEmulateMobileRendering)) ? TexCreate_SRGB : 0;
	// if no miptail is available then create the texture without a packed miptail
	if( MipTailBaseIdx == -1 )
	{
	...
	if (ConsoleSupport)
	{
		...
		if (FirstMip < Mips.Num())
		{
			...
			Size = ConsoleSupport->GetPlatformTextureSize(Format, MipMap.SizeX, MipMap.SizeY, MipCount, TexCreateFlags);
...
FTextureResource* UTexture2D::CreateResource()
{
	...
	)
	{
		...
		if( TextureFileCacheName != NAME_None )
		{
			...
			if (DlcManager == NULL || !DlcManager->GetDLCTextureCachePath(TextureFileCacheName,Filename))
#endif
			{
...
void FTexture2DResource::InitRHI()
{
	...
	// Create the RHI texture.
	DWORD TexCreateFlags = (Owner->SRGB && (GEngine == NULL || !GEngine->bEmulateMobileRendering)) ? TexCreate_SRGB : 0;
	// if no miptail is available then create the texture without a packed miptail
...
void FTexture2DResource::UpdateMipCount()
{
	...
	if ( !bUsingInPlaceRealloc || Owner->PendingMipChangeRequestStatus.GetValue() == TexState_ReadyFor_Loading )
	{
