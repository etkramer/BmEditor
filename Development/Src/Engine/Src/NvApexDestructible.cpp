...
UMaterialInterface *UApexComponentBase::GetMaterial(INT MaterialIndex) const
{
	...
	if ( Mat == NULL)
	{
		Mat = Asset ? Asset->GetMaterial(MaterialIndex): 0;
	}
...
void UApexComponentBase::UpdateBounds(void)
{
	...
	if(ApexRenderable)
	{
		const physx::PxBounds3 &PhysXBounds = ApexRenderable->getBounds();
		physx::PxVec3 Center, Extents;
		PhysXBounds.getCenter(Center);
		PhysXBounds.getExtents(Extents);
...
void UApexStaticDestructibleComponent::UpdateApexEditorState(UProperty* PropertyThatChanged)
{
	...
	UApexDestructibleAsset *DestructibleAsset = Asset ? Cast<UApexDestructibleAsset>(Asset) : 0;
...
UBOOL UApexStaticDestructibleComponent::LineCheck(FCheckResult& Result, const FVector& End, const FVector& Start, const FVector& Extent, DWORD TraceFlags)
{
	...
	if( Extent.IsZero() )
	{
		...
		if(Normal.isZero())
		{
			...
			Normal.set(nNormal.x,nNormal.y,nNormal.z);
			Normal.normalize();
		}
	...
	else
	{
		...
		if( Time < 0.0f )	// We start with penetrating
		{
			...
			if( abs(ExitTime)<abs(Time) && (ExitNormal | U2NPositionApex( Disp )) > 0.0f )
			{
				Result.Time = 1.0f;
				return TRUE;
			}
...
void UApexStaticDestructibleComponent::InitComponentRBPhys(UBOOL bFixed)
{
	...
	{
		if ( DestructibleAsset->DestructibleParameters.ForceToDamage > 0.0f )
		{
			NxParameterized::setParamU16(*params,"actorDescTemplate.group", UNX_GROUP_THRESHOLD_NOTIFY);
		}
		else
		{
			NxParameterized::setParamU16(*params,"actorDescTemplate.group", UNX_GROUP_NOTIFYCOLLIDE);
		}
	}
	...
	NxApexAsset *a_asset = DestructibleAsset->MApexAsset ? DestructibleAsset->MApexAsset->GetNxApexAsset() : 0;
	NxApexScene *ApexScene = GWorld && GWorld->RBPhysScene ? GWorld->RBPhysScene->GetApexScene() : 0;
...
void AApexDestructibleActor::TickSpecial( FLOAT DeltaSeconds )
{
