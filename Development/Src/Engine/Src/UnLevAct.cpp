...
public:
	...
	FScopedMoveActorTimer(AActor* Actor)
		: StartTime(0.0)
	    , MoveIndex(-1)
	    , bDidLineCheck(FALSE)
		...
		if(GShouldLogOutAFrameOfMoveActor)
		{
			...
			if(Depth == 0)
			{
				for(INT MoveIdx=0; MoveIdx<Moves.Num(); MoveIdx++)
				{
					...
					debugf(TEXT("MOVE%s - %s %5.2fms %d %d %s"), *Indent, *Move.Actor->GetName(), Move.Time * 1000.f, Move.bDidLineCheck, Move.bDidEncroachCheck, *Move.Actor->GetDetailedInfo());
				}
...
)
{
...
#if !FINAL_RELEASE && PERF_MOVEACTOR_STATS
	FScopedMoveActorTimer MoveTimer(Actor);
#endif // !FINAL_RELEASE && PERF_MOVEACTOR_STATS
	...
	if( Actor->Attached.Num() > 0 )
	{
		...
		const FMatrix OldMatrix = FRotationMatrix( OldRotation ).Transpose();
		const FMatrix NewMatrix = FRotationMatrix( NewRotation );
		...
		const FMatrix BaseTM = FRotationTranslationMatrix(Actor->Rotation, Actor->Location);
	...
	{
		{
			...
			{
				// LandscapeLineCheck is expensive because it uses the PhysX far-field with a mask to ignore non-Landscape bodies. 
				// It's necessary because we cannot do a swept box trace on a shape, only on the entire scene.
				// We skip this for non-zero extent collision, because we can use the Unreal octree to cull zero-extent checks.
				if( !Extent.IsZero() && (LandscapeComponentCount > 0) && (TraceFlags & TRACE_Terrain) && LandscapeLineCheck( Hits[NumHits], NULL, End, Start, Extent, TraceFlags )==0 )
				{
					Hits[NumHits].Actor = GetWorldInfo();
					Hits[NumHits].Time *= Dilation;
					NumHits++;
...
void AWorldInfo::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyThatChanged)
	{
		...
		else if (PropertyThatChanged->GetName()==TEXT("bAllowModulateBetterShadows"))
		{
			// Reattach all lights to propagate the change
			TComponentReattachContext<ULightComponent> LightReattach;
		}
		else if (PropertyThatChanged->GetName()==TEXT("bAllowLightEnvSphericalHarmonicLights")
			|| PropertyThatChanged->GetName()==TEXT("CharacterLitIndirectBrightness")
...
UBOOL AWorldInfo::GetModulateBetterShadowsAllowed()
{
	AWorldInfo* CurrentWorldInfo = this;
	if( StreamingLevels.Num() > 0 &&
		StreamingLevels(0) &&
	...
	return CurrentWorldInfo->bAllowModulateBetterShadows;
}

/** Checks whether spherical harmonic lights are allowed in this world. */
UBOOL AWorldInfo::GetSHLightsAllowed()
{
	AWorldInfo* CurrentWorldInfo = this;
	if( StreamingLevels.Num() > 0 &&
		StreamingLevels(0) &&
	...
	return CurrentWorldInfo->bAllowLightEnvSphericalHarmonicLights;
}

/** Checks whether to increase precision close to the camera at the cost of distant precision */
UBOOL AWorldInfo::GetIncreaseFogNearPrecision()
{
	AWorldInfo* CurrentWorldInfo = this;
	if( StreamingLevels.Num() > 0 &&
		StreamingLevels(0) &&
	...
	return CurrentWorldInfo->bIncreaseFogNearPrecision;
}
