				...
				for(INT CoefficientIndex = 0;CoefficientIndex < NUM_STORED_LIGHTMAP_COEF;CoefficientIndex++)
				{
					for(INT ColorIndex = 0;ColorIndex < 3;ColorIndex++)
					{
						if( CoefficientIndex == SIMPLE_LIGHTMAP_COEF_INDEX &&
							GWorld->GetWorldInfo()->bSimpleLightmapsStoredInLinearSpace )
						{
...
UBOOL FLightMap2D::IsValid(UINT BasisIndex) const
{
	...
	return (Textures[BasisIndex] != NULL);
}
...
void FLightMap2D::Serialize(FArchive& Ar)
{
	...
	UBOOL bStripSimpleLightmaps = bAllowDirectionalLightMaps;
	UBOOL bStripDirectionalLightmaps = !bAllowDirectionalLightMaps;
	if (GCookingTarget & UE3::PLATFORM_Mobile)
	{
		bStripSimpleLightmaps = FALSE;
		bStripDirectionalLightmaps = TRUE;
	}
	...
	if (Ar.IsLoading() && (!GIsEditor || (GCookingTarget & UE3::PLATFORM_Stripped)))
	{
		for(UINT CoefficientIndex = 0;CoefficientIndex < NUM_STORED_LIGHTMAP_COEF;CoefficientIndex++)
		{
			if ((bStripSimpleLightmaps && (CoefficientIndex >= SIMPLE_LIGHTMAP_COEF_INDEX))
				|| (bStripDirectionalLightmaps && (CoefficientIndex < NUM_DIRECTIONAL_LIGHTMAP_COEF)))
			{
				Textures[CoefficientIndex] = NULL;
			}
		}
