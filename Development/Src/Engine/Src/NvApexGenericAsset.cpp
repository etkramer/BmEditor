	...
	if( MApexAsset )
	{
		switch ( MApexAsset->GetType() )
		{
			...
			case AAT_CLOTHING_MATERIAL:	// asset used to represent clothing material library data.
				Info.AddItem( FString::Printf( TEXT("Clothing Material")));
				break;
...
void GetApexAssetName(UApexAsset *asset,char *dest,physx::PxU32 slen,const char *extension)
{
	...
	physx::string::sprintf_s(dest,slen,"%s%s", TCHAR_TO_ANSI(*Name), extension );
}

void StringToAnsi(const FString &str,char *dest,physx::PxU32 slen)
{
	physx::string::sprintf_s(dest,slen,"%s",TCHAR_TO_ANSI(*str));
}
