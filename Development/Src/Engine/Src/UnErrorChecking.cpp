...
void AActor::CheckForErrors()
{
	...
	{
		GWarn->MapCheck_Add( MCTYPE_ERROR, this, *FString::Printf(TEXT("%s has invalid DrawScale/ DrawScale3D"), *GetName() ), MCACTION_NONE, TEXT("InvalidDrawscale") );
	}
...
void AVolume::CheckForErrors()
{
	...
	if (CollisionComponent == NULL)
	{
		GWarn->MapCheck_Add( MCTYPE_WARNING, this,
			*FString::Printf(LocalizeSecure(LocalizeUnrealEd(TEXT("VolumeActorNULLCollisionComponent")), *GetName() )), MCACTION_DELETE, TEXT("VolumeActorCollisionComponentNULL") );
	}
	else
	{
		...
		{
			GWarn->MapCheck_Add( MCTYPE_WARNING, this,
				*FString::Printf(LocalizeSecure(LocalizeUnrealEd(TEXT("VolumeActorZeroRadius")), *GetName() )), MCACTION_DELETE, TEXT("VolumeActorZeroRadius") );
		}
	}
}
