...
FLOAT APawn::GetLedgeWalkMinFloorZ()
{
...
FVector APawn::CheckForLedges(FVector AccelDir, FVector Delta, FVector GravDir, int &bCheckedFall, int &bMustJump )
{
	...
	// See what slope we can step off ledge on to 
	FLOAT StepOffMinZ = GetLedgeWalkMinFloorZ();
	...
	FLOAT DropRatio = appSqrt(1.f - (StepOffMinZ*StepOffMinZ))/StepOffMinZ;
	...
	// This bool indicates that we are stepping off onto a slope that is allowed, but too steep to stand on
	UBOOL bStepSteepSlope = ((Hit.Time < 1.f) && (Hit.Normal.Z > StepOffMinZ) && (Hit.Normal.Z < WalkableFloorZ));

	if (Hit.Time == 1.f ||
		Hit.Normal.Z < StepOffMinZ ||
		(!bStepSteepSlope && (Hit.Time * TestDown > CylinderComponent->CollisionHeight + LedgeCheckThreshold + Min<FLOAT>(MaxStepHeight,appSqrt(1.f - Hit.Normal.Z * Hit.Normal.Z) * (CylinderComponent->CollisionRadius + DesiredDist)/Hit.Normal.Z))))
	{
	...
	if ( Hit.Time == 1.f ||	Hit.Normal.Z < StepOffMinZ )
	{
		// We have a ledge!
		if ( Controller && Controller->StopAtLedge() )
		{
...
void APawn::processLanded(FVector const& HitNormal, AActor *HitActor, FLOAT remainingTime, INT Iterations)
{
	...
	if( Hit.Time == 1.f ) //Not a valid landing
	{
		...
		if( GWorld->FindSpot(1.1f * GetCylinderExtent(), Adjusted, bCollideComplex) && (Adjusted != Location), this )
		{
...
INT AVolume::Encompasses(FVector point)
{
	...
	else
	{
		...
		{
			return !CollisionComponent->PointCheck(Hit, point, FVector(0.f, 0.f, 0.f), 0);
		}
	}
}
