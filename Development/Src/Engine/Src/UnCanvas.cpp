...
UBOOL FCanvasTileRendererItem::Render( const FCanvas* Canvas )
{
	...
	FSceneViewFamily* ViewFamily = new FSceneViewFamily(
		...
		NULL,
		SHOW_DefaultGame,
		bFreezeTime ? 0.0f : GWorld->GetTimeSeconds(),
		bFreezeTime ? 0.0f : GWorld->GetDeltaSeconds(),
		bFreezeTime ? 0.0f : GWorld->GetRealTimeSeconds(),
...
{
    return DrawStringCenteredZ(Canvas, StartX, StartY, 1.0f, Text, Font, Color);
}

INT DrawStringCenteredZ(FCanvas* Canvas,FLOAT StartX,FLOAT StartY,FLOAT Z,const TCHAR* Text,class UFont* Font,const FLinearColor& Color)
{
	INT XL, YL;
...
{
    DrawStringWrappedZ(Canvas, Draw, CurX, CurY, 1.0f, XL, YL, Font, Text, DrawColor);
}

void DrawStringWrappedZ( FCanvas* Canvas, UBOOL Draw, FLOAT CurX, FLOAT CurY, FLOAT Z, FLOAT& XL, FLOAT& YL, UFont* Font, const TCHAR* Text, FLinearColor DrawColor )
{
	if (!Font)
	{
		return;
	...
	do
	{
		...
		if ( Draw && iCleanWordEnd>0 )
		{
			...
			FLOAT LineX = CurX;
			LineX += DrawStringZ(Canvas,LineX, CurY, Z, *(TextLine.Left(iCleanWordEnd)),Font,DrawColor);
		}
...
void UCanvas::SetPos(FLOAT X, FLOAT Y, FLOAT Z)
{
	CurX = X;
	CurY = Y;
    CurZ = Z > 0.0f ? Z : 1.0f; // Ensure that Z is positive and non-zero.
}
...
INT UCanvas::WrappedPrint(UBOOL Draw, INT& out_XL, INT& out_YL, UFont* Font, FLOAT ScaleX, FLOAT ScaleY, UBOOL Center, const TCHAR* Text, const FFontRenderInfo& RenderInfo) 
{
	...
	FLOAT DrawX = OrgX + CurX, DrawY = OrgY + CurY;
	FLOAT XL = 0.f, YL = 0.f;
	for (int Idx = 0; Idx < WrappedStrings.Num(); Idx++)
	{
		FLOAT LineXL = DrawStringZ(Canvas, DrawX, DrawY, CurZ, *WrappedStrings(Idx).Value, Font, DrawColor, ScaleX, ScaleY, 0.0f, NULL, SE_BLEND_Translucent, Draw, 0.f, 1.f, RenderInfo);
...
void UCanvas::DrawRotatedMaterialTile(UMaterialInterface* Tex, FRotator Rotation, FLOAT XL, FLOAT YL, FLOAT U, FLOAT V, FLOAT UL, FLOAT VL, FLOAT AnchorX, FLOAT AnchorY)
{
	...
	Canvas->PushRelativeTransform(FinalTransform);
	DrawMaterialTile(Tex,OrgX+CurX,OrgY+CurY,CurZ,XL,YL,U,V,UL,VL);
	Canvas->PopTransform();
 ...
 */
void UCanvas::DrawTris(class UTexture* Tex, const TArray<struct FCanvasUVTri>& Triangles)
{
