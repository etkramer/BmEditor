...
void SerializeGameSessionInfo(FArchive& Archive, FGameSessionInformation& GameSession)
{
	...
	if (Archive.Ver() >= ADDED_MAPNAME_MAPURL_TO_GAMESESSIONINFO)
	{
		...
		{   
			...
			{
				GameSession.OwningNetId = FUniqueNetId(0);
 ...
 */
UBOOL SetupGameSessionInfo(FGameSessionInformation& CurrentSessionInfo, INT GameTypeId)
{
	...
	if (GWorld)
	{
		...
		if (Game)
		{
			...
			CurrentSessionInfo.OwningNetId = FUniqueNetId(0);
...
void UGameplayEventsWriter::StartLogging(FLOAT HeartbeatDelta)
{
	if (GIsGame && !GIsEditor && !CurrentSessionInfo.bGameplaySessionInProgress)
	{
		...
		if (OpenStatsFile(Filename))
		{
			...
			if (SetupGameSessionInfo(CurrentSessionInfo, eventGetGameTypeId()))
			{
				...
				if (SerializeHeader())
				{
					...
					if (Game != NULL)
					{
						...
						{
							Game->SetTimer(HeartbeatDelta, TRUE, FName(TEXT("Poll")), this);
						}
						else
						{
							Game->ClearTimer(FName(TEXT("Poll")), this);
						}
					}
				}
			}
...
void UGameplayEventsWriter::LogAllPlayerPositionsEvent(INT EventID)
{
	if (Archive)
	{
		...
		*Archive << GameEvent;
		...
		debugf(NAME_GameStats, TEXT("[%.3f]: PlayerLocationPoll[%d]: %d players"), GameEvent.TimeStamp, EventID, PlayerLocationsEvent.PlayerLocations.Num());
	}
}
