			...
			else
			{
				...
				for (INT MeshIndex = 0; MeshIndex < PrimitiveSceneInfo->StaticMeshes.Num(); MeshIndex++)
				{
					...
					if (StaticMesh.CastShadow 
						&& DistanceSquared >= StaticMesh.MinDrawDistanceSquared 
						&& DistanceSquared < StaticMesh.MaxDrawDistanceSquared * Square(GSystemSettings.MaxDrawDistanceScale))
					{
						StaticMeshWholeSceneShadowDepthMap(StaticMesh.Id) = TRUE;
						bDrawingStaticMeshes = TRUE;
					}
...
void FProjectedShadowInfo::ClearTransientArrays()
{
	...
	ModShadowReceiverPrimitives.Empty();
	...
	)
{
	...
	if(!PrimitiveSceneInfo->ShadowParent)
	{
		...
		const UINT MaxShadowResolutionSetting = (LightSceneInfo->MaxShadowResolution > 0) ? LightSceneInfo->MaxShadowResolution : GSystemSettings.RenderThreadSettings.MaxShadowResolution;
 ...
 * @param ModulateBetterShadows The list of modulate better shadows to check against
	...
	const TArray<FProjectedShadowInfo*,SceneRenderingAllocator>& ViewDependentWholeSceneShadows)
{
	for (INT PrimitiveIndex = 0; PrimitiveIndex < NumPrimitives; PrimitiveIndex++)
	{
		...
		BYTE ProcessChildrenForViews = GatherShadowsForPrimitiveInner(*Primitive, ValidViews, ModulateBetterShadows, PreShadows, ViewDependentWholeSceneShadows);
		...
		if (ProcessChildrenForViews && Primitive->ChildPrimitives.Num())
		{
			...
			GatherShadowsForPrimitives(ChildPrimitiveList, Primitive->ChildPrimitives.Num(), ProcessChildrenForViews, ModulateBetterShadows, PreShadows, ViewDependentWholeSceneShadows);
		}
	}
}
...
{
	if(ModulateBetterShadows.Num() || PreShadows.Num() || ViewDependentWholeSceneShadows.Num() || PlanarReflectionShadows.Num())
	{
			...
			PrimitiveOctreeIt.Advance())
		{
			...
			for(FScenePrimitiveOctree::ElementConstIt NodePrimitiveIt(PrimitiveOctreeNode.GetElementIt());NodePrimitiveIt;++NodePrimitiveIt)
			{
				...
				GatherShadowsForPrimitives(&PrimitiveSceneInfoCompact, 1, 0xFF, ModulateBetterShadows, PreShadows, ViewDependentWholeSceneShadows);
#else
				// gather the shadows for this one primitive
				GatherShadowsForPrimitiveInner(*NodePrimitiveIt, 0xFF, ModulateBetterShadows, PreShadows, ViewDependentWholeSceneShadows);
#endif
			}
		}
 ...
 * @param ModulateBetterShadows Collects any mod-better shadows
	...
	const FVector& Origin)
{
	...
	for (INT InteractionIndex = 0; InteractionIndex < Interactions.Num(); InteractionIndex++)
	{
		...
		else
		{
			if (Interaction->GetChildInteractions().Num() > 0)
			{
				SetupInteractionShadows(Interaction->GetChildInteractions(), VisibleLightInfo, PreShadows, ModulateBetterShadows, ViewDependentWholeSceneShadows, Origin);
			}
		}
	}
#endif
...
void FSceneRenderer::InitDynamicShadows()
{
					...
					if(bCreatePrimitiveShadows)
					{
							...
							)
						{
								...
								&& Interaction->GetPrimitiveSceneInfo()->BrightestDominantLightSceneInfo != Interaction->GetLight()))
							{
								...
								SetupInteractionShadows(LightInteractionArray, VisibleLightInfo, PreShadows, ModulateBetterShadows, CurrentLightViewDependentWholeSceneShadows, Views(0).ViewOrigin);
							}
						}
					}
				}
	...
	GatherShadowPrimitives(ModulateBetterShadows,PreShadows,ViewDependentWholeSceneShadows);
}
