...
void APylon::CheckForErrors()
{
	...
	if(!bHasValidMesh )
	{
		...
		{
			GWarn->MapCheck_Add( MCTYPE_ERROR, this, TEXT("Pylon is outside its expansion bounds! THIS PYLON WILL NOT BUILD PATHS!!"), MCACTION_NONE, TEXT("PylonOutsideBounds"));
		}
		...
		{
				GWarn->MapCheck_Add( MCTYPE_ERROR, this, TEXT("Could not find ground position for pylon, this pylon will NOT BUILD PATHS!"), MCACTION_NONE, TEXT("PylonNoGroundPos"));
		}
		else
		if(! IsPtWithinExpansionBounds(Hit.Location))
		{
			GWarn->MapCheck_Add( MCTYPE_ERROR, this, TEXT("Pylon's ground position is outside its expansion bounds, this pylon will NOT BUILD PATHS!"), MCACTION_NONE, TEXT("PylonOutsideBounds"));
		}
	...
	if(NavMeshPtr != NULL && NavMeshPtr->VersionAtGenerationTime < VER_MIN_PATHING)
	{
		...
		GWarn->MapCheck_Add( MCTYPE_ERROR, this, TEXT("Navigation Mesh Data was generated with a version of the engine which is too old, rebuild pathing!"), MCACTION_NONE, TEXT("NavMeshOutOfDate") );
	...
	for( INT Idx=0;Idx<ConflictingPylons.Num();++Idx )
	{
		...
		if(CurPylon != this)
		{
			GWarn->MapCheck_Add( MCTYPE_ERROR, this, *FString::Printf(TEXT("%s.%s and %s.%s are inside each other's bounds!  One pylon can be in another's bounds, but not both!"),*GetOutermost()->GetName(),*GetName(),*CurPylon->GetOutermost()->GetName(),*CurPylon->GetName()), MCACTION_NONE, TEXT("PylonsInsideEachOther") );
		}	
	}
	Validate();
...
{
	EdgePtrs.Reset(EdgeStorageData.Num());
	CrossPylonEdges.Reset(CrossPylonEdges.Num());
	for(INT Idx=0;Idx<EdgeStorageData.Num();Idx++)
	{
...
{
	// Go through all the verts and figure out an updated index
	static TArray<VERTID> SavedVertIndices;
	SavedVertIndices.Reset(Verts.Num());
	SavedVertIndices.AddZeroed(Verts.Num());

	INT ValidVertCount = 0;
	for( INT VertIdx = 0; VertIdx < Verts.Num(); VertIdx++ )
	{
		FMeshVertex& Vert = Verts(VertIdx);
		Vert.PolyIndices.Reset(Vert.ContainingPolys.Num());
		if( Vert.ContainingPolys.Num() > 0 )
		{
			SavedVertIndices(VertIdx) = ValidVertCount;
			ValidVertCount++;
		}
		else
		{
			SavedVertIndices(VertIdx) = MAXVERTID;
		}
	}
	...
	{
		if( SavedVertIndices(VertIdx) == MAXVERTID )
		{
	...
	if( VertHash != NULL )
	{
		...
		VertHash->Empty(SavedVertIndices.Num());
	...
	for( PolyList::TIterator Itt(BuildPolys.GetHead()); Itt; ++Itt )
	{
		...
		{
			P->PolyVerts(VertIdx) = SavedVertIndices(P->PolyVerts(VertIdx));
		}
...
UBOOL UNavigationMeshBase::SplitPolyAlongPlane(const FNavMeshPolyBase* Poly, const FPlane& Plane, TArray<FVector>& out_Poly1Verts, TArray<FVector>& out_Poly2Verts)
{
	...
	for( INT VertIdx = 0; VertIdx < Poly->PolyVerts.Num(); ++VertIdx )
	{
		...
		else
		{
			...
			if( SegmentPlaneIntersection(StartPoint, EndPoint, Plane, IntersectPoint) )
			{
...
UBOOL FTriangleAABBOverlap(const FVector& v0,const FVector& v1,const FVector& v2, const FVector& Ctr, const FVector& Extent)
{
	FSeparatingAxisPointCheck ThePointCheck(v0,v1,v2,Ctr,Extent,
		FVector(1.0f,0.f,0.f),FVector(0.f,1.0f,0.0),FVector(0.f,0.f,1.0f),100000.f);
...
UBOOL DoesBoxIntersectPolys(const FVector& Ctr,const FVector& Extent, const TArray<FNavMeshPolyBase*>& ObstacleMeshPolys)
{
	for(INT Idx=0;Idx<ObstacleMeshPolys.Num();++Idx)
	{
		...
		if( FPolyAABBIntersect(Ctr,Extent,CurPoly) && CurPoly->GetPolyPlane(LOCAL_SPACE).PlaneDot(Ctr) > 0)
		{
			return TRUE;
		}
	}
...
{
	static FVector X = FVector(1.0f,0.f,0.f);
	static FVector Y = FVector(0.0f,1.0f,0.f);
	static FVector Z = FVector(0.0f,0.f,1.0f);

	FVector HitNormal(0.f);
	FLOAT HitTime=1.0f;
	UBOOL Result = FindSeparatingAxis(v1,v2,v3,Start,End,Extent,X,Y,Z,HitTime,HitNormal);
...
UBOOL UNavigationMeshBase::LineCheckAgainstSpecificPolys(const FVector& Start, const FVector& End, const FVector& Extent, const TArray<FNavMeshPolyBase*>& Polys, FVector& out_HitLoc, FLOAT* out_HitTime/*=NULL*/, FNavMeshPolyBase** out_HitPoly/*=NULL*/)
{
	...
	for(INT Idx=0;Idx<Polys.Num();++Idx)
	{
		...
		for(INT VertIdx=1;VertIdx<CurPoly->PolyVerts.Num();++VertIdx)
		{
			...
			if( ExtentLineCheckTriangle(Vert0,Vert1,Vert2,Start,End,Extent,out_HitLoc,CurrentHitTime ) )
			{
				if(out_HitPoly!=NULL)
				{
					*out_HitPoly=CurPoly;
...
UBOOL FindEndPointsForSize(const FVector& ValidStartPos, FVector& out_SpanStart, FVector& out_SpanEnd, const FVector& Extent, const TArray<FNavMeshPolyBase*>& ObstacleMeshPolys)
{
	...
	return ( (out_SpanStart-out_SpanEnd).Size() > NAVMESHGEN_MIN_EDGE_LENGTH );	
}
 ...
 * @param PylosnToCheck - array of pylons in the area that we should do collision against
*/
#define MIN_SPAN_LENGTH 5.0f
#define MIN_SPAN_LENGTH_SQ 25.0f
void FindEdgesForSize(const FVector& Start,const FVector& End,const FVector& Extent,const TArray<FNavMeshPolyBase*>& ObstacleMeshPolys, TArray<UNavigationMeshBase::FEdgeWidthPair>& out_EdgePairs, INT& EdgeGroupID )
{
	...
	if( FindCheckStartPos(Start,End,Extent,ObstacleMeshPolys, StartCheckPos, &out_EdgePairs, This_EdgeGroupID, MinCheckDist) )
	{
		...
		if( FindEndPointsForSize(StartCheckPos,NewStart,NewEnd,Extent,ObstacleMeshPolys) )
		{
		...
		if (!bAtStart)
		{
			FindEdgesForSize(Start,NewStart,Extent,ObstacleMeshPolys,out_EdgePairs,EdgeGroupID);
		}

		if (!bAtEnd)
		{
			FindEdgesForSize(NewEnd,End,Extent,ObstacleMeshPolys,out_EdgePairs,EdgeGroupID);
		}

	}
}

void UNavigationMeshBase::BuildEdgesFromSegmentSpan(const FVector& Start, const FVector& End, TArray<FEdgeWidthPair>& out_EdgePairs)
{

	AScout* Scout = AScout::GetGameSpecificDefaultScoutObject();
	if( Scout == NULL )
	{
		warnf(TEXT("Could not get scout objectf ro EdgeSpanCreation!! wut?"));
		return;
	}
	...
	ObstacleMeshPolys.Reset(ObstacleMeshPolys.Num());
	...
	for(INT ObstacleMeshPolyIdx=ObstacleMeshPolys.Num()-1;ObstacleMeshPolyIdx>=0;--ObstacleMeshPolyIdx)
	{
		...
		{
			if( CurObstaclePoly->PolyEdges(EdgeIdx) == MAXWORD)
			{
				continue;
			}
	...
	for( INT i = Scout->PathSizes.Num()-1; i >= 0; --i)
	{
		...
		FindEdgesForSize(Start,End,Extent,ObstacleMeshPolys,out_EdgePairs,EdgeGroupID);
	}
}
/**
									...
									)
{
	...
	Edges.Reset(Edges.Num());

	BuildEdgesFromSegmentSpan(Start,End,Edges);
														...
														FLOAT MinEdgeLengthOverride/*=-1.f*/)
{
	...
	for( INT OuterIdx = 0; OuterIdx < Edges.Num(); OuterIdx++ )
	{
		...
		for( INT InnerIdx = 0; InnerIdx < Edges.Num(); InnerIdx++ )
		{
			...
			if( (ClosestPtOnInnerEdge-ClosestPtOnOuterEdge).SizeSquared() > EDGE_VERT_TOLERANCE * EDGE_VERT_TOLERANCE)
			{
				continue;
			}
...
UBOOL ExistsEdgeFromPoly1ToPoly2Fast(FNavMeshPolyBase* Poly1, FNavMeshPolyBase* Poly2)
{
	...
	APylon* Pylon2 = Poly2->NavMesh->GetPylon();
	...
	for(INT EdgeIdx=0;EdgeIdx<NumEdges;EdgeIdx++)
	{
		...
		if( Edge != NULL )
		{
			...
			{
				return ( (Poly1->Item == Edge->Poly0 && Poly2->Item == Edge->Poly1) ||
														...
														FLOAT MinEdgeLengthOverride/*=-1.f*/)
{
	...
	VertLocationCache.Reset(VertLocationCache.Num());
...
#define CACHE_STRIDE 4
	...
	for( INT OuterIdx = 0; OuterIdx < Edges.Num(); OuterIdx++ )
	{
		...
		for( INT InnerIdx = 0; InnerIdx < Edges.Num(); InnerIdx++ )
		{
			...
			// if the polys are already connected, bail
			ArePolysConnected(PolyInner,PolyOuter);
			...
			// see if the two edges are near each other
			if ( Vert0_Dist > EDGE_VERT_TOLERANCE_SQ && Vert1_Dist > EDGE_VERT_TOLERANCE_SQ && OtherVert0_Dist > EDGE_VERT_TOLERANCE_SQ && OtherVert1_Dist > EDGE_VERT_TOLERANCE_SQ )
			{
				continue;
			}
...
UBOOL UNavigationMeshBase::BuildSubMeshEdgesForJustClearedTLPoly(WORD PolyIdx)
{
	...
	SortedEdges.Reset(SortedEdges.Num());
	...
	ObstaclePolysWeHaveMessedWith.Reset(ObstaclePolysWeHaveMessedWith.Num());
		...
		if(OtherPoly->NumObstaclesAffectingThisPoly>0)
		{
			...
			if( Info != NULL )
			{
				...
				if( OtherPolySubMesh != NULL )
				{
					...
					SubmeshPolys.Reset(SubmeshPolys.Num());
...
#define DO_SUBMESHEDGES_PERF 0
	...
	if( SortedEdges.Num() > 0 )
	{
		...
		OuterEdges.Reset(OuterEdges.Num());
		for(INT VertIdx=0;VertIdx<CurPoly->PolyVerts.Num();++VertIdx)
		{
			...
			FVector SegmentEndPoint0 = CurPoly->GetVertLocation(VertIdx);
			FVector SegmentEndPoint1 = CurPoly->GetVertLocation(NextVertIdx);
			...
			for(INT EdgeIdx=0;EdgeIdx<CurPoly->GetNumEdges();++EdgeIdx)
			{
				...
				FVector Closest(0.f);
				const FVector EdgeCtr = CurEdge->GetEdgeCenter();
				if( IsPointCloseToOnSegment(EdgeCtr,SegmentEndPoint0,SegmentEndPoint1, FALSE ) )
				{
					...
					if( !bAlreadyHaveDummyLinkToPoly )
					{
						...
						EdgesInGroup.Reset(EdgesInGroup.Num());
						...
						if( BestIdx >= 0 )
						{
							...
							const FVector V0Loc = LongestEdgeInGroup->GetVertLocation(0);
							const FVector V1Loc = LongestEdgeInGroup->GetVertLocation(1);
							...
							if( OtherPoly->NavMesh != CurPoly->NavMesh )
							{
								...
								for(INT OtherPolyEdgeIdx=0;OtherPolyEdgeIdx<OtherPoly->PolyEdges.Num();++OtherPolyEdgeIdx)
								{
									OtherPolyEdge = OtherPoly->GetEdgeFromIdx(OtherPolyEdgeIdx,NULL,TRUE);
									if( OtherPolyEdge != NULL && EdgesAreEqual(OtherPolyEdge->GetVertLocation(0),OtherPolyEdge->GetVertLocation(1),V0Loc,V1Loc) )
									{
		...
		for(INT OuterEdgeIdx=0;OuterEdgeIdx<OuterEdges.Num();++OuterEdgeIdx)
		{
			...
			NewMesh->CreateEdgeConnectionsInternalFast(SortedEdges,FALSE,TRUE,TRUE,&NewMesh->SubMeshPolyIDToLinkeObstacleMap);
		}
...
extern void TryToLinkPolyToEdge(UNavigationMeshBase* ObstacleMesh, FNavMeshPolyBase& Poly, const FVector& EdgeVert0, const FVector& EdgeVert1, WORD EdgeIdx, UBOOL bDynamicEdge, FNavMeshCrossPylonEdge* CPEdge/*=NULL*/);
void UNavigationMeshBase::CreateEdgeConnections( UBOOL bTest )
{
	...
	DropDownEdges.Reset(DropDownEdges.Num());
	...
	if(GetPylon()->NeedsStaticCrossPylonEdgesBuilt())
	{
		...
		if(Pylons.Num() > 1)
		{
			for(INT IsectPylonIdx=0;IsectPylonIdx<Pylons.Num();IsectPylonIdx++)
			{
				...
				if(CurPylon == Pylon || CurPylon->NavMeshPtr == NULL || !CurPylon->NeedsStaticCrossPylonEdgesBuilt() || (CurPylon->bImportedMesh && !Pylon->bImportedMesh))
				{
					continue;
				}
			...
			Edges.Reset(Edges.Num());
			CreateEdgeConnectionsInternal(SortedEdges,bTest,TRUE,FALSE,NULL,&Edges,-1.0f,-1.0f);
				...
				for(INT Idx=0;Idx<ConnectedPolys.Num();++Idx)
				{
					...
					if(ObsMesh != NULL && ObsMesh->Polys.Num() > 0)
					{
						for(INT PolyIdx=0;PolyIdx<ObsMesh->Polys.Num();PolyIdx++)
						{
							...
							TryToLinkPolyToEdge(ObsMesh,Poly,LS_Entry,LS_Exit,MAXWORD,FALSE,NULL);
						}
					}
				}
			}
			...
			for(INT EdgeIdx=0;EdgeIdx<Edges.Num();++EdgeIdx)
			{
				...
				ConnectedPolys.AddItem(EdgeTuple.Poly0);
				ConnectedPolys.AddItem(EdgeTuple.Poly1);
				BuildEdgesFromSupportedWidths(EdgeTuple.Pt0,EdgeTuple.Pt1,ConnectedPolys,FALSE,NULL,NULL,EdgeTuple.OuterEdge,EdgeTuple.InnerEdge);
			}
	...
	CreateEdgeConnectionsInternal(SortedEdges,bTest,FALSE);
	...
	if(NewEdge != NULL)
	{
		...
		if ( ObstMesh != NULL )
		{
			for(INT PolyIdx=0; PolyIdx< ObstMesh->Polys.Num(); ++ PolyIdx)
			{
				...
				TryToLinkPolyToEdge(ObstMesh,Poly,StartPt,EndPt,EdgeId,FALSE,NewEdge);
			}
		}
...
void APylon::GetIntersectingPolys(const FVector& Loc, const FVector& Extent, TArray<FNavMeshPolyBase*>& out_Polys, UBOOL bIgnoreDynamic, UBOOL bReturnBothDynamicAndStatic)
{
	checkSlowish(NavMeshPtr != NULL);
	NavMeshPtr->GetIntersectingPolys(Loc,Extent,out_Polys,WORLD_SPACE,bIgnoreDynamic,bReturnBothDynamicAndStatic);
}
...
void ConditionalAddToOutputPolys(TArray<UNavigationMeshBase*>& out_Submeshes, FNavMeshPolyBase* Poly, TArray<FNavMeshPolyBase*>& out_Polys, UBOOL bIgnoreDynamic, UBOOL bReturnBothDynamicAndStatic)
{
	...
	{
		out_Submeshes.AddItem(Poly->GetSubMesh());
	}
 ...
 */
void UNavigationMeshBase::GetIntersectingPolys(const FVector& Loc, const FVector& Extent, TArray<FNavMeshPolyBase*>& out_Polys, UBOOL bWorldSpace, UBOOL bIgnoreDynamic/*=FALSE*/, UBOOL bReturnBothDynamicAndStatic/*=FALSE*/, UBOOL bRecursedForSubmesh/*=FALSE*/)
{
	...
	if( KDOPInitialized )
	{
		...
		if(bWorldSpace)
		{
			...
			LS_Box = FBox::BuildAABB(W2LTransformFVector(Loc),Extent);
		}
		else
		{
			LS_Box = FBox::BuildAABB(Loc,Extent);
			Box = FBox::BuildAABB(L2WTransformFVector(Loc),FVector(Extent.X,Extent.Y,Max<FLOAT>(Extent.Z,NAVMESHGEN_MAX_POLY_HEIGHT)));
		}
		
		static TArray<INT> TriIndices; 
		TriIndices.Reset(TriIndices.Num());
		...
		FNavMeshCollisionDataProvider Prov(this,ForNavMesh);
	...
	{
		const FVector LS_Loc = (bWorldSpace) ? FVector(W2LTransformFVector(Loc)) : Loc;

		FBox LS_Box = FBox::BuildAABB(LS_Loc,Extent);
...
void UNavigationMeshBase::GetIntersectingPolys(const TArray<VERTID>& PolyVerts, TArray<FNavMeshPolyBase*>& out_Polys)
{
	FBox PolyBounds(0);
	static TArray<FVector> PolyVertLocs;
	PolyVertLocs.Reset(PolyVertLocs.Num());
	for(INT Idx=0;Idx<PolyVerts.Num();Idx++)
	{
		PolyVertLocs.AddItem(GetVertLocation(PolyVerts(Idx),LOCAL_SPACE));
	}

	GetIntersectingPolys(PolyVertLocs,out_Polys,LOCAL_SPACE);
}
...
*/	
void UNavigationMeshBase::GetIntersectingPolys(const TArray<FVector>& PolyVerts, TArray<FNavMeshPolyBase*>& out_Polys, UBOOL bWorldSpace)
{
	FBox PolyBounds(0);
	static TArray<FVector> PolyVertLocs;
	PolyVertLocs.Reset(PolyVertLocs.Num());
	...
	PolysThatIsect.Reset(PolysThatIsect.Num());
	...
	GetIntersectingPolys(PolyBounds.GetCenter(), PolyBounds.GetExtent(),PolysThatIsect,LOCAL_SPACE,TRUE);
...
UBOOL   APylon::IsPtWithinExpansionBounds(const FVector& Pt, FLOAT Buffer)
{
	...
	if(PathObjectsThatAffectThisPylon != NULL)
	{
		for( APylon::PathObjectList::TIterator It(PathObjectsThatAffectThisPylon->GetHead()); It; ++It )
		{
			IInterface_NavMeshPathObject* CurrentPO = *It;
								   ...
								   PolyList* OutPutPolys=NULL)
{
	...
	for(SortedEdgeList::TIterator Itt(SegmentEdges.GetHead());Itt;++Itt)
	{
		...
		if(NewEventDist > GAP_EPSILON)
		{
			...
			FNavMeshPolyBase* NewPoly = ObstacleMesh->AddPoly(Verts,200.f,WORLD_SPACE);
	...
	for(INT Idx=0;Idx<NormalMesh->Polys.Num();Idx++)
	{
		...
		if( !bSecondPass )
		{
			for( INT Idx=0;Idx<PolyObstacleInfo->LinkedObstacles.Num();++Idx)
			{
				...
				{			
					Verts.Reset(Verts.Num());
					LinkedObstacle->GetBoundingShape(Verts);
					...
					for(INT Vertidx=0; Vertidx< Verts.Num();++Vertidx)
					{
						...
						if( PolyObstacleInfo->Poly->ContainsPoint( (CurPt+NextPt)*0.5f, LOCAL_SPACE) )
						{
							...
							FNavMeshPolyBase* NewPoly = ObstacleMesh->AddPoly(AddVerts,200.f,LOCAL_SPACE);
							if( NewPoly != NULL )
							{
								bAddedGeo=TRUE;
											}
...
UBOOL BuildObstacleMesh(UNavigationMeshBase* NormalMesh, UNavigationMeshBase* ObstacleMesh, UBOOL bSkipSimplification=FALSE, PolyList* OutPutPolys=NULL)
{
	...
	for(INT Idx=0;Idx<NormalMesh->Polys.Num();Idx++)
	{
		...
		for(INT VertIdx=0;VertIdx<CurPoly.PolyVerts.Num();VertIdx++)
		{
			...
			Temp_Edges.Reset(Temp_Edges.Num());
			...
			AdjacentEdges.Reset(AdjacentEdges.Num());
			...
			if ( AddObstacleGeoForPolySegment(NormalMesh,ObstacleMesh,SegmentEndPoint0,SegmentEndPoint1,Vert0, Vert1, SegmentEdges,OffsetVec,&CurPoly, OutPutPolys) )
			{
				bAddedGeo = TRUE;
			}
		}
	...
	if( Polys.Num() > 0 )
	{
		...
		{			
			...
			//    the segments of the poly that are associated with the edges linked to it
			{
				...
				EdgesToConsider.Reset(EdgesToConsider.Num());
				...
				for(INT EdgeToConsiderIdx=0;EdgeToConsiderIdx<EdgesToConsider.Num();++EdgeToConsiderIdx)
				{
					...
					NewPolyPts.Reset(NewPolyPts.Num());
				...
				AddObstacleGeoForPolySegment(MeshThatOwnsTheEdges,this,PolySegmentPt0,PolySegmentPt1,SegmentPt0, SegmentPt1, SegmentEdges,HeightOffset,NULL);
			}
		}
	}
 ...
 */
void FNavMeshPolyBase::AdjustPositionToDesiredHeightAbovePoly(FVector& out_Pt, FLOAT DesiredHeight)
{
	...
	const FPlane PolyPlane = GetPolyPlane();
...
void APylon::CreateExtraMeshData( AScout* Scout )
{
	...
	for( APylon::PathObjectList::TIterator It(PathObjects.GetHead()); It; ++It)
	{
		GInitRunaway(); // path objects may end up calling eventFUNC which will eventually cause runaway loop assert if there are a lot of them
		IInterface_NavMeshPathObject* CurrentPO = *It;
...
void AScout::GetNavMeshPassList( TArray<NavMashPassFunc>& PassList )
{
...
UBOOL APylon::NavMeshPass_InitialExploration()
{
	...
	PathObjectsThatAffectThisPylon = new APylon::PathObjectList();
	...
	for(APylon::PathObjectList::TIterator It(PathObjects.GetHead());It;++It)
	{
		...
		{
			PathObjectsThatAffectThisPylon->AddTail(CurrentPO);
		}
	}
 ...
 */
void FNavMeshEdgeBase::UpdateEdgeCenter( UNavigationMeshBase* NavMesh )
{
	if( NavMesh )
	{
...
// 		GWorld->GetWorldInfo()->DrawDebugBox( EntityPosition, FVector(3), 0, 0, 255, TRUE );
	...
	if( (EdgePerp | (PolyEntIsIn->GetPolyCenter(WORLD_SPACE)-V0).SafeNormal()) < 0.f )
	{
		EdgePerp *= -1.0f;
	}
	...
	if( Handle->PointReachable(ClosestEdgePoint,InEntityPosition) )
	{
		DestinationPosition = ClosestEdgePoint;
	}
	...
	else if( DistFromEdgeRey < PathParams.SearchExtent.X && DistToEdge > PathParams.SearchExtent.X )
	{
		...
		if( !PolyEntIsIn->ContainsPoint(DestinationPosition,WORLD_SPACE) )
		{
 ...
 * @param EdgeDir - optional parameter, of non-null will be populated with this edge's direction
 * @return - the perp vector
 */
FVector FNavMeshEdgeBase::GetEdgePerpDir(UBOOL bWorldSpace,FVector* EdgeDir)
{
	FVector ThisEdgeDir = (GetVertLocation(0,bWorldSpace) - GetVertLocation(1,bWorldSpace)).SafeNormal();
...
/**
 *	Overidden to call SpecialMoveThruEdge on pawn
 * @param C - controller about to traverse this edge
 * @param out_Movept - point where this we should move to cross this edge (stuffed with non-special edge value to begin)
 * @return TRUE if this edge handled all movement to this edge, FALSE if the AI should still move to movept
 */
UBOOL FNavMeshSpecialMoveEdge::PrepareMoveThru( AController* C, FVector& out_MovePt )
{
	if( C != NULL && C->Pawn != NULL )
	{
		return C->Pawn->eventSpecialMoveThruEdge( EdgeType, MoveDir, out_MovePt, *MoveDest, *RelActor, RelItem );
	}
...
UBOOL FNavMeshMantleEdge::PrepareMoveThru( AController* C, FVector& out_MovePt )
{
	if( C != NULL && C->Pawn != NULL )
	{
		...
		{
			return C->Pawn->eventSpecialMoveThruEdge( EdgeType, MoveDir, out_MovePt, out_MovePt+Offset, *RelActor, RelItem );
		}
	}
...
UBOOL FNavMeshDropDownEdge::PrepareMoveThru( AController* C, FVector& out_MovePt )
{
	if( C != NULL && C->Pawn != NULL )
	{
		...
		if( (Closest-PawnLoc).Size2D() < ArrivalOffset )
		{
			...
			return C->Pawn->eventSpecialMoveThruEdge( GetEdgeType(), appTrunc(DropHeight), out_MovePt, MoveDestPoint);
		}
		else
		{
			return FALSE;
...
void FNavMeshEdgeBase::GetAllStaticEdgesInGroup(FNavMeshPolyBase* SrcPoly, TArray<FNavMeshEdgeBase*>& out_EdgesInGroup)
{
	...
	for( INT EdgeIdx = 0; EdgeIdx < SrcPoly->PolyEdges.Num(); ++EdgeIdx )
	{
		FNavMeshEdgeBase* CurEdge = SrcPoly->NavMesh->GetEdgeAtIdx(SrcPoly->PolyEdges(EdgeIdx));  
...
UBOOL FNavMeshCoverSlipEdge::PrepareMoveThru( class AController* C, FVector& out_MovePt )
{
	...
	if( C != NULL && C->Pawn != NULL )
	{
		return C->Pawn->eventSpecialMoveThruEdge( GetEdgeType(), 0, out_MovePt, out_MovePt, RelActor,RelItem );
	}
 ...
 */
FLOAT ExpansionObstacleEdgeVertTolerance = 0.1f;
#define OBSTACLE_EDGE_VERT_TOLERANCE ExpansionEdgeVertTolerance
void TryToLinkPolyToEdge(UNavigationMeshBase* ObstacleMesh, FNavMeshPolyBase& Poly, const FVector& EdgeVert0, const FVector& EdgeVert1, WORD EdgeIdx, UBOOL bDynamicEdge, FNavMeshCrossPylonEdge* CPEdge=NULL)
{
	check(!bDynamicEdge);
	...
	if(bDynamicEdge)
	{
		// if this is a dynamic edge only disable obstacle geo if both the obstacle mesh edge and the real edge have similar endpoints

		FLOAT Dist1 = Min<FLOAT>((EdgeVert0-BottomVert0).SizeSquared2D(),(EdgeVert0-BottomVert1).SizeSquared2D());
		FLOAT Dist2 = Min<FLOAT>((EdgeVert1-BottomVert0).SizeSquared2D(),(EdgeVert1-BottomVert1).SizeSquared2D());
...
// 	}
	...
	if( GetPylon()->bImportedMesh )
	{
		...
		BorderEdgeSegments.Reset(BorderEdgeSegments.Num());
		for(INT PolyIdx=0;PolyIdx<Polys.Num();PolyIdx++)
		{
			FNavMeshPolyBase* Poly = &Polys(PolyIdx);
...
void UNavigationMeshBase::CreateDynamicEdgesForPylonAtoPylonB(APylon* PylonA, APylon* PylonB)
{
	...
	PylonB->GetIntersectingPolys(PylonABounds.GetCenter(), PylonABounds.GetExtent(),PylonBIntersectingPolys,TRUE);
	...
	if(LocalBorderEdgeSegList.Num() < 1)
	{
		...
		PylonA->GetIntersectingPolys(PylonBBounds.GetCenter(), PylonBBounds.GetExtent(),PylonAIntersectingPolys,TRUE);
	...
	for(INT BorderEdgeSegIdx=0;BorderEdgeSegIdx<LocalBorderEdgeSegList.Num();BorderEdgeSegIdx++)
	{
		...
		if(PylonBBounds.IsInside(Ctr))
		{
			...
			for(INT PolyIdx=0;PolyIdx<PylonBIntersectingPolys.Num();PolyIdx++)
			{
				...
				if(Poly->IntersectsPoly2D(CPLS_Pt0,CPLS_Pt1,Entry,Exit))
				{
					...
					if((Entry-Exit).Size() > EDGE_VERT_TOLERANCE)
					{
						...
						FVector PolyCtr = Poly->GetPolyCenter(WORLD_SPACE);
						PointDistToSegment(PolyCtr,WS_Entry,WS_Exit,ClosestPt);				

						if( Abs<FLOAT>(ClosestPt.Z - PolyCtr.Z) < NAVMESHGEN_MAX_STEP_HEIGHT )
						{
							...
							for(INT Idx=0;Idx<ConnectingPolys.Num();++Idx)
							{
								...
								if(ObsMesh != NULL && ObsMesh->Polys.Num() > 0)
								{
									...
									{
										FNavMeshPolyBase& Poly = ObsMesh->Polys(PolyIdx);
										TryToLinkPolyToEdge(ObsMesh,Poly,LS_Entry,LS_Exit,MAXWORD,FALSE);
									}
								}
							}
							...
							EdgePairs.Reset(EdgePairs.Num());
							BuildEdgesFromSegmentSpan(WS_Entry,WS_Exit,EdgePairs);
							...
							for(INT EdgePairIdx=0;EdgePairIdx<EdgePairs.Num();++EdgePairIdx)
							{
								...
								PylonB->NavMeshPtr->AddDynamicCrossPylonEdge<FNavMeshCrossPylonEdge>(EdgePair.Pt0,EdgePair.Pt1,ConnectingPolys,EdgePair.SupportedWidth,EdgePair.EdgeGroupID);	
...
void UNavigationMeshBase::PruneDynamicVerts()
{
	...
	if( Waste > MAX_WASTE || NumDynamicVerts > MAX_VERT_THRESH )
	{
		...
		VertsInUse.Reset(VertsInUse.Num());
...
FNavMeshPolyBase* UNavigationMeshBase::AddPoly( const TArray<FVector>& inVerts, FLOAT PolyHeight, UBOOL bWorldSpace, FLOAT MaxVertSnapHeight/*=-1.0f*/, FLOAT MaxVertSnapDist/*=-1.0f*/  )
{
	...
	VertIndices.Reset(VertIndices.Num());
 ...
 * @param Poly - the poly to snap internal concave verts for
 */
#define CLOSE_EDGE_TOLERANCE NAVMESHGEN_STEP_SIZE * 0.8f
#define MIN_Z_SNAP_HEIGHT 5.0f
...
void UNavigationMeshBase::SnapCloseInternalConcaveVertsForPoly(FNavMeshPolyBase* Poly)
{
	...
	for( INT OuterVertIdx=0; OuterVertIdx<Poly->PolyVerts.Num(); ++OuterVertIdx)
	{
		...
		if( IsVertexOnEdge(Poly->PolyVerts(OuterVertIdx),this,EList,FALSE) ||
			IsVertexOnEdge(Poly->PolyVerts(OuterVertIdx),this,BlankList,TRUE))
		{
			continue;
		}
		...
		for(INT InnerVertIdx=0;InnerVertIdx<Poly->PolyVerts.Num();++InnerVertIdx)
		{
				...
				&& T < 1.0f -KINDA_SMALL_NUMBER)
			{
				...
				if( ZDelta > MIN_Z_SNAP_HEIGHT && ZDelta < NAVMESHGEN_MAX_STEP_HEIGHT )
				{
					//GWorld->GetWorldInfo()->DrawDebugLine(CurrentCheckVert,Closest,255,255,0,TRUE);
					VERTID NewVert = AddVert(Closest,TRUE,5.f);
	...
	for(INT LocalVertIdx=0;LocalVertIdx<Poly->PolyVerts.Num();++LocalVertIdx)
	{
		...
		for(INT VertIdx=0;VertIdx<NearVerts.Num();++VertIdx)
		{
			...
			if ( NoZDist < EDGE_VERT_TOLERANCE )
			{
				...
				if( VertDist > NAVMESHGEN_EDGE_ALIGN_VERT_DIST && VertDist < NAVMESHGEN_EDGE_ALIGN_VERT_DIST_MAX)
				{
...
UBOOL IsVertexOnEdgeSegment(const FVector& VertLoc, const FVector& SegPt0, const FVector& SegPt1, UBOOL bEndPointExclusive, FLOAT Tolerance=-1.0f)
{
	...
	{
		if(!bEndPointExclusive || 
			( !(Closest-NextVertNoZ).IsNearlyZero() && !(Closest-CurVertNoZ).IsNearlyZero())
			)
		{
			return TRUE;
		}
...
/**
 * returns TRUE If hte passed point is ont he segment formed bey Pt0,Pt1
 ...
 * @return - the length of hte longest poly
 */
...
{
	if( ! DO_SIMPLIFICATION || !ExpansionDoPolyMerge || ExpansionDoConcaveSlabsOnly )
	{
		return 0;
	}
	...
	while( NumRemoved == -1 || NumRemoved > 0 )
	{
		...
		{		
			...
			{				
				...
				CountOccurences(Poly->PolyVerts,NumOccurances);
				checkSlowish(!Poly->PolyVerts.ContainsItem(65535));
			}
		}
										...
										)
{
	...
	PolyA->NavMesh->CreateEdgeConnectionsInternal(TempEdges,FALSE,FALSE,FALSE,NULL,&Edge_Segments,0.1f,GetStepSize(NAVMESHGEN_MAX_SUBDIVISIONS));
	...
	for(INT OuterEdgeIdx=Edge_Segments.Num()-1;OuterEdgeIdx>=0;--OuterEdgeIdx)
	{
		for(INT EdgeIdx=0;EdgeIdx<Edge_Segments.Num();++EdgeIdx)
		{
			...
			if( 
				( FPointsAreNear(Edge_Segments(EdgeIdx).Pt0,Edge_Segments(OuterEdgeIdx).Pt0,0.01f) ||
				  FPointsAreNear(Edge_Segments(EdgeIdx).Pt0,Edge_Segments(OuterEdgeIdx).Pt1,0.01f))
				&& 				
				( FPointsAreNear(Edge_Segments(EdgeIdx).Pt1,Edge_Segments(OuterEdgeIdx).Pt0,0.01f) || 
				  FPointsAreNear(Edge_Segments(EdgeIdx).Pt1,Edge_Segments(OuterEdgeIdx).Pt1,0.01f))
			  )
			{
 ...
 */
UBOOL PopBestState(OpenListType& OpenList,FDecompositionState& out_BestState)
{
...
UBOOL WalkPolyAndTryToSplit( TArray<INT> &InflectionVerts,
							INT InflectionIdx,
							const FDecompositionState &BaseState,
...
#define CCW_DIR -1
	...
	const VERTID Inflection_NextID = BaseState.ConcavePoly((InnerConcaveVertIdx+1) % BaseState.ConcavePoly.Num());
	const VERTID Inflection_PrevID = BaseState.ConcavePoly((InnerConcaveVertIdx==0) ? BaseState.ConcavePoly.Num()-1 : InnerConcaveVertIdx-1);
	...
	for(INT InnerLoopCount=0;InnerLoopCount<BaseState.ConcavePoly.Num();++InnerLoopCount)	
	{
		Total++;
		InnerConcaveVertIdx = Increment(InnerConcaveVertIdx,CW_DIR,BaseState.ConcavePoly.Num());
		const VERTID InnerConcaveVertID = BaseState.ConcavePoly(InnerConcaveVertIdx);
		...
		const VERTID Next_InnerConcaveVertID = BaseState.ConcavePoly(Increment(InnerConcaveVertIdx,CW_DIR,BaseState.ConcavePoly.Num()));
		const VERTID Prev_InnerConcaveVertID = BaseState.ConcavePoly(Increment(InnerConcaveVertIdx,CCW_DIR,BaseState.ConcavePoly.Num()));
		...
		PolyA.Reset(PolyA.Num());
		PolyB.Reset(PolyB.Num());

		SplitPolyAtLocalVertIndexes(BaseState.ConcavePoly,InflectionVerts(InflectionIdx),InnerConcaveVertIdx,PolyA,PolyB);
		...
		// if this split intersects existing edges of the poly.. skip it
		if(DoesSplitIntersectExistingEdge(Mesh,InflectionID,InnerConcaveVertID,BaseState.ConcavePoly,TRUE))
		{
			EdgeIsect++;
			continue;
		}

		checkSlow(PolyA.Num() + PolyB.Num() == BaseState.ConcavePoly.Num()+2);



		INT NewConvexSize=0;
		FDecompositionState NewState;
		NewState.InitFrom(BaseState);
 ...
 */
UBOOL AddSplitPermutationsToOpenList(UNavigationMeshBase* Mesh, OpenListType& OpenList, ClosedListType& ClosedList, const FDecompositionState& BaseState, UBOOL bUseAllVerts)
{
	FDecompositionState NewState;

	static TArray<INT> InflectionVerts;
	InflectionVerts.Reset(InflectionVerts.Num());
	FindInflectionVerts(Mesh,BaseState.ConcavePoly,InflectionVerts, bUseAllVerts);
	...
	{
		const VERTID InflectionID = BaseState.ConcavePoly(InflectionVerts(InflectionIdx));
		// walk from inflection point and link to verts
		if ( WalkPolyAndTryToSplit(InflectionVerts, InflectionIdx, BaseState, InflectionID, Mesh, OpenList, ClosedList) )
		{
			return TRUE;
		}
	}
...
INT UNavigationMeshBase::DecomposePolyToConvexPrimitives( FNavMeshPolyBase* Poly, INT FailDepth )
{
	...
	if( FailDepth < MAX_FAIL_DEPTH && FailDepth > 0 )
	{

		for(INT VertIdx=0;VertIdx<Poly->PolyVerts.Num();++VertIdx)
		{
			const FVector Current = GetVertLocation(Poly->PolyVerts(VertIdx))+FVector(0,0,60);
			const FVector Next = GetVertLocation(Poly->PolyVerts( (VertIdx+1) % Poly->PolyVerts.Num()))+FVector(0,0,60);
		}
	...
	while(PopBestState(OpenList,CurrentState))
	{
		...
		{
			CurrentState = OpenList.GetHead()->GetValue();
		}

		// if there is nothing concave left over, we're done!
		if(CurrentState.ConcavePoly.Num() == 0)
		{
			bFoundSolution=TRUE;
			break;
		}
...
*/
	...
	if(bDrawDebug && !bFoundSolution)
	{
		...
		for(INT ConcaveIdx=0;ConcaveIdx<CurrentState.ConcavePoly.Num();++ConcaveIdx)
		{
			...
			GWorld->GetWorldInfo()->DrawDebugLine(Current,Next,0,0,255,TRUE);
		}
	}
#endif 
...
UBOOL UNavigationMeshBase::PerformEdgeSmoothingStep( 				
	...
	if(NAVMESHGEN_DO_EDGESMOOTHING)
	{
			...
			)
		{
			...
			if( GetPylon() && 
				(!GetPylon()->IsPtWithinExpansionBounds(WS_Cur,-2.0f*NAVMESHGEN_STEP_SIZE) ||
				 !GetPylon()->IsPtWithinExpansionBounds(WS_Next,-2.0f*NAVMESHGEN_STEP_SIZE) ||
				 !GetPylon()->IsPtWithinExpansionBounds(WS_Prev,-2.0f*NAVMESHGEN_STEP_SIZE))
				  &&
				 OffDist > 0.1f)
			{
				return FALSE;
...
/**
 * Determines if hte passed vertex is part of an edge that the passed point is on top of
...
// 		GWorld->GetWorldInfo()->DrawDebugLine(BoxBounds.GetCenter(),InPoint,255,0,0,TRUE);
	...
	if(Abs<FLOAT>(NavMesh->L2WTransformNormal(PolyNormal).Z)>= 0.7f)
	{
		...
		else
		{
			...
			for (INT Idx = 0, Jdx = PolyVerts.Num()-1; Idx < PolyVerts.Num(); Jdx = Idx++)
			{
				...
				VertI += (VertI - PolyCenter).SafeNormal()*Epsilon; // expand out so we include verticies/edges of original poly
				FVector VertJ = NavMesh->Verts(PolyVerts(Jdx));
				VertJ += (VertJ - PolyCenter).SafeNormal()*Epsilon; // expand out so we include verticies/edges of original poly
 ...
 */
UBOOL UNavigationMeshBase::IntersectsPolyBounds(const FVector& Center, const FVector& Extent, FNavMeshPolyBase*& out_IntersectingPoly, UBOOL bWorldSpace )
{
	static TArray<FNavMeshPolyBase*> Polys;
	Polys.Reset(Polys.Num());
	GetIntersectingPolys(Center,Extent,Polys,bWorldSpace,TRUE);
...
UBOOL FNavMeshPolyBase::IntersectsPoly2D(const FVector& In_SegPt0, const FVector& In_SegPt1, FVector& out_EntryPoint, FVector& out_ExitPoint,UBOOL bWorldSpace/*=LOCAL_SPACE*/)
{
	...
	if(SegPt0.Equals(SegPt1,0.1f))
	{
		out_ExitPoint=SegPt0;
		out_EntryPoint=SegPt0;
		return ContainsPoint(SegPt0,bWorldSpace);
	}
 ...
 */
UBOOL UNavigationMeshBase::IntersectsPoly(TArray<FVector>& InPolyVertLocs, FNavMeshPolyBase*& out_IntersectingPoly,TArray<FNavMeshPolyBase*>* ExclusionPolys, UBOOL bWorldSpace, const FLOAT MinNormalZ/*=-1.f*/)
{
	FBox PolyBounds(0);
	...
	static TArray<FNavMeshPolyBase*> Polys;
	Polys.Reset(Polys.Num());
	GetIntersectingPolys(PolyBounds.GetCenter(),PolyBounds.GetExtent(),Polys,LOCAL_SPACE,TRUE);
 ...
 * @return TRUE if the poly described by the list of points intersects the mesh
 */
UBOOL UNavigationMeshBase::IntersectsPoly(TArray<VERTID>& InPoly)
{
	FBox PolyBounds(0);
	...
	return IntersectsPoly(PolyVertLocs,Poly);
...
void UNavigationMeshBase::GetAllVertsNearPoint(const FVector& Pt, const FVector& Extent, TArray<VERTID>& Result)
{
	...
	if( KDOPInitialized )
	{
		...
		FNavMeshCollisionDataProvider Prov(this,this);
...
FNavMeshPolyBase* UNavigationMeshBase::GetPolyFromBox(const FBox& Box, FLOAT MinWalkableZ, UBOOL bWorldSpace)
{
	...
	for(INT PolyIdx=0;PolyIdx<Polys.Num();++PolyIdx)
	{
		...
		{
			if(CurPoly->ContainsPoint(LS_Box.GetCenter(),LOCAL_SPACE))
			{
...
void UNavigationMeshBase::BuildKDOP(UBOOL bFromBuildStructures)
{
	...
	KDOPTriangles.Reset(KDOPTriangles.Num());
	...
	if( bFromBuildStructures )
	{
		...
		INT Count = 0;
		BuildPolyIndexMap.Reset(BuildPolys.Num());
...
UBOOL UNavigationMeshBase::LineCheck( UNavigationMeshBase* ForNavigationMesh, FCheckResult& Result, const FVector& End, const FVector& Start, const FVector& Extent, DWORD TraceFlags, FNavMeshPolyBase** out_HitPoly )
{
	...
	FNavMeshCollisionDataProvider Prov(this,ForNavigationMesh);
...
#define BESTCYLLOCISECT_THRESH 5.0f
	...
	{
		AdjustPositionToDesiredHeightAbovePoly(ClosestEdgePt,CylHalfHeight);
	...
	if( BestCornerDist < BIG_NUMBER )
	{
		...
		AdjustPositionToDesiredHeightAbovePoly(ClosestCornerPt,CylHalfHeight);
...
FNavMeshEdgeBase* FNavMeshPolyBase::GetEdgeFromIdx(INT Idx,UNavigationMeshBase* MeshToUse, UBOOL bAllowInvalidEdges)
{
	...
	{
		Edge = MeshToUse->GetEdgeAtIdx(PolyEdges(Idx));
	}
	else
	{
		Idx -= PolyEdges.Num();
	...
	if(!bAllowInvalidEdges && !Edge->IsValid())
	{
		return NULL;
	}
 ...
 * @param EdgeIdx - the Index of the edge (local to this poly)
 */
void FNavMeshPolyBase::UpdateDynamicObstaclesForEdge(INT SessionID, INT EdgeIdx)
{
	FNavMeshEdgeBase* Edge = GetEdgeFromIdx(EdgeIdx);
	if(Edge == NULL)
	{
		return;
	}

	FNavMeshPolyBase* OtherPoly = Edge->GetOtherPoly(this);
	if(OtherPoly == NULL)
	{
		return;
	}
	...
	if(TopMostMesh->SavedSessionID == SessionID)
	{
		return;
	}
...
UBOOL IInterface_NavMeshPathObstacle::RegisterObstacleListWithNavMesh(TArray<IInterface_NavMeshPathObstacle*>& Obstacles)
{
	...
	for( INT ObstacleIdx=0;ObstacleIdx<Obstacles.Num();++ObstacleIdx)
	{
		...
		BoundingShape.Reset(BoundingShape.Num());

		if(!CurObstacle->GetBoundingShape(BoundingShape))
		{
			continue;
		}
		...
		for(INT PylonIdx=0;PylonIdx<Pylons.Num();++PylonIdx)
		{
			...
			if( CurPylon->IsValid() )
			{
				CurPylon->GetIntersectingPolys(Ctr,Extent,Polys,TRUE,TRUE);
			}
		}
...
UBOOL IInterface_NavMeshPathObstacle::RegisterObstacleWithNavMesh()
{
	...
	if(!GetBoundingShape(BoundingShape))
	{
		return FALSE;
	}
	...
	UBOOL bRegisterStatus = RegisterObstacleWithPolys(BoundingShape,Polys);
...
void IInterface_NavMeshPathObstacle::UpdateAllDynamicObstaclesInPylonList(TArray<APylon*>& Pylons)
{
	...
	ObstaclesThatWereJustBuilt.Reset(ObstaclesThatWereJustBuilt.Num());
...
UBOOL IInterface_NavMeshPathObstacle::UnregisterObstacleWithNavMesh()
{
	...
	static TArray<FNavMeshPolyBase*> PolysThatNeedEdgeFixup;
	PolysThatNeedEdgeFixup.Reset(PolysThatNeedEdgeFixup.Num());
	...
	// make sure edge delete notifications aren't sent out while we're cleaning up!
	{
		...
		for(INT PolyIdx=0;PolyIdx<Polys.Num();++PolyIdx)
		{
			...
			if(CurPoly != NULL)
			{
				...
				// if we just removed the last one, remove the info from the list
				if(CurPoly->NumObstaclesAffectingThisPoly == 0)
				{
					PolysThatNeedEdgeFixup.AddItem(CurPoly);
	...
	DoEdgeFixupForNewlyClearedPolys(PolysThatNeedEdgeFixup);
...
UBOOL UNavigationMeshBase::SplitMeshAboutEdge(const FVector& EdgePt0, const FVector& EdgePt1, const FVector& Up, FLOAT Height)
{
	...
	for(PolyList::TIterator It(BuildPolys.GetTail());It;)
	{
		...
		if(!bHeightEarlyOut && (!bCheckPlaneAABB||FPlaneAABBIsect(SplitPlane,TransformedBox)) && (!bOnlySplitEdgeIntersectors||CurPoly->IntersectsPoly2D(LS_EdgePt0,LS_EdgePt1,EntryPt,ExitPt,LOCAL_SPACE)))
		{
			...
			Poly1.Reset(Poly1.Num());
			static TArray<FVector> Poly2;
			Poly2.Reset(Poly2.Num());
...
void UNavigationMeshBase::CleanupMeshReferences(FPolyObstacleInfo* ObstacleInfoForThisMesh/*=NULL*/)
{
	...
	Edges_that_ref_me.Reset(Edges_that_ref_me.Num());
...
UBOOL UNavigationMeshBase::BuildSubMeshForPoly(WORD PolyIdx, TArray<FPolyObstacleInfo*>& out_ObstaclesThatWereJustBuilt)
{
	...
	CurPolyVerts.Reset(CurPoly->PolyVerts.Num());
	...
	for(INT AffectingObstacleIdx=0;AffectingObstacleIdx<Info->LinkedObstacles.Num();++AffectingObstacleIdx)
	{
		...
		if(!CurObstacle->GetBoundingShape(ShapeInfo.Shape))
		{
			continue;
		}
	...
	InternalMeshes.Reset(InternalMeshes.Num());
