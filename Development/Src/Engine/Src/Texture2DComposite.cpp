...
*/
static void ValidateSourceRegions(const TArray<FSourceTexture2DRegion>& SourceRegions,TArray<FSourceTexture2DRegion>& ValidRegions)
{
...
void UTexture2DComposite::UpdateCompositeTexture(INT NumMipsToGenerate)
{
	...
	ValidateSourceRegions(SourceRegions,ValidRegions);
	...
	else
	{
		...
		const INT MaxTexMips = appCeilLogTwo(Max(SrcSizeX,SrcSizeY))+1;
		...
		CompositeResource->SizeX = SrcSizeX;
		CompositeResource->SizeY = SrcSizeY;
...
void UTexture2DComposite::RenderThread_CopyRectRegions(const TArray<FSourceTexture2DRegion>& ValidRegions)
{
	...
	DWORD CreateFlags = ( CompositeResource->bSRGB && (GEngine == NULL || !GEngine->bEmulateMobileRendering) ) ? TexCreate_SRGB : 0;
	...
	for( INT MipIdx=0; MipIdx < CompositeResource->NumMips; MipIdx++ )
	{
		...
		for( INT RegionIdx=0; RegionIdx < ValidRegions.Num(); RegionIdx++ )		
		{
				...
				Region.Texture2D->ResidentMips == Region.Texture2D->RequestedMips )
			{
				...
				check( Region.Texture2D->ResidentMips >= CompositeResource->NumMips );
				...
				FCopyTextureRegion2D& CopyRegion = *new(CopyRegions) FCopyTextureRegion2D(
					...
					Region.Texture2D->ResidentMips - CompositeResource->NumMips
					);
			}
		}
