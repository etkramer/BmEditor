...
UBOOL FPrimitiveSceneProxy::IsShadowCast(const FSceneView* View) const
{
...
#endif
	{
		...
		CONSOLE_PREFETCH(&MaxDrawDistance);
	...
	if(DistanceSquared * Square(View->LODDistanceFactor) > Square(MaxDrawDistance))
	{
		return FALSE;
	} 
...
UBOOL FPrimitiveSceneProxy::HasRelevantStaticDecals(const FSceneView* View) const
{
	if( View->Family->ShowFlags & SHOW_Decals )
	{
		...
		for( INT DecalIndex = 0 ; DecalIndex < Decals.Num() ; ++DecalIndex )
		{
			...
			if( Interaction->DecalStaticMesh )
			{
				return TRUE;
			}
		}
...
{
	if( View->Family->ShowFlags & SHOW_Decals &&
		GSystemSettings.RenderThreadSettings.bAllowUnbatchedDecals )
	{
#if !FINAL_RELEASE
		if( IsRichView(View) )
		{
 ...
 * @param Platform		The platform to get material textures for.
 * @param bAllPlatforms	Whether to iterate for all platforms. The Platform parameter is ignored if this is set to TRUE.
 */
void UPrimitiveComponent::GetUsedTextures(TArray<UTexture*> &OutTextures, EMaterialShaderPlatform Platform /*= MSP_BASE*/, UBOOL bAllPlatforms /*= FALSE*/)
{
...
UBOOL UPrimitiveComponent::ShouldComponentAddToScene() const
{
	...
	const UBOOL bShowInGame					= !HiddenGame && (!Owner || !Owner->bHidden || bIgnoreOwnerHidden);
	const UBOOL bDetailModeAllowsRendering	= DetailMode <= GSystemSettings.DetailMode;
	return bDetailModeAllowsRendering && ((GIsGame && bShowInGame) || (!GIsGame && bShowInEditor) || bCastHiddenShadow);
}
