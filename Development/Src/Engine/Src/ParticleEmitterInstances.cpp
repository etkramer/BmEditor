...
void FParticleEmitterInstance::SetCurrentLODIndex(INT InLODIndex, UBOOL bInFullyProcess)
{
	if (SpriteTemplate != NULL)
	{
		...
		if (bInFullyProcess == TRUE)
		{
			...
			{
				if (SpawnModule->BurstList(BurstIndex).Time < EmitterTime)
				{
...
FLOAT FParticleEmitterInstance::Spawn(FLOAT DeltaTime)
{
	...
	if (GSystemSettings.DetailMode != DM_High && SpriteTemplate->MediumDetailSpawnRateScale == 0)
	{
		// Disable any spawning if MediumDetailSpawnRateScale is 0 and we are not in high detail mode
		// We don't scale down spawn counts based on MediumDetailSpawnRateScale (except to disable completely) 
		// For spawn per unit and bursts because it would look obviously wrong
		SpawnRate = 0;
		SpawnCount = 0;
		BurstCount = 0;
	}
