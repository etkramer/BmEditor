	...
	FShaderFrequencyUniformExpressionValues& InValues) const
{
	...
	{
		// Update the cache
		InValues.Update(InExpressions, MaterialRenderContext, Material, !MaterialRenderContext.MaterialRenderProxy->bCacheable);
		CachedValues = &InValues;
	}
	else 
	{
		// Update the temporary values since we are not caching
		NewValues.Update(InExpressions, MaterialRenderContext, Material, TRUE);
	...
	) const
{
	if (PrimitiveSceneInfo)
	{
		...
		if (ObjectOrientationParameter.IsBound())
		{
			...
			SetShaderValue(Shader,ObjectOrientationParameter,Mesh.LocalToWorld.GetAxis(2).SafeNormal());
		}
...
void FMaterialPixelShaderParameters::Set(FShader* PixelShader,const FMaterialRenderContext& MaterialRenderContext, ESceneDepthUsage DepthUsage/*=SceneDepthUsage_Normal*/) const
{
	...
	if( GUsingMobileRHI )
	{
		...
		Material->FillMobileMaterialPixelParams(MobileMaterialPixelParams);
...
#endif
	{
		...
		for(INT ParameterIndex = 0;ParameterIndex < UniformPixelCubeShaderResourceParameters.Num();ParameterIndex++)
		{
			...
			UniformExpressionSet.UniformCubeTextureExpressions(UniformResourceParameter.Index)->GetTextureValue(MaterialRenderContext,*Material,Value);
			if (!Value)
			{
	...
	if( EnableScreenDoorFadeParameter.IsBound() )
	{
		...
		if( bIsCurrentlyFading )
		{
			...
			FVector4 ScreenDoorFadeSettings2;
			{
				...
				// Set the noise texture UV scaling amount.  Note that currently the noise texture is 1024x1024.
				ScreenDoorFadeSettings.W = 8.0f * 0.001f;		// W = Noise texture scale
				...
				ScreenDoorFadeSettings2.Z = 0.0f;
				ScreenDoorFadeSettings2.W = 0.0f;
			}
			SetPixelShaderValue(
				PixelShader->GetPixelShader(),
...
/** Sets domain shader parameters that are material specific but not FMeshElement specific. */
...
/** Sets hull shader parameters that are material specific but not FMeshElement specific. */
...
{
	FMaterialShaderParameters::SetMeshShader(HullShader->GetHullShader(),PrimitiveSceneInfo,Mesh,View);
}
...
void FMaterialVertexShaderParameters::Set(FShader* VertexShader,const FMaterialRenderContext& MaterialRenderContext) const
{
	...
	if( GUsingMobileRHI )
	{
		...
		Material->FillMobileMaterialVertexParams(MobileMaterialVertexParams);
		//overrides PER proxy (mesh particles)
		MaterialRenderProxy.FillMobileMaterialVertexParams(MobileMaterialVertexParams);
...
#if NGP
//@todo ngp clean: hackery
FVector GPrevBestLightDirection;
FLinearColor GPrevBestLightColor;
#endif
	...
	) const
{
	...
	if( GUsingMobileRHI )
	{
		...
		MobileMeshVertexParams.bEnableSkyLight = PrimitiveSceneInfo != NULL ? PrimitiveSceneInfo->HasDynamicSkyLighting() : FALSE;
...
#if NGP
				GPrevBestLightDirection = MobileMeshVertexParams.BrightestLightDirection;
				GPrevBestLightColor = MobileMeshVertexParams.BrightestLightColor;
#endif
			}
			else
			{
#if NGP
				MobileMeshVertexParams.BrightestLightDirection = GPrevBestLightDirection;
				MobileMeshVertexParams.BrightestLightColor = GPrevBestLightColor;
#else
		...
		MobileMeshVertexParams.LocalToWorld = &Mesh.LocalToWorld;
...
{
	FMaterialShaderMap* Result = GIdToMaterialShaderMap[InPlatform].FindRef(StaticParameterSet);

#if BATMAN
	// BM3 shader cache entries may have extra static switches that our engine doesn't find
	// in the cooked material graph. Do a subset match: if the lookup's switches all exist
	// by name+value in a cache entry with the same BaseMaterialId, use that entry.
	if (!Result && StaticParameterSet.BaseMaterialId.IsValid())
	{
		FMaterialShaderMap* BestMatch = NULL;
		INT BestExtraCount = INT_MAX; // prefer entries with fewest extra switches

		for (TMap<FStaticParameterSet, FMaterialShaderMap*>::TIterator It(GIdToMaterialShaderMap[InPlatform]); It; ++It)
		{
			const FStaticParameterSet& CacheSet = It.Key();
			FMaterialShaderMap* CacheMap = It.Value();

			// Only consider BM3 cache entries with matching BaseMaterialId
			if (!CacheMap->IsFromBmCache() || CacheSet.BaseMaterialId != StaticParameterSet.BaseMaterialId)
			{
				continue;
			}

			// Cache entry must have >= as many switches as the lookup
			if (CacheSet.StaticSwitchParameters.Num() < StaticParameterSet.StaticSwitchParameters.Num())
			{
				continue;
			}

			// Check that every switch in the lookup exists in the cache with the same name+value
			UBOOL bAllMatch = TRUE;
			for (INT i = 0; i < StaticParameterSet.StaticSwitchParameters.Num() && bAllMatch; i++)
			{
				const FStaticSwitchParameter& LookupSwitch = StaticParameterSet.StaticSwitchParameters(i);
				UBOOL bFound = FALSE;
				for (INT j = 0; j < CacheSet.StaticSwitchParameters.Num(); j++)
				{
					if (CacheSet.StaticSwitchParameters(j).ParameterName == LookupSwitch.ParameterName)
					{
						bFound = (CacheSet.StaticSwitchParameters(j).Value == LookupSwitch.Value);
						break;
					}
				}
				if (!bFound)
			...
			// Also check component mask parameters match
			if (bAllMatch && CacheSet.StaticComponentMaskParameters.Num() >= StaticParameterSet.StaticComponentMaskParameters.Num())
			{
				for (INT i = 0; i < StaticParameterSet.StaticComponentMaskParameters.Num() && bAllMatch; i++)
				{
					const FStaticComponentMaskParameter& LookupMask = StaticParameterSet.StaticComponentMaskParameters(i);
					UBOOL bFound = FALSE;
					for (INT j = 0; j < CacheSet.StaticComponentMaskParameters.Num(); j++)
					{
						if (CacheSet.StaticComponentMaskParameters(j).ParameterName == LookupMask.ParameterName)
						{
							bFound = (CacheSet.StaticComponentMaskParameters(j).R == LookupMask.R
								&& CacheSet.StaticComponentMaskParameters(j).G == LookupMask.G
								&& CacheSet.StaticComponentMaskParameters(j).B == LookupMask.B
								&& CacheSet.StaticComponentMaskParameters(j).A == LookupMask.A);
							break;
						}
					}
					if (!bFound)
			...
			else if (CacheSet.StaticComponentMaskParameters.Num() < StaticParameterSet.StaticComponentMaskParameters.Num())
			{
				bAllMatch = FALSE;
			}
			...
			if (bAllMatch)
			{
				INT ExtraCount = CacheSet.StaticSwitchParameters.Num() - StaticParameterSet.StaticSwitchParameters.Num();
