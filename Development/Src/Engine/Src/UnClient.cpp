...
void FViewport::BeginRenderFrame()
{
	...
	GSceneRenderTargets.SetBackBuffer( RHIGetViewportBackBuffer( GetViewportRHI() ) );
}
...
void FViewport::UpdateViewportRHI(UBOOL bDestroyed,UINT NewSizeX,UINT NewSizeY,UBOOL bNewIsFullscreen)
{
...

	{
		...
		GSceneRenderTargets.SetBackBuffer(NULL);
...
{
#if STATS
	SET_FLOAT_STAT( STAT_InputLatencyTime, (FLOAT) (DeltaTime * GSecondsPerCycle) );
