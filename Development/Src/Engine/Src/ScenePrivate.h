...
public:
		...
		UBOOL& bOutPrimitiveIsDefinitelyUnoccluded)
	{
		...
		if (bIsOccludable && View.PrecomputedVisibilityData && VisibilityId >= 0)
		{
...
#if WITH_ES2_RHI
		// ES2 renderer doesn't yet support occlusion queries
		const UBOOL bSupportsOcclusionQueries = !GUsingES2RHI;
#elif USE_GXM_RHI
		// Our GxmRHI doesn't support OQ yet
		const UBOOL bSupportsOcclusionQueries = FALSE;
#else
		const UBOOL bSupportsOcclusionQueries = TRUE;
#endif
		if( !bSupportsOcclusionQueries )
		{
		...
		else
		{
			...
			else if (bIsOccludable)
			{
				...
				DWORD NumPixels = 0;
				if (IsValidRef(PrimitiveOcclusionHistory->GetPastQuery(View.FrameNumber)))
				{
					// NOTE: RHIGetOcclusionQueryResult should never fail when using a blocking call, rendering artifacts may show up.
					if ( RHIGetOcclusionQueryResult(PrimitiveOcclusionHistory->GetPastQuery(View.FrameNumber),NumPixels,TRUE) )
					{
						...
						// Take downsampling into account when calculating the total number of pixels
						const FLOAT NumPossiblePixels = 
							GSceneRenderTargets.UseDownsizedOcclusionQueries() && IsValidRef(GSceneRenderTargets.GetSmallDepthSurface()) ? 
						...
						PrimitiveOcclusionHistory->LastPixelsPercentage = NumPixels / NumPossiblePixels;
		...
		{
			const FBoxSphereBounds OcclusionBounds = GetOcclusionBounds(Primitive.GetOccluderBounds());
			...
			UBOOL bIgnoreNearPlaneIntersection = Primitive.IgnoresNearPlaneIntersection();
			if (View.bHasNearClippingPlane && bIgnoreNearPlaneIntersection)
			{
			...
			if ( bAllowBoundsTest &&
				(View.bHasNearClippingPlane && (ClipDistance < -BoundsPushOut || bIgnoreNearPlaneIntersection) ||
				!View.bHasNearClippingPlane && OcclusionBounds.SphereRadius < HALF_WORLD_MAX) )
			{
				...
				if (bRunQuery && !(View.Family->ShowFlags & SHOW_Wireframe))
				{
					...
					{
						PrimitiveOcclusionHistory->SetCurrentQuery(View.FrameNumber, View.IndividualOcclusionQueries.BatchPrimitive(TranslatedOcclusionBounds));	
					}
					else
					{
						PrimitiveOcclusionHistory->SetCurrentQuery(View.FrameNumber, View.GroupedOcclusionQueries.BatchPrimitive(TranslatedOcclusionBounds));	
					}
				}
			}
			else
...
#if BATMAN
	/** Base pass draw list - BM3 APlus3D character lighting */
