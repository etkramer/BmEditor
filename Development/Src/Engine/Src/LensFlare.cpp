...
void ULensFlareComponent::PostLoad()
{
	...
	NextTraceTime = -0.2f + 0.2f * appFrand();
	Super::PostLoad();
}
				...
				for (AController* Controller = Info->ControllerList; Controller != NULL; Controller = Controller->NextController)
				{
					...
					if (PlayerController && PlayerController->IsLocalPlayerController())
					{
						if (PlayerController->PlayerCamera != NULL)
						{
							...
							FVector POVLoc = PlayerController->PlayerCamera->LastFrameCameraCache.POV.Location;
							FVector POVDir = PlayerController->PlayerCamera->LastFrameCameraCache.POV.Rotation.Vector();
							FVector BoundsOffset = (POVLoc -  LensFlareActor->Location);
							BoundsOffset.Normalize();
							if ( (POVDir | (-1.f*BoundsOffset)) < 0.7f )
							{
							...
							{
							  BoundsOffset *= Bounds.SphereRadius;
							  ...
							  bVisibleForMobile = (Hit.Actor == NULL);
							}
						}
