...
public:
    ...
    TArray<FPostProcessSceneProxy*> PostProcessSceneProxies;
...
public:
	...
	/** The min draw distance override.  When doing visibility calculations, the maximum of
	    this and the object max draw distance will be used.  The default is 0. */
	FLOAT MinViewDistanceSquaredOverride;
    ...
    UBOOL RenderDPGBasePass(UINT DPGIndex, FViewInfo& View, UBOOL bIsTiledRendering);
...
private:
	...
	*/
	...
	UBOOL RenderLightMapDensities(UINT DPGIndex, UBOOL bIsTiledRendering);
	...
	*/
	...
	void ViewSetViewport(UINT ViewIndex, UBOOL bTiledRenderingPass, FLOAT ZNear=0.0f, FLOAT ZFar=1.0f );
	 ...
	 */
	BYTE ProcessPrimitiveCullingInner(const FPrimitiveSceneInfoCompact& CompactPrimitiveSceneInfo, BYTE ValidViews, INT Depth);
	 ...
	 * @param Depth Current hierarchy depth (0 for root primitives)
	 */
	void ProcessPrimitiveCulling(const FPrimitiveSceneInfoCompact** Primitives, INT NumPrimitives, BYTE ValidViews, INT Depth);
};
