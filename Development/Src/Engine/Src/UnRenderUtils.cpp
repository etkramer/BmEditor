...
{
	// We assume GEngine is always initialized
	checkf( GEngine != NULL );
	if( GEngine->bEmulateMobileRendering )
	{
	...
	)
{
	...
	if (!(ShowFlags & SHOW_LOD) && bSelected && PrimitiveInfo->ReplacementPrimitiveMapKey)
	{
		...
		if (Path)
		{
			...
			PDI->DrawLine( Mesh.LocalToWorld.GetOrigin(), CompactInfo.Bounds.Origin, FColor(255, 255, 0), SDPG_World );
...
void EmitMeshDrawEvents(const FPrimitiveSceneInfo* PrimitiveSceneInfo, const FMeshElement& Mesh)
{
#if !FINAL_RELEASE
