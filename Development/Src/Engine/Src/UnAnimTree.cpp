...
private: 
	...
	void ClearAnimNodeSequence(INT Index)
	{
		...
		if (AnimSlotNodeSequencePool(Index).SeqNode)
		{
			...
			// TODO & FIXME: We kind of need to default back here instead of picking variables to clear
			// this is cheaper, but it will be harder to maintain in the future if we add any new variables
...
void UAnimNode::ResetAnimNodeToSource(UAnimNode* SourceNode)
{
	...
	NodeTickTag = NodeInitTag = NodeCachedAtomsTag = SearchTag = 0;
...
// 		{
// 			// log position
// 			debugf(TEXT("(%s) playing animation %s - PreviousTime (%0.2f), CurrentTime (%0.2f)"), *SkelComponent->GetOwner()->GetName(), *InAnimSeqName.GetNameString(), SeqNode->PreviousTime, SeqNode->CurrentTime);
// 		}
	}
}
...
void UAnimNodeSlot::AddToSynchGroup(FName GroupName)
{
	...
	if( SynchNode == NULL && SkelComponent )
	{
		TArray<UAnimNode*> Nodes;
...
void UAnimNodeBlendByProperty::TickAnim(FLOAT DeltaSeconds)
{
	if( SkelComponent && CachedOwner )
	{
		...
		{
			// Look up property in owning actor
			UProperty* CachedProperty = FindField<UProperty>( CachedOwner->GetClass(), *PropertyName.ToString() );
