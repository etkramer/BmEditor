...
void UGuidCache::Merge(UGuidCache* Other)
{
	for (TMap<FName, FGuid>::TIterator It(Other->PackageGuidMap); It; ++It)
	{
		...
		{
			warnf(NAME_Warning, TEXT("Mismatched Guid while merging guid caches for package %s, overwriting with Other"), *It.Key().ToString());
		}
...
void UNetConnection::Close()
{
	if (Driver != NULL)
	{
		...
		if (State != USOCK_Closed)
		{
			...
			debugf(NAME_NetComeGo, TEXT("Close %s %s %s %s"), *Driver->GetDescription(), *GetName(), *LowLevelGetRemoteAddress(TRUE), appTimestamp());
#if WITH_STEAMWORKS
			if( GSteamworksInitialized )
			{
				// Clientside termination with Steam
				if (Driver->ServerConnection != NULL )
				{
				extern void appSteamworksTerminateGameConnection(DWORD Addr, INT Port);
				appSteamworksTerminateGameConnection(GetAddrAsInt(), GetAddrPort());
			}
				// IMPORTANT: The server >MUST< mark the UID as disconnected with Steam, otherwise the client will not be able to
				//		rejoin, as Steam thinks they are still ingame
				else if (UniquePlayerIdForAuth.Uid != 0)
				{
					extern void appSteamworksSendUserDisconnect(FUniqueNetId UID);
					appSteamworksSendUserDisconnect(UniquePlayerIdForAuth);
				}
			}
#endif
		}
...
{
	FString PackageName, ForcedExportName;
	FNetControlMessage<NMT_Uses>::Receive(Bunch, Info.Guid, PackageName, Info.Extension, Info.PackageFlags, Info.RemoteGeneration, ForcedExportName, Info.LoadingPhase);
...
void UNetConnection::SendPackageInfo(FPackageInfo& Info)
{
	...
	FNetControlMessage<NMT_Uses>::Send(this, Info.Guid, PackageName, Info.Extension, Info.PackageFlags, Info.LocalGeneration, ForcedExportName, Info.LoadingPhase);
}
...
void UNetConnection::ReceivedRawPacket( void* InData, INT Count )
{
	...
	debugfSuppressed( NAME_DevNetTraffic, TEXT("%6.3f: Received %i"), appSeconds() - GStartTime, Count );
...
void UNetConnection::ReceivedPacket( FBitReader& Reader )
{
	...
	while( !Reader.AtEnd() && State!=USOCK_Closed )
	{
		...
		if( IsAck )
		{
			...
			debugfSuppressed( NAME_DevNetTraffic, TEXT("   Received ack %i (%.1f)"), AckPacketId, (Reader.GetPosBits()-StartPos)/8.f );
			for( INT i=OpenChannels.Num()-1; i>=0; i-- )
			{
				UChannel* Channel = OpenChannels(i);
		...
		else
		{
			...
			if( Bunch.bReliable )
			{
				debugfSuppressed( NAME_DevNetTraffic, TEXT("   Reliable Bunch, Channel %i Sequence %i: Size %.1f+%.1f"), Bunch.ChIndex, Bunch.ChSequence, (HeaderPos-IncomingStartPos)/8.f, (Reader.GetPosBits()-HeaderPos)/8.f );
			}
			else
			{
			...
			if (Channels[Bunch.ChIndex] == NULL && (Bunch.ChIndex != 0 || Bunch.ChType != CHTYPE_Control))
			{
				...
				// so reject it
				else if (Actor == NULL && Driver->ClientConnections.ContainsItem(this))
				{
...
/**
 * Determine if a voice packet should be replicated from the local sender the a peer connection as identified by a net id.
 *
 * @param DestPlayer - destination net id of the peer connection to send voice packet to
 *
 * @return true if it should be sent on this connection, false otherwise
 */
UBOOL UNetConnection::ShouldReplicateVoicePacketTo(const FUniqueNetId& DestPlayer)
{
	if (Actor &&
		// Has the handshaking of the mute list completed?
		Actor->bHasVoiceHandshakeCompleted)
	{
			...
			Actor->IsPlayerMuted(DestPlayer) == FALSE)
		{
			...
			for (INT Index = 0; Index < Children.Num(); Index++)
			{
				if (Children(Index)->ShouldReplicateVoicePacketTo(DestPlayer) == FALSE)
				{
					// A child wants to mute, so skip
					return FALSE;
				}
