...
//@DEBUG:
// Set to 1 to dump all dynamic component notifications to the log
#define STREAM_NOTIFICATIONS_LOG	0
...
//		FLOAT MipFactor = WantedMips - ResidentMips;
	...
	INT				WantedMips;
	/** Number of wanted mip-maps for dynamic primitives. */
	...
	FLOAT			MinDistance;
	/** Distance to the closest dynamic instance of this texture. */
	FLOAT			DynamicMinDistance;
			...
			if ( appStristr(TextureName, *TrackedTextureName) != NULL )
			{
				if ( bEnableLogging )
				{
					...
					if ( LastEvent->NumResidentMips != Texture->ResidentMips ||
						 ...
						 (StreamingTexture && LastEvent->WantedMips != StreamingTexture->WantedMips) ||
						 (StreamingTexture && LastEvent->DynamicWantedMips != StreamingTexture->DynamicWantedMips) ||
						 bIsDestroying )
					{
						...
						NewEvent.WantedMips				= LastEvent->WantedMips			= StreamingTexture ? StreamingTexture->WantedMips : Texture->RequestedMips;
						NewEvent.DynamicWantedMips		= LastEvent->DynamicWantedMips	= StreamingTexture ? StreamingTexture->DynamicWantedMips : INDEX_NONE;
						FLOAT BoostFactor				= StreamingTexture ? StreamingTexture->BoostFactor : 1.0f;
						...
						debugf( NAME_DevStreaming, TEXT("Texture: \"%s\", ResidentMips: %d/%d, RequestedMips: %d, WantedMips: %d, DynamicWantedMips: %d, StreamingStatus: %d, Forced: %d, Boost: %.1f (%s)"),
							TextureName, LastEvent->NumResidentMips, Texture->Mips.Num(), bIsDestroying ? 0 : LastEvent->NumRequestedMips,
							LastEvent->WantedMips, LastEvent->DynamicWantedMips, LastEvent->StreamingStatus, bForceMipLevelsToBeResident,
...
private:
	...
	void DoWork()
	{
		...
		// Calculate DynamicWantedMips and DynamicMinDistance for all dynamic textures.
...
//		StreamingManager.CalcDynamicWantedMips();
		...
		for ( INT Index=0; Index < StreamingManager.StreamingTextures.Num() && !IsAborted(); ++Index )
		{
			...
			StreamingTexture.bUsesDynamicHeuristics = (StreamingTexture.DynamicWantedMips != INDEX_NONE) ? TRUE : FALSE;
...
/**
 * Removes all views and camera locations from the streaming manager. This is also called by Tick().
 */
void FStreamingManagerBase::ClearViewInformation( )
{
	ViewInfos.Empty();
...
void FStreamingManagerBase::Tick( FLOAT DeltaTime, UBOOL bProcessEverything/*=FALSE*/ )
{
	...
	// Empty view info and slave location arrays to be populated again next frame.
...
{
	// Add slave locations, if we have a main view to base them from.
	if ( ViewInfos.Num() > 0 )
	{
		// Re-use settings from the first view.
		FStreamingViewInfo ViewInfo = ViewInfos(0);

		// Add the slave locations if we have any.
		for ( INT SlaveLocationIndex=0; SlaveLocationIndex < SlaveLocations.Num(); SlaveLocationIndex++ )
		{
			AddViewInformation( SlaveLocations( SlaveLocationIndex ), ViewInfo.ScreenSize, ViewInfo.FOVScreenSize );
		}
		SlaveLocations.Empty();
	}
...
INT FStreamingManagerCollection::StreamAllResources( UBOOL bIncludePlayerLocations, FLOAT TimeLimit/*=0.0f*/ )
{
	...
	INT NumPendingRequests = BlockTillAllRequestsFinished( TimeLimit );
 ...
 * @return				Number of streaming requests still in flight, if the time limit was reached before they were finished.
 */
INT FStreamingManagerCollection::BlockTillAllRequestsFinished( FLOAT TimeLimit /*= 0.0f*/ )
{
	...
	{
		FStreamingManagerBase* StreamingManager = StreamingManagers(ManagerIndex);
		NumPendingRequests += StreamingManager->BlockTillAllRequestsFinished( TimeLimit );
	}
...
void FStreamingManagerTexture::UpdateThreadData()
{
	...
	for ( INT LevelIndex=0; LevelIndex < PendingLevels.Num(); ++LevelIndex )
	{
		...
		if ( bAllowSwitchingStreamingSystem || bUsePriorityStreaming )
		{
			...
			ThreadLevelData.TextureInstances.Empty( Level->TextureToInstancesMap.Num() );
			for ( TMap<UTexture2D*,TArray<FStreamableTextureInstance> >::TConstIterator It(Level->TextureToInstancesMap); It; ++It )
			{
				...
				if ( Texture2D )
				{
					...
					TArray<FStreamableTextureInstance4>& TextureInstances4 = ThreadLevelData.TextureInstances.Set( Texture2D, TArray<FStreamableTextureInstance4>() );
			...
			if ( bAllowSwitchingStreamingSystem == FALSE )
			{
	...
	ThreadSettings.ViewInfos = ViewInfos;
...
void FStreamingManagerTexture::UpdateDynamicPrimitiveCache()
{
	...
	for ( TMap<const UPrimitiveComponent*,FSpawnedPrimitiveData>::TIterator It(ThreadSettings.SpawnedPrimitives); It; ++It )
	{
		...
		PrimitiveData.bAttached = Primitive->IsAttached();
	}
	STAT( GStreamingDynamicPrimitivesTime += appSeconds() - StartTime );
}
...
UBOOL FStreamingManagerTexture::AddDynamicPrimitive( const UPrimitiveComponent* Primitive, EDynamicPrimitiveType DynamicType )
{
	...
	{
		debugf(TEXT("AddDynamicPrimitive(\"%s\"), IsValid=%d"), *SkeletalMeshComponent->SkeletalMesh->GetName(), Primitive->IsValidComponent());
	}
	else if ( StaticMeshComponent && StaticMeshComponent->StaticMesh )
	{
		debugf(TEXT("AddDynamicPrimitive(\"%s\"), IsValid=%d"), *StaticMeshComponent->StaticMesh->GetName(), Primitive->IsValidComponent());
	}
	else
	{
		debugf(TEXT("AddDynamicPrimitive(\"%s\"), IsValid=%d"), *Primitive->GetName(), Primitive->IsValidComponent());
	}
#endif
...
void FStreamingManagerTexture::RemoveLevel( ULevel* Level )
{
	...
	if ( LevelData && !LevelData->Value.bRemove )
	{
		...
		// Note: We do this immediately, since ULevel is being deleted.
		for ( TMap<UTexture2D*,UBOOL>::TIterator It(Level->ForceStreamTextures); It; ++It )
		{
			UTexture2D* Texture2D = It.Key();
			if ( Texture2D && IsManagedStreamingTexture( Texture2D ) )
...
void FStreamingManagerTexture::NotifyPrimitiveDetached( const UPrimitiveComponent* Primitive )
{
	...
	if ( bUseDynamicStreaming && Primitive )
	{
		...
		if ( SkeletalMeshComponent && SkeletalMeshComponent->SkeletalMesh )
		{
			debugf(TEXT("NotifyPrimitiveDetached(\"%s\"), IsValid=%d"), *SkeletalMeshComponent->SkeletalMesh->GetName(),Primitive->IsValidComponent());
		}
		else if ( StaticMeshComponent && StaticMeshComponent->StaticMesh )
		{
 ...
 */
FStreamingTexture& FStreamingManagerTexture::GetStreamingTexture( UTexture2D* Texture )
{
	return StreamingTextures( Texture->StreamingIndex );
}

/** Returns TRUE if this is a streaming texture that is managed by the streaming manager. */
UBOOL FStreamingManagerTexture::IsManagedStreamingTexture( UTexture2D* Texture )
{
	return Texture->StreamingIndex >= 0 && FStreamingTexture::IsStreamingTexture( Texture );
}
...
void FStreamingManagerTexture::UpdateStreamingTextures( FStreamingContext& Context, INT StageIndex, INT NumUpdateStages )
{
	...
	for ( INT Index=StartIndex; Index < EndIndex; ++Index )
	{
		...
		PREFETCH( ((BYTE*)StreamingTexture.Texture) );
		PREFETCH( ((BYTE*)StreamingTexture.Texture) + CACHE_LINE_SIZE );
...
void FStreamingManagerTexture::StreamTextures( UBOOL bProcessEverything )
{
	...
	if ( bLimitedPoolSize )
	{
		...
		STAT_FAST( INT NonStreamingUsage = TotalMemoryUsed - ThreadStats.TotalResidentSize - TempMemoryUsed );
	...
	if ( bLimitedPoolSize && !bPauseTextureStreaming )
	{
		...
		while ( HighPrioIndex < LowPrioIndex && TempMemoryUsed < MaxTempMemoryUsed )
		{
			...
			if ( HighPrioTexture.Texture != NULL )
			{
				...
				{
					//@TODO: TempSize=0 when using in-place reallocation.
 ...
 * @return				Number of streaming requests still in flight, if the time limit was reached before they were finished.
 */
INT FStreamingManagerTexture::BlockTillAllRequestsFinished( FLOAT TimeLimit /*= 0.0f*/ )
{
...
{
	STAT_FAST(INT PerfectWantedMips = StreamingTexture.DynamicWantedMips);
	INT WantedMips = StreamingTexture.DynamicWantedMips;
	FLOAT MinDistance = StreamingTexture.DynamicMinDistance;
	...
	if ( StreamingTexture.MinAllowedMips != StreamingTexture.MaxAllowedMips && GUseTextureStreaming )
	{
		...
		if( WantedMips == INDEX_NONE )
		{
			...
			FLOAT HandlerDistance = FLT_MAX;
			INT HandlerPerfectWantedMips = INDEX_NONE;
			WantedMips = FallbackStreamingHandler.GetWantedMips( *this, StreamingTexture, HandlerPerfectWantedMips, HandlerDistance );
			MinDistance = Min( MinDistance, HandlerDistance );
			STAT_FAST( PerfectWantedMips = Max( PerfectWantedMips, HandlerPerfectWantedMips ) );
		}
	}
...
UBOOL FStreamingManagerTexture::StartStreaming( FStreamingTexture& StreamingTexture, INT WantedMips, FStreamingContext& Context, UBOOL bIgnoreMemoryConstraints )
{
	...
	if ( StreamingTexture.WantedMips != StreamingTexture.ResidentMips && StreamingTexture.bReadyForStreaming )
	{
		...
		if ( (StreamingTexture.WantedMips < StreamingTexture.ResidentMips) || bCanRequestTextureIncrease )						
		{
...
UBOOL FStreamingManagerTexture::Exec( const TCHAR* Cmd, FOutputDevice& Ar )
{
	...
	{
		DWORD StreamingTexturesSize = GET_MEMORY_STAT_FAST(STAT_StreamingTexturesSize);
		DWORD OptimalTextureSize = GET_MEMORY_STAT_FAST(STAT_OptimalTextureSize);
		INT StreamingOverBudget = INT(GET_MEMORY_STAT_FAST(STAT_StreamingOverBudget)) - INT(GET_MEMORY_STAT_FAST(STAT_StreamingUnderBudget));
		DWORD TexturePoolAllocatedSize = GET_MEMORY_STAT_FAST(STAT_TexturePoolAllocatedSize);
		DWORD LargestHoleSize = GET_MEMORY_STAT_FAST(STAT_TexturePool_LargestHole);
		DWORD NumWantingTextures = GET_MEMORY_STAT_FAST(STAT_NumWantingTextures);

		Ar.Logf( TEXT("Current Texture Streaming Stats") );
		Ar.Logf( TEXT("  Textures In Memory, Current (KB) = %f"), StreamingTexturesSize / 1024.0f);
		Ar.Logf( TEXT("  Textures In Memory, Target (KB) =  %f"), OptimalTextureSize / 1024.0f );
		Ar.Logf( TEXT("  Over Budget (KB) =                 %f"), StreamingOverBudget / 1024.0f );
		Ar.Logf( TEXT("  Pool Memory Used (KB) =            %f"), TexturePoolAllocatedSize / 1024.0f );
		Ar.Logf( TEXT("  Largest free memory hole (KB) =    %f"), LargestHoleSize / 1024.0f );
		Ar.Logf( TEXT("  Num Wanting Textures =             %d"), NumWantingTextures );
	...
	else if (ParseCommand(&Cmd,TEXT("StreamingManagerMemory")))
	{
		...
		for ( INT LevelIndex=0; LevelIndex < ThreadSettings.LevelData.Num(); ++LevelIndex )
		{
			...
			for ( TMap<const UTexture2D*,TArray<FStreamableTextureInstance4> >::TConstIterator It(LevelData.Value.TextureInstances); It; ++It )
			{
	...
	for ( INT TextureIndex=0; TextureIndex < StreamingTextures.Num(); ++TextureIndex )
	{
		...
		if ( TextureName.InStr( InvestigateTextureName, FALSE, TRUE ) >= 0 )
		{
			...
			for ( INT LevelIndex=0; LevelIndex < ThreadSettings.LevelData.Num(); ++LevelIndex )
			{
				...
				TArray<FStreamableTextureInstance4>* TextureInstances = LevelData.TextureInstances.Find( StreamingTexture.Texture );
				if ( TextureInstances )
				{
					for ( INT InstanceIndex=0; InstanceIndex < TextureInstances->Num(); ++InstanceIndex )
					{
							...
							if ( CenterX[PartialIndex] < 3.402823466e+30F )
							{
								...
								for( INT ViewIndex=0; ViewIndex < ThreadNumViews(); ViewIndex++ )
								{
									...
									if( DistSqMinusRadiusSq > 1.f )
									{
										...
										const FLOAT ScreenSize = ViewInfo.ScreenSize * ScreenSizeFactor;
	...
	for ( INT InstanceIndex=0; InstanceIndex < PrimitiveData.TextureInstances.Num(); ++InstanceIndex )
	{
		...
		if ( Instance.Texture2D == Texture2D )
		{
			...
			for( INT ViewIndex=0; ViewIndex < ThreadNumViews(); ViewIndex++ )
			{
				...
				if( DistSqMinusRadiusSq > 1.f )
				{
					FLOAT TexelFactorScale = appIsNearlyZero(Instance.OriginalRadius) ? 1.0f : (PrimitiveData.BoundingSphere.W / Instance.OriginalRadius);
			...
			debugf( TEXT("%s: Wanted=%dx%d, Distance=%.1f, TexelFactor=%.2f, CurrentRadius=%5.1f, OriginalRadius=%5.1f, Position=(%d,%d,%d), IsAttached=%d, Mesh=\"%s\", Component=\"%s\""),
				...
				MinDistance, Instance.TexelFactor, PrimitiveData.BoundingSphere.W, Instance.OriginalRadius,
...
INT FStreamingHandlerTextureStatic::GetWantedMips2( FStreamingManagerTexture& StreamingManager, FStreamingTexture& StreamingTexture, INT& PerfectWantedMips, FLOAT& MinDistance )
{
	...
	if( StreamingManager.ThreadNumViews() /*&& StreamingTexture.Instances.Num() > 0*/ )
	{
		...
		VectorRegister MinDistanceSq4 = VectorSet(FLT_MAX, FLT_MAX, FLT_MAX, FLT_MAX);
		VectorRegister MaxTexels = VectorSet(-FLT_MAX, -FLT_MAX, -FLT_MAX, -FLT_MAX);
			...
			if ( TextureInstances )
			{
				...
				for ( INT InstanceIndex=0; InstanceIndex < TextureInstances->Num() && !bShouldAbortLoop; ++InstanceIndex )
				{
					...
					for( INT ViewIndex=0; ViewIndex < StreamingManager.ThreadNumViews() && !bShouldAbortLoop; ViewIndex++ )
					{
						...
						const FLOAT ScreenSizeFloat = ViewInfo.ScreenSize * ScreenSizeFactor;
...
//						const FLOAT DistSqMinusRadiusSq = DistSq - Square(TextureInstance.BoundingSphere.W);
				...
				if ( MinDistanceSq > 1.0f )
				{
					...
					// WantedMipCount is the number of mips so we need to adjust with "+ 1".
					WantedMipCount = ScreenTexelsToMipLevel(ScreenSizeInTexels);
...
void FStreamingManagerTexture::CalcDynamicWantedMips()
{
	...
	for ( INT Index=0; Index < StreamingTextures.Num(); ++Index )
	{
		FStreamingTexture& StreamingTexture = StreamingTextures( Index );
		StreamingTexture.DynamicWantedMips = INDEX_NONE;
		StreamingTexture.DynamicMinDistance = FLT_MAX;
	}
					...
					if ( bIsPrimitiveUpToDate )
					{
						...
						for( INT ViewIndex=0; ViewIndex < ThreadNumViews(); ViewIndex++ )
						{
							...
							if( DistSqMinusRadiusSq > 1.f )
							{
								...
								FLOAT TexelFactorScale = appIsNearlyZero(TextureInstance.OriginalRadius) ? 1.0f : (PrimitiveData.BoundingSphere.W / TextureInstance.OriginalRadius);
								FLOAT TexelFactor = TextureInstance.TexelFactor * TexelFactorScale;

								// Calculate the maximum screen space dimension in pixels.
								const FLOAT	ScreenSizeInTexels	= TexelFactor * appInvSqrtEst( DistSqMinusRadiusSq ) * ScreenSize;
								// WantedMipCount is the number of mips so we need to adjust with "+ 1".
								StreamingTexture.DynamicWantedMips	= Max<INT>( StreamingTexture.DynamicWantedMips, ScreenTexelsToMipLevel(ScreenSizeInTexels) );
								StreamingTexture.DynamicMinDistance	= Min<FLOAT>( StreamingTexture.DynamicMinDistance, appSqrt( DistSqMinusRadiusSq ) );
							}
							// Request all miplevels to be loaded if we're inside the bounding sphere.
							else
							{
								StreamingTexture.DynamicWantedMips	= StreamingTexture.MaxAllowedMips;
								StreamingTexture.DynamicMinDistance	= 1.0f;
								break;
							}
						}
					}
...
INT FStreamingHandlerTextureLastRender::GetWantedMips( FStreamingManagerTexture& StreamingManager, FStreamingTexture& StreamingTexture, INT& PerfectWantedMips, FLOAT& MinDistance )
{
	...
	if( SecondsSinceLastRender < 45 && GStreamWithTimeFactor )
	{
		PerfectWantedMips = StreamingTexture.MaxAllowedMips;
		MinDistance = 0;
		return StreamingTexture.MaxAllowedMips;
	}
	else if( SecondsSinceLastRender < 90 && GStreamWithTimeFactor )
	{
