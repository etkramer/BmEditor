...
FSystemSettingsFriendly ConvertToFriendlySettings( const FSystemSettingsData& Settings )
{
	...
	// MSAA
	{
		EFriendlySettingsLevel Level = FindMatchingSystemSetting<FSystemSettingsDataAntialiasing>(Settings);
		const FSystemSettingsData* Data = &GSystemSettings;
		if (Level != FSL_Custom)
		{
			Data = &GSystemSettings.GetDefaultSettings(Level);
...
#define PROPERTY_TEXT_TO_ADDRESS_MAPPING											\
	...
	{																				\
		...
		{ TEXT("bAllowTemporalAA")			, &bAllowTemporalAA },					\
		MOBILE_SWITCHES																\
		APEX_SWITCHES																\
	};																				\
																					\
	...
	{																				\
		{ TEXT("MaxDrawDistanceScale")				, &MaxDrawDistanceScale },				\
		...
		{ TEXT("CSMMinimumFOV")						, &CSMMinimumFOV },						\
		{ TEXT("CSMFOVRoundFactor")					, &CSMFOVRoundFactor },					\
		...
		{ TEXT("TemporalAA_MinDepth")				, &TemporalAA_MinDepth },				\
		{ TEXT("TemporalAA_StartDepthVelocityScale"), &TemporalAA_StartDepthVelocityScale },\
		{ TEXT("TemporalAA_JitterStartDepthScale")	, &TemporalAA_JitterStartDepthScale },	\
		MOBILE_FLOAT_VALUES																	\
		APEX_FLOAT_VALUES																	\
	};
 ...
 * ctor
 */
FSystemSettingsDataAntialiasing::FSystemSettingsDataAntialiasing()
{
}
...
void FSystemSettings::Initialize( UBOOL bSetupForEditor )
{
	...
	for (INT Level=0;Level<ARRAY_COUNT(Defaults);++Level)
	{
		for (INT SSLevel=1;SSLevel<ARRAY_COUNT(Defaults[0]);++SSLevel)
		{
			...
			Defaults[Level][SSLevel].LoadFromIni(*SSSection);
		}
	}
...
void FSystemSettings::ApplySystemSettings_RenderingThread(const FSystemSettings::FRenderThreadSettings& NewSettings, UBOOL bInUpdateRenderTargets)
{
	...
	const UBOOL bUpdateRenderTargets =
		...
		(GSystemSettings.RenderThreadSettings.bAllowTemporalAA		!= NewSettings.bAllowTemporalAA) ||
...
void FSystemSettings::ApplySettingsAtRuntime(const FSystemSettingsData& NewSettings, UBOOL bWriteToIni)
{
	...
	// MSAA
	UBOOL bMSAAChanged = (const FSystemSettingsDataAntialiasing&)(*this) != (const FSystemSettingsDataAntialiasing&)NewSettings;
...
void FSystemSettings::ApplyOverrides()
{
	...
	if (bUseMaxQualityMode)
	{
		...
		// Don't composite dynamic lights into light environments, we want them to have proper falloff
		bUseCompositeDynamicLights = FALSE;
