...
void UPrimitiveComponent::execSetRBCollisionChannels( FFrame& Stack, RESULT_DECL )
{
	...
	SetRBCollisionChannels((FRBCollisionChannelContainer)Channels);
}
...
AKActorFromStatic* AKActorFromStatic::MakeDynamic(UStaticMeshComponent* MovableMesh)
{
	...
	AKActorFromStatic* MyKActor = Cast<AKActorFromStatic>(GWorld->SpawnActor(AKActorFromStatic::StaticClass(), NAME_None, MovableMesh->LocalToWorld.GetOrigin(), MovableMesh->LocalToWorld.Rotator(), NULL, TRUE, FALSE, MovableMesh->GetOwner()));
