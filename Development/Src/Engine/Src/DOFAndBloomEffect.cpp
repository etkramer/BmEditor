...
*/
void FDOFAndBloomPostProcessSceneProxy::RenderGatherPass(FViewInfo& View, EGatherData DestinationData, FSceneRenderTargetIndex FilterColorIndex, FLOAT CustomColorScale, INT Quality, UINT DoFQuality)
{
	...
	switch(DestinationData)
	{
		case EGD_DepthOfField:
			if (DoFQuality == 0)
			{
				SetupGather2x2<TDOFGatherPixelShader<NumFPFilterSamples, 0> >(View, GatherVertexShader, GSceneRenderTargets.GetTranslucencyBufferTexture(), CustomColorScale, bBilinearFiltered);
			}
			else
			{
				SetupGather2x2<TDOFGatherPixelShader<NumFPFilterSamples, 1> >(View, GatherVertexShader, GSceneRenderTargets.GetTranslucencyBufferTexture(), CustomColorScale, bBilinearFiltered);
			}
			break;
		case EGD_Bloom:
			// We read from HalfResPostProcess to get Bloom from motion blurred content (more correct).
			SetupGather2x2<TBloomGatherPixelShader<NumFPFilterSamples> >(View, GatherVertexShader, GSceneRenderTargets.GetHalfResPostProcessTexture(), CustomColorScale, bBilinearFiltered);
			break;
		case EGD_MotionBlur:
...
void FDOFAndBloomPostProcessSceneProxy::DownSampleSceneAndDepth(FViewInfo& View)
{
	...
	const FTexture2DRHIRef* SrcTexture = &GSceneRenderTargets.GetSceneColorTexture();

#if XBOX
	// On Xbox360 we store the scene color in the 32 bit and use fetch7e3() to reconstruct.
	SrcTexture = &GSceneRenderTargets.GetSceneColorRawTexture();
#endif
