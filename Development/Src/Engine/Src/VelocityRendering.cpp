...
public:
	...
	void SetMesh(const FPrimitiveSceneInfo* PrimitiveSceneInfo,const FMeshElement& Mesh,const FViewInfo& View,const FMatrix& PreviousLocalToWorld)
	{
...
public:
	...
	void SetMesh(const FPrimitiveSceneInfo* PrimitiveSceneInfo,const FMeshElement& Mesh,const FViewInfo& View,UBOOL bBackFace, FLOAT MotionBlurInstanceScale, UBOOL bFullMotionBlur)
	{
	...
	virtual UBOOL Serialize(FArchive& Ar)
	{
		...
		else if (Ar.IsLoading())
		{
			...
			ZeroAr << PrevViewProjectionMatrixParameter;
		}
#endif
		return bShaderHasOutdatedParameters;
	}
	...
	) const
{
	...
	if (Mesh.MBInfo != NULL)
	{
		VertexShader->SetMesh(PrimitiveSceneInfo,Mesh,View,Mesh.MBInfo->PreviousLocalToWorld);
	}
	...
	{
		VertexShader->SetMesh(PrimitiveSceneInfo,Mesh,View,PreviousLocalToWorld);
	}	
	else
	{
		VertexShader->SetMesh(PrimitiveSceneInfo,Mesh,View,Mesh.LocalToWorld);
	}
	...
	PixelShader->SetMesh(PrimitiveSceneInfo,Mesh,View,bBackFace,PrimitiveSceneInfo->MotionBlurInstanceScale, bFullMotionBlur);
