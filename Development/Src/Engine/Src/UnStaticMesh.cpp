		...
		for(INT i=0;i<LODModels.Num();i++)
		{
			...
			for(INT ElementIndex = 0;ElementIndex < RenderData.Elements.Num();ElementIndex++)
			{
				if(ElementIndex < ThisLODInfo.Elements.Num())
				{
                    ...
                    // UDKUltimate MOD - auto enable shadow casting and poly collision for imported meshes
                    ElementInfo.bEnableShadowCasting = TRUE;
                    ElementInfo.bEnableCollision = TRUE;
