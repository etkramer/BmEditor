...
void FLandscapeStaticLightingMesh::GetStaticLightingVertex(INT VertexIndex, FStaticLightingVertex& OutVertex) const
{
	...
	const FColor* Data = &HeightData(X + Y * NumVertices); //DataInterface->GetHeightData( X, Y );
	...
	OutVertex.WorldPosition = LandscapeComponent->LocalToWorld.TransformFVector( FVector4( LocalX, LocalY, ((FLOAT)Height - 32768.f) * LANDSCAPE_ZSCALE ) );

	FLOAT UVFactor = DataInterface->LightMapRatio / (FLOAT)(NumVertices * LandscapeComponent->GetLandscapeActor()->StaticLightingResolution);

	OutVertex.TextureCoordinates[0] = FVector2D((FLOAT)X, (FLOAT)Y); 
	OutVertex.TextureCoordinates[LANDSCAPE_LIGHTMAP_UV_INDEX].X = (FLOAT)(X * LandscapeComponent->GetLandscapeActor()->StaticLightingResolution + 0.5f) * UVFactor;
	OutVertex.TextureCoordinates[LANDSCAPE_LIGHTMAP_UV_INDEX].Y = (FLOAT)(Y * LandscapeComponent->GetLandscapeActor()->StaticLightingResolution + 0.5f) * UVFactor;
}
