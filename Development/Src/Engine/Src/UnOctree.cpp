...
static FORCEINLINE_DEBUGGABLE UBOOL LineBoxIntersectSIMD(const FOctreeNodeBounds& InParentBounds,INT LeftChildIndex,const FPrimitiveOctree& o,FOctreeNodeBounds& OutBounds)
{
	...
	VectorRegister LeftMin = VectorSubtract(LeftBoxCenter,BoxRadii);
	VectorRegister LeftMax = VectorAdd(LeftBoxCenter,BoxRadii);
				...
				if( lineChkRes )
				{
					...
					if(Hit.Normal.SizeSquared() < Square(0.99f))
					{
						debugf( TEXT("ZELC: Component '%s' with Owner '%s' returned normal with incorrect length (%f).  ChkActor: %s"),
							*TestPrimitive->GetDetailedInfo(),*PrimOwner->GetDetailedInfo(),Hit.Normal.Size(), *o->ChkActor->GetName() );
...
void FPrimitiveOctree::AddPrimitive(UPrimitiveComponent* Primitive)
{
	...
	if( (GWorld->HasBegunPlayAndNotAssociatingLevel() && (PrimOwner == NULL || PrimOwner->IsStatic() == FALSE))  || (PrimOwner && PrimOwner->bForceOctreeSNFilter) )
	{
