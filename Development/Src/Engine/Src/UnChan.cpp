	...
	else
	{
		...
		while( InRec )
		{
			if( InRec->ChSequence!=Connection->InReliable[ChIndex]+1 )
				break;
			debugfSuppressed( NAME_DevNetTraffic, TEXT("      Unleashing queued bunch") );
...
#if WITH_STEAMWORKS
		else if ((HelloMessage[0] == NMT_Hello) || (HelloMessage[0] == NMT_AuthHello))
#else
		else if (HelloMessage[0] == NMT_Hello)
#endif
		{
			// Get platform id
...
#if WITH_STEAMWORKS
				case NMT_AuthHello:
					FNetControlMessage<NMT_AuthHello>::Discard(Bunch);
					break;
				case NMT_AuthServer:
					FNetControlMessage<NMT_AuthServer>::Discard(Bunch);
					break;
				case NMT_AuthBlob:
...
static void AddActorPropertiesFromStruct(const UStructProperty* StructProp, INT BaseOffset, TArray<FReplicatedActorProperty>& ReplicatedActorProperties)
{
	...
	for (UProperty* Prop = StructProp->Struct->PropertyLink; Prop != NULL; Prop = Prop->PropertyLinkNext)
	{
		...
		if (ObjectProp != NULL)
		{
			...
			{
				new(ReplicatedActorProperties) FReplicatedActorProperty(BaseOffset + ObjectProp->Offset, ObjectProp);
			}
		}
		else
		{
			...
			{
				AddActorPropertiesFromStruct(InnerStructProp, BaseOffset, ReplicatedActorProperties);
			}
		}
	}
}
	...
	for (UProperty* Prop = ActorClass->PropertyLink; Prop != NULL; Prop = Prop->PropertyLinkNext)
	{
		if (Prop->PropertyFlags & CPF_Net)
		{
			...
			if (ObjectProp != NULL)
			{
				if (ObjectProp->PropertyClass != NULL && ObjectProp->PropertyClass->IsChildOf(AActor::StaticClass()))
				{
					new(ReplicatedActorProperties) FReplicatedActorProperty(ObjectProp->Offset, ObjectProp);
				}
			}
			else
			{
