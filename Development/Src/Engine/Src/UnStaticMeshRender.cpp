...
void UStaticMeshComponent::GenerateDecalRenderData(FDecalState* Decal, TArray< FDecalRenderData* >& OutDecalRenderDatas) const
{
	...
	const INT InstanceCount = IsInstanced() ? GetInstanceCount() : 1;
	...
	WireframeColor(Component->WireframeColor)
{
		...
		Component->bAcceptsDynamicDecals )
	{
		for( INT DecalIndex = 0 ; DecalIndex < Decals.Num() ; ++DecalIndex )
		{
			...
			Decals(DecalIndex)->DecalState.TransformFrustumVerts( GetInstanceLocalToWorld( Decals(DecalIndex)->RenderData->InstanceIndex ) );
		}
	}
...
UBOOL FStaticMeshSceneProxy::GetMeshElement(INT LODIndex,INT ElementIndex,INT FragmentIndex,BYTE InDepthPriorityGroup,const FMatrix& WorldToLocal,FMeshElement& OutMeshElement, const UBOOL bUseSelectedMaterial, const UBOOL bUseHoveredMaterial) const
{
	...
	if(OutMeshElement.NumPrimitives > 0)
	{
		...
		OutMeshElement.MaterialRenderProxy = ProxyLODInfo.Elements(ElementIndex).Material->GetRenderProxy(bUseSelectedMaterial,bUseHoveredMaterial);
		...
		OutMeshElement.LODIndex = FMeshElement::QuantizeLODIndex(LODs.Num() > 1 ? LODIndex : INDEX_NONE);
...
UBOOL FStaticMeshSceneProxy::GetWireframeMeshElement(INT LODIndex, const FMaterialRenderProxy* WireframeRenderProxy, BYTE InDepthPriorityGroup, const FMatrix& WorldToLocal, FMeshElement& OutMeshElement) const
{
	const FStaticMeshRenderData& LODModel = StaticMesh->LODModels(LODIndex);
	...
	OutMeshElement.MinVertexIndex = 0;
	OutMeshElement.MaxVertexIndex = LODModel.NumVertices - 1;
	OutMeshElement.ReverseCulling = LocalToWorldDeterminant < 0.0f ? TRUE : FALSE;
	...
	SetIndexSource(LODIndex, 0, 0, OutMeshElement, TRUE);
...
//@todo decal - decal bounds are not getting updated, so this culling won't work
			...
			if( bIsDecalVisible )
			{
				...
				checkSlow( !IsInstanced() || RenderData->InstanceIndex != INDEX_NONE );
				const FMatrix& InstanceLocalToWorld = GetInstanceLocalToWorld( RenderData->InstanceIndex );
				const FLOAT InstanceLocalToWorldDeterminant = InstanceLocalToWorld.Determinant();

				MeshElement.LocalToWorld = InstanceLocalToWorld;
				MeshElement.WorldToLocal = InstanceLocalToWorld.Inverse();
						...
						for(INT FragmentIndex = 0;FragmentIndex < LODs(LODIndex).Elements(ElementIndex).NumFragments;FragmentIndex++)
						{
							...
							if(GetMeshElement(LODIndex,ElementIndex,FragmentIndex,DPGIndex,WorldToLocal,MeshElement, bIsSelected, bHovered))
							{
								...
								if( View->Family->ShowFlags & SHOW_VertexColors && AllowDebugViewmodes() )
								{
									...
									FMeshElement ModifiedMeshElement = MeshElement;
...
{   
	...
	if(View->Family->ShowFlags & SHOW_StaticMeshes)
	{
		...
		if (!(View->Family->ShowFlags & SHOW_Materials))
		{
