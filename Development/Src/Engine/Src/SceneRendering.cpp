		...
		if ( ViewState )
		{
			...
			if (View.bRequiresPrevTransforms)
			{
				...
				// console variables can limit MotionBlurTimeScale
				{
					...
					FLOAT MinScale = CVarMin->GetFloat();
					FLOAT MaxScale = CVarMax->GetFloat();

					ViewState->MotionBlurTimeScale = Clamp(ViewState->MotionBlurTimeScale, MinScale, MaxScale);
				}
...
// The scene color alpha channel (depth information) has to be cleared according to GUsesInvertedZ flag
	...
	if(ViewRelevance.bStaticRelevance)
	{
		...
		for(INT MeshIndex = 0;MeshIndex < StaticMeshesNum;MeshIndex++)
		{
			...
			if(	LODFactorDistanceSquared >= StaticMesh.MinDrawDistanceSquared 
				&& LODFactorDistanceSquared < StaticMesh.MaxDrawDistanceSquared * Square(GSystemSettings.MaxDrawDistanceScale) )
			{
	    ...
	    for(INT MeshIndex = 0;MeshIndex < StaticMeshesNum;MeshIndex++)
	    {
		    ...
		    if(	LODToRender != INDEX_NONE && StaticMesh.LODIndex == LODToRender
				|| LODToRender == INDEX_NONE && LODFactorDistanceSquared >= StaticMesh.MinDrawDistanceSquared && LODFactorDistanceSquared < StaticMesh.MaxDrawDistanceSquared * Square(GSystemSettings.MaxDrawDistanceScale) )
		    {
