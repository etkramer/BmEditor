...
void UGameEngine::Init()
{
	...
	{	
		// Init client.
#if PLATFORM_MACOSX
		UClass* ClientClass = StaticLoadClass( UClient::StaticClass(), NULL, TEXT("MacDrv.MacClient"), NULL, LOAD_None, NULL );
#else
...
#if XBOX
	// update inis and the guid cache with patch files (must be done between loading global guid cache
	// and installing any DLC)
	extern void appXenonPatchGame();
	appXenonPatchGame();
#endif
...
//		GParticleDataManager.ResetParticleMemoryMaxValues();
	...
	else if (ParseCommand( &Cmd, TEXT("Ad")))
	{
		UInGameAdManager* AdManager = GetAdManager();
		if (AdManager)
		{
		...
		{
			Ar.Logf(TEXT("No Ad Manager has been created. See [Engine.GameEngine] : InGameAdManagerClassName"));
		}
		return TRUE;
	}
	else if (ParseCommand( &Cmd, TEXT("FB")))
	{
		if (GEngine->FacebookIntegration)
		{
			if (ParseCommand(&Cmd, TEXT("auth")))
			{
				GEngine->FacebookIntegration->eventAuthorize();
			}
			else if (ParseCommand(&Cmd, TEXT("isauthed")))
			{
				Ar.Logf(TEXT("Authorized? %d"), GEngine->FacebookIntegration->eventIsAuthorized());
			}
			else if (ParseCommand(&Cmd, TEXT("username")))
			{
				Ar.Logf(TEXT("FB username is %s"), *GEngine->FacebookIntegration->UserName);
			}
			else if (ParseCommand(&Cmd, TEXT("disconnect")))
			{
				GEngine->FacebookIntegration->eventDisconnect();
			}
		}
		else
		{
			Ar.Logf(TEXT("This platform has no Facebook support"));
		}
		return TRUE;
	}
#endif // !FINAL_RELEASE
...
UBOOL UGameEngine::LoadMap( const FURL& URL, UPendingLevel* Pending, FString& Error )
{
	...
	MALLOC_PROFILER(GMalloc->Exec(TEXT("SNAPSHOTMEMORYTEXT UGameEngine::LoadMap START"),*GLog));
	...
	for(FLocalPlayerIterator It(this);It;++It)
	{
		...
		if(!It->SpawnPlayActor(URL.String(1),Error2))
		{
			if (Error2 != TEXT(""))
			{
				...
				Error2.ParseIntoArray(&Pieces, TEXT("."), TRUE);
				if (Pieces.Num() >= 3)
				{
					Error2 = Localize(*Pieces(1), *Pieces(2), *Pieces(0), NULL, TRUE);
				}
			}
	...
	// Trigger initial (blocking) texture streaming next tick.
	bTriggerTextureStreaming = TRUE;

	// Trigger a call to PostLoadMap() the next Tick. PostLoadMap() will turn off the loading screen.
	...
	// RedrawViewports() may have added a dummy playerstart location. Clear it so that proper locations can be added for the next Tick().
	GStreamingManager->ClearViewInformation();

	MALLOC_PROFILER(GMalloc->Exec(*FString::Printf(TEXT("SNAPSHOTMEMORYTEXT UGameEngine::LoadMap %s END"),*GWorld->GetOutermost()->GetName()),*GLog));
...
void UGameEngine::Tick( FLOAT DeltaSeconds )
{
	...
	if( GIsClient )
	{
		...
		if ( bTriggerTextureStreaming )
		{
			// Update level streaming, forcing existing levels to be unloaded and their streaming objects removed from the world info.
			GWorld->FlushLevelStreaming( NULL, TRUE );
	...
	bTriggerTextureStreaming = FALSE;
	...
	// tick facebook integration
	if (FacebookIntegration)
	{
		FacebookIntegration->Tick(DeltaSeconds);
	}
}
 ...
 * Script to C++ thunking code
 */
void UGameEngine::execGetAdManager( FFrame& Stack, RESULT_DECL )
{
	P_FINISH;
	*(class UInGameAdManager**)Result = UGameEngine::GetAdManager();
}
...
void UGameEngine::InitGameSingletonObjects(void)
{
	...
	if (InGameAdManagerClassName.Len())
	{
		// Load and create the DLC manager object
		UClass* AdManagerClass = LoadClass<UInGameAdManager>(NULL, *InGameAdManagerClassName, NULL, LOAD_None, NULL);
		if (AdManagerClass != NULL)
		{
			AdManager = ConstructObject<UInGameAdManager>(AdManagerClass);

			// assuming it's all been made, allow it to initialize itself
			if (AdManager != NULL)
			{
				AdManager->eventInit();
			}
			...
			{
				warnf(TEXT("Failed to create AdManager class (%s)"),*InGameAdManagerClassName);
			}
		}
		else
		{
			warnf(TEXT("Failed to load AdManager class (%s)"),*InGameAdManagerClassName);
		}
	}
}
