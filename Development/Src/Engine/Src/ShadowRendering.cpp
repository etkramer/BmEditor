...
public:
	...
	void Set(ShaderRHIParamRef Shader, const FSceneView& View, const FProjectedShadowInfo* ShadowInfo, const FMaterialRenderProxy* MaterialRenderProxy)
	{
		SetShaderValue(
			...
			FTranslationMatrix(ShadowInfo->PreShadowTranslation - View.PreViewTranslation) * ShadowInfo->SubjectAndReceiverMatrix
			);
...
void FModShadowProjectionVertexShader::SetParameters(	
	...
	const FProjectedShadowInfo* ShadowInfo)
{
	FShadowProjectionVertexShader::SetParameters(View);
}
...
FShadowDepthDrawingPolicy::FShadowDepthDrawingPolicy(
	...
	FMeshDrawingPolicy(InVertexFactory,InMaterialRenderProxy,FALSE,bInCastShadowAsTwoSided),
	GeometryShader(NULL),
	...
	bOnePassPointLightShadow(bInOnePassPointLightShadow)
{
	...
	const FMaterial* VertexShaderMaterialResource = (MaterialResource->IsMasked() || MaterialResource->CastLitTranslucencyShadowAsMasked() || MaterialResource->MaterialModifiesMeshPosition()) ? MaterialResource : DefaultMaterialResource;

#if WITH_D3D11_TESSELLATION
	HullShader = NULL;
	DomainShader = NULL;

	const EMaterialTessellationMode TessellationMode = MaterialResource->GetD3D11TessellationMode();
	...
	// Pixel shaders
	if (MaterialResource->IsMasked() || MaterialResource->CastLitTranslucencyShadowAsMasked())
	{
		if (bUsePerspectiveCorrectShadowDepths)
		{
			...
			PixelShader = MaterialResource->GetShader<TShadowDepthPixelShader<PixelShadowDepth_PerspectiveCorrect,FALSE> >(InVertexFactory->GetType());
		}
...
#else
	return RHICreateBoundShaderState(VertexDeclaration, StreamStrides, VertexShader->GetVertexShader(), GETSAFERHISHADER_PIXEL(PixelShader));	
#endif
}
	...
	if (StaticMesh->CastShadow)
	{
		...
		if ((!IsTranslucentBlendMode(BlendMode) && BlendMode != BLEND_DitheredTranslucent) || Material->CastLitTranslucencyShadowAsMasked())
		{
			...
			Scene->DPGs[StaticMesh->DepthPriorityGroup].WholeSceneShadowDepthDrawList.AddMesh(
				...
				FShadowDepthDrawingPolicy(StaticMesh->VertexFactory,MaterialRenderProxy,FALSE,FALSE,TRUE,TRUE,FALSE,StaticMesh->PrimitiveSceneInfo->bCastShadowAsTwoSided,StaticMesh->ReverseCulling,FALSE)
				);
		}
	}
}
...
void FProjectedShadowInfo::RenderDepth(const FSceneRenderer* SceneRenderer, BYTE DepthPriorityGroup, UBOOL bTranslucentPreShadow)
{
	...
	else if (SubjectMeshElements.Num() > 0)
	{
		...
		{
			SharedDrawingPolicy.SetMeshRenderState(*FoundView, FirstShadowMesh.Mesh->PrimitiveSceneInfo, *FirstShadowMesh.Mesh, FALSE, FMeshDrawingPolicy::ElementDataType());
			SharedDrawingPolicy.DrawMesh(*FirstShadowMesh.Mesh);
		}
		...
		for (INT ElementIndex = 1; ElementIndex < SubjectMeshElements.Num(); ElementIndex++)
		{
			...
			if (FoundView->StaticMeshShadowDepthMap(ShadowMesh.Mesh->Id))
			{
				...
				SharedDrawingPolicy.SetMeshRenderState(*FoundView, ShadowMesh.Mesh->PrimitiveSceneInfo, *ShadowMesh.Mesh, FALSE, FMeshDrawingPolicy::ElementDataType());
				SharedDrawingPolicy.DrawMesh(*ShadowMesh.Mesh);
			}
		}
	}
...
void DrawStencilingSphere(const FSphere& Sphere, const FVector& PreViewTranslation)
{
	...
	FLOAT Radius = Sphere.W / appCos(RadiansPerRingSegment);
	...
	TArray<FVector, TInlineAllocator<NumVerts> > Verts;
	...
	for (INT s = 0; s < NumSides + 1; s++)
	{
		...
		{
			const INT VIx = (NumRings + 1) * s + v;
			Verts.AddItem(Sphere.Center + PreViewTranslation + Radius * ArcRot.TransformFVector(ArcVerts(v)));
		}
	}
...
#ifdef _DEBUG
		static UBOOL bCWEnable = FALSE;
		RHISetColorWriteEnable(bCWEnable);
#endif
	...
	if (GOnePassDominantLight && (ViewFamily.ShowFlags & SHOW_Lighting))
	{
		for(TSparseArray<FLightSceneInfoCompact>::TConstIterator LightIt(Scene->Lights);LightIt;++LightIt)
		{
				...
				&& LightSceneInfo->LightShadowMode == LightShadow_Normal)
			{
				...
				const UBOOL bHasProjectedShadows = CheckForProjectedShadows( LightSceneInfo, DPGIndex );
				const UBOOL bHasLightFunction = CheckForLightFunction( LightSceneInfo, DPGIndex );
				if(bDrawShadows && (bHasProjectedShadows || bHasLightFunction))
				{
					return TRUE;
				}
			}
...
void FSceneRenderer::RenderDominantLightShadowsForBasePass(UINT DPGIndex)
{
	...
	{
		// MSAA makes the scene depth buffer 2x bigger in EDRAM, overlapping the light attenuation buffer
		// With MSAA (and tiling) on, there's not enough EDRAM to efficiently render normal shadows for the dominant light and apply them in the base pass
		check(!GUseTilingCode);
		...
		for (INT ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			...
			for (TSparseArray<FLightSceneInfoCompact>::TConstIterator LightIt(Scene->Lights); LightIt; ++LightIt)
			{
					...
					&& LightSceneInfo->LightShadowMode == LightShadow_Normal)
				{
					...
					if (bDrawShadows && (bHasShadows || bHasLightFunction))
					{
...
UBOOL FSceneRenderer::RenderProjectedShadows( const FLightSceneInfo* LightSceneInfo, UINT DPGIndex, UBOOL bRenderingBeforeLight )
{
	...
	if (!GUsingMobileRHI)
	{
		...
		while (NumShadowsRendered < Shadows.Num())
		{
			...
			if (PassNumber > 0 || !(GOnePassDominantLight && Shadows(0)->IsPrimaryWholeSceneDominantShadow()))
			{
	...
	else
	{
		...
		if (DPGIndex == SDPG_World)
		{
			...
			for (INT ShadowIndex = 0; ShadowIndex < Shadows.Num(); ShadowIndex++)
			{
				FProjectedShadowInfo* Shadow = Shadows(ShadowIndex);
...
#else
		BoundShaderState = RHICreateBoundShaderState(VertexDeclaration, StreamStrides, VertexShader->GetVertexShader(), PixelShader->GetPixelShader());
#endif
	...
	) const
{
	...
	if( Light->LightType == LightType_Directional || Light->LightType == LightType_DominantDirectional )
	{
		const FLOAT BigNumber = 10000.f;
		LightPosition = Light->GetDirection().SafeNormal() * (-BigNumber) + Mesh.LocalToWorld.GetOrigin() + View.PreViewTranslation;
	}
	else
	{
		LightPosition = Light->GetOrigin() + View.PreViewTranslation;
	}

	VertexShader->SetLightPosition(VertexShader,FVector4(LightPosition,1.f));

	// Set transforms
	VertexShader->SetMesh(Mesh,View,*PrimitiveSceneInfo);
	PixelShader->SetMesh(Mesh,View,bBackFace,*PrimitiveSceneInfo);

#if WITH_D3D11_TESSELLATION
	if(HullShader && DomainShader)
	{
		DomainShader->SetLightPosition(DomainShader,FVector4(LightPosition,1.f));
