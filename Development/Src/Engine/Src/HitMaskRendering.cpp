...
FMeshDrawingPolicy(InVertexFactory,InMaterialRenderProxy)
{
	...
	VertexShader = MaterialResource->GetShader<FHitMaskVertexShader>(InVertexFactory->GetType());
	PixelShader = MaterialResource->GetShader<FHitMaskPixelShader>(InVertexFactory->GetType());
}
	...
	)
{
	...
	FHitMaskDrawingPolicy DrawingPolicy(Mesh.VertexFactory,MaterialRenderProxy);
	DrawingPolicy.DrawShared(&View,DrawingPolicy.CreateBoundShaderState(Mesh.GetDynamicVertexStride()), HitInfo);
	DrawingPolicy.SetMeshRenderState(View,PrimitiveSceneInfo,Mesh,bBackFace,HitInfo);
