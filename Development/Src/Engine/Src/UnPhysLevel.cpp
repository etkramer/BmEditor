...
DECLARE_CYCLE_STAT(TEXT("Phys LineCheck Time"),STAT_PhysicsLineCheckTime,STATGROUP_Physics);
	...
	if( !RBPhysScene )
	{
		...
		if ( (RBPhysScene != NULL) && (RBPhysScene->ApexScene != NULL) )
		{
			...
			if ( ApexScene != NULL )
			{
				...
				if ( (WorldInfo != NULL) && (WorldInfo->ApexLODResourceBudget >= 0.0f) )
				{
...
void UWorld::TickWorldRBPhys(FLOAT DeltaSeconds)
{
	...
	// before physics runs, poll async line checker
	UseScene->AsyncLineCheckManager.Tick(DeltaSeconds);
...
//		NxCellConfig::setSceneParamInt(NovodexScene, NxCellConfig::NX_CELL_SCENE_PARAM_SPU_CLOTH, 0);
	...
	QueryDesc.executeMode = NX_SQE_SYNCHRONOUS;
...
void DestroyRBPhysScene(FRBPhysScene* Scene)
{
	...
	// ensure we're not waiting on any line checks
...
void DeferredRBResourceCleanup(FRBPhysScene* PhysScene, UBOOL bCalledFromTick)
{
	...
	if (GNovodexPendingKillCloths.Num() > 0)
	{
		...
		PhysScene->PhysXEmitterManager->SyncPhysXData();
		nxScene->simulate(0);
...
void TickRBPhysScene(FRBPhysScene* Scene, FLOAT DeltaTime)
{
	...
	if(NovodexScene)
	{
		...
		if ( Scene->ApexScene )
		{
			Scene->ApexScene->Simulate(UseDelta);
		}
		else
#endif
		{
...
void WaitRBPhysScene(FRBPhysScene* Scene)
{
	...
	{
		SCOPE_CYCLE_COUNTER(STAT_PhysicsLineCheckTime);
		// to make async we need to end and then fire off line checks for the next frame!
		// okie so for this change we need to do a lot of in depth testing to make certain this actually works well
		// right now there is a debug assert when you run this SceneQuery.cpp ln 410 (also we need to make a line check handler on the engine side
		// as PhysX does not manage the line checks. Something like: block on finishing line checks, run physics, after physics has completed,
		// start up this frame's line checks. )
...
void FRBPhysScene::AsyncRayCheck(const FVector& Origin, const FVector& Direction, FLOAT Distance, UBOOL bDynamicObjects, FAsyncLineCheckResult* Result)
{
	...
	if(ScenePair)
	{
		...
		bool bHit = ScenePair->SceneQuery->raycastAnyShape(nRay, TestTypes, 0xFFFFFFFF, Distance*U2PScale, 0, 0, Result);

		// To make async and sync both work, call the callback now is sync
		if(ScenePair->SceneQuery->getExecuteMode() == NX_SQE_SYNCHRONOUS)
		{
			Result->bHit = bHit;
			Result->bCheckCompleted = TRUE;
		}

		AsyncLineCheckManager.TrackLineCheckResult(Result);
	}
#endif
}
...
void ListAwakeRigidBodies()
{
	...
	for( TObjectIterator<URB_BodyInstance> It; It; ++It )
	{
		...
		if(!BodyInst->IsTemplate() && BodyInst->IsValidBodyInstance())
		{
			...
			if(!nActor->isSleeping() && !nActor->readBodyFlag(NX_BF_KINEMATIC))
			{
