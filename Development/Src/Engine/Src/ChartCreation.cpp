...
void UEngine::TickFPSChart( FLOAT DeltaSeconds )
{
	...
	DWORD MaxThreadTimeValue = Max3<DWORD>( GRenderThreadTime, GGameThreadTime, GGPUFrameTime );
	...
	DWORD PossibleGPUTime = GGPUFrameTime;
	if( PossibleGPUTime == 0 )
	{
		// if we are over
		PossibleGPUTime = static_cast<DWORD>(Max( FrameTime, DeltaSeconds ) / GSecondsPerCycle );
		MaxThreadTimeValue = Max3<DWORD>( GGameThreadTime, GRenderThreadTime, PossibleGPUTime );
	}
	...
	if( DeltaSeconds < 1.f )
	{
		...
		if( CurrentFPS < 30 )
		{
			// then based on order of comparisons this will bias things toward the non GPU for ties
			...
			else if( GRenderThreadTime >= (MaxThreadTimeValue - Epsilon) )
			{
				//warnf( TEXT( "Render" ) );
			...
			{
				warnf( TEXT( "Game %d Render %d GPU %d  Max %d Frame %f DeltaTme %f" ),GGameThreadTime, GRenderThreadTime , PossibleGPUTime, MaxThreadTimeValue, FrameTime, DeltaSeconds );
			}
		}
	}
...
void UEngine::DumpFPSChartToStatsLog( FLOAT TotalTime, FLOAT DeltaTime, INT NumFrames )
{
	...
	FString OutputDir = appProfilingDir() + TEXT("FPSChartStats") + PATH_SEPARATOR;
...
void UEngine::DumpFPSChartToHTML( FLOAT TotalTime, FLOAT DeltaTime, INT NumFrames, UBOOL bOutputToGlobalLog )
{
	...
	if( bAreAllHTMLPartsLoaded )
	{
		...
		// Create folder for FPS chart data.
		const FString OutputDir = appProfilingDir() + TEXT("FPSChartStats") + PATH_SEPARATOR;
		GFileManager->MakeDirectory( *OutputDir );
...
void UEngine::DumpMemoryChartToHTML( FLOAT TotalTime, FLOAT DeltaTime, INT NumFrames, UBOOL bOutputToGlobalLog )
{
	...
	if( bAreAllHTMLPartsLoaded )
	{
		...
		const FString OutputDir = appProfilingDir() + TEXT("MemoryChartStats") + PATH_SEPARATOR;
		GFileManager->MakeDirectory( *OutputDir );
