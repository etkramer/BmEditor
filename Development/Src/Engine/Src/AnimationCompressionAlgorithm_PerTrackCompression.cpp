...
protected:
	...
	void CompressTranslation_16_16_16(const FTranslationTrack& TranslationData, FLOAT ZeroingThreshold)
	{
		...
		for (INT i = 0; i < NumKeys; ++i)
		{
			...
			const FVector DecompressedV(
				bHasX ? FAnimationCompression_PerTrackUtils::DecompressFixed16(X, LogScale) : 0.0f,
				bHasY ? FAnimationCompression_PerTrackUtils::DecompressFixed16(Y, LogScale) : 0.0f,
				bHasZ ? FAnimationCompression_PerTrackUtils::DecompressFixed16(Z, LogScale) : 0.0f);
	...
	void CompressRotation_Uncompressed(const FRotationTrack& RotationData)
	{
		...
		// Write the header out
		INT Header = MakeHeader(NumKeys, ACF_Float96NoW, 0);
		AppendBytes(&Header, sizeof(Header));
	...
	void CompressRotation_16_16_16(const FRotationTrack& RotationData, FLOAT ZeroingThreshold)
	{
		...
		for (INT i = 0; i < NumKeys; ++i)
		{
			...
			Decompressor.X = bHasX ? FAnimationCompression_PerTrackUtils::DecompressFixed16(X) : 0.0f;
			Decompressor.Y = bHasY ? FAnimationCompression_PerTrackUtils::DecompressFixed16(Y) : 0.0f;
			Decompressor.Z = bHasZ ? FAnimationCompression_PerTrackUtils::DecompressFixed16(Z) : 0.0f;
	...
	void CompressRotation_Fixed32(const FRotationTrack& RotationData)
	{
		// Write the header out
		const INT NumKeys = RotationData.RotKeys.Num();
		const INT Header = MakeHeader(NumKeys, ACF_Fixed32NoW, 0);
		AppendBytes(&Header, sizeof(Header));
	...
	void CompressRotation_Float32(const FRotationTrack& RotationData)
	{
		// Write the header out
		const INT NumKeys = RotationData.RotKeys.Num();
		const INT Header = MakeHeader(NumKeys, ACF_Float32NoW, 0);
		AppendBytes(&Header, sizeof(Header));
