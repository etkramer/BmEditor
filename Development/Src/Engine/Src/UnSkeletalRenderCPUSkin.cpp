...
static void SkinVertices( FFinalSkinVertex* DestVertex, FBoneAtom* ReferenceToLocal, INT LODIndex, FStaticLODModel& LOD, TArray<FActiveMorph>& ActiveMorphs, UBOOL bUseBoneInfluences, const TArray<FVertexInfluence>& VertexInfluences );
#else
template<typename VertexType>
static FORCEINLINE void SkinVertices( FFinalSkinVertex* DestVertex, FBoneAtom* ReferenceToLocal, INT LODIndex, FStaticLODModel& LOD, TArray<FActiveMorph>& ActiveMorphs, UBOOL bUseBoneInfluences, const TArray<FVertexInfluence>& VertexInfluences );
#endif
/** 
 ...
 */
void CalculateBoneWeights(FFinalSkinVertex* DestVertex, FStaticLODModel& LOD, const TArray<INT>& BonesOfInterest, SkinColorRenderMode ColorMode, UBOOL bUseBoneInfluences, const TArray<FVertexInfluence>& VertexInfluences );
...
void FSkeletalMeshObjectCPUSkin::CacheVertices(INT LODIndex, UBOOL bForce, UBOOL bUpdateDecalVertices) const
{
		...
		IsValidRef(MeshLOD.VertexBuffer.VertexBufferRHI) )
	{
...

		{
			...
			{
				// do actual skinning
				SkinVertices< TGPUSkinVertexFloat32Uvs<> >( DestVertex, ReferenceToLocal, DynamicData->LODIndex, LOD, DynamicData->ActiveMorphs, MeshLODInfo.bUseInstancedVertexInfluences, MeshLOD.VertexInfluenceBuffer.Influences );
			}
			else
			{
				// do actual skinning
				SkinVertices< TGPUSkinVertexFloat16Uvs<> >( DestVertex, ReferenceToLocal, DynamicData->LODIndex, LOD, DynamicData->ActiveMorphs, MeshLODInfo.bUseInstancedVertexInfluences, MeshLOD.VertexInfluenceBuffer.Influences );
			}

			if (RenderColorMode!=ESCRM_None)
			{
				//Transfer bone weights we're interested in to the UV channels
				CalculateBoneWeights(DestVertex, LOD, BonesOfInterest, RenderColorMode, MeshLODInfo.bUseInstancedVertexInfluences, MeshLOD.VertexInfluenceBuffer.Influences );
			}
		}
		...
		if((SkeletalMesh->bEnableClothTearing || SkeletalMesh->bEnableValidBounds) && (SkeletalMesh->ClothWeldingMap.Num() == 0))
		{
			...
			if(IsValidRef(MeshLOD.DynamicIndexBuffer.IndexBufferRHI))
			{
				...
				*/
				...
				{
					WORD* Buffer = (WORD *)RHILockIndexBuffer(MeshLOD.DynamicIndexBuffer.IndexBufferRHI,0, TotalNumIndices * sizeof(DWORD));
...
#if DEBUG_CPU_SKINNING
template<typename VertexType>
void SkinVertices( FFinalSkinVertex* DestVertex, FBoneAtom* ReferenceToLocal, INT LODIndex, FStaticLODModel& LOD, TArray<FActiveMorph>& ActiveMorphs, UBOOL bUseBoneInfluences, const TArray<FVertexInfluence>& VertexInfluences )
#else
template<typename VertexType>
FORCEINLINE void SkinVertices( FFinalSkinVertex* DestVertex, FBoneAtom* ReferenceToLocal, INT LODIndex, FStaticLODModel& LOD, TArray<FActiveMorph>& ActiveMorphs, UBOOL bUseBoneInfluences, const TArray<FVertexInfluence>& VertexInfluences )
#endif
{
	...
	for(INT ChunkIndex = 0;ChunkIndex < LOD.Chunks.Num();ChunkIndex++)
	{
		...
		static FFinalSkinVertex* LocalDestVertex;
