...
#define ENABLE_DYNAMIC_BUFFERS      0  // Disabled, we need to make sure we don't discard when locking VBs or IBs
 ...
 */
class FApexRenderVertexBuffer : public NxUserRenderVertexBuffer
{
private:
...
public:
	FApexRenderVertexBuffer(const physx::NxUserRenderVertexBufferDesc &Desc)
	{
		...
		for(UINT i=0; i<physx::NxRenderVertexSemantic::NUM_SEMANTICS; i++)
		{
			...
			if(Format != physx::NxRenderDataFormat::UNSPECIFIED)
			{
				...
				else if((Usage == VEU_Normal || Usage == VEU_Tangent || Usage == VEU_Binormal) && (FormatType == VET_Float3 || FormatType == VET_Float4))
				{
	...
	*/
	void Init(const physx::NxUserRenderVertexBufferDesc &Desc)
	{
		...
		if(BufferSize > 0)
		{
			const EResourceUsageFlags UsageHint = GetUsageFlag(Desc.hint);

			VertexBuffer.InitResource();
			VertexBuffer.VertexBufferRHI = RHICreateVertexBuffer(BufferSize, 0, UsageHint);

			void *VB = RHILockVertexBuffer(VertexBuffer.VertexBufferRHI, 0, BufferSize, FALSE);
			check(VB);
			if(VB)
			{
				appMemset(VB, 0, BufferSize);
			}
	...
	const FVertexBuffer      *getVertexBuffer(void) const       { return &VertexBuffer; }
...
public:
	...
	*/
	virtual void writeBuffer(const NxApexRenderVertexBufferData &data, physx::PxU32 firstVertex, physx::PxU32 numVertices)
	{
		...
		if ( IsInRenderingThread() ) // If we are in the rendering thread, then process this write buffer call.
		{
			...
			BYTE *VBData = (BYTE*)RHILockVertexBuffer(VertexBuffer.VertexBufferRHI, 0, BufferSize, FALSE);
			check(VBData);
			...
			for(physx::PxU32 i=0; i<NxRenderVertexSemantic::NUM_SEMANTICS; i++)
			{
				...
				{
					writeBufferSemantic(VBData,semantic, semanticData.data, semanticData.stride, firstVertex, numVertices);
				}
			}
	...
	*/
	void writeBuffer(BYTE *SrcVBData)
	{
		...
		BYTE *DstVBData = (BYTE*)RHILockVertexBuffer(VertexBuffer.VertexBufferRHI, 0, BufferSize, FALSE);
		check(DstVBData);
							 ...
							 physx::PxU32 NumElements)
	{
		...
		if(AVE.IsValid())
		{
			...
			if(VBData)
			{
				...
				{
					CopyNormals(VBData+AVE.Offset, BufferStride, SrcData, SrcStride, NumElements);
				}
				...
				else if(Semantic == physx::NxRenderVertexSemantic::TEXCOORD0 )
				{
...
private:
	...
	void CopyNormals(void *RESTRICT Dest, UINT DestStride, const void *RESTRICT Src, UINT SrcStride, UINT Num)
	{
		while(Num--)
		{
 ...
 */
class FApexRenderIndexBuffer : public NxUserRenderIndexBuffer
{
private:
	static EResourceUsageFlags GetUsageFlag(physx::NxRenderBufferHint::Enum hint)
	{
...
public:
	...
	void Init(const NxUserRenderIndexBufferDesc &Desc)
	{
		...
		EResourceUsageFlags UsageHint	= GetUsageFlag(Desc.hint);
		...
		if(BufferSize > 0)
		{
			...
			IndexBuffer.IndexBufferRHI = RHICreateIndexBuffer(BufferStride, BufferSize, 0, UsageHint);
			BYTE *IB = (BYTE*)RHILockIndexBuffer(IndexBuffer.IndexBufferRHI, 0, BufferSize);
			if(IB)
			{
				appMemset(IB, 0, BufferSize);
			}
	...
	const FIndexBuffer *getIndexBuffer(void) const
	{
		return &IndexBuffer;
	}
...
public:
	...
	virtual void writeBuffer(const void *SrcData, physx::PxU32 SrcStride, physx::PxU32 FirstDestElement, physx::PxU32 NumElements)
	{
		...
		if ( IsInRenderingThread() ) // if we are in the rendering thread, then process this write buffer call immediately
		{
    		...
    		BYTE *IB = (BYTE*)RHILockIndexBuffer(IndexBuffer.IndexBufferRHI, Offset, LockSize);
    		if(IB)
    		{
    			for(UINT i=0; i<NumElements; i++)
    			{
...
//		UINT NumVBs = (UINT)Desc.vertexBuffers.size();
	...
	UBOOL loadMeshElement(FMeshElement &MeshElement, const NxApexRenderContext &Context, UBOOL bSelected)
	{
		UBOOL OK = FALSE;
		...
		MeshElement.IndexBuffer                = IndexBuffer ? IndexBuffer->getIndexBuffer() : NULL;
		MeshElement.FirstIndex                 = FirstIndex;
		MeshElement.NumPrimitives              = GetNumPrimitives();
		...
		MeshElement.MinVertexIndex             = FirstVertex;
		MeshElement.MaxVertexIndex             = NumVertices ? FirstVertex+NumVertices-1 : 0;
		...
		{
			MeshElement.LocalToWorld           = N2UTransformApex(Context.local2world);
			MeshElement.WorldToLocal           = N2UTransformApex(Context.world2local);
		}

		if(MeshElement.IndexBuffer && MeshElement.NumPrimitives > 0 && MeshElement.VertexFactory && MeshElement.MaterialRenderProxy)
		{
			OK = TRUE;
		}
...
IMPLEMENT_VERTEX_FACTORY_TYPE(FGPUSkinVertexFactoryApexDestructible, "ApexGpuSkinVertexFactory_1Bone", TRUE, FALSE, TRUE, FALSE, VER_MOTIONBLURSKINNING, 0);
...
IMPLEMENT_VERTEX_FACTORY_TYPE(FGPUSkinVertexFactoryApexClothing, "ApexGpuSkinVertexFactory", TRUE, FALSE, TRUE, FALSE, VER_REMOVE_MAXBONEINFLUENCE, 0);
