			...
			if (bUseCompositedTranslucentLighting)
			{
				...
				for( const FLightPrimitiveInteraction* Interaction = PrimitiveSceneInfo->LightList; Interaction; Interaction = Interaction->GetNextLight() )
				{
					...
					if (!MergedDynamicLight 
						...
						// Don't use a sky or SH light as the merged light, since it will be applied as a directional light
	...
	FHitProxyId HitProxyId	)
{
	...
	if( bRender )
	{
		FDepthDrawingPolicy DrawingPolicy(Mesh.VertexFactory,MaterialRenderProxy,TRUE,FALSE,bIsPostRenderDepthPass);
...
* @param SceneColorPass 0:before SceneColor resolve 1:using SceneColor 2:separate translucency
