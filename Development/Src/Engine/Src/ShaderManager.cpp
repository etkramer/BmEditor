...
static UBOOL IsParameterBoundByView(const FString& ParameterName, EShaderFrequency Frequency)
{
	...
	if (Frequency == SF_Vertex)
	{
		bParameterBoundByView = ParameterName == TEXT("ViewProjectionMatrix") 
			...
			|| ParameterName == TEXT("TemporalAAParameters");
	}
	else if (Frequency == SF_Hull)
	{
		bParameterBoundByView = ParameterName == TEXT("GlobalTesselationFactor");
	}
	else if (Frequency == SF_Domain)
	{
		bParameterBoundByView = ParameterName == TEXT("ViewProjectionMatrixDS")
			...
			|| ParameterName == TEXT("TemporalAAParametersDS");
	}
...
void GetShaderIncludes(const TCHAR* Filename, TArray<FString> &IncludeFilenames, UINT DepthLimit)
{
	...
	while (IncludeBegin != NULL && SearchCount < MaxSearchCount && DepthLimit > 0)
	{
		...
		if (ExtractedIncludeFilename != TEXT("VertexFactory.usf"))
		{
			...
			ExtractedIncludeFilename = FFilename(ExtractedIncludeFilename).GetBaseFilename();
...
#if PLATFORM_MACOSX
			WORD *UTF16Contents = (WORD *)appMalloc((IncludeFileContents.Len() + 1) * 2);
			appMacUTF32ToUTF16((DWORD *)*IncludeFileContents, UTF16Contents, IncludeFileContents.Len());
			HashState.Update((const BYTE*)UTF16Contents, IncludeFileContents.Len() * sizeof(WORD));
			appFree(UTF16Contents);
#else
			HashState.Update((const BYTE*)*IncludeFileContents, IncludeFileContents.Len() * sizeof(TCHAR));
#endif
		}
...
#if PLATFORM_MACOSX
		WORD *UTF16Contents = (WORD *)appMalloc((FileContents.Len() + 1) * 2);
		appMacUTF32ToUTF16((DWORD *)*FileContents, UTF16Contents, FileContents.Len());
		HashState.Update((const BYTE*)UTF16Contents, FileContents.Len() * sizeof(WORD));
		appFree(UTF16Contents);
#else
		HashState.Update((const BYTE*)*FileContents, FileContents.Len() * sizeof(TCHAR));
#endif
		HashState.Final();
...
void GenerateBinaryShaderFiles( )
{
	...
	for (INT ShaderIndex = 0; ShaderIndex < ShaderFiles.Num(); ShaderIndex++)
	{
		...
		const FString DestFolder = FString( appShaderDir() ) * TEXT("Binaries");
