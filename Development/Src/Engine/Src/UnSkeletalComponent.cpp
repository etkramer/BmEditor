...
void USkeletalMeshComponent::Attach()
{
	...
	if( SkeletalMesh != NULL && SkeletalMesh->FaceFXAsset != NULL && SkeletalMesh->FaceFXAsset->FaceFXActor != NULL )
	{
...
void USkeletalMeshComponent::UpdateTransform()
{
	...
	if ( Clothing != NULL )
	{
		...
		if((Clothing->IsVisible(TRUE) || !bAutoFreezeApexClothingWhenNotRendered) && (!PhysicsAssetInstance || PhysicsWeight == 0) && !GWorld->bPostTickComponentUpdate)
		{
...
void USkeletalMeshComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if ( PropertyChangedEvent.Property != NULL &&
		 PropertyChangedEvent.Property->GetFName() == TEXT( "AnimSets" ) )
	{
...
void USkeletalMeshComponent::TickAnimNodes(FLOAT DeltaTime)
{
	...
	for(INT i=0; i<AnimNodeCount; ++i)
	{
		...
		FLOAT& NodeWeight = AnimTickWeightsArray(i);
...
void USkeletalMeshComponent::AttachComponentToSocket(UActorComponent* Component,FName SocketName)
{
	if( SkeletalMesh != NULL )
	{
		...
		{
#if !PS3 // hopefully will be found on PC
			debugf(NAME_DevAnim,TEXT("AttachComponentToSocket : Could not find socket '%s' in %s  attaching: %s[%s]"), *SocketName.ToString(), *SkeletalMesh->GetPathName(), *Component->GetName(), *Component->GetDetailedInfo() );
#endif
		}
	}
	else
...
void USkeletalMeshComponent::GetStreamingTextureInfo(TArray<FStreamingTexturePrimitiveInfo>& OutStreamingTextures) const
{
	if( SkeletalMesh )
	{
		const FLOAT LocalTexelFactor = SkeletalMesh->GetStreamingTextureFactor(0);
		...
		for( INT MatIndex = 0; MatIndex < NumMaterials; MatIndex++ )
		{
			...
			if(MaterialInterface)
			{
				...
				MaterialInterface->GetUsedTextures(Textures, MSP_BASE, TRUE);
...
FMatrix USkeletalMeshComponent::CalcCurrentLocalToWorld(const FMatrix& ParentMatrix)
{
	...
	if( SkeletalMesh && !bForceRawOffset && !(ParentAnimComponent && bTransformFromAnimParent) )
	{
		if( !AbsoluteRotation )
		{
			ResultMatrix = FRotationMatrix(SkeletalMesh->RotOrigin) * ResultMatrix;
		}

		if( !AbsoluteTranslation )
		{
			ResultMatrix = FTranslationMatrix(SkeletalMesh->Origin) * ResultMatrix;
		}
	}
...
#if !FINAL_RELEASE
			else if( BoneIndex == INDEX_NONE && BoneName != NAME_None && GIsEditor )
			{
				...
				warnf(TEXT("BuildComposeSkeletonPasses Bone %s not found for POST-PASS. Mesh: %s, Tree: %s"), *BoneName.ToString(), *SkeletalMesh->GetFName().ToString(), *AnimTreeTemplate->GetFName().ToString());
			}
#endif
		}
...
void USkeletalMeshComponent::ApplyControllersForBoneIndex(INT BoneIndex, UBOOL bPrePhysControls, UBOOL bPostPhysControls, const UAnimTree* Tree, UBOOL bRenderedRecently, const TArray<INT>& BoneProcessed)
{
	checkSlow(Tree);
			...
			if( (bRenderedRecently || !Control->bIgnoreWhenNotRendered) && (PredictedLODLevel < Control->IgnoreAtOrAboveLOD) && (ControlWeight > ZERO_ANIMWEIGHT_THRESH) && bCorrectPhysPhase )
			{
				...
				if( AffectedBones.Num() > 0 )
				{
					...
					if( ControlAlpha > ZERO_ANIMWEIGHT_THRESH && (bTransformingAffectedBones || bScalingAffectedBones) )
					{
						...
						// also set 0 scale for invisible bones
						for(INT AffectedIdx=0; AffectedIdx<AffectedBones.Num(); AffectedIdx++)
						{
							const INT AffectedBoneIndex	= AffectedBones(AffectedIdx);
...
// 	debugf(TEXT("(ComposeSkeleton) [%3.3f] SkelMesh: %s"), GWorld->GetWorldInfo()->TimeSeconds, *SkeletalMesh->GetName());
	...
	UBOOL bDoPostPhysControls = (PhysicsAssetInstance == NULL) || (GIsEditor && !GIsGame);
		...
		for (INT BoneIndex=0; BoneIndex<BoneVisibilityStates.Num(); ++BoneIndex)
		{
			if (BoneVisibilityStates(BoneIndex)!=BVS_Visible)
			{
				if (BoneIndex != 0 )
				{
					...
					const INT ParentIndex = SkeletalMesh->RefSkeleton(BoneIndex).ParentIndex;
					LocalAtoms(BoneIndex) = LocalAtoms(ParentIndex);
					LocalAtoms(BoneIndex).SetScale(ScalarZero);
					SpaceBases(BoneIndex) = SpaceBases(ParentIndex);
					SpaceBases(BoneIndex).SetScale(ScalarZero);
				}
				else
				{
					LocalAtoms(BoneIndex).SetScale(ScalarZero);
...
// 	}
	...
	// Low Frame Rate Update. In that case use cached version.
...
UBOOL USkeletalMeshComponent::LegLineCheck(const FVector& Start, const FVector& End, FVector& HitLocation, FVector& HitNormal, const FVector& Extent)
{
	if( Owner )
	{
		...
		{
			// Skip all KAsset and KActors. They're unstable and we shouldn't be stepping on them.
			if( Hit->Actor && (Hit->Actor->IsA(AKActor::StaticClass()) || Hit->Actor->IsA(AKAsset::StaticClass()))  )
			{
				continue;
			}
...
void USkeletalMeshComponent::UpdateBounds()
{
	...
	// Check to see whether or not the Physics Asset has any valid bones
	UBOOL bHasValidBones = FALSE;
	if( PhysicsAsset )
	{
		for( INT BoneIndex = 0; BoneIndex < PhysicsAsset->BodySetup.Num(); BoneIndex++ )
		{
			if( MatchRefBone( PhysicsAsset->BodySetup( BoneIndex )->BoneName ) != INDEX_NONE )
			{
				bHasValidBones = TRUE;
			}
		}
	...
	if(ParentAnimComponent)
	{
		if(BoneIdx < (DWORD)ParentBoneMap.Num())
		{
			...
			else
			{
				...
				debugf(NAME_Warning, TEXT("GetBoneMatrix : ParentBoneIndex(%d) out of range of ParentAnimComponent->SpaceBases for %s owned by %s"), BoneIdx, *GetName(), *Owner->GetName());
#endif
...
void USkeletalMeshComponent::InitAnimTree(UBOOL bForceReInit)
{
	...
	if( (bForceReInit || !bAnimTreeInitialised) && Animations && ParentAnimComponent == NULL )
	{
#if PERF_ENABLE_INITANIM_STATS
		InitAnimStats.Empty();
...
{
	// Increase InitTag to triggered DeferredInitAnim()
	InitTag++;
