...
*/
FBoxSphereBounds FSceneViewState::GetOcclusionBounds(const FBoxSphereBounds& Bounds)
{
...
#if USE_STATIC_ES2_RHI || ( PS3 && USE_NULL_RHI )
	if(NumBatchedPrimitives >= MaxBatchedPrimitives)
#elif PLATFORM_MACOSX
	// gcc doesn't understand IsValidRef(CurrentBatchOcclusionQuery), so we use !CurrentBatchOcclusionQuery instead
	// @todo Mac - figure this out and add better documentation
	if(!CurrentBatchOcclusionQuery || NumBatchedPrimitives >= MaxBatchedPrimitives)
#else
