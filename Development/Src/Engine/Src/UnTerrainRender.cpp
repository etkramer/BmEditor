 ...
 *
 *  @param	MatPlatform			The EMaterialShaderPlatform the terrain materil is being compiled for
 ...
 */
INT FTerrainMaterialResource::CompileTerrainMaterial(EMaterialShaderPlatform MatPlatform,EMaterialProperty Property,FMaterialCompiler* Compiler,UTerrainMaterial* TerrainMaterial,UBOOL Highlighted, FColor& HighlightColor) const
{
	...
	INT	Result = Compiler->ForceCast(Material->MaterialResources[MatPlatform]->CompileProperty(MatPlatform,Property,&ProxyCompiler),GetMaterialPropertyType(Property));
 ...
 */
INT FTerrainMaterialResource::CompileProperty(EMaterialShaderPlatform MatPlatform,EMaterialProperty Property,FMaterialCompiler* Compiler) const
{
	...
	{
		return GEngine->DefaultMaterial->GetRenderProxy(0)->GetMaterial()->CompileProperty(MatPlatform,Property,Compiler);
	}
}
...
*/
void ATerrain::CacheResourceShaders(EShaderPlatform ShaderPlatform, UBOOL bFlushExistingShaderMaps, UBOOL bForceAllPlatforms)
{
	...
	{
		EMaterialShaderPlatform MatPlatform = (EMaterialShaderPlatform)PlatformIndex;
		...
		for( INT MatIdx=0; MatIdx < CachedMaterials.Num(); MatIdx++ )
		{
			...
			if( CachedMat )
			{
				...
				if (bForceAllPlatforms || PlatformIndex == RequestedMaterialPlatform)
				{
					...
					{
						CachedMat->CacheShaders(CurrentShaderPlatform);
						CachedMat->LightingGuid = appCreateGuid();
					}
					else
					{
						CachedMat->InitShaderMap(CurrentShaderPlatform);
					}
				}
			}
		}
 ...
 *	be recompiled.
 */
void FTerrainMaterialResource::PostLoad(EMaterialShaderPlatform MatPlatform)
{
			...
			if(Mask.Get(MaterialIndex))
			{
				if (MaterialIndex < Terrain->WeightedMaterials.Num())
				{
					...
					if (TerrainMaterial)
					{
						...
						if (MatRes)
						{
							UMaterial* Material = MatRes->GetMaterial(MatPlatform);
							if (Material && Material->MaterialResources[MatPlatform])
							{
								if (IdIndex < MaterialIds.Num())
								{
									FGuid CheckGuid = MaterialIds(IdIndex++);
									if (Material->MaterialResources[MatPlatform]->GetId() != CheckGuid)
									{
										bTossShader = TRUE;
										MaterialIds.Empty();
										break;
 ...
 */
void FTerrainMaterialResource::PreSave(EMaterialShaderPlatform MatPlatform)
{
			...
			if (MaterialIndex < Terrain->WeightedMaterials.Num())
			{
				...
				if (TerrainMaterial)
				{
					...
					if (MatRes)
					{
						...
						if (Material && Material->MaterialResources[MatPlatform])
						{
							MaterialIds.AddItem(Material->MaterialResources[MatPlatform]->GetId());
						}
						else
						{
...
FMaterialRenderProxy* ATerrain::GetCachedMaterial(const FTerrainMaterialMask& Mask, UBOOL& bIsTerrainResource)
{
	...
	if( !Result )
	{
		...
		Result = GenerateCachedMaterial(Mask,MatPlatform);
		// cache its shaders
		if( Result )
		{
			// compile for the current RHI platform that is running
			Result->CacheShaders(GRHIShaderPlatform);
...
*/
FTerrainMaterialResource* ATerrain::GenerateCachedMaterial(const FTerrainMaterialMask& Mask, EMaterialShaderPlatform MatPlatform)
{
	FTerrainMaterialResource* Result = NULL;
	// get the cached terrain materials for the platform 		
	TArrayNoInit<FTerrainMaterialResource*>& CachedMaterials = GetCachedTerrainMaterials(MatPlatform);
			...
			for ( INT i = 0 ; i < DecalInteraction->RenderData->ReceiverResources.Num() ; ++i )
			{
				...
				if( Decal )
				{
					...
					if( DecalInteraction->DecalStaticMesh && i == 0 )
					{
						...
						DecalInteraction->DecalStaticMesh->IndexBuffer = DecalIndexBuffer;
						DecalInteraction->DecalStaticMesh->NumPrimitives = Decal->GetSmoothIndexBuffer()->NumTriangles;
					}
				}
			}
		}		
	...
	, MaxTessellation(1)
{
	...
	// Make sure terrain material resource parameters are cached for the current material platform
	TArrayNoInit<FTerrainMaterialResource*>& CachedMaterials = Terrain->GetCachedTerrainMaterials(GCurrentMaterialPlatform);
	for( INT MatIdx = 0; MatIdx < CachedMaterials.Num(); MatIdx++ )
	{
...
void FTerrainComponentSceneProxy::AddDecalInteraction_RenderingThread(const FDecalInteraction& DecalInteraction)
{
	...
	if( TerrainObject )
	{
		...
		if (TerrainObject->GetRepackRequired() == TRUE)
		{
			...
			TerrainObject->UpdateResources_RenderingThread(TessParam, Decals);
 ...
 */
void FTerrainComponentSceneProxy::PreRenderView(const FSceneViewFamily* ViewFamily, const TBitArray<FDefaultBitArrayAllocator>& VisibilityMap, INT FrameNumber)
{
