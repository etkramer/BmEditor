...
UBOOL FPoly::IsConvex()
{
	...
	{
		const FVector* vtx1 = &Vertices(v);
		const FVector* vtx2 = &Vertices((v+1)%Vertices.Num());

		FPlane plane( *vtx1, *vtx2, (*vtx2) + (Normal * 16.f) );
		Planes.AddItem( plane );
	}

	// A polygon is convex if all the vertices lie behind each edge plane.

	FLOAT dot;

	for( INT p = 0 ; p < Planes.Num() ; ++p )
	{
		FPlane* plane = &Planes(p);

		for( INT v = 0 ; v < Vertices.Num() ; ++v )
		{
			dot = plane->PlaneDot( Vertices(v) );
...
void FPoly::GetOutsideWindings( ABrush* InOwnerBrush, TArray<FPoly>& InPolygons, TArray< TArray<FVector> >& InWindings )
{
	InWindings.Empty();
