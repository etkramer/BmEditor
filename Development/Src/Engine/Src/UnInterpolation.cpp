...
void AMatineeActor::AddAIGroupActor(UInterpGroupInstAI * AIGroupInst)
{
	if (AIGroupInst->AIGroup)
	{
		...
		if ( GroupName!=NAME_None && GroupActor!=NULL ) 	
		{
...
void UInterpData::UpdateBakeAndPruneStatus()
{
	...
	for (INT StatusIdx = BakeAndPruneStatus.Num() - 1; StatusIdx >= 0; StatusIdx--)
	{
		FAnimSetBakeAndPruneStatus& Status = BakeAndPruneStatus(StatusIdx);
		UBOOL bFound = FALSE;
		for (INT AnimSetIdx = 0; (AnimSetIdx < FoundAnimSetNames.Num()) && !bFound; AnimSetIdx++)
		{
		...
		{
			// Remove it!
			BakeAndPruneStatus.Remove(StatusIdx);
		}
	}
...
void USeqAct_Interp::UpdateObject()
{
	...
	// check for changes to output links
	{
		...
		INT NumberOfSavedOutputLinks = OutputLinks.Num() - EventNames.Num();
...
void UInterpTrackInstFloatProp::InitTrackInst(UInterpTrack* Track)
{
	...
	InterpTools::EnableCameraPostProcessFlag(Actor, TrackProp->PropertyName);
}
...
void UInterpTrackBoolProp::UpdateTrack(FLOAT NewPosition, UInterpTrackInst* TrackInst, UBOOL bJump)
{
	...
	*(BITFIELD*)PropInst->BoolProp = NewBoolValue;
...
void UInterpTrackInstBoolProp::InitTrackInst( UInterpTrack* Track )
{
	...
	BoolProp = Actor->GetInterpBoolPropertyRef(TrackProp->PropertyName);
...
void UInterpTrackInstLinearColorProp::InitTrackInst(UInterpTrack* Track)
{
	...
	ColorProp = Actor->GetInterpLinearColorPropertyRef(TrackProp->PropertyName);
...
void UInterpTrackInstFloatMaterialParam::SaveActorState(UInterpTrack* Track)
{
	...
	if (ParamTrack != NULL)
	{
		for (INT i = 0; i < ParamTrack->Materials.Num() && i < MICInfos.Num(); i++)
		{
			...
			{
				if (MICInfos(i).MICs(j) == NULL || !MICInfos(i).MICs(j)->GetScalarParameterValue(ParamTrack->ParamName, MICInfos(i).MICResetFloats(j)))
				{
					MICInfos(i).MICResetFloats(j) = 0.f;
				}
			}
		}
	}
...
void UInterpGroupInstAI::CreatePreviewPawn()
{
	...
	if (AIGroup->PreviewPawnClass != NULL && StageMarkActor != NULL)
	{
		if (PreviewPawn == NULL)
		{
			...
			else if (PreviewPawn->Mesh && PreviewPawn->Mesh->AnimTreeTemplate == NULL)
			{
				// set animtree templates first
...
void UInterpGroupInstAI::UpdateStageMarkGroupActor(USeqAct_Interp * Seq)
{
	if ( Seq && AIGroup )
	{
		...
		{
			// print error
			debugf(TEXT("Matinee AI Group's Stage Mark is missing or valid actor isn't connected : %s"), *AIGroup->StageMarkGroup.GetNameString());
		}
	}
}
...
void UInterpTrackHeadTracking::UpdateTrack(FLOAT NewPosition, UInterpTrackInst* TrInst, UBOOL bJump)
{
	...
	for( TMap<class AActor*,struct FActorToLookAt*>::TIterator It(HeadTrackingInst->CurrentActorMap); It; ++It )
	{
		...
		if ( !ActorToLookAt->Actor || ActorToLookAt->Actor->bDeleteMe)
		{
			delete It.Value();
			It.RemoveCurrent();
		}
