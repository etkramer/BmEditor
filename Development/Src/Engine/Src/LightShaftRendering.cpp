				...
				&& (LightSceneInfo->BloomScale > 0.001f || LightSceneInfo->OcclusionMaskDarkness < 1.0f))
			{
				...
				for(INT ViewIndex = 0;ViewIndex < Views.Num();ViewIndex++)
				{
						...
						|| DistanceToBlurOrigin < LightSceneInfo->GetRadius() * PointLightRadiusFadeFactor))
					{
						...
						SetGlobalBoundShaderState(ApplyLightShaftsBoundShaderState, GApplyLightShaftsVertexDeclaration.VertexDeclarationRHI, *ApplyLightShaftsVertexShader, *ApplyLightShaftsPixelShader, sizeof(FVector2D));
