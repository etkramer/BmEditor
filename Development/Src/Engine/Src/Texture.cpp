...
// to detect changes from specific enum values to a new value
static BYTE GPreBackupLODGroup = 0;
// to detect if the the Pre and Post don't match (meaning this implementation doesn't work as intended)
static UTexture* GPreBackupTexturePointer = 0;
...
void UTexture::PreEditChange(UProperty* PropertyAboutToChange)
{
	...
	GPreBackupLODGroup = LODGroup;
	GPreBackupTexturePointer = this;
}
...
void UTexture::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if( PropertyThatChanged )
	{
		...
		if(	appStricmp( *PropertyName, TEXT("LODGroup")						) == 0)
		{
			...
			GPreBackupTexturePointer = 0;
			GPreBackupLODGroup = 0;
		}
