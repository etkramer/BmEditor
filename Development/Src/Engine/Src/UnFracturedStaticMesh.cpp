...
void UFracturedStaticMeshComponent::CheckForErrors()
{
	...
	if (StaticMesh)
	{
		...
		{
			GWarn->MapCheck_Add(MCTYPE_WARNING, Owner, *FString::Printf(TEXT("%s : Fractured Static Mesh needs to be resliced."), *FracturedStaticMesh->GetPathName()), MCACTION_NONE);
		}
		...
		if (LODData.Num() > 0 && LODData(0).LightMap)
		{
			...
			if (LightMap1D)
			{
				GWarn->MapCheck_Add(MCTYPE_WARNING, Owner, *FString::Printf(TEXT("Fractured component using vertex lightmaps, will waste a lot of memory, use texture lightmaps instead.")), MCACTION_NONE);
			}
		}
	}
}
