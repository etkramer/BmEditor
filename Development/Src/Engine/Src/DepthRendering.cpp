...
FDepthDrawingPolicy::FDepthDrawingPolicy(
	...
	FMeshDrawingPolicy(InVertexFactory,InMaterialRenderProxy,FALSE,FALSE,0.f,bOutputDepthToSceneColorAlpha)
{
	...
	if (bForceUsePixelShader || MaterialResource->IsMasked())
	{
		bNeedsPixelShader = TRUE;

		if( bOutputDepthToSceneColorAlpha )
		{
			PixelShader = MaterialResource->GetShader<FTranslucencyPostRenderDepthPixelShader>(InVertexFactory->GetType());
		}
		else
		{
			PixelShader = MaterialResource->GetShader<TDepthOnlySolidPixelShader>(InVertexFactory->GetType());
		}
	}
	else
	{
	...
	if (Mesh.bUseAsOccluder)
	{
		...
		else if (!IsTranslucentBlendMode(BlendMode))
		{
			...
			if(bDraw)
			{
				...
				FDepthDrawingPolicy DrawingPolicy(Mesh.VertexFactory,MaterialRenderProxy,FALSE,bForceDefaultPixelShader,DrawingContext.bOutputDepthToSceneAlpha);
				DrawingPolicy.DrawShared(&View,DrawingPolicy.CreateBoundShaderState(Mesh.GetDynamicVertexStride()));
