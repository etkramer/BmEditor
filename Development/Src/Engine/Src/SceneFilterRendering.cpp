	...
	const FSceneView* View)
{
	...
	if(View)
	{
		...
		FSamplerStateRHIParamRef SamplerStateRHI = TStaticSamplerState<SF_Point,AM_Clamp,AM_Clamp,AM_Clamp>::GetRHI();
		...
		PixelShader->SetParameters(View, SamplerStateRHI, Src);
	...
	{
		// down sample 4:1 with simple averaging (e.g. for high quality bloom)
		SetFilterShaders(
	...
	ResolveRect.X2 = DstLeftTop.X + SrcRect.Width();
	ResolveRect.Y2 = DstLeftTop.Y + SrcRect.Height();
