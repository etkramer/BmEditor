...
UBOOL FLandscapeEditDataInterface::GetComponentsInRegion(INT X1, INT Y1, INT X2, INT Y2, TSet<ULandscapeComponent*>* OutComponents /*= NULL*/)
{
	...
	for( INT ComponentIndexY=ComponentIndexY1;ComponentIndexY<=ComponentIndexY2;ComponentIndexY++ )
	{
		...
		{		
			...
			if( Component )
			{				
				bNotLocked = bNotLocked && ( !FLevelUtils::IsLevelLocked(Component->GetLandscapeProxy()->GetLevel()) );
				if (OutComponents)
				{
...
namespace
{
	FORCEINLINE void CalcComponentIndices(const INT X1, const INT Y1, const INT X2, const INT Y2, const INT ComponentSizeQuads, 
...
void FLandscapeEditDataInterface::GetHeightDataTemplFast(const INT X1, const INT Y1, const INT X2, const INT Y2, TStoreData& StoreData, TStoreData* NormalData /*= NULL*/)
{
	...
	CalcComponentIndices(X1, Y1, X2, Y2, ComponentSizeQuads, ComponentIndexX1, ComponentIndexY1, ComponentIndexX2, ComponentIndexY2);
		...
		for( INT ComponentIndexX=ComponentIndexX1;ComponentIndexX<=ComponentIndexX2;ComponentIndexX++ )
		{
			...
			else
			{
				...
				if ( bShouldSearchX || (!NoBorderX1 && !BorderComponent[0]))
				{
					...
					for (INT X = ComponentIndexX-1; X >= ComponentIndexX1; X--)
					{
						BorderComponent[0] = Landscape->XYtoComponentMap.FindRef(ALandscape::MakeKey(X*ComponentSizeQuads,ComponentIndexY*ComponentSizeQuads));
				...
				if ( bShouldSearchX || (!NoBorderX2 && !BorderComponent[1]))
				{
					...
					for (INT X = ComponentIndexX+1; X <= ComponentIndexX2; X++)
					{
						BorderComponent[1] = Landscape->XYtoComponentMap.FindRef(ALandscape::MakeKey(X*ComponentSizeQuads,ComponentIndexY*ComponentSizeQuads));
				...
				if ( bShouldSearchY || (!NoBorderY1(ComponentIndexXX) && !BorderComponentY1(ComponentIndexXX)))
				{
					...
					for (INT Y = ComponentIndexY-1; Y >= ComponentIndexY1; Y--)
					{
						BorderComponentY1(ComponentIndexXX) = BorderComponent[2] = Landscape->XYtoComponentMap.FindRef(ALandscape::MakeKey(ComponentIndexX*ComponentSizeQuads,Y*ComponentSizeQuads));
				...
				if ( bShouldSearchY || (!NoBorderY2(ComponentIndexXX) && !BorderComponentY2(ComponentIndexXX)))
				{
					...
					for (INT Y = ComponentIndexY+1; Y <= ComponentIndexY2; Y++)
					{
						BorderComponentY2(ComponentIndexXX) = BorderComponent[3] = Landscape->XYtoComponentMap.FindRef(ALandscape::MakeKey(ComponentIndexX*ComponentSizeQuads,Y*ComponentSizeQuads));
				...
				CornerComponent[0] = Landscape->XYtoComponentMap.FindRef(ALandscape::MakeKey((ComponentIndexX-1)*ComponentSizeQuads,(ComponentIndexY-1)*ComponentSizeQuads));
				CornerComponent[1] = Landscape->XYtoComponentMap.FindRef(ALandscape::MakeKey((ComponentIndexX+1)*ComponentSizeQuads,(ComponentIndexY-1)*ComponentSizeQuads));
				CornerComponent[2] = Landscape->XYtoComponentMap.FindRef(ALandscape::MakeKey((ComponentIndexX-1)*ComponentSizeQuads,(ComponentIndexY+1)*ComponentSizeQuads));
				CornerComponent[3] = Landscape->XYtoComponentMap.FindRef(ALandscape::MakeKey((ComponentIndexX+1)*ComponentSizeQuads,(ComponentIndexY+1)*ComponentSizeQuads));
...
void ULandscapeComponent::DeleteLayer(FName LayerName, TMap<FName, FLandscapeLayerInfo>& LayerInfosMap, FLandscapeEditDataInterface* LandscapeEdit /*= NULL*/)
{
...
void FLandscapeEditDataInterface::DeleteLayer(FName LayerName)
{
	...
	Landscape->GetAllEditableComponents(&AllLandscapeComponents);
	// Iterate all the components currently visible...
	for( INT ComponentIdx=0;ComponentIdx<AllLandscapeComponents.Num();ComponentIdx++ )
	{
		ULandscapeComponent* Component = AllLandscapeComponents(ComponentIdx);
		Component->DeleteLayer(LayerName, LayerInfosMap, this);
	}
}
