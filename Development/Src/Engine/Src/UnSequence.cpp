 ...
 */
BITFIELD* AActor::GetInterpBoolPropertyRef( FName InPropName )
{
	// get the property name and the Object its in. handles property being in a component.
	INT PropOffset;
	UObject* PropObject = FindObjectAndPropOffset( PropOffset, this, InPropName );
		...
		for (INT i = 0; i < VariableLinks.Num(); i++)
		{
			...
			for (INT j = 0; j < VarLink.LinkedVariables.Num(); j++)
			{
				...
				if (VarLink.bModifiesLinkedObject && ObjectVar != NULL)
				{
					...
					{
						GWarn->MapCheck_Add(MCTYPE_KISMET, TestActor, *FString::Printf(TEXT("Kismet object %s will fail to modify Actor referenced through a linked variable: %s"), *GetPathName(), *Reason));
					}
				}
				...
				{
					GWarn->MapCheck_Add(MCTYPE_KISMET, this, *FString::Printf(TEXT("Kismet object %s has a deprecated property %s referenced by a linked variable: %s"), *GetPathName(), *VarLink.PropertyName.ToString(), *ObjectVar->GetPathName()));
				}
			}
			...
			if (VarLink.bModifiesLinkedObject && Prop != NULL)
			{
				...
				for (INT i = 0; i < ObjArray.Num(); i++)
				{
					...
					if (TestActor != NULL && !TestActor->SupportsKismetModification(this, Reason))
					{
						GWarn->MapCheck_Add(MCTYPE_KISMET, TestActor, *FString::Printf(TEXT("Kismet object %s will fail to modify Actor referenced through an editable property: %s"), *GetPathName(), *Reason));
					}
				}
			}
		}
...
FVector* USeqVar_Object::GetRef()
{
	...
	if( ActorValue )
	{
		...
		AController* Controller = Cast<AController>(ActorValue);
		if( Controller != NULL )
		{
...
void USeqAct_ConvertToString::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if( PropertyThatChanged && PropertyThatChanged->GetFName() == FName(TEXT("NumberOfInputs")) )
	{
		...
		else if (CurrentNumOfInputs < NumberOfInputs)
		{
			...
			USeqAct_ConvertToString *DefAction = GetDefault<USeqAct_ConvertToString>();
			while (NumToAdd-- > 0)
			{
...
UBOOL USeqAct_StreamInTextures::UpdateOp(FLOAT DeltaTime)
{
	...
	if ( bStreamingActive )
	{
		...
		{
			// Handle location-based streaming here. Update every frame while this action is active.
	...
	if ( bWasActive != bStreamingActive )
	{
		...
		for (INT TargetIndex = 0; TargetIndex < Targets.Num(); TargetIndex++)
		{
			...
			if ( Target )
			{
				...
				AController* TargetController = Cast<AController>(Target);
				...
				for (AController *Controller = GWorld->GetFirstController(); Controller != NULL; Controller = Controller->NextController)
				{
					...
					if ( PC && !PC->IsLocalPlayerController() )
					{
						...
						PC->eventClientPrestreamTextures( Target, Seconds, bStreamingActive, SelectedCinematicTextureGroups );
					}
				}
			}
		}
...
void USeqAct_CameraFade::Activated()
{
	...
	for (INT Idx = 0; Idx < Targets.Num(); Idx++)
	{
		...
		if (PC != NULL)
		{
			...
			PC->eventClientSetCameraFade(TRUE, FadeColor, FadeAlpha, FadeTime);
...
void USeqVar_Character::CheckForErrors()
{
	...
	if (GWarn != NULL && GWarn->MapCheck_IsActive())
	{
		GWarn->MapCheck_Add(MCTYPE_KISMET, NULL, *FString::Printf(TEXT("%s is obsolete! Replace it with a player variable and an Assign Player Slot action."), *GetPathName()));
	}
}
#endif
