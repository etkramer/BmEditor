...
void UMorphNodeWeightBase::DrawMorphNode(FCanvas* Canvas, const TArray<UAnimObject*>& SelectedNodes)
{
	...
	const FColor& BorderColor = bSelected ? SelectedColor : FColor(0,0,0);
	...
	for(INT i=0; i<NodeConns.Num(); i++)
	{
		...
		for(INT j=0; j<Conn.ChildNodes.Num(); j++)
		{
			...
			if( ChildNode )
			{
				...
				const FColor& LineColor = ( bSelected || SelectedNodes.ContainsItem( ChildNode ) ) ? SelectedColor : MorphConnColor;
