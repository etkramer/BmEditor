...
#define	SHOW_Tessellation			MAKE_SHOW_FLAG(69)	// Post processing: Tessellation
...
#define	SHOW_SSAO					MAKE_SHOW_FLAG(73)	// Post processing: Screen Space Ambient Occlusion
#define	SHOW_VisualizeSSAO			MAKE_SHOW_FLAG(74)	// Post processing: Visualize Screen Space Ambient Occlusion
#define	SHOW_LightShafts			MAKE_SHOW_FLAG(75)	// Post processing: Light shafts
#define SHOW_TemporalAA				MAKE_SHOW_FLAG(76)	// Post processing: Temporal AA
...
struct FTemporalAAParameters
{
	FVector2D Offset;
	FLOAT StartDepth;
	FLOAT JitterStartDepth;

	/** Initialization constructor. */
	FTemporalAAParameters(const FVector2D& InOffset,FLOAT InStartDepth,FLOAT InJitterStartDepth)
	:	Offset(InOffset)
	,	StartDepth(InStartDepth)
	,	JitterStartDepth(InJitterStartDepth)
	{}

	/** Default constructor. */
	FTemporalAAParameters(): Offset(0,0), StartDepth(0), JitterStartDepth(0) {}

	/** Returns the parameters packed into a 4-vector. */
	FVector4 GetVector() const
	{
		return FVector4(Offset.X,Offset.Y,JitterStartDepth,StartDepth);
	}
};
