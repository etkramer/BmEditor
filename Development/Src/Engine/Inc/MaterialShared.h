...
struct FProxyMaterialCompiler: FMaterialCompiler
{
	...
	virtual INT ForceCast(INT Code,EMaterialValueType DestType,UBOOL bExactMatch=FALSE,UBOOL bReplicateValue=FALSE) { return Compiler->ForceCast(Code,DestType,bExactMatch,bReplicateValue); }
...
public:
	...
	*/
	UBOOL CacheShaders(EShaderPlatform Platform=GRHIShaderPlatform, UBOOL bFlushExistingShaderMap=TRUE);

	/**
	* Caches the material shaders for the given static parameter set and platform
	*/
	UBOOL CacheShaders(class FStaticParameterSet* StaticParameters, EShaderPlatform Platform, UBOOL bFlushExistingShaderMap, UBOOL bDebugDump = FALSE);
	...
	UBOOL InitShaderMap(EShaderPlatform Platform=GRHIShaderPlatform);

	/** Initializes the material's shader map. */
	UBOOL InitShaderMap(FStaticParameterSet* StaticParameters, EShaderPlatform Platform);
	...
	virtual INT CompileProperty(EMaterialShaderPlatform MatPlatform,EMaterialProperty Property,FMaterialCompiler* Compiler) const = 0;
	...
	virtual UBOOL RequiresSimpleStaticLighting(EMaterialShaderPlatform MatPlatform) const { return FALSE; }
	...
	virtual UBOOL IsMobileTextureCoordinateTransformed () const { return FALSE; }
...
protected:
	 ...
	 */
	virtual void FillMobileMaterialVertexParams (const UMaterial* InMaterial, FMobileMaterialVertexParams& OutVertexParams) const;
	/**
	 * Internal helper functions to fill in the vertex params struct
	 * @param InMaterial - The Material to draw the parameters from
	 * @param OutVertexParams - Vertex parameter structure to pass to the shader system
	 */
	virtual void FillMobileMaterialPixelParams (const UMaterial* InMaterial, FMobileMaterialPixelParams& OutVertexParams) const;
...
public:
	...
	*/
	UBOOL GetMaterialExpressionSource( FString& OutSource, TMap<UMaterialExpression*,INT>* OutExpressionCodeMap );
...
public:
	...
	virtual UBOOL IsMobileTextureCoordinateTransformed () const;
	virtual void FillMobileMaterialVertexParams (FMobileMaterialVertexParams& OutVertexParams) const;
	virtual void FillMobileMaterialPixelParams (FMobileMaterialPixelParams& OutPixelParams) const;
	virtual QWORD GetMobileMaterialSortKey (void) const;
...
public:
	...
	virtual UBOOL Compile( class FStaticParameterSet* StaticParameters, EShaderPlatform Platform, TRefCountPtr<FMaterialShaderMap>& OutShaderMap, UBOOL bForceCompile=FALSE, UBOOL bDebugDump=FALSE);
