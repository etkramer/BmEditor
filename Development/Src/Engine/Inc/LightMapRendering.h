...
public:
	...
	public:
			...
			UINT NumLightmapTextures) const
		{
			...
			if( GUsingMobileRHI )
			{
				// we assume here that platforms that use flattened materials only have 1 lightmap texture
				checkf(NumLightmapTextures == 1, TEXT("When using flattened materials, directional lightmaps aren't supported"));

				// bypass texture setting via parameter so that we directly set the texture by type (base or lightmap)
				FTexture* LightMapTexture = LightMapTextures[0]->Resource;
				LightMapTexture->LastRenderTime = GCurrentTime;
				RHISetMobileTextureSamplerState(PixelShader->GetPixelShader(), Lightmap_MobileTexture, LightMapTexture->SamplerStateRHI, LightMapTexture->TextureRHI, 0.0f, -1.0f, -1.0f);
			}
			else
#endif
			{
...
public:
	...
	static UBOOL ShouldCache(EShaderPlatform Platform, const FMaterial* Material, const FVertexFactoryType* VertexFactoryType, UBOOL bEnableSkyLight=FALSE)
	{
		// Only xbox and PS3 have GOnePassDominantLight == TRUE
		return (Platform == SP_XBOXD3D || Platform == SP_PS3)
