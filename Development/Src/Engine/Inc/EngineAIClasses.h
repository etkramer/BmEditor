...
struct AITree_DMC_Base_eventCalculateUtilityRating_Parms
{
    class UAICommandNodeBase* CommandNode;
    AITree_DMC_Base_eventCalculateUtilityRating_Parms(EEventParm)
    {
    }
};
class UAITree_DMC_Base : public UObject
{
public:
    //## BEGIN PROPS AITree_DMC_Base
    //## END PROPS AITree_DMC_Base

    void eventCalculateUtilityRating(class UAICommandNodeBase* CommandNode)
    {
        AITree_DMC_Base_eventCalculateUtilityRating_Parms Parms(EC_EventParm);
        Parms.CommandNode=CommandNode;
        ProcessEvent(FindFunctionChecked(ENGINE_CalculateUtilityRating),&Parms);
    }
    DECLARE_CLASS(UAITree_DMC_Base,UObject,0,Engine)
	void ClearDMC();
...
struct FAITreeUtilityInfo
{
    ...
    /** Constructors */
    FAITreeUtilityInfo() {}
    FAITreeUtilityInfo(EEventParm)
    {
        appMemzero(this, sizeof(FAITreeUtilityInfo));
    }
};

struct FAITreeHandle
{
    FName ActiveRootName;
    class UAICommandNodeRoot* ActiveRoot;
    TArrayNoInit<class UAICommandNodeBase*> DisabledNodes;
    TArrayNoInit<struct FAITreeUtilityInfo> LastUtilityRatingList;
    TArrayNoInit<struct FAITreeUtilityInfo> LastUtilityRatingListAtChange;

		UBOOL IsNodeDisabled( UAICommandNodeBase* Node );
	
};

class UAITree : public UK2GraphBase
{
public:
    //## BEGIN PROPS AITree
    TArrayNoInit<class UAICommandNodeRoot*> RootList;
    class UK2GraphBase* GatherList;
    //## END PROPS AITree

    virtual UBOOL SetActiveRoot(FName InName,struct FAITreeHandle& Handle);
    virtual TArray<class UClass*> EvaluateTree(class AAIController* InAI,struct FAITreeHandle& Handle);
    DECLARE_FUNCTION(execSetActiveRoot)
    {
        P_GET_NAME(InName);
        P_GET_STRUCT_INIT_REF(struct FAITreeHandle,Handle);
        P_FINISH;
        *(UBOOL*)Result=this->SetActiveRoot(InName,Handle);
    }
    DECLARE_FUNCTION(execEvaluateTree)
    {
        P_GET_OBJECT(AAIController,InAI);
        P_GET_STRUCT_INIT_REF(struct FAITreeHandle,Handle);
        P_FINISH;
        *(TArray<class UClass*>*)Result=this->EvaluateTree(InAI,Handle);
    }
    DECLARE_CLASS(UAITree,UK2GraphBase,0,Engine)
	virtual void PostLoad();
	virtual void FillRootList();
};

class UAICommandNodeBase : public UK2NodeBase
{
public:
    //## BEGIN PROPS AICommandNodeBase
    class UClass* CommandClass;
    class UDMC_Prototype* UtilityDMC;
    //## END PROPS AICommandNodeBase

    virtual class UAICommandNodeBase* SelectBestChild(class AAIController* InAI,struct FAITreeHandle& Handle);
    DECLARE_FUNCTION(execSelectBestChild)
    {
        P_GET_OBJECT(AAIController,InAI);
        P_GET_STRUCT_INIT_REF(struct FAITreeHandle,Handle);
        P_FINISH;
        *(class UAICommandNodeBase**)Result=this->SelectBestChild(InAI,Handle);
    }
    DECLARE_CLASS(UAICommandNodeBase,UK2NodeBase,0,Engine)
#if WITH_EDITOR
...
class UAICommandNodeRoot : public UAICommandNodeBase
{
public:
    //## BEGIN PROPS AICommandNodeRoot
    FName RootName;
    //## END PROPS AICommandNodeRoot

    DECLARE_CLASS(UAICommandNodeRoot,UAICommandNodeBase,0,Engine)
#if WITH_EDITOR
	virtual FString GetDisplayName();
	virtual void CreateAutoConnectors();
#endif
...
class UAIGatherNodeBase : public UK2NodeBase
{
public:
    //## BEGIN PROPS AIGatherNodeBase
    FLOAT LastUpdateTime;
    FStringNoInit NodeName;
    //## END PROPS AIGatherNodeBase

    DECLARE_CLASS(UAIGatherNodeBase,UK2NodeBase,0,Engine)
#if WITH_EDITOR
	virtual FString GetDisplayName();
	virtual void CreateAutoConnectors();
#endif
...
AUTOGENERATE_FUNCTION(UAITree,-1,execEvaluateTree);
AUTOGENERATE_FUNCTION(UAITree,-1,execSetActiveRoot);
AUTOGENERATE_FUNCTION(UAICommandNodeBase,-1,execSelectBestChild);
...
#define AUTO_INITIALIZE_REGISTRANTS_ENGINE_AI \
	...
	UAITree::StaticClass(); \
	GNativeLookupFuncs.Set(FName("AITree"), GEngineUAITreeNatives); \
	UAICommandNodeBase::StaticClass(); \
	GNativeLookupFuncs.Set(FName("AICommandNodeBase"), GEngineUAICommandNodeBaseNatives); \
	UAICommandNodeRoot::StaticClass(); \
	UAIGatherNodeBase::StaticClass(); \
...
FNativeFunctionLookup GEngineUAITreeNatives[] = 
{ 
	MAP_NATIVE(UAITree, execEvaluateTree)
	MAP_NATIVE(UAITree, execSetActiveRoot)
	{NULL, NULL}
};

FNativeFunctionLookup GEngineUAICommandNodeBaseNatives[] = 
{ 
	MAP_NATIVE(UAICommandNodeBase, execSelectBestChild)
	{NULL, NULL}
};
...
VERIFY_CLASS_SIZE_NODIE(UAITree_DMC_Base)
...
VERIFY_CLASS_OFFSET_NODIE(UAITree,AITree,RootList)
VERIFY_CLASS_OFFSET_NODIE(UAITree,AITree,GatherList)
VERIFY_CLASS_SIZE_NODIE(UAITree)
VERIFY_CLASS_OFFSET_NODIE(UAICommandNodeBase,AICommandNodeBase,CommandClass)
VERIFY_CLASS_OFFSET_NODIE(UAICommandNodeBase,AICommandNodeBase,UtilityDMC)
VERIFY_CLASS_SIZE_NODIE(UAICommandNodeBase)
VERIFY_CLASS_OFFSET_NODIE(UAICommandNodeRoot,AICommandNodeRoot,RootName)
VERIFY_CLASS_SIZE_NODIE(UAICommandNodeRoot)
VERIFY_CLASS_OFFSET_NODIE(UAIGatherNodeBase,AIGatherNodeBase,LastUpdateTime)
VERIFY_CLASS_OFFSET_NODIE(UAIGatherNodeBase,AIGatherNodeBase,NodeName)
VERIFY_CLASS_SIZE_NODIE(UAIGatherNodeBase)
...
VERIFY_CLASS_OFFSET_NODIE(UNavMeshPath_Toward,NavMeshPath_Toward,GoalActor)
...
VERIFY_CLASS_OFFSET_NODIE(UNavMeshPathGoalEvaluator,NavMeshPathGoalEvaluator,NumNodesProcessed)
...
VERIFY_CLASS_OFFSET_NODIE(UNavMeshGoal_GenericFilterContainer,NavMeshGoal_GenericFilterContainer,MyNavigationHandle)
