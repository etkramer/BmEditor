...
public:
	...
	virtual void SetMesh(FShader* VertexShader,const FMeshElement& Mesh,const FSceneView& View) const = 0;
};
 ...
 */
#define IMPLEMENT_VERTEX_FACTORY_TYPE(FactoryClass,ShaderFilename,bUsedWithMaterials,bSupportsStaticLighting,bSupportsDynamicLighting,bPrecisePrevWorldPos,InMinPackageVersion,InMinLicenseePackageVersion) \
...
public:
	...
	void SetMesh(FShader* Shader,const FMeshElement& Mesh,const FSceneView& View) const
	{
		if(Parameters)
		{
			Parameters->SetMesh(Shader,Mesh,View);
		}
	}
