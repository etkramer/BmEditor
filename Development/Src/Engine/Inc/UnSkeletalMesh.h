...
class USkeletalMeshComponent : public UMeshComponent
{
	...
	*/
	...
	void ApplyControllersForBoneIndex(INT BoneIndex, UBOOL bPrePhysControls, UBOOL bPostPhysControls, const UAnimTree* Tree, UBOOL bRenderedRecently, const TArray<INT>& BoneProcessed);
	...
	*/
	...
	/**
	* Returns the FIApexClothing interfrace pointer.
	**/
...
public:
	...
	friend FArchive& operator<<( FArchive& Ar, FSkeletalMeshVertexInfluences& W )
	{
		...
		{
			if( Ar.Ver() < VER_DWORD_SKELETAL_MESH_INDICES_FIXUP || Ar.LicenseeVer() >= VER_BATMAN2 )
			{
				if(  Ar.Ver() >= VER_DWORD_SKELETAL_MESH_INDICES || Ar.LicenseeVer() >= VER_BATMAN2 )
				{
					BYTE IndexSize;
...
public:
	...
	UBOOL IsIndexBufferValid() { return IndexBuffer != NULL; }
	 ...
	 */
	void GetIndexBufferData( FMultiSizeIndexContainerData& OutData );
...
public:
	...
	*/
	void GetVertices(TArray<FSoftSkinVertex>& Vertices);
	...
	*/
	void BuildVertexBuffers(class USkeletalMesh* Mesh, UBOOL bUsePackedPosition);
...
public:
	...
	*/
	 ...
	 */
	void DebugDrawBones(FPrimitiveDrawInterface* PDI,const FSceneView* View, const TArray<FBoneAtom>& InSpaceBases, const class FStaticLODModel& LODModel, const FColor& LineColor);
...
protected:
	...
	struct FSectionElementInfo
	{
		...
		{}
		*/
		FSectionElementInfo(UMaterialInterface* InMaterial, UBOOL bInEnableShadowCasting, INT InUseMaterialIndex)
		:	Material( InMaterial )
