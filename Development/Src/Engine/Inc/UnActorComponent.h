...
enum ELightShadowMode
{
	...
	* renders receiver geometry in order to prevent shadows on backfaces and emissive areas
	*/
...
public:
	...
	FLOAT LightEnv_BouncedLightBrightness;

	/** The color of bounced lighting from this light in DynamicLightEnvironments. */
	FColor LightEnv_BouncedModulationColor;
...
public:
	...
	*/
	...
	/**
	 * Computes the intensity of the bounced lighting from this light on a specific point.
	 */
	virtual FLinearColor GetBouncedIntensity(const FVector& Point) const;
...
public:
	...
	void InvalidateLightingCacheInner(UBOOL bRecreateLightGuids);
};
...
class UAudioComponent : public UActorComponent
{
	 ...
	 */
	void HandleInteriorVolumes( UAudioDevice* AudioDevice, class AWorldInfo* WorldInfo );
...
public:
	...
	/** Initialization constructor. */
	FWindSourceSceneProxy(const FVector& InPosition,FLOAT InStrength,FLOAT InPhase,FLOAT InFrequency,FLOAT InSpeed,FLOAT InRadius):
		Position(InPosition),
...
public:
	...
	/**
	 * Determine if the radial blur proxy is renderable for hte current view
	 *
	 * @param View - View region that is currently being rendered for the scene
	 * @param DPGIndex - Current Depth Priority Group being rendered for the scene
	 * @param Flags - Scene rendering flags
