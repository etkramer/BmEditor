...
struct DMC_Base_eventDMCTakeDamage_Parms
{
    INT DamageAmount;
    FVector HitLocation;
    FVector Momentum;
    DMC_Base_eventDMCTakeDamage_Parms(EEventParm)
    {
    }
};
struct DMC_Base_eventDMCCreate_Parms
{
    DMC_Base_eventDMCCreate_Parms(EEventParm)
    {
    }
};
class ADMC_Base : public AActor
{
public:
    //## BEGIN PROPS DMC_Base
    TArrayNoInit<class UActorComponent*> CreatedComponents;
    //## END PROPS DMC_Base

    virtual class UActorComponent* AddComponent(class UActorComponent* Template);
    DECLARE_FUNCTION(execAddComponent)
    {
        P_GET_OBJECT(UActorComponent,Template);
        P_FINISH;
        *(class UActorComponent**)Result=this->AddComponent(Template);
    }
    void eventDMCTakeDamage(INT DamageAmount,FVector HitLocation,FVector Momentum)
    {
        DMC_Base_eventDMCTakeDamage_Parms Parms(EC_EventParm);
        Parms.DamageAmount=DamageAmount;
        Parms.HitLocation=HitLocation;
        Parms.Momentum=Momentum;
        ProcessEvent(FindFunctionChecked(ENGINE_DMCTakeDamage),&Parms);
    }
    void eventDMCCreate()
    {
        ProcessEvent(FindFunctionChecked(ENGINE_DMCCreate),NULL);
    }
    DECLARE_CLASS(ADMC_Base,AActor,0,Engine)
	void ClearDMC();
...
class UDMC_Prototype : public UK2GraphBase
{
public:
    //## BEGIN PROPS DMC_Prototype
    class UClass* ParentClass;
    class UClass* GeneratedClass;
    FStringNoInit FunctionCode;
    FStringNoInit DefaultPropText;
    TArrayNoInit<struct FDMCNewVar> NewVars;
    //## END PROPS DMC_Prototype

    DECLARE_CLASS(UDMC_Prototype,UK2GraphBase,0,Engine)
	virtual void PostLoad();
};
...
AUTOGENERATE_FUNCTION(ADMC_Base,-1,execAddComponent);
...
#define AUTO_INITIALIZE_REGISTRANTS_ENGINE_PREFAB \
	ADMC_Base::StaticClass(); \
	GNativeLookupFuncs.Set(FName("DMC_Base"), GEngineADMC_BaseNatives); \
...
#ifdef NATIVES_ONLY
FNativeFunctionLookup GEngineADMC_BaseNatives[] = 
{ 
	MAP_NATIVE(ADMC_Base, execAddComponent)
	{NULL, NULL}
};
...
VERIFY_CLASS_OFFSET_NODIE(ADMC_Base,DMC_Base,CreatedComponents)
VERIFY_CLASS_SIZE_NODIE(ADMC_Base)
...
VERIFY_CLASS_OFFSET_NODIE(UDMC_Prototype,DMC_Prototype,ParentClass)
VERIFY_CLASS_OFFSET_NODIE(UDMC_Prototype,DMC_Prototype,NewVars)
VERIFY_CLASS_SIZE_NODIE(UDMC_Prototype)
