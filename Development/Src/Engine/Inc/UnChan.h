...
#define DEFINE_CONTROL_CHANNEL_MESSAGE_ZEROPARAM(Name, Index) \
	...
	{ \
	public: \
	static BYTE Initialize() { return 0; } \
	static void Send(UNetConnection* Conn) {} \
	};
#define DEFINE_CONTROL_CHANNEL_MESSAGE_ONEPARAM(Name, Index, TypeA) \
	enum { NMT_##Name = Index }; \
...
#define IMPLEMENT_CONTROL_CHANNEL_MESSAGE(Name) static BYTE Dummy##_FNetControlMessage_##Name = FNetControlMessage<NMT_##Name>::Initialize();

// message type definitions
#if UDK
DEFINE_CONTROL_CHANNEL_MESSAGE_FOURPARAM(Hello, 0, BYTE, INT, INT, FGuid); // initial client connection message
#else
DEFINE_CONTROL_CHANNEL_MESSAGE_THREEPARAM(Hello, 0, BYTE, INT, INT); // initial client connection message
#endif // UDK
...
DEFINE_CONTROL_CHANNEL_MESSAGE_TWOPARAM(Login, 5, FString, FString); // client requests to be admitted to the game
DEFINE_CONTROL_CHANNEL_MESSAGE_ONEPARAM(Failure, 6, FString); // indicates connection failure
DEFINE_CONTROL_CHANNEL_MESSAGE_SEVENPARAM(Uses, 7, FGuid, FString, FString, DWORD, INT, FString, BYTE); // server tells client about a package they should have/acquire
DEFINE_CONTROL_CHANNEL_MESSAGE_TWOPARAM(Have, 8, FGuid, INT); // client tells server what version of a package it has
DEFINE_CONTROL_CHANNEL_MESSAGE_ONEPARAM(Join, 9, FUniqueNetId); // final join request (spawns PlayerController)
...
#if WITH_STEAMWORKS
DEFINE_CONTROL_CHANNEL_MESSAGE_THREEPARAM(AuthHello, 16, BYTE, INT, INT); // client request for server auth blob.
DEFINE_CONTROL_CHANNEL_MESSAGE_TWOPARAM(AuthServer, 17, QWORD, BYTE); // server reports basic auth info.
DEFINE_CONTROL_CHANNEL_MESSAGE_THREEPARAM(AuthBlob, 18, BYTE, BYTE, FString); // client reports another chunk of auth blob to server.
#endif
DEFINE_CONTROL_CHANNEL_MESSAGE_ONEPARAM(ActorChannelFailure, 19, INT); // client tells server that it failed to open an Actor channel sent by the server (e.g. couldn't serialize Actor archetype)
DEFINE_CONTROL_CHANNEL_MESSAGE_ONEPARAM(DebugText, 20, FString); // debug text sent to all clients or to server
...
DEFINE_CONTROL_CHANNEL_MESSAGE_ONEPARAM(PeerListen, 21, DWORD);
/** Server tells all clients about a new listening client peer that is ready for connection requests */
DEFINE_CONTROL_CHANNEL_MESSAGE_ONEPARAM(PeerConnect, 22, FClientPeerInfo);
/** Peer requesting to join another client peer */
DEFINE_CONTROL_CHANNEL_MESSAGE_ONEPARAM(PeerJoin, 23, FUniqueNetId);
/** Peer response to another peer requesting join */
DEFINE_CONTROL_CHANNEL_MESSAGE_ONEPARAM(PeerJoinResponse, 24, BYTE);
/** Peer notification to other peers about server disconnect */
DEFINE_CONTROL_CHANNEL_MESSAGE_ONEPARAM(PeerDisconnectHost, 25, FUniqueNetId);
/** Peer notification to other peers about a new host being selected during host migration */
DEFINE_CONTROL_CHANNEL_MESSAGE_ONEPARAM(PeerNewHostFound, 26, FUniqueNetId);
...
DEFINE_CONTROL_CHANNEL_MESSAGE_ONEPARAM(PeerNewHostTravel, 27, FClientPeerTravelInfo);
...
/** Peer notification to other peers about a new host being selected during host migration */
DEFINE_CONTROL_CHANNEL_MESSAGE_ONEPARAM(PeerNewHostTravelSession, 28, FClientPeerTravelSessionInfo);
