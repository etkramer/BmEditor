...
struct FSystemSettingsDataAntialiasing
{
	FSystemSettingsDataAntialiasing();

	friend bool operator==(const FSystemSettingsDataAntialiasing& LHS, const FSystemSettingsDataAntialiasing& RHS)
	{
		return LHS.MaxMultiSamples						== RHS.MaxMultiSamples
			&& LHS.bAllowD3D9MSAA						== RHS.bAllowD3D9MSAA
			&& LHS.bAllowTemporalAA						== RHS.bAllowTemporalAA
			&& LHS.TemporalAA_MinDepth					== RHS.TemporalAA_MinDepth
			&& LHS.TemporalAA_StartDepthVelocityScale	== RHS.TemporalAA_StartDepthVelocityScale
			&& LHS.TemporalAA_JitterStartDepthScale		== RHS.TemporalAA_JitterStartDepthScale;
	}
	friend bool operator!=(const FSystemSettingsDataAntialiasing& LHS, const FSystemSettingsDataAntialiasing& RHS)
	{
		return !(LHS==RHS);
	...
	FLOAT	TemporalAA_JitterStartDepthScale;
};
...
struct FSystemSettingsData 
	...
	, public FSystemSettingsDataAntialiasing
...
public:
	...
	{
		// Elements only need to be here if they change at runtime.
		// (not through "scale set" because that ensures the RT gets the data e.g. MotionBlurSkinningRT)
		UBOOL	bAllowMotionBlur;
		UBOOL	bAllowAmbientOcclusion;
		UBOOL	bAllowDynamicShadows;
		UBOOL	bAllowHardwareShadowFiltering;
		...
		FRenderThreadSettings(const FSystemSettings& Src)
		{
			...
			bAllowTemporalAA				= Src.bAllowTemporalAA;
