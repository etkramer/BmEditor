...
public:
	...
	FORCEINLINE UBOOL IsInSameGroupAs(FNavMeshEdgeBase* OtherEdge)
	{
		...
		return ( (MyPoly0 == OtherPoly0 && MyPoly1 == OtherPoly1) || (MyPoly0 == OtherPoly0 && MyPoly1 == OtherPoly1) );
	...
	*/
	 ...
	 */
	FVector GetEdgePerpDir(UBOOL bWorldSpace=WORLD_SPACE,FVector* EdgeDir=NULL);
...
public:
	 ...
	 */
	void AdjustPositionToDesiredHeightAbovePoly(FVector& out_Pt, FLOAT DesiredHeight);
...
#define DEFAULT_BOX_PADDING 10.f
	...
	*/
	 ...
	 */
	void UpdateDynamicObstaclesForEdge(INT SessionID, INT EdgeIdx);
...
public:
	...
	/**
	 * @returns the pylon assoicated with this navmesh
	 */
	APylon* GetPylon();
	 ...
	 */
	UBOOL IsObstacleMesh() { return (GetPylon() && GetPylon()->ObstacleMesh==this);}

	/**
	 *	@Returns TRUE if this mesh is an obstacle mesh that was created at runtime
	 */
	UBOOL IsDynamicObstacleMesh() { return (GetPylon() && GetPylon()->DynamicObstacleMesh==this);}

	/**
	 *	@Returns TRUE if this mesh is a submesh of another higher level mesh
	 */
	UBOOL IsSubMesh() { return (GetPylon() && GetPylon()->NavMeshPtr != this && GetPylon()->ObstacleMesh != this && GetPylon()->DynamicObstacleMesh != this); }


	/**
	 *	@Returns the obstacle mesh associated with this mesh (could be this)
	 */
	UNavigationMeshBase* GetObstacleMesh() { return (IsObstacleMesh()) ? this : (GetPylon()) ? GetPylon()->ObstacleMesh : NULL; }

	// this will return the topmost mesh (if this is a submesh, get the parent, if not return self)
	UNavigationMeshBase* GetTopLevelMesh() {return (GetPylon()) ? GetPylon()->NavMeshPtr : NULL;}
...
public:
	...
	*/
	 ...
	 */
	UBOOL IntersectsPoly(TArray<VERTID>& Poly);
	 ...
	 */
	UBOOL IntersectsPoly(TArray<FVector>& InPolyVertLocs, FNavMeshPolyBase*& out_IntersectingPoly,TArray<FNavMeshPolyBase*>* ExclusionPolys=NULL,UBOOL bWorldSpace=WORLD_SPACE, const FLOAT MinNormalZ=-1.f);
	 ...
	 */
	UBOOL IntersectsPolyBounds(const FVector& Center, const FVector& Extent, FNavMeshPolyBase*& out_IntersectingPoly, UBOOL bWorldSpace=WORLD_SPACE );
	...
	*/
	 ...
	 * @param EdgeGroupID - internal param used during recursion to identify edge groups one from another
	*/
	...
	void BuildEdgesFromSegmentSpan(const FVector& Start, const FVector& End, TArray<FEdgeWidthPair>& out_EdgePairs);
	...
	*/
	 ...
	 */
	void GetIntersectingPolys(const FVector& Loc, const FVector& Extent, TArray<FNavMeshPolyBase*>& out_Polys, UBOOL bWorldSpace=WORLD_SPACE, UBOOL bIgnoreDynamic=FALSE, UBOOL bReturnBothDynamicAndStatic=FALSE, UBOOL bRecursedForSubmesh=FALSE);
	 ...
	 */	
	void GetIntersectingPolys(const TArray<FVector>& PolyVerts, TArray<FNavMeshPolyBase*>& out_Polys, UBOOL bWorldSpace);
	...
	* @param out_Polys - output array of intersecting polys
	*/	
	void GetIntersectingPolys(const TArray<VERTID>& PolyVerts, TArray<FNavMeshPolyBase*>& out_Polys);
...
T* UNavigationMeshBase::AddEdgeData(T& NewEdge, WORD& out_EdgeIdx)
{
	...
	T TempEdge;
	appMemcpy(Edge,&TempEdge,sizeof(T));
...
public:
	...
	FORCEINLINE FNavMeshCollisionDataProvider(class UNavigationMeshBase* ObstacleNavMesh, class UNavigationMeshBase* InNavMesh) :
	  ...
	  /*FORCEINLINE*/ UBOOL ShouldCheckMaterial(INT MaterialIndex) const
	  {
		  ...
		  else
		  {
			  ...
			  for(INT EdgeIdx=0;EdgeIdx<LinkedPoly->GetNumEdges();EdgeIdx++)
			  {
				  // special designator to disable this surface even if edges are not loaded/valid
				  if( LinkedPoly->PolyEdges(EdgeIdx) == MAXWORD)
				  {
					  return FALSE;
				  }
			  }
