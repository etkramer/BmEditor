...
public:
	 ...
	 */
	static FLOAT DecompressFixed16(WORD Value, INT Log2MaxValue = 0)
	{
		const INT QuantOffset = (1 << (15 - Log2MaxValue)) - 1;
		const FLOAT QuantFactor = (FLOAT)(QuantOffset >> Log2MaxValue);

		return ((INT)Value - QuantOffset) / QuantFactor;
	}
		...
		INT& FixedComponentCount, INT& FixedComponentSize)
	{
		...
		KeyComponentCount = CompressedRotationNum[KeyFormat];

		switch (KeyFormat)
		{
		case ACF_Float96NoW:
			{
				// For translation we can support per component storage.
				INT const ComponentCount = (FormatFlags & 1) + ((FormatFlags >> 1) & 1) + ((FormatFlags >> 2) & 1);
				if( ComponentCount != 0 )
				{
					KeyComponentCount = ComponentCount;
				}
				break;
			}
		case ACF_None:
		case ACF_Fixed32NoW:
		case ACF_Float32NoW:
			// No changes from the defaults (no extra non-key data stored, no components skipped)
			break;
		case ACF_Fixed48NoW:
			// The number of components stored depends on the type flags
			KeyComponentCount = (FormatFlags & 1) + ((FormatFlags >> 1) & 1) + ((FormatFlags >> 2) & 1);
			break;
		case ACF_IntervalFixed32NoW:
			// Min/Range floats for all non-zero channels
			FixedComponentCount = 2 * ((FormatFlags & 1) + ((FormatFlags >> 1) & 1) + ((FormatFlags >> 2) & 1));
			FixedComponentSize = sizeof(FLOAT);
			break;
		default:
			appErrorf( TEXT("Unknown or unsupported key compression format '%i'"), KeyFormat );
		}
	}
	...
	static FORCEINLINE_DEBUGGABLE void DecompressTranslation(INT Format, INT FormatFlags, FVector& Out, const BYTE* RESTRICT TopOfStream, const BYTE* RESTRICT KeyData)
	{
		...
		else if (Format == ACF_Fixed48NoW)
		{
			...
			if (FormatFlags & 1)
			{
				Out.X = FAnimationCompression_PerTrackUtils::DecompressFixed16(*TypedKeyData++, LogScale);
			}
			else
			{
				Out.X = 0.0f;
			...
			if (FormatFlags & 2)
			{
				Out.Y = FAnimationCompression_PerTrackUtils::DecompressFixed16(*TypedKeyData++, LogScale);
			}
			else
			{
				Out.Y = 0.0f;
			...
			if (FormatFlags & 4)
			{
				Out.Z = FAnimationCompression_PerTrackUtils::DecompressFixed16(*TypedKeyData++, LogScale);
			}
			else
			{
				Out.Z = 0.0f;
	...
	static FORCEINLINE_DEBUGGABLE void DecompressRotation(INT Format, INT FormatFlags, FQuat& Out, const BYTE* RESTRICT TopOfStream, const BYTE* RESTRICT KeyData)
	{
		...
		else if (Format == ACF_Fixed48NoW)
		{
			...
			if (FormatFlags & 1)
			{
				Out.X = FAnimationCompression_PerTrackUtils::DecompressFixed16(*TypedKeyData++);
			}
			else
			{
				Out.X = 0.0f;
			...
			if (FormatFlags & 2)
			{
				Out.Y = FAnimationCompression_PerTrackUtils::DecompressFixed16(*TypedKeyData++);
			}
			else
			{
				Out.Y = 0.0f;
			...
			if (FormatFlags & 4)
			{
				Out.Z = FAnimationCompression_PerTrackUtils::DecompressFixed16(*TypedKeyData++);
			}
			else
			{
				Out.Z = 0.0f;
