 ...
 */
#define INIT_SHADERS_ON_DEMAND	CONSOLE
...
{
	// Can't use shader compression if codermode is enabled, 
	// Since codermode inlines shader maps in materials instead of storing them in a shader cache.
	return !GUseCoderMode && (Platform == SP_XBOXD3D || Platform == SP_PS3);
}
...
public:
	...
	inline void SetShaderParamName(const TCHAR* Name)
	{
		...
		ShaderParamSlotIndex = RHIGetMobileUniformSlotIndexByName(Name);
#endif
	}
...
public:
	...
	*/
	FORCEINLINE const FVertexShaderRHIRef& GetVertexShader()
	{
		if (!IsInitialized())
		{
	...
	* @return the shader's vertex shader
	*/
	FORCEINLINE const FPixelShaderRHIRef& GetPixelShader()
	{
		if (!IsInitialized())
		{
