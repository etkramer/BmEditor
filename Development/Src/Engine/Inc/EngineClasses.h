...
#define FOREACH_ENUM_EMOBILETEXTURETRANSFORMTARGET(op) \
    op(MTTT_BaseTexture) \
    op(MTTT_DetailTexture) 
...
#define FOREACH_ENUM_EK2CONNECTORDIRECTION(op) \
    op(K2CD_Input) \
    op(K2CD_Output) 
...
#define FOREACH_ENUM_EK2CONNECTORTYPE(op) \
    op(K2CT_Bool) \
    op(K2CT_Int) \
    op(K2CT_Float) \
    op(K2CT_Vector) \
...
public:
    ...
    void DrawTris(class UTexture* Tex,const TArray<struct FCanvasUVTri>& Triangles);
    void SetPos(FLOAT PosX,FLOAT PosY,FLOAT PosZ=1.000000);
    ...
    DECLARE_FUNCTION(execSetPos)
    {
        P_GET_FLOAT(PosX);
        P_GET_FLOAT(PosY);
        P_GET_FLOAT_OPTX(PosZ,1.000000);
        P_FINISH;
...
public:
    ...
    TDoubleLinkedList<class IInterface_NavMeshPathObject*>* PathObjectsThatAffectThisPylon;
...
public:
    ...
    DECLARE_FUNCTION(execSetSettingsDataString)
    {
        ...
        P_GET_STR(InString);
        P_FINISH;
        this->SetSettingsDataString(Data,InString);
    }
    ...
    DECLARE_FUNCTION(execGetSettingsDataString)
    {
        P_GET_STRUCT_REF(struct FSettingsData,Data);
        P_FINISH;
        *(FString*)Result=this->GetSettingsDataString(Data);
    }
...
{
    FString Response;
    FacebookIntegration_eventOnWebRequestComplete_Parms(EEventParm)
    {
    }
};
struct FacebookIntegration_eventOnFacebookRequestComplete_Parms
{
    FString JsonString;
    FacebookIntegration_eventOnFacebookRequestComplete_Parms(EEventParm)
    {
    }
};
struct FacebookIntegration_eventOnAuthorizationComplete_Parms
{
    UBOOL bSucceeded;
    FacebookIntegration_eventOnAuthorizationComplete_Parms(EEventParm)
    {
    }
};
...
struct FacebookIntegration_eventWebRequest_Parms
{
    FString URL;
    FString POSTPayload;
    FacebookIntegration_eventWebRequest_Parms(EEventParm)
    {
    }
};
...
public:
    ...
    TArrayNoInit<FScriptDelegate> AuthorizationDelegates;
    TArrayNoInit<FScriptDelegate> FacebookRequestCompleteDelegates;
    TArrayNoInit<FScriptDelegate> WebRequestCompleteDelegates;
    FScriptDelegate __OnAuthorizationComplete__Delegate;
    FScriptDelegate __OnFacebookRequestComplete__Delegate;
    FScriptDelegate __OnWebRequestComplete__Delegate;
    //## END PROPS FacebookIntegration

    virtual UBOOL Init();
    virtual UBOOL Authorize();
    virtual UBOOL IsAuthorized();
    virtual void WebRequest(const FString& URL,const FString& POSTPayload);
    ...
    DECLARE_FUNCTION(execWebRequest)
    {
        P_GET_STR(URL);
        P_GET_STR(POSTPayload);
        P_FINISH;
        this->WebRequest(URL,POSTPayload);
    }
    ...
    void delegateOnWebRequestComplete(const FString& Response)
    {
        FacebookIntegration_eventOnWebRequestComplete_Parms Parms(EC_EventParm);
        Parms.Response=Response;
        ProcessDelegate(ENGINE_OnWebRequestComplete,&__OnWebRequestComplete__Delegate,&Parms);
    }
    void delegateOnFacebookRequestComplete(const FString& JsonString)
    {
        FacebookIntegration_eventOnFacebookRequestComplete_Parms Parms(EC_EventParm);
        Parms.JsonString=JsonString;
        ProcessDelegate(ENGINE_OnFacebookRequestComplete,&__OnFacebookRequestComplete__Delegate,&Parms);
    }
    void delegateOnAuthorizationComplete(UBOOL bSucceeded)
    {
        ...
        ProcessDelegate(ENGINE_OnAuthorizationComplete,&__OnAuthorizationComplete__Delegate,&Parms);
    }
    ...
    void eventWebRequest(const FString& URL,const FString& POSTPayload)
    {
        FacebookIntegration_eventWebRequest_Parms Parms(EC_EventParm);
        Parms.URL=URL;
        Parms.POSTPayload=POSTPayload;
        ProcessEvent(FindFunctionChecked(ENGINE_WebRequest),&Parms);
    }
    ...
    DECLARE_CLASS(UFacebookIntegration,UObject,0|CLASS_Transient|CLASS_Config,Engine)
    static const TCHAR* StaticConfigName() {return TEXT("Engine");}
...
struct GameplayEventsWriter_eventGetGameTypeId_Parms
{
    INT ReturnValue;
    GameplayEventsWriter_eventGetGameTypeId_Parms(EEventParm)
    {
    }
};
...
public:
    ...
    {
        GameplayEventsWriter_eventGetGameTypeId_Parms Parms(EC_EventParm);
        Parms.ReturnValue=0;
        ProcessEvent(FindFunctionChecked(ENGINE_GetGameTypeId),&Parms);
        return Parms.ReturnValue;
    }
    DECLARE_CLASS(UGameplayEventsWriter,UGameplayEvents,0,Engine)
	/** 
...
public:
    ...
    DECLARE_CLASS(UGameViewportClient,UObject,0|CLASS_Transient|CLASS_Config,Engine)
...
public:
    ...
    virtual void ProcessIniLocFile(const FString& Filename,const TArray<BYTE>& FileData);
    ...
    DECLARE_FUNCTION(execProcessIniLocFile)
    {
        ...
        P_FINISH;
        this->ProcessIniLocFile(Filename,FileData);
    }
...
class UK2Connector : public UObject
{
public:
    //## BEGIN PROPS K2Connector
    class UK2NodeBase* OwningNode;
    FStringNoInit ConnName;
    BYTE Type;
    SCRIPT_ALIGN;
    //## END PROPS K2Connector

    DECLARE_CLASS(UK2Connector,UObject,0,Engine)
#if WITH_EDITOR
...
class UK2Input : public UK2Connector
{
public:
    //## BEGIN PROPS K2Input
    class UK2Output* FromOutput;
    //## END PROPS K2Input

    DECLARE_CLASS(UK2Input,UK2Connector,0,Engine)
#if WITH_EDITOR
	void Break();
...
class UK2Output : public UK2Connector
{
public:
    //## BEGIN PROPS K2Output
    TArrayNoInit<class UK2Input*> ToInputs;
    //## END PROPS K2Output

    DECLARE_CLASS(UK2Output,UK2Connector,0,Engine)
#if WITH_EDITOR
...
class UK2GraphBase : public UObject
{
public:
    //## BEGIN PROPS K2GraphBase
    TArrayNoInit<class UK2NodeBase*> Nodes;
    //## END PROPS K2GraphBase

    DECLARE_CLASS(UK2GraphBase,UObject,0,Engine)
    NO_DEFAULT_CONSTRUCTOR(UK2GraphBase)
};

class UK2NodeBase : public UObject
{
public:
    //## BEGIN PROPS K2NodeBase
    TArrayNoInit<class UK2Input*> Inputs;
    TArrayNoInit<class UK2Output*> Outputs;
    INT NodePosX;
    INT NodePosY;
    //## END PROPS K2NodeBase

    DECLARE_CLASS(UK2NodeBase,UObject,0,Engine)
#if WITH_EDITOR
...
AUTOGENERATE_FUNCTION(UFacebookIntegration,-1,execWebRequest);
...
#define AUTO_INITIALIZE_REGISTRANTS_ENGINE \
	...
	UK2Connector::StaticClass(); \
	UK2Input::StaticClass(); \
	UK2Output::StaticClass(); \
	UK2GraphBase::StaticClass(); \
...
{ 
	...
	MAP_NATIVE(UFacebookIntegration, execWebRequest)
...
VERIFY_CLASS_OFFSET_NODIE(UCheatManager,CheatManager,DebugCameraControllerRef)
...
VERIFY_CLASS_OFFSET_NODIE(UFacebookIntegration,FacebookIntegration,__OnWebRequestComplete__Delegate)
...
VERIFY_CLASS_OFFSET_NODIE(UFont,Font,MaxCharHeight)
...
VERIFY_CLASS_OFFSET_NODIE(UGameplayEventsWriter,GameplayEventsWriter,Game)
...
VERIFY_CLASS_OFFSET_NODIE(UIniLocPatcher,IniLocPatcher,__OnReadTitleFileComplete__Delegate)
...
VERIFY_CLASS_OFFSET_NODIE(UK2Connector,K2Connector,OwningNode)
VERIFY_CLASS_OFFSET_NODIE(UK2Connector,K2Connector,Type)
VERIFY_CLASS_SIZE_NODIE(UK2Connector)
VERIFY_CLASS_OFFSET_NODIE(UK2Input,K2Input,FromOutput)
VERIFY_CLASS_SIZE_NODIE(UK2Input)
VERIFY_CLASS_OFFSET_NODIE(UK2Output,K2Output,ToInputs)
VERIFY_CLASS_SIZE_NODIE(UK2Output)
VERIFY_CLASS_OFFSET_NODIE(UK2GraphBase,K2GraphBase,Nodes)
VERIFY_CLASS_SIZE_NODIE(UK2GraphBase)
VERIFY_CLASS_OFFSET_NODIE(UK2NodeBase,K2NodeBase,Inputs)
VERIFY_CLASS_OFFSET_NODIE(UK2NodeBase,K2NodeBase,NodePosY)
VERIFY_CLASS_SIZE_NODIE(UK2NodeBase)
...
VERIFY_CLASS_OFFSET_NODIE(ULocalPlayer,LocalPlayer,TagContext)
