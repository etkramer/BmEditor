...
extern UBOOL GMobileUsePostProcess;
...
// ES2 is available for use if we're either using a static ES2 RHI or a dynamic ES2 RHI
#define WITH_ES2_RHI ( ( USE_STATIC_ES2_RHI ) || ( USE_DYNAMIC_ES2_RHI ) )
...
struct FCopyTextureRegion2D
{
	...
	INT FirstMipIdx;
	/** constructor */
	FCopyTextureRegion2D(FTexture2DRHIParamRef InSrcTextureRef,void* InSrcTextureObject, INT InOffsetX=-1,INT InOffsetY=-1,INT InSizeX=-1,INT InSizeY=-1,INT InFirstMipIdx=0)
...
#define GETSAFERHISHADER_PIXEL(Shader) (Shader ? Shader->GetPixelShader() : FPixelShaderRHIRef())
#define GETSAFERHISHADER_VERTEX(Shader) (Shader ? Shader->GetVertexShader() : FVertexShaderRHIRef())
