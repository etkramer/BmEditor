 ...
 */
struct FParticleCollisionActorInstancePayload
{
...
protected:
	...
	TMap<FParticleVertexFactory*,UBOOL>	VertexFactoriesInUse;
};
...
struct FDynamicSpriteEmitterDataBase : public FDynamicEmitterDataBase
{
	 ...
	 *	@param	InVertexSize			Stride of these verts, only used for verification
	 */
	void BuildViewFillDataAndSubmit(const FSceneViewFamily* ViewFamily,const TBitArray<FDefaultBitArrayAllocator>& VisibilityMap,UBOOL bOnlyOneView,INT InVertexCount,INT InVertexSize)
	{
...
struct FDynamicSortableSpriteEmitterDataBase : public FDynamicSpriteEmitterDataBase
{
	...
	INT PrimitiveCount;
	/** The (optional) particle ordering to use for sorted emitter */
};
...
struct FDynamicMeshEmitterData : public FDynamicSpriteEmitterDataBase
{
	...
	*/
	...
	/** True if we are using the optimization where the game thread fills VB's */
	BITFIELD bUseNxFluid:1;
...
public:
	...
	UBOOL UsesPointSprites() const { return bUsePointSprites; }
...
protected:
	...
	BITFIELD bUsePointSprites : 1;	//mobile-only point sprites
