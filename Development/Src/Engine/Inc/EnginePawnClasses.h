...
struct Pawn_eventIsValidEnemyTargetFor_Parms
{
    const class APlayerReplicationInfo* PRI;
    UBOOL bNoPRIisEnemy;
    UBOOL ReturnValue;
    Pawn_eventIsValidEnemyTargetFor_Parms(EEventParm)
    {
    }
};
...
public:
    ...
    UBOOL eventSpecialMoveThruEdge(BYTE Type,INT Dir,FVector MoveStart,FVector MoveDest,class AActor* RelActor=NULL,INT RelItem=0)
    {
    ...
    UBOOL eventIsValidEnemyTargetFor(const class APlayerReplicationInfo* PRI,UBOOL bNoPRIisEnemy)
    {
        Pawn_eventIsValidEnemyTargetFor_Parms Parms(EC_EventParm);
        Parms.ReturnValue=FALSE;
        Parms.PRI=PRI;
        Parms.bNoPRIisEnemy=bNoPRIisEnemy ? FIRST_BITFIELD : FALSE;
        ProcessEvent(FindFunctionChecked(ENGINE_IsValidEnemyTargetFor),&Parms);
        return Parms.ReturnValue;
    }
