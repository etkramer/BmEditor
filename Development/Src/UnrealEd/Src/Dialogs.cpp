 ...
 */
WxSuppressableWarningDialog::WxSuppressableWarningDialog( const FString& InPrompt, const FString& InTitle, const FString& InIniSettingName )
...
/*-----------------------------------------------------------------------------
WxDlgMeshLODFbxImport.
-----------------------------------------------------------------------------*/

BEGIN_EVENT_TABLE(WxDlgMeshLODFbxImport, wxDialog)
END_EVENT_TABLE()
...
WxDlgMeshLODFbxImport::WxDlgMeshLODFbxImport( wxWindow* InParent, const TArray<FString>& InLODGroupNames, const TArray<TArray<FString>*>& LODString, const TCHAR* SrcFileName )
:	wxDialog(InParent,wxID_ANY,*LocalizeUnrealEd("WxDlgFbxStaticMeshLODImport_Title"),wxDefaultPosition,wxDefaultSize,wxDEFAULT_DIALOG_STYLE|wxRESIZE_BORDER)
{
	wxBoxSizer* MainSizer = new wxBoxSizer(wxVERTICAL);
	{
		// filename text
		...
		wxGridSizer* MorphListGridSizer = new wxGridSizer(InLODGroupNames.Num() + 1, 2, 0, 0);
		{
			// label for morph target list
			wxStaticText* MeshNamesText = new wxStaticText(
				this,
				wxID_ANY,
				*LocalizeUnrealEd("WxDlgMorphFbxImport_LODGroupNames")
				);
			wxStaticText* LODLevelText = new wxStaticText(
				this,
				wxID_ANY,
				*LocalizeUnrealEd(TEXT("ChooseLODLevel"))
				);

			MorphListGridSizer->Add(MeshNamesText, 0, wxALIGN_CENTER_HORIZONTAL|wxALIGN_CENTER_VERTICAL|wxALL, 5);    
			MorphListGridSizer->Add(LODLevelText, 0, wxALIGN_CENTER_HORIZONTAL|wxALIGN_CENTER_VERTICAL|wxALL, 5);

			INT MeshIndex;
			for ( MeshIndex = 0; MeshIndex < InLODGroupNames.Num(); MeshIndex++ )
			{
				// fill in the text for each LOD index
				wxArrayString LODListText;
				TArray<FString>* LODGroupStrings = LODString(MeshIndex);
				for( INT LODIdx=0; LODIdx < LODGroupStrings->Num(); LODIdx++ )
				{
					LODListText.Add( *(*LODGroupStrings)(LODIdx) );
				}
				wxStaticText* MeshNameText = new wxStaticText(
					this,
					wxID_ANY,
					*InLODGroupNames(MeshIndex)
					);
				// create morph target name text
				LODLevelList.AddItem( new wxComboBox(
					this,
					wxID_ANY,
					LODListText[MeshIndex],
					wxDefaultPosition,
					wxDefaultSize,
					LODListText,
					wxCB_DROPDOWN|wxCB_READONLY
				...
				MorphListGridSizer->Add(MeshNameText, 0, wxALIGN_CENTER_HORIZONTAL|wxALIGN_CENTER_VERTICAL|wxALL, 5);    
				MorphListGridSizer->Add(LODLevelList(MeshIndex), 0, wxALIGN_CENTER_HORIZONTAL|wxALIGN_CENTER_VERTICAL|wxALL, 5);
			}
		}
		...
		MainSizer->Add( FileNameText, 0, wxEXPAND|wxALL, 4 );
		MainSizer->Add( MorphListGridSizer, 0, wxEXPAND );
		MainSizer->Add(ButtonSizer, 0, wxALIGN_RIGHT | wxALL, 2);
	}
...
enum EBuildPlayDevice
{
	...
	BPD_RESOLUTION_MAX,
	BPD_MOBILE_FIRST_DEVICE = BPD_IPHONE_3GS,
	BPD_NUM_MOBILE_DEVICES = BPD_RESOLUTION_MAX - BPD_IPHONE_3GS
};
...
void WxDlgEditPlayWorldURL::BuildResolutionOptions (const UBOOL bUseMobilePreview)
{
	...
	GetResolution(BPD_DEFAULT, OUT PrintWidth, OUT PrintHeight);
	...
	GetResolution(BPD_CUSTOM, OUT PrintWidth, OUT PrintHeight);
...
INT WxDlgEditPlayWorldURL::GetCurrentResolutionIndex(const UBOOL bUseMobilePreview) const
{
	...
	for (INT i = 0; i < BPD_RESOLUTION_MAX; ++i)
	{
		...
		GetResolution(i, OUT TestWidth, OUT TestHeight);
...
INT WxDlgEditPlayWorldURL::GetCurrentFeaturesIndex (void) const
{
	for (INT i = 0; i < BPD_RESOLUTION_MAX; ++i)
	{
		FString DeviceName = GetDeviceName(i);
		if (DeviceName == GEditor->MobileFeatures)
		{
			return i;
		}
	}
...
void WxDlgEditPlayWorldURL::GetResolution(const INT DeviceIndex, INT& OutWidth, INT& OutHeight) const
{
	switch (DeviceIndex)
	{
		case BPD_DEFAULT:
...
/** Returns string associated with a particular device */
FString WxDlgEditPlayWorldURL::GetDeviceName (const int DeviceIndex ) const
{
	FString DeviceName;
	switch (DeviceIndex)
	{
...
void WxDlgEditPlayWorldURL::SetFeaturesIndex(INT FeaturesIndex, const UBOOL bUpdateControl)
{
	...
	{
		GEditor->MobileFeatures = GetDeviceName(FeaturesIndex);
...
void WxDlgEditPlayWorldURL::UpdateCustomResolution(const INT ResolutionIndex)
{
	...
	GetResolution(ResolutionIndex, NewWidth, NewHeight);
...
void WxDlgEditPlayWorldURL::OnResolutionChanged( wxCommandEvent& In )
{
	if (bInited)
	{
		...
		GetResolution(NewResolutionIndex, GEditor->PlayInEditorWidth, GEditor->PlayInEditorHeight);
