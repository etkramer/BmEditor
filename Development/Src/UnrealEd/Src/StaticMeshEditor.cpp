...
void FStaticMeshEditorViewportClient::Draw(FViewport* Viewport,FCanvas* Canvas)
{
	...
	StaticMeshComponent->bCanHighlightSelectedSections = true;

	UStaticMesh* StatMesh = StaticMeshEditor->StaticMesh;
	for( INT LodIndex = 0; LodIndex < StatMesh->LODInfo.Num(); LodIndex++ )
	{
		FStaticMeshLODInfo& LodInfo = ( FStaticMeshLODInfo& )StatMesh->LODInfo( LodIndex );
		for( INT ElemIndex = 0; ElemIndex < LodInfo.Elements.Num(); ElemIndex++ )
		{
			FStaticMeshLODElement& LODElement = LodInfo.Elements( ElemIndex );
			UBOOL bIsSelected = StaticMeshEditor->PropertyWindow->IsPropertyOrChildrenSelected( TEXT( "Elements" ), ElemIndex );
			if( LODElement.bSelected != bIsSelected )
			{
				LODElement.bSelected = bIsSelected;
				StatMesh->PostEditChange();
			}
		}
	}
	...
	,	LightMapCoordinateIndex(0)
{
	...
	// Add docking windows.
	{
		...
		ShowDockingWindow( GenerateUVsWindow, ( bSimplifyMode == FALSE ) );
...
void PopulateFBXStaticMeshLODInfo(UnFbx::CFbxImporter* FbxImporter, KFbxNode* Node, TArray<KFbxNode*>& LODNodeList, INT& MaxLODCount, TArray<FString>& LODGroupNames, TArray<TArray<FString>*>& LODGroupStrings)
{
	if (Node->GetNodeAttribute() && Node->GetNodeAttribute()->GetAttributeType() == KFbxNodeAttribute::eLODGROUP)
	{
		...
		LODGroupNames.AddItem(ANSI_TO_TCHAR(FbxImporter->MakeName(Node->GetName())));

		TArray<FString>* GroupLODs = new TArray<FString>;
		GroupLODs->AddZeroed(Node->GetChildCount() + 1);
		(*GroupLODs)(0) = FString::Printf( TEXT("None") );
		(*GroupLODs)(1) = FString::Printf( TEXT("Base") );
		for(INT i = 2; i < Node->GetChildCount() + 1; i++)
		{
			(*GroupLODs)(i) = FString::Printf( TEXT("%d"), i - 1 );
		}
		LODGroupStrings.AddItem(GroupLODs);
	...
	else
	{
		...
		{
			PopulateFBXStaticMeshLODInfo(FbxImporter, Node->GetChild(ChildIndex), LODNodeList, MaxLODCount, LODGroupNames, LODGroupStrings);
		}
	}
}
#endif
		...
		else
		{
			...
			if (bIsFBX)
			{
				...
				else
				{
					...
					{
						WxDlgMeshLODFbxImport Dlg = WxDlgMeshLODFbxImport(this, LODGroupNames, LODGroupStrings, *Filename);
						...
						if ( Dlg.ShowModal() == wxID_OK )
						{
							...
							{
								INT LODGroupIdx;
								for ( LODGroupIdx = 0; LODGroupIdx < SelectedLODIdxs.Num(); LODGroupIdx++ )
								{
									if ( (SelectedLODIdxs(LODGroupIdx) - 1) == LODLevel )
									{
										break;
									}
								}

								if ( LODGroupIdx == SelectedLODIdxs.Num() )
								{
									continue;
								}
								UStaticMesh* TempStaticMesh;
								TempStaticMesh = (UStaticMesh*)FbxImporter->ImportStaticMesh( UObject::GetTransientPackage(), LODNodeList(LODGroupIdx)->GetChild(LODLevel), NAME_None, 0, NULL, 0 );

								// Extract data to LOD
								BuildStaticMeshLOD(TempStaticMesh, LODLevel);
							}

							// Update mesh component
							StaticMesh->MarkPackageDirty();
							UpdateToolbars();
							if ( SelectedLODIdxs.Num() == 1)
							{
								appMsgf( AMT_OK, LocalizeSecure(LocalizeUnrealEd("LODImportSuccessful"), SelectedLODIdxs(0) - 1) );
							}
							else if ( SelectedLODIdxs.Num() > 1 )
							{ 
								appMsgf( AMT_OK, *LocalizeUnrealEd("FbxLODImportSuccessful") );
							}
						}
					}
					for (INT x = 0; x < LODGroupStrings.Num(); x++)
					{
						delete LODGroupStrings(x);
					}
				}
...
//@todo cb: GB conversion
		...
		if(NewFracturedMesh)
		{
			...
			if(NewFracturedMesh->LightMapResolution > 0)
			{
				...
				GenerateUVs(NewFracturedMesh, 0, NewFracturedMesh->LightMapCoordinateIndex, TRUE, MinChartSpacingPercent, BorderSpacingPercent, bUseMaxStretch, NULL, MaxCharts, MaxStretch );
			}
