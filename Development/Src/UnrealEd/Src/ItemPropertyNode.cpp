...
void WxItemPropertyControl::OnPaint( wxPaintEvent& In )
{
	...
	if ( bIsEditConst )
	{
		...
		{
			// highlight the entire area
			dc.DrawRectangle( wxRect( rc.x + IndentX, rc.y, rc.GetWidth(), ItemHeight ) );
		}
	}
	...
	if(PropertyNode->HasNodeFlags(EPropertyNodeFlags::CanBeExpanded) && (bHasChildren || bMightHaveChildrenWhenOpened))
	{
		// Get additional property offset to remove from the indent, dependent on the type of window (set at create time)
		...
		const wxRect wkrc( rc.x + IndentX - Offset, rc.y, rc.width - IndentX, rc.height );
		...
		if (!bSuppressExpansion)
		{
			...
			dc.DrawBitmap( *ArrowBitmap, IndentX - Offset, wkrc.y+((PROP_DefaultItemHeight - GPropertyWindowManager->ArrowDownB.GetHeight())/2), bNormalExpansion ? true : false);
		}
	}
...
void WxItemPropertyControl::RenderItemName( wxBufferedPaintDC& RenderDeviceContext, const wxRect& ClientRect )
{
	...
	else
	{
		...
		{
			LeftText = *FString::Printf(TEXT("[%i]"), PropertyNode->GetArrayIndex() );
		}
		else
		{
			LeftText = *FString::Printf(TEXT("[%s]"), *Property->ArraySizeEnum->GetEnum(PropertyNode->GetArrayIndex()).ToString());
		}
	}
	...
	UBOOL bAssumeHasChildren = PropertyNode->HasNodeFlags(EPropertyNodeFlags::InitChildrenOnDemand) && !PropertyNode->HasNodeFlags(EPropertyNodeFlags::HasEverBeenExpanded);
	const UBOOL bCanBeExpanded = PropertyNode->HasNodeFlags(EPropertyNodeFlags::CanBeExpanded) && ((PropertyNode->GetNumChildNodes()>0) || bAssumeHasChildren);

	RenderDeviceContext.DrawText( *LeftText, ClientRect.x+IndentX+PROP_Indent-Offset,ClientRect.y+YOffset );

	UBOOL bIsChildOfFavorite = PropertyNode->IsChildOfFavorite();

	//if favorites are enabled
	WxPropertyWindow* PropertyWindow = GetPropertyWindow();
	if (PropertyWindow && PropertyWindow->IsFavoritesFeatureEnabled() && !bIsChildOfFavorite)
	{
		...
		const WxBitmap& FavoritesBmp = bIsItemFavorite ? GPropertyWindowManager->FavoritesOnImage : GPropertyWindowManager->FavoritesOffImage;
				...
				else if ( PropertyNode->GetArrayIndex() == INDEX_NONE && InProperty->ArrayDim > 1 )
				{
					for ( INT Idx = 0; !bDiffersFromDefault && Idx < InProperty->ArrayDim; Idx++ )
					{
						bDiffersFromDefault = !InProperty->Identical(
							...
							PPF_DeltaComparison|PPF_DeepComparison
							);
					}
				}
				else
...
wxRect WxItemPropertyControl::GetConditionalRect (void) const
{
	...
	wxRect ConditionalRect (rc.x + IndentX - PROP_Indent, rc.y, PROP_Indent, rc.GetBottom() - rc.GetTop());
 ...
 */
UBoolProperty* WxCustomPropertyItem_ConditionalItem::GetEditConditionProperty() const
{
 ...
 */
UBOOL WxCustomPropertyItem_ConditionalItem::GetEditConditionPropertyAddress( UBoolProperty*& ConditionProperty, TArray<BYTE*>& ConditionPropertyAddresses )
{
	UBOOL bResult = FALSE;
...
UBOOL WxCustomPropertyItem_ConditionalItem::SupportsEditConditionCheckBox() const
{
	...
	{
		UBoolProperty* ConditionalProperty = const_cast<UBoolProperty*>( GetEditConditionProperty() );
...
void WxCustomPropertyItem_ConditionalItem::RenderItemName( wxBufferedPaintDC& RenderDeviceContext, const wxRect& ClientRect )
{
	...
	UBOOL bIsConditionalPropertyVisible = FALSE;
	{
		UProperty* ConditionalProperty = GetEditConditionProperty();
		if( ConditionalProperty != NULL )
		{
			if( ConditionalProperty->HasAllPropertyFlags( CPF_Edit ) )
			{
...
UBOOL WxCustomPropertyItem_ConditionalItem::ClickedPropertyItem( wxMouseEvent& Event )
{
	...
	if ( Property == NULL || PropertyNode->IsEditConst() || GetEditConditionProperty() == NULL )
	{
