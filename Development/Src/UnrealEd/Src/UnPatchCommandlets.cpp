...
void FComparisonResults::BuildScriptDataForBinary(FScriptPatchData* ScriptPatch, INT StructIndex, UBOOL bNonIntelByteCode)
{
	...
	{
		ScriptPatch->Data = Struct->Script;
	}
}
...
void FScriptPatchExporter::SaveToFile()
{
	...
	for (INT PackageIndex = 0; PackageIndex < AllPackagePatches.Num(); PackageIndex++)
	{
		...
		BinFile->SerializeCompressed(Buffer.GetData(), Buffer.Num(), (ECompressionFlags)(COMPRESS_LZX | COMPRESS_BiasMemory));
		delete BinFile;
...
extern void appGetEngineScriptPackageNames(TArray<FString>& PackageNames, UBOOL bCanIncludeEditorOnlyPackages);
extern void appGetGameNativeScriptPackageNames(TArray<FString>& PackageNames, UBOOL bCanIncludeEditorOnlyPackages);
extern void appGetGameScriptPackageNames(TArray<FString>& PackageNames, UBOOL bCanIncludeEditorOnlyPackages);
...
UBOOL FScriptPatchWorker::InitCommandlet()
{
	...
	appGetGameScriptPackageNames(NonNativeScriptPackageNames, TRUE);
...
// 	NonNativeScriptPackageNames.AddItem(TEXT("WarfareGameContentWeapons"));
	...
	// the list of package names returned from appGetGameScriptPackageNames will include both native and non-native packages, so remove any that are also found in
	// the list of native script packages.
	for ( INT PackageIndex = 0; PackageIndex < NativeScriptPackageNames.Num(); PackageIndex++ )
	{
		NonNativeScriptPackageNames.RemoveItem(NativeScriptPackageNames(PackageIndex));
	}

	if ( bStrippedScriptPatch )
	{
		// when generating a patch for a stripped (console/dedicated server) platform, remove editor-only packages from the list of native packages
		TArray<FString> NonEditorPackageNames;
		appGetEngineScriptPackageNames(NonEditorPackageNames, FALSE);
		appGetGameNativeScriptPackageNames(NonEditorPackageNames, FALSE);

		for ( INT PackageIndex = NativeScriptPackageNames.Num() - 1; PackageIndex >= 0; PackageIndex-- )
		{
			// don't make this a ref, or removal below will leave a bad ref
			FString PackageName = NativeScriptPackageNames(PackageIndex);
			if ( !NonEditorPackageNames.ContainsItem(PackageName)

			// Localized seekfree files are not included in the results of GetEngine/GameScriptPackageNames, so skip over any
			// packages that end with the loc suffix (otherwise, we'd remove all loc packages).  Instead, we'll remove the loc file
			// when/if we remove the non-loc version.
			&& PackageName.Right(appStrlen(LOCALIZED_SEEKFREE_SUFFIX)) != LOCALIZED_SEEKFREE_SUFFIX )
			{
				NativeScriptPackageNames.Remove(PackageIndex);

				INT LocPackageLocation = NativeScriptPackageNames.FindItemIndex(PackageName + LOCALIZED_SEEKFREE_SUFFIX);
				if ( LocPackageLocation != INDEX_NONE )
				{
					if ( LocPackageLocation == PackageIndex - 1 )
					{
						PackageIndex--;
					}

					NativeScriptPackageNames.Remove(LocPackageLocation);
				}
			}
		}
	}
