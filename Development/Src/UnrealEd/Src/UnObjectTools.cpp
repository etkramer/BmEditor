...
namespace GBCompatTools
{
	/** @return	a pointer to the generic browser instance */
	WxGenericBrowser* GetGenericBrowser()
	{
		return GUnrealEd->GetBrowser<WxGenericBrowser>(TEXT("GenericBrowser"));
	}
};
...
namespace ObjectTools
{
	...
	UBOOL IsObjectBrowsable( UObject* Obj, const TMap< UClass*, TArray< UGenericBrowserType* > >* InResourceTypes )	// const
	{
		...
		const UBOOL bUsageFilterEnabled = FALSE; //GenericBrowser->IsUsageFilterEnabled();
		...
		if( ObjectPackage != NULL && !bIsRedirector && !bIsPackageObj )
		{
			...
			if( ObjectPackage != UObject::GetTransientPackage()
				...
				&& Obj->GetOuter()->IsA(UPackage::StaticClass())	// Don't display objects embedded in other objects (e.g. font textures, sequences)
	...
	void DuplicateWithRefs( TArray<UObject*>& SelectedObjects, const TMap< UClass*, TArray< UGenericBrowserType* > >* InBrowsableObjectTypes )
	{
	 ...
	 */
	void RenameObjectsWithRefsInternal( TArray<UObject*>& Objects, UBOOL bLocPackages, const TMap< UObject*, FString >* ObjectToLanguageExtMap, const TMap< UClass*, TArray< UGenericBrowserType* > >* InBrowsableObjectTypes )
	{
		...
		for( INT Index = 0; Index < Objects.Num(); Index++ )
		{
			...
			for ( INT ObjectIndex = 0 ; ObjectIndex < ReferencedTopLevelObjects.Num() && !bMoveFailed ; ++ObjectIndex )
			{
				...
				else
				{
					...
					if( bLocPackages && TgtPackage != RefObject->GetOutermost()->GetName() )
					{
						...
						if( ObjectToLanguageExtMap )
						{
							...
							const FString* FoundLanguageExt = ObjectToLanguageExtMap->Find( RefObject );
				...
				else
				{
					...
					else if ( ExistingOutermostPackage && ExistingOutermostPackageFilename.Len() > 0 && !FEditorFileUtils::IsFilenameValidForSaving( ExistingOutermostPackageFilename ) )
					{
						ErrorMessage += LocalizeUnrealEd("Error_DestinationPackageFilenameLength");
						bMoveFailed = TRUE;
					}
					else if( !ObjName.Len() )
					{
		...
		if ( bSawSuccessfulRename )
		{
			...
			// @todo CB: GCallbackEvent->Send( CALLBACK_RefreshEditor_GenericBrowser );
			// @todo CB: RightContainer->Update();
		}
	}
	 ...
	 * @param InBrowsableObjectTypes	A mapping of classes to their generic browser type for finding object references only if they are browsable.
	 */
	void RenameObjectsWithRefs( TArray< UObject* >& SelectedObjects, UBOOL bIncludeLocInstances, const TMap< UClass*, TArray< UGenericBrowserType* > >* InBrowsableObjectTypes ) 
	{
		if( !bIncludeLocInstances )
		{
			RenameObjectsWithRefsInternal( SelectedObjects, bIncludeLocInstances, NULL, InBrowsableObjectTypes );
		}
		else
		{
			...
			for( INT Index = 0; Index < SelectedObjects.Num(); Index++ )
			{
				...
				if( Object )
				{
					...
					if( Wave )
					{
						...
						RenameObjectsWithRefsInternal( LocObjects, bIncludeLocInstances, &ObjectToLanguageExtMap, InBrowsableObjectTypes );
					}
				}
			}
		}
	 ...
	 */
	void AssembleListOfImportFactories( TArray<UFactory*>& out_Factories, FString& out_Filetypes, FString& out_Extensions )
	{
		...
		GenerateFactoryFileExtensions( out_Factories, out_Filetypes, out_Extensions );
	}
	...
	UBOOL FindFileFromFactory (UFactory* InFactory, const FString& InMessage, FString& OutFileName)
	{
		...
		ObjectTools::GenerateFactoryFileExtensions(InFactory, FileTypes, AllExtensions);
	...
	void RefreshResourceType( UClass* ResourceType )
	{
		...
		if( bUpdated )
		{
			...
			{
				GenericBrowserPtr->LeftContainer->RegenerateCustomResourceFilterType();
			}
		}
	}
		...
		if( RenderInfo != NULL && RenderInfo->Renderer != NULL )
		{
			...
			else
			{
				...
				if( bWantLabels )
				{
					...
					UMemCountThumbnailLabelRenderer* MemCountLabelRenderer = NULL; //GenericBrowser->GetMemCountLabelRenderer();
