...
void FInterpEdViewportClient::AddKeysFromHitProxy( HHitProxy* HitProxy, TArray<FInterpEdSelKey>& Selections )
{
						...
						else if(HitResult->IsA(HInterpEdTrackGraphPropBtn::StaticGetType()))
						{
							...
							if( SubTrackGroupIndex != -1 )
							{
								...
								for( INT Index = 0; Index < SubTrackGroup.TrackIndices.Num(); ++Index )
								{
									...
									InterpEd->AddTrackToCurveEd( *Group->GroupName.ToString(), Group->GroupColor, Track->SubTracks( SubTrackIndex ) );
								}
							}
							else
							{
								...
								{
									InterpEd->AddTrackToCurveEd( *Group->GroupName.ToString(), Group->GroupColor, Track );
								}
							}
						}
...
void WxInterpEd::DrawModeHUD(FEditorLevelViewportClient* ViewportClient,FViewport* Viewport,const FSceneView* View,FCanvas* Canvas)
{
	...
	if( ViewportClient->AllowMatineePreview() )
	{
		...
		{
			// Get the size of the viewport to determine where to draw the lines
			const INT SizeX = Viewport->GetSizeX();
			const INT SizeY = Viewport->GetSizeY();
		...
		if( IsEditingCrosshairEnabled() )
		{
			...
			FLOAT CenterX = Viewport->GetSizeX() / 2.0f + .5f;
			FLOAT CenterY = Viewport->GetSizeY() / 2.0f + .5f;
