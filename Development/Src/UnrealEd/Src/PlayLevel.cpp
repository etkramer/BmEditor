...
void UEditorEngine::EndPlayOnConsole()
{
	...
	if( CurrentPlayWorldDestination != INDEX_NONE )
	{
		...
		if( CurrentConsole )
		{
			CurrentConsole->ExitGame(NULL, 0, TRUE);
		}
 ...
 */
void UEditorEngine::PlayMap( FVector* StartLocation, FRotator* StartRotation, INT Destination, INT InPlayInViewportIndex, UBOOL bUseMobilePreview )
{
...
void UEditorEngine::StartQueuedPlayMapRequest()
{
	...
	{
		PlayOnConsole(PlayWorldDestination, bUseMobilePreviewForPlayWorld);
	}
}
...
void UEditorEngine::PlayInEditor()
{
	...
	UBOOL bStartInSpectatorMode = FALSE;
...
void UEditorEngine::PlayOnConsole(INT ConsoleIndex, UBOOL bUseMobilePreview)
{
	...
	if(NumTargets > 0 || ConsoleName == CONSOLESUPPORT_NAME_PC)
	{
		...
		else if ( ConsoleName == CONSOLESUPPORT_NAME_PC )
		{
			...
			if( bUseMobilePreview )
			{
				...
				//features
				if (GEditor->MobileFeatures.Len())
				{
					LevelNames += FString::Printf(TEXT(" -SystemSettings=%s"), *GEditor->MobileFeatures);
				}
	...
	if(PackageTools::CheckForReferencesToExternalPackages(NULL, NULL, LevelToCheck, &ExternalObjects ))
	{
		for(INT ObjectIndex = 0; ObjectIndex < ExternalObjects.Num(); ++ObjectIndex)
		{
			...
			if(!ExternalObject->HasAnyFlags(RF_PendingKill))
			{
				...
				if( bAddForMapCheck ) 
				{
					GWarn->MapCheck_Add(MCTYPE_WARNING, ExternalObject, *FString::Printf( LocalizeSecure( LocalizeUnrealEd("Warning_UsingExternalObject"), *ExternalObject->GetFullName())), MCACTION_DELETE );
				}
			}
		}
	}
