...
//	EditorFrame->Maximize();
	...
	GApp->LastDir[LD_COLLADA_ANIM]			= GConfig->GetStr( TEXT("Directories2"), TEXT("COLLADAAnim"),		GEditorUserSettingsIni );
#if WITH_FBX
...
int WxUnrealEdApp::OnExit()
{
	...
	GConfig->SetString( TEXT("Directories2"), TEXT("COLLADAAnim"),		*GApp->LastDir[LD_COLLADA_ANIM],		GEditorUserSettingsIni );
#if WITH_FBX
...
WxDlgStaticMeshLightingInfo* WxUnrealEdApp::GetDlgStaticMeshLightingInfo(void)
{
	if (!DlgStaticMeshLightingInfo)
	{
		...
		DlgStaticMeshLightingInfo->Show(0);
	}
...
WxDlgDensityRenderingOptions* WxUnrealEdApp::GetDlgDensityRenderingOptions(void)
{
	if (!DlgDensityRenderingOptions)
	{
		...
		DlgDensityRenderingOptions->Show(0);
	}
...
void WxUnrealEdApp::KeyPressed(wxKeyEvent& Event)
{
	...
	if(wxDynamicCast(Event.GetEventObject(), wxTextCtrl) != 0)
	{
		Event.Skip();
		return;
	}
	...
	if( AutosaveState == AUTOSAVE_Saving && KeyName == TEXT("Escape"))
	{
	...
	if( bShiftPressed && !GEditorModeTools().IsModeActive( EM_InterpEdit ) && GApp->KismetWindows.Num() == 0)
	{
		switch(KeyCode)
		{
		case '1': 
