...
public:
	...
	*/
	virtual void DebugDrawBonesSubset(FPrimitiveDrawInterface* PDI,const FSceneView* View, const TArray<FBoneAtom>& InSpaceBases, const class FStaticLODModel& LODModel, const TArray<INT>& BonesOfInterest, const FColor& LineColor)
	{
		...
		if (DrawDPG == DPGIndex)
		{
			for(INT SectionIndex = 0;SectionIndex < LODModel.Sections.Num();SectionIndex++)
			{
				...
				if (SortStripData.Num() > 0)
				{
					...
					Mesh.IndexBuffer = LODModel.MultiSizeIndexContainer.GetIndexBuffer();
					...
					if (PDI->IsHitTesting())
					{
						...
						for (INT i=0;i<SortStripData.Num();i++)
						{
							...
							Mesh.FirstIndex = SortStripData(i).FirstIndex;
							Mesh.NumPrimitives = Min<INT>(SortStripData(i).NumTris, MaxNumPrimitives-((Mesh.FirstIndex-BaseIndex)/3));
						...
						(ASV->SelectedSortStripSectionIndex == SectionIndex) )
					{
						...
						Mesh.FirstIndex = SortStripData->FirstIndex;
						Mesh.NumPrimitives = Min<INT>(SortStripData->NumTris, MaxNumPrimitives-((Mesh.FirstIndex-BaseIndex)/3));
						...
						if( Mesh.NumPrimitives > 0 )
						{
							...
							if((ASV->bSortStripMoveForward||ASV->bSortStripMoveBackward) && ASV->MouseOverSortStripIndex != INDEX_NONE)
							{
								...
								Mesh.FirstIndex = SortStripData->FirstIndex;
								Mesh.NumPrimitives = SortStripData->NumTris;
	...
	virtual void DrawDynamicElements(FPrimitiveDrawInterface* PDI,const FSceneView* View,UINT DPGIndex,DWORD Flags)
	{
		...
		if (!PDI->IsHitTesting())
		{
			...
			{	
				...
				const FStaticLODModel& LODModel = SkeletalMesh->LODModels(LODIndex);
				DebugDrawBones(PDI, View, SkeletalMeshComponent->SpaceBases, LODModel, BoneColor);
			}
			...
			{ 
				...
				INT LODIndex = GetCurrentLODIndex();
				const FStaticLODModel& LODModel = SkeletalMesh->LODModels(LODIndex);
				DebugDrawBonesSubset(PDI, View, SkeletalMeshComponent->SpaceBases, LODModel, SkeletalMeshComponent->BonesOfInterest, BoneColor);
			}
...
void FASVViewportClient::Draw(FViewport* Viewport, FCanvas* Canvas)
{
	...
	for( INT LodIndex = 0; LodIndex < SkelMesh->LODModels.Num(); LodIndex++ )
	{
		...
		for( INT Section = 0; Section < LodModel.Sections.Num(); Section++ )
		{
			...
			UBOOL bIsSelected = AnimSetViewer->MeshProps->IsPropertyOrChildrenSelected( TEXT( "bEnableShadowCasting" ), Section )
				|| AnimSetViewer->MeshProps->IsPropertyOrChildrenSelected( TEXT( "TriangleSortSettings" ), Section )
				|| AnimSetViewer->MeshProps->IsPropertyOrChildrenSelected( TEXT( "Materials" ), Section )
				|| AnimSetViewer->MeshProps->IsPropertyOrChildrenSelected( TEXT( "ClothingAssets" ), Section );
...
#if WITH_APEX
	EVT_MENU(IDM_ANIMSET_APEX_CLEAR_ALL, WxAnimSetViewer::OnApexDebug )

	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_CLOTHING_SKINNED_POSITIONS, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_CLOTHING_SPHERE_COLLISION, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_CLOTHING_MAX_DISTANCE_OUT, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_CLOTHING_MAX_DISTANCE_IN, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_CLOTHING_ALL_SKIN_MAP, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_CLOTHING_BAD_SKIN_MAP, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_CLOTHING_SKIN_MAP, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_CLOTHING_RENDER_NORMALS, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_CLOTHING_RENDER_TANGENTS, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_CLOTHING_PHYSICS_MESH_WIRE, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_CLOTHING_PHYSICS_MESH_SOLID, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_CLOTHING_SKELETON, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_CLOTHING_BONE_FRAMES, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_CLOTHING_BONE_NAMES, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_CLOTHING_VELOCITIES, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_CLOTHING_GRAPHICAL_VERTEX_BONES, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_CLOTHING_PHYSICAL_VERTEX_BONES, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_CLOTHING_ACOLLISION_UMBRELLAS, WxAnimSetViewer::OnApexDebug )

	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_DESTRUCTIBLE_BOUNDS, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_DESTRUCTIBLE_SUPPORT, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_DESTRUCTIBLE_ACTOR_POSE, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_DESTRUCTIBLE_FRAGMENT_ACTOR_POSE, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_DESTRUCTIBLE_ACTOR_NAME, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_LOD_DISTANCE_DESTRUCTIBLE_ACTOR_POSE, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_LOD_DISTANCE_DESTRUCTIBLE_ACTOR_NAME, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_LOD_DISTANCE_DESTRUCTIBLE_FRAGMENT_ACTOR_POSE, WxAnimSetViewer::OnApexDebug )

	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_LOD_BENEFITS, WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_LOD_DISTANCE_SCALE, WxAnimSetViewer::OnApexDebug )
#if WITH_APEX_PARTICLES
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_TOTAL_INJECTED_AABB,WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_APEX_EMITTER_ACTOR_POSE,WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_THRESHOLD_DISTANCE_APEX_EMITTER_ACTOR_POSE,WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_APEX_EMITTER_ACTOR_NAME,WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_THRESHOLD_DISTANCE_APEX_EMITTER_ACTOR_NAME,WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_GROUND_EMITTER_SPHERE,WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_GROUND_EMITTER_GRID,WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_GROUND_EMITTER_RAYCAST,WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_GROUND_EMITTER_ACTOR_POSE,WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_GROUND_EMITTER_ACTOR_NAME,WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_IMPACT_EMITTER_RAYCAST,WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_IMPACT_EMITTER_ACTOR_NAME,WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_THRESHOLD_DISTANCE_IMPACT_EMITTER_ACTOR_NAME,WxAnimSetViewer::OnApexDebug )

	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_IOFX_BOUNDING_BOX,WxAnimSetViewer::OnApexDebug )
	EVT_MENU(IDM_ANIMSET_APEX_VISUALIZE_IOFX_ACTOR_NAME,WxAnimSetViewer::OnApexDebug )
#endif
...
{
	bListenForFileChanges = !bListenForFileChanges;
	SetFileSystemNotifications(bListenForFileChanges);
}
...
{
#if WITH_MANAGED_CODE
	SetFileSystemNotification("psa", bListenOnOff);
	SetFileSystemNotification("psk", bListenOnOff);
	SetFileSystemNotification("fbx", bListenOnOff);
#endif
}
...
{
	In.Check( bListenForFileChanges == TRUE );
}
