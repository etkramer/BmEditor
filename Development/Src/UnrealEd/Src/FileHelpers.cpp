...
static UBOOL SaveAsImplementation( const FFilename& DefaultFilename, const UBOOL bAllowStreamingLevelRename )
{
	...
	while( !bFilenameIsValid )
	{
		if( dlg.ShowModal() == wxID_OK )
		{
			...
			if( !FEditorFileUtils::IsFilenameValidForSaving( FileToCheck ) )
			{
				FFilename Filename = FileToCheck;
				appMsgf( AMT_OK, LocalizeSecure( LocalizeUnrealEd( "Error_FilenameIsTooLongForCooking" ), *Filename.GetBaseFilename(), FEditorFileUtils::MAX_UNREAL_FILENAME_LENGTH ) );
 ...
 */
UBOOL FEditorFileUtils::PromptToCheckoutPackages(UBOOL bCheckDirty, const TArray<UPackage*>& PackagesToCheckOut, TArray<UPackage*>* OutPackagesNotNeedingCheckout, const UBOOL bPromptingAfterModify )
{
	UBOOL bResult = TRUE;
	...
	for ( TArray<UPackage*>::TConstIterator PackageIter( PackagesToCheckOut ); PackageIter; ++PackageIter )
	{
		...
		// Package does not need to be checked out if its already checked out, we are ignoring it for source control, or we dont care about the scc state
		UBOOL bSCCCanEdit = (SCCState == SCC_CheckedOut) || (SCCState == SCC_Ignore) || (SCCState == SCC_DontCare);
		
		if ( !bSCCCanEdit && (SCCState != SCC_NotInDepot) && ( !bCheckDirty || ( bCheckDirty && CurPackage->IsDirty() ) ) && bPkgReadOnly)
		{
	...
	if ( bPackagesAdded )
	{
		...
		INT ButtonID = PkgCheckOutDlg.ShowDialog( *LocalizeUnrealEd( TEXT("CheckOutDlg_Title") ), *LocalizeUnrealEd( TEXT("CheckOutDlg_Message") ), WindowSize, "CheckoutPacakgesDlg" );
...
void FEditorFileUtils::LoadMap(const FFilename& Filename)
{
	...
	if( !bIsLoadingSimpleStartupMap )
	{
		// Don't set the last directory when loading the simple map as it is confusing to users
 ...
 */
UBOOL FEditorFileUtils::AutosaveContentPackages(const FString& AbsoluteAutosaveDir, INT AutosaveIndex)
{
	const FScopedBusyCursor BusyCursor;
	DOUBLE SaveStartTime = appSeconds();
...
static INT InternalSavePackage( UPackage* PackageToSave, UBOOL& bOutPackageLocallyWritable )
{
	...
	else
	{
		...
		if (bIsMapPackage)
		{
			FinalPackageFilename = FString::Printf( TEXT("Untitled.%s"), *GApp->MapExt );
		}
		else
		{
			...
			if( SaveFileDialog.ShowModal() == wxID_OK )
			{
				...
				if( FinalPackageFilename.Len() > 0 && FFilename( FinalPackageFilename ).GetExtension().Len() == 0 )
				{
					if( bIsMapPackage )
					{
						...
						FinalPackageFilename += FString::Printf( TEXT(".%s"), *GApp->MapExt );
					}
					else
					{
				...
				if( !FEditorFileUtils::IsFilenameValidForSaving( FinalPackageFilename ) )
				{
 ...
 */
UBOOL FEditorFileUtils::IsFilenameValidForSaving( const FString& Filename )
{
	UBOOL bFilenameIsValid = FALSE;
	...
	if( BaseFilename.Len() > 0 )
	{
		...
		if( BaseFilename.Len() <= MAX_UNREAL_FILENAME_LENGTH ||
			( GIsCooking && GCookingTarget == UE3::PLATFORM_Windows ) )
		{
			bFilenameIsValid = TRUE;
		}
	}
