...
UBOOL UGameStatsDatabase::UploadSession(const FString& SessionID)
{
	...
	FString DBUploaderClassname;
	if(!GConfig->GetString( TEXT("UnrealEd.GameStatsBrowser"), TEXT("GameStatsDBUploaderClassname"), DBUploaderClassname, GEditorIni))
	{
		DBUploaderClassname = TEXT("UnrealEd.GameStatsDBUploader");
	}

	UClass* DBUploaderClass = LoadClass<UGameStatsDBUploader>(NULL, *DBUploaderClassname, NULL, LOAD_None, NULL);
	if (DBUploaderClass != NULL)
	{
		UGameplayEventsReader* StatsFileReader = ConstructObject<UGameplayEventsReader>(UGameplayEventsReader::StaticClass());
		UGameStatsDBUploader* DBUploader = ConstructObject<UGameStatsDBUploader>(DBUploaderClass);
		if (DBUploader != NULL && DBUploader->IsA(UGameStatsDBUploader::StaticClass()))
		{
			DOUBLE TimeToUpload = 0;
			{
				SCOPE_SECONDS_COUNTER(TimeToUpload);
				if (StatsFileReader->OpenStatsFile(*SessionFilename) == TRUE)
				{
					StatsFileReader->eventRegisterHandler(DBUploader);
					if (DBUploader->UploadMetadata() == TRUE)
					{
						StatsFileReader->ProcessStream(); 
						bSuccess = TRUE;
					}
					else
					{
						debugf(TEXT("Failed to upload %s"), **SessionFilename);
					}

					StatsFileReader->CloseStatsFile();
				}
				else
				{
					debugf(TEXT("Failed to open %s for upload"), **SessionFilename);
				}
			}

			debugf(TEXT("Time Talking: %f"), DBUploader->DBUploader->TimeSpentTalkingWithDB);
			debugf(TEXT("Time Total: %f"), TimeToUpload);
		}
		else
		{
			debugf(TEXT("Class %s is not a valid game stats database uploader type"), *DBUploaderClassname);
		}
	}
	else
	{
		debugf(TEXT("Failed to load class %s for game stats visualization"), *DBUploaderClassname);
	}
