	...
	bUseCustomImportanceVolume(FALSE)
{
	...
	for ( INT LevelIndex=0; LevelIndex < GWorld->Levels.Num(); LevelIndex++ )
	{
		...
		{
			SkippedLevels += FString(TEXT(", ")) + Level->GetName();
		}
	}
	...
	for( INT LevelIndex=0; LevelIndex<GWorld->Levels.Num(); LevelIndex++ )
	{
				...
				if (bBuildActorLighting)
				{
					...
					if (!Options.bOnlyBuildVisibility)
					{
						...
						if (Light)
						{
							...
							Light->InvalidateLightingForRebuild();
						}
						else
						{
		...
		if (Options.bOnlyBuildSelected)
		{
			GWarn->LightingBuild_Add(MCTYPE_WARNING, NULL, 
				*Localize(TEXT("Lightmass"), TEXT("LightmassError_BuildSelected"), TEXT("UnrealEd")));

			if (!bObjectsToBuildLightingForFound)
			{
				GWarn->LightingBuild_Add(MCTYPE_ERROR, NULL, 
					*Localize(TEXT("Lightmass"), TEXT("LightmassError_BuildSelectedNothingSelected"), TEXT("UnrealEd")));
			}
		}
...
UBOOL FStaticLightingSystem::LightmassProcess()
{
	...
	FLightmassProcessor* LightmassProcessor = new FLightmassProcessor(*this, Options.bDumpBinaryResults, Options.bOnlyBuildVisibility);
		...
		if (LightingMeshBounds.GetExtent().SizeSquared() > (MinimumImportanceVolumeExtentWithoutWarning * MinimumImportanceVolumeExtentWithoutWarning))
		{
			...
			if (LightmassExporter->GetImportanceVolumes().Num() == 0)
			{
				...
				GWarn->LightingBuild_Add(MCTYPE_CRITICALERROR, NULL, 
					*Localize(TEXT("Lightmass"), TEXT("LightmassError_MissingImportanceVolume"), TEXT("UnrealEd")));
			}
		}
	}
