		...
		if( StaticMeshComponent != NULL )
		{
			...
			if( StaticMeshComponent->LODData.Num() > PaintingMeshLODIndex )
			{
				...
				if(InstanceMeshLODInfo->OverrideVertexColors)
				{
					...
					InstanceMeshLODInfo->VertexColorPositions.Empty();
	...
	else
	{
		...
		if( StaticMeshComponent )
		{
			...
			for( INT CurLODIndex = 0; CurLODIndex < StaticMeshComponent->StaticMesh->LODModels.Num(); ++CurLODIndex )
			{
				...
				if( ColBuffer )
				{
					UINT NumVertices = ColBuffer->GetNumVertices();
					for( UINT VertexIndex = 0; VertexIndex < NumVertices; VertexIndex++ )
					{
						CopiedColors.AddItem( ColBuffer->VertexColor( VertexIndex ) );
					}
					CopiedColorNums.AddItem( NumVertices );
				}
			}
		}
	}
...
void FEdModeMeshPaint::PasteInstanceVertexColors()
{
	...
	for( INT iMesh = 0; iMesh < SelectedActors.Num(); iMesh++ )
	{
		...
		if( StaticMeshComponent )
		{
			...
			if( !NumLods || ( NumLods != CopiedColorNums.Num() ) )
			{
				continue;
			}
			UBOOL bMatch = TRUE;
			...
			{
				if ( StaticMeshComponent->StaticMesh->LODModels( CurLODIndex ).NumVertices != CopiedColorNums( CurLODIndex ) )
				{
					bMatch = FALSE;
					break;
				}
			...
			UINT TotalVertexIndex = 0;
			for( INT CurLODIndex = 0; CurLODIndex < NumLods; ++CurLODIndex )
			{
				INT NumColors = CopiedColorNums( CurLODIndex );
				...
				else
				{
					...
					ComponentLODInfo.OverrideVertexColors->InitFromColorArray( &CopiedColors( TotalVertexIndex ), NumColors );
				}
				BeginInitResource( ComponentLODInfo.OverrideVertexColors );

				TotalVertexIndex += NumColors;
			}
