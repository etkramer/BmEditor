...
void WxAnimSetViewer::ImportCOLLADAAnim()
{
	if(!SelectedAnimSet)
	{
		appMsgf( AMT_OK, LocalizeSecure(LocalizeUnrealEd("Error_SelectAnimSetForImport"), ANSI_TO_TCHAR("COLLADA") ));
		return;
	}
	...
	if( ImportFileDialog.ShowModal() == wxID_OK )
	{
		...
		if(bSetWasNew)
		{
			if( !SelectedAnimSet->CanPlayOnSkeletalMesh(SelectedSkelMesh) )
			{
				...
				{
					appMsgf( AMT_OK, LocalizeSecure(LocalizeUnrealEd("Error_AnimSetCouldNotBePlayed"), ANSI_TO_TCHAR("COLLADA")) );
...
#if WITH_FBX
// TODO: merge this function with ImportPSA(), also for ImportCOLLADAAnim()
void WxAnimSetViewer::ImportFbxAnim()
{
	...
	if( ImportFileDialog.ShowModal() == wxID_OK )
	{
		...
		for(UINT FileIndex = 0; FileIndex < ImportFilePaths.Count(); FileIndex++)
		{
			...
			UEditorEngine::ImportFbxANIMIntoAnimSet( SelectedAnimSet, *Filename, SelectedSkelMesh, TRUE );
...
void WxAnimSetViewer::ImportMeshLOD(USkeletalMesh* InSkeletalMesh, INT DesiredLOD)
{
	...
	// Shut down the skeletal mesh component that is previewing this mesh.
	{
		...
		for(INT MatIdx = 0; MatIdx < InSkeletalMesh->Materials.Num(); MatIdx++)
		{
			...
			{
				SelectedSkelMesh->Materials.FindItemIndex( InSkeletalMesh->Materials(MatIdx) );
			}
		...
		else
		{
			...
			if (bIsFBX)
			{
				...
				else
				{
					...
					// Generate data for LOD import dialog and find the maximum LOD level
					for (INT i = 0; i < MeshArray.Num(); i++)
					{
						...
						for (INT j = 0; j < NodeArray.Num(); j++)
						{
							...
							if (Node->GetNodeAttribute() && Node->GetNodeAttribute()->GetAttributeType() == KFbxNodeAttribute::eLODGROUP)
							{
								...
								{
									MaxLODGroupLevel = Node->GetChildCount() - 1;

									if (MaxLODLevel < MaxLODGroupLevel)
									{
										MaxLODLevel = MaxLODGroupLevel;
									}
								}
							}
						}

						LODGroupStrings->AddZeroed(MaxLODGroupLevel + 2);
						(*LODGroupStrings)(0) = FString::Printf( TEXT("None") );
						(*LODGroupStrings)(1) = FString::Printf( TEXT("Base") );
						for(INT i = 2; i < MaxLODGroupLevel + 2; i++)
						{
							(*LODGroupStrings)(i) = FString::Printf( TEXT("%d"), i - 1 );
						}
						LODStrings.AddItem(LODGroupStrings);
					}

					if ( MeshArray.Num() == 0 || MaxLODLevel == 0 )
					{
						appMsgf( AMT_OK, *LocalizeUnrealEd("Prompt_NoMeshFound") );
					}
					else if ( MaxLODLevel > 3 )
					{
						appMsgf( AMT_OK, LocalizeSecure(LocalizeUnrealEd("Prompt_MoreThanThreeLOD"), MaxLODLevel) );
					}
					else
					{						
						TArray<FString> LODGroupNames;
						INT MeshIndex;
						// create a list of Fbx mesh names to display in the dlg
						for ( MeshIndex = 0; MeshIndex < MeshArray.Num(); MeshIndex++ )
						{
							LODGroupNames.AddItem(ANSI_TO_TCHAR(FbxImporter->MakeName((*MeshArray(MeshIndex))(0)->GetName())));
						}

						WxDlgMeshLODFbxImport Dlg = WxDlgMeshLODFbxImport(this, LODGroupNames, LODStrings, *Filename);
						UBOOL bValidInput = FALSE;
						while ( !bValidInput && Dlg.ShowModal() == wxID_OK )
						{
							bValidInput = TRUE;
							UBOOL bFailed = FALSE;
							
							TArray<INT> SelectedLODIdxs;
							Dlg.GetLODLevels(SelectedLODIdxs);

							// sort LOD
							for ( INT LODLevel = 0; LODLevel <= MaxLODLevel; LODLevel++ )
							{
								INT MeshArrayIdx;
								for ( MeshArrayIdx = 0; MeshArrayIdx < SelectedLODIdxs.Num(); MeshArrayIdx++ )
								{
									if ( (SelectedLODIdxs(MeshArrayIdx) - 1) == LODLevel )
									{
										break;
									}
								}

								if ( MeshArrayIdx == SelectedLODIdxs.Num() )
								{
									continue;
								}

								INT n = SelectedSkelMesh->LODInfo.Num();
								n = SelectedSkelMesh->LODModels.Num();
								if ( (SelectedLODIdxs(MeshArrayIdx) - 1) > SelectedSkelMesh->LODInfo.Num() )
								{
									bValidInput = FALSE;
									appMsgf( AMT_OK, LocalizeSecure(LocalizeUnrealEd("Prompt_InvalidLODIndex"), SelectedLODIdxs(MeshArrayIdx) - 1));
									break;
								}
								...
								for (INT j = 0; j < NodeArray.Num(); j++)
								{
									...
									if (Node->GetNodeAttribute() && Node->GetNodeAttribute()->GetAttributeType() == KFbxNodeAttribute::eLODGROUP)
									{
										...
										{
											SkelMeshNodeArray.AddItem(Node->GetChild(LODLevel));
										}
								...
								TempSkelMesh = (USkeletalMesh*)FbxImporter->ImportSkeletalMesh(UObject::GetTransientPackage(), 
									SkelMeshNodeArray, NAME_None, 0, Filename.GetBaseFilename());

								if( TempSkelMesh )
								{
									ImportMeshLOD(TempSkelMesh, LODLevel);
								...
								{
									bFailed = TRUE;
									appMsgf( AMT_OK, LocalizeSecure(LocalizeUnrealEd("LODImportFail"), LODLevel) );
								}
							}
							if ( bValidInput && !bFailed )
							{
								// Pop up a box saying it worked.
								appMsgf( AMT_OK, *LocalizeUnrealEd("LODALLImportSuccessful") );
							}
						}
					}
					for (INT i=0; i<MeshArray.Num(); i++)
					...
					for (INT x = 0; x < LODStrings.Num(); x++)
					{
						delete LODStrings(x);
					}
					...
					if (Mesh)
					{
						...
						if (ShapeCount > 0)
						{
							for (INT ShapeIndex = 0; ShapeIndex < ShapeCount; ShapeIndex++)
							{
								...
								{
									Result = ConstructObject<UMorphTarget>( UMorphTarget::StaticClass(), SelectedMorphSet, ShapeName );
									// add it to the set
...
void WxAnimSetViewer::ImportMorphTarget(UBOOL bImportToLOD, UBOOL UseImportName)
{
	...
	#else
							appMsgf( AMT_OK, TEXT( "COLLADA support must be enabled to import Collada morph targets.  See the WITH_COLLADA build definition." ) );
	#endif
						}					
					}
				}
...
void WxAnimSetViewer::UpdateMorphTargetWeight(const INT Index, const FLOAT Weight)
{
	...
	PreviewMorphPose->AddMorphTarget(SelectedMorphSet->Targets(Index)->GetFName(), Weight);
	if (SelectedMorphSet->Targets(Index))
	{
		PreviewSkelComp->UpdateMorphTargetMaterial(SelectedMorphSet->Targets(Index), Weight);
		PreviewSkelCompRaw->UpdateMorphTargetMaterial(SelectedMorphSet->Targets(Index), Weight);
	}
}
...
void WxAnimSetViewer::CopySelectedSequence()
{
	...
	if (!RemoveAdditiveSequences(Sequences, DestAnimSet))
	{
		return;
	}
	...
	for(INT i=0; i<Sequences.Num(); i++)
	{
		...
		if( AnimSeq )
		{
			...
			else
			{
				...
				if (bReplaceSequenceName == FALSE)
				{
					// if it's different animset, ask if they'd like to replace
					bDoReplace = appMsgf( AMT_YesNo, LocalizeSecure(LocalizeUnrealEd("Prompt_25"), *AnimSeq->SequenceName.ToString()) );
				}
...
void WxAnimSetViewer::AddAdditiveAnimationToSelectedSequence(UBOOL bPerformSubtraction)
{
	...
	for(INT i=0; i<Sequences.Num(); i++)
	{
		...
		if( AnimSeq )
		{
			...
			{
				UBOOL bDoReplace = appMsgf( AMT_YesNo, LocalizeSecure(LocalizeUnrealEd("Prompt_25"), *DestinationName.ToString()) );
				if( !bDoReplace )
				{
					continue; // Move on to next sequence...
				}
