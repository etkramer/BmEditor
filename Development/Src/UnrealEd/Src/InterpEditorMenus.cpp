...
void WxInterpEd::OnContextNewGroup( wxCommandEvent& In )
{
	...
	const UBOOL bAIGroup = ( In.GetId() == IDM_INTERP_NEW_AI_GROUP );
	const UBOOL bLightingGroup = ( In.GetId() == IDM_INTERP_NEW_LIGHTING_GROUP );
	...
	for( TArray<UInterpGroup*>::TIterator NewGroupIt(NewGroups); NewGroupIt; ++NewGroupIt )
	{
		...
		else
		{
			...
			if( In.GetId() == IDM_INTERP_NEW_LIGHTING_GROUP )
			{
...
//@todo cb: GB conversion
	...
	FString PackageName, GroupName, ObjName;
	{
		...
		if( GBrowser != NULL )
		{
			SelectedPkg = GBrowser->GetTopPackage();
			SelectedGrp = GBrowser->GetGroup();
		}
	...
	if( Interp != NULL )
	{
		WxFileDialog ImportFileDialog(this,
							...
							*(GApp->LastDir[LD_GENERIC_IMPORT]),
							TEXT(""),
#if WITH_COLLADA && WITH_FBX
							TEXT("COLLADA document|*.dae|FBX document|*.fbx|All document|*.*"),
#elif WITH_FBX
							TEXT("FBX document|*.fbx"),
#else // WITH_COLLADA
							TEXT("COLLADA document|*.dae"),
#endif 
...
#else
	appMsgf( AMT_OK, TEXT( "COLLADA or FBX support must be enabled to import Matinee sequences. See the WITH_COLLADA and WITH_FBX build definition." ) );
#endif
}
...
void WxInterpEd::OnMenuExport( wxCommandEvent& In )
{
	...
	if( Interp != NULL )
	{
		...
		if( ExportFileDialog.ShowModal() == wxID_OK )
		{
			...
			if (bIsFBX)
			{
				Exporter = UnFbx::FbxExporter::GetInstance();
			}
			else 
			{
#endif
				...
				Exporter = UnCollada::CExporter::GetInstance();
#endif
#if WITH_FBX
			}
#endif
...
#else
	appMsgf( AMT_OK, TEXT( "COLLADA or FBX support must be enabled to export Matinee sequences. See the WITH_COLLADA and WITH_FBX build definition." ) );
#endif
}
