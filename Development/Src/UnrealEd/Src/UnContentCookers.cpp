...
extern void GatherMaterialKeyData (ProgramKeyData& MaterialKeyData, const UMaterialInterface* Material, const UWorld* InWorld);
...
public:
	 ...
	 * Constructor
 */
	FConsoleMobileShaderCooker( void )
	{
		bSupported = FALSE;
		bReportKeys = FALSE;
	 ...
	 * Virtual destructor
	 */
	virtual ~FConsoleMobileShaderCooker( void )
	{
	}
	 ...
	 */
	void Init( UE3::EPlatformType InPlatform, const TCHAR *InCookedDirectory )
	{
		Platform = InPlatform;
		if ((Platform == PLATFORM_IPhone) || (Platform == PLATFORM_Android) || (Platform == PLATFORM_NGP))
		{
			CookedDirectory = FFilename(InCookedDirectory) * TEXT("Shaders\\ES2\\");
			bSupported = TRUE;
			Load();
			// Create folder for cooked data.
			if( !GFileManager->MakeDirectory( *CookedDirectory, TRUE ) )
			{
				appErrorf(TEXT("Couldn't create %s"), *CookedDirectory);
			}
		}
	}

	//During particle cooking, they will send their material to the mobile shader cooker for future processing
	void SetParticleMaterialUsage(const UMaterialInterface* InParticleMaterial, const BYTE InInterpolationMethod, const UBOOL bUseMobilePointSprites, const BYTE InSpriteScreenAlignment, const UBOOL bInMeshParticle )
	{
		if (bSupported && InParticleMaterial)
		{
			FParticleMaterialDesc NewDesc;
			InParticleMaterial->GetName(NewDesc.MaterialName);
			NewDesc.bUsePointSprites = bUseMobilePointSprites;
	...
	/** Helper function to find out if this particle material is used with and/or without point sprites */
	void FindParticleUsage(const UMaterial* InMaterial, UBOOL& bOutUsedWithoutPointSprites, UBOOL& bOutUsedWithPointSprites, UBOOL& bUsedAsParticle, UBOOL& bUsedAsParticleSubUV, UBOOL& bMeshParticle, DWORD& OutSpriteScreenAlignment, DWORD& OutScreenAlignmentMask)
	{
		check(InMaterial);

		bOutUsedWithoutPointSprites = FALSE;
		bOutUsedWithPointSprites = FALSE;
		...
		for (INT i = 0; i < ParticleMaterialDescs.Num(); ++i)
		{
			if (ParticleMaterialDescs(i).MaterialName == InMaterial->GetName())
			{
				...
				{
					bOutUsedWithoutPointSprites = bOutUsedWithoutPointSprites || !ParticleMaterialDescs(i).bUsePointSprites;
					bOutUsedWithPointSprites    = bOutUsedWithPointSprites    ||  ParticleMaterialDescs(i).bUsePointSprites;
	 ...
	 */
	void AddMobileShaderKey( const UMaterial* InMaterial, const UWorld* InWorld )
	{
		check(InMaterial);

		//NEEDED - better way to reject useless materials.  Temp for demo
		if (InMaterial->GetName() == TEXT("CubeMaterial"))
		{
			return;
		}
		...
		if (bSupported) // && !InMaterial->bUsedAsSpecialEngineMaterial)
		{
			...
			ProgramKeyData KeyData;

			//extract some flags early
			UBOOL bIsDecal = InMaterial->GetUsageByFlag(MATUSAGE_Decals);
			UBOOL bParticle = !bIsDecal && InMaterial->GetUsageByFlag(MATUSAGE_ParticleSprites);
			UBOOL bParticleSubUV = !bIsDecal && InMaterial->GetUsageByFlag(MATUSAGE_ParticleSubUV);
			UBOOL bSkeletalMesh = !bIsDecal && InMaterial->GetUsageByFlag(MATUSAGE_SkeletalMesh);
			UBOOL bLensFlare = !bIsDecal && InMaterial->GetUsageByFlag(MATUSAGE_LensFlare);

			/** If particles, cache off some data*/
			UBOOL bUsedWithoutPointSprites = FALSE;
			UBOOL bUsedWithPointSprites = FALSE;
			...
			FindParticleUsage(InMaterial, bUsedWithoutPointSprites, bUsedWithPointSprites, bUsedAsParticle, bUsedAsParticleSubUV, bUsedAsMeshParticle, ScreenAlignment, ScreenAlignmentMask);
			...
			else
			{
				...
				ASSIGN_PROGRAM_KEY_VALUE( PKDT_IsGradientFogEnabled, KeyData, FALSE );
			}
			...
			{
				FullyIteratedFields.AddItem(PKDT_ParticleScreenAlignment);
			}
			else
			{
				ASSIGN_PROGRAM_KEY_VALUE( PKDT_ParticleScreenAlignment, KeyData, ScreenAlignment );
			}
			...
			//Bump offset is dynamically turned on and off if disabled
				...
				{
					RESET_PROGRAM_KEY_VALUE( PKDT_IsBumpOffsetEnabled, KeyData);
					FullyIteratedFields.AddItem(PKDT_IsBumpOffsetEnabled);
				}
			#endif
			...
			{
				FullyIteratedFields.AddItem(PKDT_IsSubUV);
			}
			else
			{
				ASSIGN_PROGRAM_KEY_VALUE( PKDT_IsSubUV, KeyData, bUsedAsParticleSubUV );
			}

			//mesh particles
			if (bUsedAsMeshParticle)
			{
				OVERRIDE_PROGRAM_KEY_VALUE(PKDT_UseUniformColorMultiply, KeyData, TRUE);
			}

			//Skeletal mesh
			if( bSkeletalMesh )
			{
				FullyIteratedFields.AddItem( PKDT_IsSkinned );
			}
			else
			{
				ASSIGN_PROGRAM_KEY_VALUE( PKDT_IsSkinned, KeyData, FALSE );
			}

			// Decals
			ASSIGN_PROGRAM_KEY_VALUE( PKDT_IsDecal, KeyData, bIsDecal );

			//Used lightmaps
			UBOOL bUsedWithLightMap = FALSE;
			UBOOL bUsedWithoutLightMap = FALSE;
			if (KeyData.FieldValue[PKDT_IsLightingEnabled])
			{
				//NEED to resolve BSP usage
				//FindLightmapUsage(InMaterial, InWorld, bUsedWithoutLightMap, bUsedWithLightMap);
				//if (bUsedWithLightMap && bUsedWithoutLightMap)
				{
					FullyIteratedFields.AddItem(PKDT_IsLightmap);
				}
				//else
				//{
				//	ASSIGN_PROGRAM_KEY_VALUE( PKDT_IsLightmap, KeyData, bUsedWithLightMap );
				//}
			}
			else
			{
				ASSIGN_PROGRAM_KEY_VALUE( PKDT_IsLightmap, KeyData, FALSE);
			}

			//PICK THE PRIMITIVE TYPES
			if (bUsedWithoutPointSprites)
			{
				PrimitiveTypes.AddItem(EPT_Particle);
			}
			if (bUsedWithPointSprites)
			{
				PrimitiveTypes.AddItem(EPT_PointSpriteParticle);
			}
			if (bLensFlare)
			{
			...
			if (bReportKeys)
			{
				warnf(NAME_Warning, TEXT("MOBILE MATERIAL - %s"), *InMaterial->GetName());
			}
	...
	void GenerateAllKeys(TArray <EMobilePrimitiveType>& InPrimitiveTypes, ProgramKeyData InKeyData, TArray <INT>& FullyIteratedFields, const INT FieldIndex, const DWORD ScreenAlignmentMask)
	{
		...
		if (FieldIndex == FullyIteratedFields.Num())
		{
			for (INT PrimitiveIndex = 0; PrimitiveIndex < InPrimitiveTypes.Num(); ++PrimitiveIndex)
			{
				...
				{
					ProgramKeyData FinalKeyData = InKeyData;
					ASSIGN_PROGRAM_KEY_VALUE( PKDT_PrimitiveType, FinalKeyData, PrimitiveType );
					ASSIGN_PROGRAM_KEY_VALUE( PKDT_PlatformFeatures, FinalKeyData, PlatformFeatures );
					FinalKeyData.Stop();
					QWORD NewKey = PackProgramKeyData(FinalKeyData);

					if (bReportKeys)
					{
						FString NewKeyString = FString::Printf(TEXT("0x%llx"), NewKey);
						warnf(NAME_Warning, TEXT("    Key - %s"), *NewKeyString);
					}

					AllRequestedShaderKeys.Num();
					AllRequestedShaderKeys.AddUniqueItem(NewKey);
				}
			}
		}
		else
		{
			...
			INT MaxValue = ES2ShaderProgramKeyFields[FieldEnum].MaxValue;
			for (INT IterValue = 0; IterValue < MaxValue; IterValue++)
			{
				//unused alignment, but more than one specified
				if ((FieldEnum == PKDT_ParticleScreenAlignment) && ((ScreenAlignmentMask & (1<<IterValue)) == 0))
				{
					continue;
				}
				//make a copy of the current state of things
				ProgramKeyData CopyKeyData = InKeyData;
				ASSIGN_PROGRAM_KEY_VALUE( FieldEnum, CopyKeyData, IterValue );
	 ...
	 */
	QWORD CalcGlobalShaderKey( EMobileGlobalShaderType GlobalShaderType )
	{
		ProgramKeyData KeyData;
		KeyData.Start();
		for (INT FieldIndex = 0; FieldIndex < PKDT_MAX; ++ FieldIndex)
		{
			ASSIGN_PROGRAM_KEY_VALUE( FieldIndex, KeyData, 0 );
		}
		OVERRIDE_PROGRAM_KEY_VALUE( PKDT_PrimitiveType, KeyData, EPT_GlobalShader );
		OVERRIDE_PROGRAM_KEY_VALUE( PKDT_GlobalShaderType, KeyData, GlobalShaderType );
		KeyData.Stop();
		QWORD Key = PackProgramKeyData( KeyData );
		return Key;
	}
	...
	void AddFallbackShaders ()
	{
		for (INT PrimitiveType = 0; PrimitiveType < EPT_MAX; ++PrimitiveType)
		{
			...
			{
				ProgramKeyData KeyData;	//reset the key data to be filled in again
				KeyData.Start();
				for (INT FieldIndex = 0; FieldIndex < PKDT_MAX; ++ FieldIndex)
				{
					ASSIGN_PROGRAM_KEY_VALUE( FieldIndex, KeyData, 0 );
				}
				OVERRIDE_PROGRAM_KEY_VALUE(PKDT_PrimitiveType, KeyData, PrimitiveType);
				KeyData.Stop();

				QWORD Key = PackProgramKeyData( KeyData );
				//make sure to always have a fallback of 0
				AllRequestedShaderKeys.AddUniqueItem(Key);
			}
		}
	}
	...
	void AddGlobalShaders()
	{
		...
		for ( INT GlobalShaderIndex=1; GlobalShaderIndex < EGST_MAX; ++GlobalShaderIndex )
		{
			QWORD Key = CalcGlobalShaderKey( (EMobileGlobalShaderType) GlobalShaderIndex );
			AllRequestedShaderKeys.AddUniqueItem( Key );
		}
	}
	...
	void Save( const TCHAR* PlatformEngineConfigFilename )
	{
		if (bSupported)
		{
			TMap<FString,TArray<QWORD> > PixelShaderPreprocessedText;
			TMap<FString,TArray<QWORD> > VertexShaderPreprocessedText;
			...
			if ( Platform & PLATFORM_NGP )
			{
				bSuccess = appLoadFileToString( CommonShaderPrefixFile, *( appEngineDir() + TEXT("Shaders\\NGP\\Prefix_Common.usf") ) );
				bSuccess = bSuccess && appLoadFileToString( VertexShaderPrefixFile, *( appEngineDir() + TEXT("Shaders\\NGP\\Prefix_VertexShader.usf") ) );
				bSuccess = bSuccess && appLoadFileToString( PixelShaderPrefixFile, *( appEngineDir() + TEXT("Shaders\\NGP\\Prefix_PixelShader.usf") ) );
			}
			else
			{
				bSuccess = appLoadFileToString( CommonShaderPrefixFile, *( appEngineDir() + TEXT("Shaders\\ES2\\Prefix_Common.glsl") ) );
				bSuccess = bSuccess && appLoadFileToString( VertexShaderPrefixFile, *( appEngineDir() + TEXT("Shaders\\ES2\\Prefix_VertexShader.glsl") ) );
				bSuccess = bSuccess && appLoadFileToString( PixelShaderPrefixFile, *( appEngineDir() + TEXT("Shaders\\ES2\\Prefix_PixelShader.glsl") ) );
			}
			if (!bSuccess)
			{
			...
			//sort shaders by key
			Sort<USE_COMPARE_CONSTREF(QWORD,UnContentCookers)>( AllRequestedShaderKeys.GetTypedData(), AllRequestedShaderKeys.Num() );

			// load or preprocess shaders and build equivalence maps

			for( INT KeyIndex = 0; KeyIndex < AllRequestedShaderKeys.Num(); KeyIndex++ )
			{
				QWORD ProgramKey = AllRequestedShaderKeys(KeyIndex);
				// Unpack the program key
				ProgramKeyData KeyData;
				UnpackProgramKeyData( ProgramKey, KeyData );
				EMobilePrimitiveType PrimitiveType =(EMobilePrimitiveType)KeyData.FieldValue[PKDT_PrimitiveType];
				const EMobileGlobalShaderType GlobalShaderType = (EMobileGlobalShaderType) KeyData.FieldValue[PKDT_GlobalShaderType];
				const FString PreprocessedDirectoryName = FString::Printf( TEXT( "%s0x%llx\\" ), *CookedDirectory, ProgramKey );

				const FFilename VertexFileName = 
					appConvertRelativePathToFull(FString::Printf( TEXT( "%s%s.glsl.i" ), 
				...
				FString PreprocessedPixelShader;
				{
					...
					GConfig->GetFloat( TEXT("SystemSettings"), TEXT("MobileLODBias"), MobileLODBias, PlatformEngineConfigFilename );
					GConfig->GetInt( TEXT("SystemSettings"), TEXT("MobileBoneCount"), MobileBoneCount, PlatformEngineConfigFilename );
					GConfig->GetInt( TEXT("SystemSettings"), TEXT("MobileBoneWeightCount"), MobileBoneWeightCount, PlatformEngineConfigFilename );
				...
				// vertex
				{
					TArray<QWORD>* Existing = VertexShaderPreprocessedText.Find(PreprocessedVertexShader);
					if (!Existing)
					{
						TArray<QWORD> Blank;
				...
				// pixel
				{
					TArray<QWORD>* Existing = PixelShaderPreprocessedText.Find(PreprocessedPixelShader);
					if (!Existing)
					{
						TArray<QWORD> Blank;
			...
			FString CachedProgramKeyFileContents = FString::Printf( TEXT( "version:%d\r\n"),SHADER_MANIFEST_VERSION);
			// vertex shader equivalence map
			for (TMap<FString,TArray<QWORD> >::TIterator It(VertexShaderPreprocessedText);It;++It)
			{
				if (It.Value().Num() > 1)
				{
					Sort<USE_COMPARE_CONSTREF(QWORD,UnContentCookers)>( It.Value().GetTypedData(), It.Value().Num() );
					UBOOL First = TRUE;
					for (TArray<QWORD>::TIterator ItQ(It.Value());ItQ;++ItQ,First = FALSE)
					{
						CachedProgramKeyFileContents += FString::Printf( TEXT( "%s0x%llx"),
							First ? TEXT("vse:") : TEXT(","), 
							*ItQ );
					}
			...
			for (TMap<FString,TArray<QWORD> >::TIterator It(PixelShaderPreprocessedText);It;++It)
			{
				if (It.Value().Num() > 1)
				{
					Sort<USE_COMPARE_CONSTREF(QWORD,UnContentCookers)>( It.Value().GetTypedData(), It.Value().Num() );
			...
			for( INT KeyIndex = 0; KeyIndex < AllRequestedShaderKeys.Num(); KeyIndex++ )
			{
				...
				CachedProgramKeyFileContents += FString::Printf( TEXT( "0x%llx\r\n"),AllRequestedShaderKeys(KeyIndex)  );
			}
				...
				if( VersionString.StartsWith(VersionToken) )
				{
					...
					else
					{
						...
						for( INT KeyIndex = 1; KeyIndex < KeyStrings.Num(); KeyIndex++ )  // start with 1 because the version is on line 0
						{
							...
							{
								QWORD ProgramKey = HexStringToQWord(*ProgramKeyName);
								// Unpack the program key
								ProgramKeyData KeyData;
								UnpackProgramKeyData( ProgramKey, KeyData );

								AllRequestedShaderKeys.AddItem(ProgramKey);
							}
						}
					}
				}
	...
	struct FParticleMaterialDesc
	{
		UBOOL operator==( const FParticleMaterialDesc& Other) const
		{
			if ((MaterialName == Other.MaterialName) &&
				(bUsePointSprites == Other.bUsePointSprites) &&
		...
		UBOOL bUsePointSprites;
		BYTE SpriteScreenAlignment;
		BYTE InterpolationMethod;
		BYTE bMeshParticle;
	};

	TArray<QWORD> AllRequestedShaderKeys;
	UBOOL bSupported;
	UBOOL bReportKeys;
 ...
 */
static void DumpShaderCompileErrors( FMaterialResource* MaterialResource )
{
...
FString UCookPackagesCommandlet::GetTrueCookedDirectory()
{
	...
	// use the single cooked directory calculation function (emulating GUseCoderMode with no side effects)
	UBOOL bOldUserCoderMode = GUseCoderMode;
	GUseCoderMode = bCoderMode;
	appGetCookedContentPath(Platform, CookedDir);

	// restore the global
	GUseCoderMode = bOldUserCoderMode;
	...
	if (bIsInUserMode)
	{
		...
		switch (Platform)
		{
			...
			case PLATFORM_WindowsConsole:
				CookedDir += FString(TEXT("Published\\CookedPCConsole"));
				break;

			case PLATFORM_WindowsServer:
				CookedDir += FString(TEXT("Published\\CookedPCServer"));
				break;
			...
			case PLATFORM_Xbox360:
				CookedDir += FString::Printf(TEXT("DLC\\Xbox360\\%s\\CookedXbox360\\"),*UserModName);
				break;
		}
		CookedDir += PATH_SEPARATOR;
	}
	else
	{
...
{
	return GetConfigOutputPrefix() + appPlatformTypeToString(Platform) + TEXT("-");
}
...
{
	return Platform == PLATFORM_Xbox360 || Platform == PLATFORM_PS3;
}
...
void FPersistentFaceFXAnimSetGenerator::GenerateMangledFaceFXInformation( FString& PersistentLevelName, FMangledFaceFXInfo& PFFXData )
{
	...
	{
		FString NewMangledName = FString::Printf(TEXT("%d"), FaceFXMangledNameCount++);
...
void UCookPackagesCommandlet::CookTexture( UPackage* Package, UTexture2D* Texture2D, UBOOL bIsSavedInSeekFreePackage )
{
	...
	if( !Texture2D->IsTemplate( RF_ClassDefaultObject ) )
	{
		...
		if ( Texture2D->Format == PF_A1 || ( (Platform & PLATFORM_Mobile) && Texture2D->IsA(ULightMapTexture2D::StaticClass()) && !Texture2D->GetName().StartsWith("Simple") ) )
		{
		...
		if (Texture2D->Mips.Num() > 0)
		{
			...
			UBOOL bDoesPlatformNeedRecompression = 
				(Platform == PLATFORM_IPhone || Platform == PLATFORM_NGP ||
				(Platform == PLATFORM_Android && AndroidTextureFormat == ATF_PVRTC) ||
				(Platform == PLATFORM_Android && AndroidTextureFormat == ATF_ATITC));
			if ((bIsDXT || bWillBeDXT) && bDoesPlatformNeedRecompression)
			{
				// iphone expects precached mips, make them if they don't exist
				if (AndroidTextureFormat == ATF_ATITC)
				{
					...
					ConditionalCacheATITCTextures(Texture2D, FALSE, TRUE, NULL);
				}
				else
				{
			...
			if( Platform == PLATFORM_PS3 || Platform == PLATFORM_Xbox360 || (Platform & PLATFORM_Mobile))
			{
...
void UCookPackagesCommandlet::CookParticleSystem(UParticleSystem* ParticleSystem)
{
	...
	if (GCookingTarget & UE3::PLATFORM_Stripped)
	{
		...
		if (bSkipPSysDuplicateModules == TRUE)
		{
			...
			ParticleSystem->RemoveAllDuplicateModules(TRUE);
		}
								...
								if (MeshTD->Mesh != NULL)
								{
									...
									if (MeshTD->Mesh->LODModels.Num() > 0)
									{
										if (MeshTD->Mesh->LODModels(0).Elements.Num() > 0)
										{
											...
											{
												GMobileShaderCooker.SetParticleMaterialUsage(MeshTD->Mesh->LODModels(0).Elements(0).Material, LODLevel->RequiredModule->InterpolationMethod, ParticleSystem->bUseMobilePointSprites, SpriteScreenAlignment, bMeshParticle);
											}
											else
											{
												GMobileShaderCooker.SetParticleMaterialUsage(GEngine->DefaultMaterial, LODLevel->RequiredModule->InterpolationMethod, ParticleSystem->bUseMobilePointSprites, SpriteScreenAlignment, bMeshParticle);
											}
										}
									}
								}
								GMobileShaderCooker.SetParticleMaterialUsage(LODLevel->RequiredModule->Material, LODLevel->RequiredModule->InterpolationMethod, ParticleSystem->bUseMobilePointSprites, SpriteScreenAlignment, bMeshParticle);
								...
								for (INT ModuleIdx = 0; ModuleIdx < LODLevel->Modules.Num(); ModuleIdx++)
								{
									...
									if (MeshMaterialModule != NULL)
									{
										for (INT MMIdx = 0; MMIdx < MeshMaterialModule->MeshMaterials.Num(); MMIdx++)
										{
											...
											{
												GMobileShaderCooker.SetParticleMaterialUsage(MeshMaterialModule->MeshMaterials(MMIdx), LODLevel->RequiredModule->InterpolationMethod, ParticleSystem->bUseMobilePointSprites, SpriteScreenAlignment, bMeshParticle);
											}
										}
									}
								}
						...
						for (INT ModuleIdx = LODLevel->Modules.Num() - 1; ModuleIdx >= -3; ModuleIdx--)
						{
							...
							if (Module != NULL)
							{
								...
								if (EmitterIndexArray != NULL)
								{
									...
									{
//										debugf(TEXT("\t\tClearing out module %2d - %s"), ModuleIdx, *(Module->GetName()));
										Module->ClearFlags(RF_ForceTagExp);
										Module->SetFlags(RF_MarkedByCooker|RF_Transient);
										if (ModuleIdx >= 0)
										{
						...
						LODLevel->SetFlags(RF_MarkedByCooker|RF_Transient);
					}
				}
			...
			for (INT SectionIndex=0; SectionIndex<NumSections; ++SectionIndex)
			{
				...
				if (Platform == PLATFORM_PS3)
				{
					...
					else
					{
						...
						DWORD* NewIndices = NewIndexArray.GetTypedData() + FirstNewIndex;
						appMemcpy(NewIndices, OldIndices, Section.NumTriangles*3*sizeof(DWORD));
						Section.BaseIndex = FirstNewIndex;
					}
				}	//End PS3
				else if (Platform & PLATFORM_Mobile)
				{
					...
					for (UINT VertexIndex = 0; VertexIndex < VertexBuffer.GetNumVertices(); ++VertexIndex)
					{
						...
						check(BaseVertex);
						//loop through all "extra" weights to verify the aren't too large
						for (INT WeightIndex = MobileBoneWeightCountLimit; WeightIndex < MAX_INFLUENCES; ++WeightIndex)
						{
							if (BaseVertex->InfluenceWeights[WeightIndex] > SMALL_NUMBER)
							{
								warnf(NAME_Warning, TEXT("%s skeletal mesh uses more bone influences than are allowed by this platform (%d)"), *SkeletalMesh->GetName(), MobileBoneWeightCountLimit);
								return;
							}
						}
					}
...
void UCookPackagesCommandlet::CookLandscapeComponent(ULandscapeComponent* LandscapeComponent)
{
	...
	if (Platform == PLATFORM_PS3)
	{
		...
		INT Mip = 0;
		INT MipSubsectionSizeVerts = LandscapeComponent->SubsectionSizeQuads+1;
		while( MipSubsectionSizeVerts > 1 )
		{
			...
			for( INT SubsectionY = 0;SubsectionY < LandscapeComponent->NumSubsections;SubsectionY++ )
			{
				for( INT SubsectionX = 0;SubsectionX < LandscapeComponent->NumSubsections;SubsectionX++ )
				{
					WORD* DstData = &Heights(Heights.AddZeroed((MipSubsectionSizeVerts + Padding) * MipSubsectionSizeVerts));

					FVector4 HeightmapScaleBias = LandscapeComponent->HeightmapScaleBias;
					HeightmapScaleBias.Z += LandscapeComponent->HeightmapSubsectionOffset * (FLOAT)SubsectionX;
					HeightmapScaleBias.W += LandscapeComponent->HeightmapSubsectionOffset * (FLOAT)SubsectionY; 
					INT BaseMipOfsX = appRound(HeightmapScaleBias.Z * (FLOAT)HeightmapTexture->SizeX);
					INT BaseMipOfsY = appRound(HeightmapScaleBias.W * (FLOAT)HeightmapTexture->SizeY);
		...
		LandscapeComponent->PlatformDataSize = Heights.Num() * sizeof(WORD) + LodInfoSize;
		LandscapeComponent->PlatformData = appMalloc(LandscapeComponent->PlatformDataSize);
		appMemzero(LandscapeComponent->PlatformData, LandscapeComponent->PlatformDataSize);

		DWORD* LodInfoData = (DWORD*)LandscapeComponent->PlatformData;
		...
		appMemcpy((BYTE*)LandscapeComponent->PlatformData + LodInfoSize, &Heights(0), Heights.Num() * sizeof(WORD));
	}
}
	...
	if (MatInst->Parent != NULL)
	{
		for (INT MPIdx = 0; MPIdx < MP_MAX; MPIdx++)
		{
			...
			if (MatInst->Parent->GetTexturesInPropertyChain(MaterialProp, IgnoreTextures, &RefdParamNames, 
				(bSkipStaticSwitchClean ? MatInst->StaticParameters[MSP_BASE] : NULL)) == TRUE)
			{
...
void UCookPackagesCommandlet::CleanupMaterials(UPackage* Package)
{
	...
	for (INT MaterialIdx = 0; MaterialIdx < FoundMaterials.Num(); MaterialIdx++)
	{
		...
		for (INT ExpIdx = Material->Expressions.Num() - 1; ExpIdx >= 0; ExpIdx--)
		{
			...
			if (ReferencedExpressionsMap.Find(Expression) == NULL)
			{
				...
				if (Expression->bUsedByStaticParameterSet == TRUE)
				{
				...
				else
				{
					...
					Expression->SetFlags(RF_Transient|RF_MarkedByCooker);
	...
	for (INT MICIdx = 0; MICIdx < FoundConstant.Num(); MICIdx++)
	{
		...
		if (MIC->Parent)
		{
			...
			{
				UTexture*const* ParentTexture = RefdTextureParamsMap.Find(MIC->TextureParameterValues(CheckIdx).ParameterName);
				...
				if (ParentTexture == NULL && MIC->TextureParameterValues(CheckIdx).ParameterName.ToString().Left(9) != TEXT("Weightmap") )
				{
	...
	if (ClearedTextures.Num() > 0)
	{
		for (INT TextureIdx = ClearedTextures.Num() - 1; TextureIdx >= 0; TextureIdx--)
		{
			...
			if (Texture != NULL)
			{
				...
				{
					Texture->SetFlags(RF_MarkedByCooker);
					Texture->ClearFlags(RF_ForceTagExp);
				}
			}
			else
			{
				debugf(TEXT("CleanupMaterials> NULL TEXTURE???"));
			}
		}
	}
}
...
{	
		...
		)
	{
		...
		if(MaterialResource->Compile( &EmptySet, ShaderPlatform, MaterialShaderMapRef, FALSE ))
		{
			...
			{
				// swap the material resource shader map...
				MaterialResource->SetShaderMap(MaterialShaderMapRef);
			}
		}
...
{	
		...
		)
	{
		...
		{
			const UMaterial* BaseMaterial = MaterialInstance->GetMaterial(MaterialPlatform);
...
// 				||
// 				// NotForClient & NotForServer & NotForEdit
// 				(Object->HasAnyFlags(RF_NotForClient) && Object->HasAnyFlags(RF_NotForServer) && Object->HasAnyFlags(RF_NotForEdit))
			))
		{
		...
		for (INT CachedMatIndex = 0; CachedMatIndex < CachedMaterials.Num(); CachedMatIndex++)
		{
			...
			if( TMatRes && ShaderCache )
			{
				...
				if (TMatRes->Compile(&EmptySet, ShaderPlatform, MaterialShaderMapRef, FALSE))
				{
					...
					{
						// swap the material resource shader map...
						TMatRes->SetShaderMap(MaterialShaderMapRef);
					}
				}
			}
		}
...
UBOOL UCookPackagesCommandlet::SetPlatform(const FString& Params)
{
	...
	if (Platform == PLATFORM_Android)
	{
		...
		{
			warnf(TEXT("When cooking for Android, you MUST specify a texture format with -TexFormat=[dxt | pvrtc | atitc]"));
			return FALSE;
		}
		// now set the enum
		if (TextureFormat == TEXT("dxt"))
		{
			AndroidTextureFormat = ATF_DXT;
		}
		else if (TextureFormat == TEXT("pvrtc"))
		{
			AndroidTextureFormat = ATF_PVRTC;
		}
		else if (TextureFormat == TEXT("atitc"))
		{
			AndroidTextureFormat = ATF_ATITC;
		}
		else
		{
			warnf(TEXT("Unrecognized texture format specified with -TexFormat. Must be one of [dxt | pvrtc | atitc]"));
			return FALSE;
		}
	}
...
UBOOL UCookPackagesCommandlet::BindDLLs()
{
	...
	if( Platform & PLATFORM_Console )
	{
		...
		ConsoleSupport = FConsoleSupportContainer::GetConsoleSupportContainer()->GetConsoleSupport(*appPlatformTypeToString(Platform));
		if(!ConsoleSupport)
		{
			appDebugMessagef(TEXT("Couldn't bind to support DLL for %s."), *appPlatformTypeToString(Platform));
			return FALSE;
		}
...
void UCookPackagesCommandlet::CreateIniFiles(UBOOL bAddForHashing)
{
	...
	if (Platform & (PLATFORM_Console | PLATFORM_WindowsServer))
	{
		...
		if (!bIsMTChild)  // child processes don't need to do this and the files would clash
		{
			if (Platform & PLATFORM_Console)
			{
				...
				GConfig->CoalesceFilesFromDisk(*(appGameConfigDir() + GetConfigOutputDirectory()), *GetTrueCookedDirectory(), 
					ShouldByteSwapData(), PlatformEngineConfigFilename, *appPlatformTypeToString(Platform), 
					bCookCurrentLanguageOnly | bCoderMode, MultilanguageCookingMask);
...
UBOOL UCookPackagesCommandlet::PrepareShaderFiles()
{
	...
	else if ((bIsInUserMode && Platform == PLATFORM_PS3))
	{
...
UBOOL UCookPackagesCommandlet::Init( const TCHAR* Params, UBOOL& bQuitAfterInit )
{
	...
	bCoderMode					= Switches.FindItemIndex(TEXT("CODERMODE")) != INDEX_NONE;
	...
	if (bCoderMode)
	{
		if (bIsInUserMode)
		{
			warnf(NAME_Warning, TEXT("*** Disabling CoderMode due to UserMode!"));
			bCoderMode = FALSE;
		}
		else
		{
			warnf(NAME_Log, TEXT("*** Cooking in Coder mode!"));
		}
	}
	...
	// For PS3 mods . . .
	if (bIsInUserMode && (Platform == PLATFORM_PS3 || Platform == PLATFORM_Xbox360 || Platform == PLATFORM_WindowsConsole || Platform == PLATFORM_WindowsServer))
	{
	...
	if (Switches.FindItemIndex(TEXT("SINGLETHREAD")) == INDEX_NONE )
	{
		...
		else if (bVerifyTextureFileCache || bIniFilesOnly || bAnalyzeReferencedContent || bIsInUserMode || bDumpBulkData
	...
	{
		if (bVerifyTextureFileCache || bIniFilesOnly || bAnalyzeReferencedContent || bIsInUserMode || bDumpBulkData)
		{
	...
	GMobileShaderCooker.Init(Platform, *CookedDir);
	...
	GetNativeScriptPackageNames(PackageNames, FALSE, FALSE);
	...
	if (bCoderMode)
	{
		warnf(NAME_Log, TEXT("CoderMode: Disabling TextureFileCache!"));
	...
	if (bUseTextureFileCache == TRUE && !bIsInUserMode
		&& !bIsMTChild  // children have slightly different TFC names and it is ok if they are missing
		)
	{
...
void UCookPackagesCommandlet::InitializeMaterialCleanupSettings()
{
	...
	if (bCleanupMaterials == TRUE)
	{
		...
		if (bCleanStaticMeshMaterials == TRUE)
		{
			...
			TArray<FString> GameNativeScriptPackages;
			appGetGameNativeScriptPackageNames(GameNativeScriptPackages, FALSE);
			appGetGameScriptPackageNames(GameScriptPackages, FALSE);

			for (INT NativeIdx = 0; NativeIdx < GameNativeScriptPackages.Num(); NativeIdx++)
			{
				GameScriptPackages.RemoveSingleItem(GameNativeScriptPackages(NativeIdx));
			}
...
static FString GetCookedPackageExtension(EPlatformType Platform)
{
	if (Platform&PLATFORM_Console)
	{
		return TEXT(".xxx");
...
TArray<FPackageCookerInfo> UCookPackagesCommandlet::GeneratePackageList( INT& FirstStartupIndex, INT& FirstScriptIndex, INT& FirstGameScriptIndex, INT& FirstMapIndex )
{
	...
	TArray<FString> GFxPackageList;
	GFx_AddExtraPackages(GFxPackageList);
	SeekFreeAlwaysCookMaps += GFxPackageList;
#endif // WITH_GFx
	...
	for (INT Pass = 0; Pass < 2; Pass++)
	{
		for( INT PackageIndex = 0; PackageIndex < PackageList.Num(); PackageIndex++ )
		{
			...
			if (bCoderMode)
			{
				//warnf(NAME_Log, TEXT("*** Disabling all seekfree/combined cooking for CoderMode!"));
				bIsCombinedStartupPackage	= FALSE;
				bIsStandaloneSeekfree		= FALSE;
				bIsSeekfree					= FALSE;
			}
			...
			else
			{
				...
				{
					DstFileTime = GFileManager->GetFileTimestamp(*DstFilename);
				}
			}
				...
				&&	( !bIsCombinedStartupPackage ) )
			{
				...
				{
					if (bCoderMode && bIsMapFile)
					{
						debugf(TEXT("CODERMODE: Skipping dependency check for map %s"), *(SrcFilename.GetBaseFilename()));
					}
					else
					{
			...
			else if (bIsNonNativeScriptFile && (GCookingTarget == PLATFORM_Windows || bCoderMode) && !bIsEditorScriptFile)
			{
	...
	if( ScriptFilenamePairs.Num() )
	{
		...
		if (bCoderMode || GCookingTarget == PLATFORM_Windows)
		{
			for (INT GameScriptIndex = 0; GameScriptIndex < GameScriptPackageNames.Num(); GameScriptIndex++)
			{
				ScriptPackageNames.AddUniqueItem(GameScriptPackageNames(GameScriptIndex));
			}
		}
...
void UCookPackagesCommandlet::SaveMipToTextureFileCache( UPackage* Package, UTexture2D* Texture, INT MipIndex )
{
	...
	{
		TextureFileCache = FName(*GetTextureCacheName(Texture)); // we want the appendage version here
...
void UCookPackagesCommandlet::SaveCookedPackage( UPackage* Package, UObject* Base, EObjectFlags TopLevelFlags, const TCHAR* DstFilename, UBOOL bStripEverythingButTextures, UBOOL bCleanupAfterSave, UBOOL bRememberSavedObjects )
{
	...
	if ((Platform == PLATFORM_Windows) || bCoderMode)
	{
	...
	if( bCleanupAfterSave )
	{
		...
		for( FObjectIterator It; It; ++It )
		{
			...
			if (bRememberSavedObjects && !bCoderMode)
			{
...
void UCookPackagesCommandlet::CreateCustomEngine()
{
	...
	if ( !SetPlatform( appCmdLine() ) )
	{
		...
		warnf(NAME_Error, TEXT("Platform not specified. You must use:\n   platform=[xenon|pc|pcserver|pcconsole|ps3|linux|macosx|iphone|android|ngp]"));
		CLEAR_WARN_COLOR();
	}
...
UBOOL UCookPackagesCommandlet::StartChildren(INT NumFiles)
{
	...
	Parse(appCmdLine(), TEXT("Processes="),NumChildProcesses);
	...
	warnf(NAME_Log, TEXT("*** Multithreaded cook, starting %d processes for %d files."),NumChildProcesses,NumFiles);
...
void UCookPackagesCommandlet::LoadParentPersistentCookerData()
{
	...
	if (ParentPersistentCookerDataFilename.FileExists()) // sometimes the parent doesn't give us one because it doesn't have enough new to bother
	{
		...
		PersistentCookerData->CopyTFCInfo(NewPersistentCookerData);
	...
	for( TObjectIterator<UObject> It; It; ++It )
	{
		...
		if( Texture2D )
		{
			for( INT MipLevel=0; MipLevel<Texture2D->Mips.Num(); MipLevel++ )
			{
				...
				FString MipPart = FString::Printf(TEXT("MipLevel_%i"),MipLevel);
...
INT UCookPackagesCommandlet::Main( const FString& Params )
{
	...
	if( Platform & UE3::PLATFORM_PS3 || Platform == PLATFORM_NGP)
	{
	...
	else if (Platform == PLATFORM_IPhone || Platform == PLATFORM_Android)
	{
...
//					warnf(NAME_Log,TEXT("pkg %s"),*PackageList(PackageIndex).DstFilename);
		...
		for( INT PackageIndex=0; !bAbortCooking && PackageIndex<PackageList.Num(); PackageIndex++ )
		{
			...
			if (bCoderMode )
			{
				//if (bIsCombinedStartupPackage || bShouldBeSeekFree || bIsStandaloneSeekFreePackage || bShouldBeFullyCompressed)
				//{
				//	warnf(NAME_Log, TEXT("*** Disabling all seekfree cooking for CoderMode!"));
				//}
				bIsCombinedStartupPackage		= FALSE;
				bShouldBeSeekFree				= FALSE;
				bIsStandaloneSeekFreePackage	= FALSE;
				bShouldBeFullyCompressed		= FALSE;
			}
			...
			UBOOL bStripEverythingButTextures	= (PackageIndex < FirstScriptIndex) && (Platform != PLATFORM_Windows);
			...
			if (Package && (!bShouldOnlyLoad || !bQuickMode) )
			{
				...
				if (bCoderMode)
				{
					// Set the flag to inline shaders...
					Package->PackageFlags |= PKG_ContainsInlinedShaders;
				}
					...
					if( GIsBuildMachine )
					{
						...
						if( LightingQuality != Quality_Production && LightingQuality != Quality_MAX)
						{
							...
							if( PersistentMapList.ContainsItem( PackageName ) )
							{
								...
								warnf( TEXT("[REPORT] %s was built without production quality lighting!"), *PackageName );
							}
						}
					}
				}
				...
				for( FObjectIterator It; It; ++It )
				{
...
#endif
					{
						...
						if( !bStripEverythingButTextures || Object->IsA(UTexture2D::StaticClass()) || bCoderMode )
						{
							// Cook object.
				...
				if ((Platform & PLATFORM_Mobile) && ShaderCache)
				{
					for( TObjectIterator<UMaterialInstance> It; It; ++It )
					{
						...
						if (!MaterialInstance->HasAnyFlags(RF_ClassDefaultObject|RF_Transient))
						{
							...
							{
								if (MaterialInstance->StaticPermutationResources[Platform])
								{
									MaterialInstance->StaticPermutationResources[Platform]->RemoveUniformExpressionTextures();
								}
							}
							...
							for (INT LangIndex = 1; LangIndex < KnownLanguageExtensions.Num(); LangIndex++) // start at 1 here, we don't want to do INT again
							{
								if (MultilanguageCookingMask & (1 << LangIndex))
								{
									...
									for( INT ResourceIndex=0; ResourceIndex<IntObjectReferencer->ReferencedObjects.Num(); ResourceIndex++ )
									{
										...
										LocObjectName += TEXT("_");
										LocObjectName += KnownLanguageExtensions(LangIndex);
...
//														debugf(TEXT("Found inners %s"),*ItObject->GetFullName());
		...
		if (bIsMTChild)
		{
			...
			BulletProofPCDSave(PersistentCookerData,*PersistentCookerDataFilename);
			if (!GIsBuildMachine) // build machines don't reuse the local shader cache for anything anyway
			{
				SCOPE_SECONDS_COUNTER_COOK(SaveShaderCacheTime);
				if (Platform & (UE3::PLATFORM_Windows|UE3::PLATFORM_WindowsConsole))
				{
		...
		else if (bIsMTMaster)
		{
			...
			GMobileShaderCooker.Save( PlatformEngineConfigFilename );
		}
		else
		{
			// Save the shader hashes used for this cook
	...
	UBOOL bInProcessShaderCaches)
{
	...
	if (bInProcessShaderCaches && bInShouldBeSeekFree && (Platform & UE3::PLATFORM_Console))
	{
	...
	if (!bInStripEverythingButTextures && !bCoderMode)
	{
		// Clean up materials???
...
void UCookPackagesCommandlet::GenerateSHAHashes()
{
	...
	if (FilenameToScriptSHAMap.Num())
	{
		...
		SHAWriter->Serialize(HASHES_SHA_DIVIDER, strlen(HASHES_SHA_DIVIDER) + 1);

		for (TMap<FString, TArray<BYTE> >::TConstIterator It(FilenameToScriptSHAMap); It; ++It)
		{
			// write out filename
			SHAWriter->Serialize(TCHAR_TO_ANSI(*It.Key()), It.Key().Len() + 1);

			// write out hash
			SHAWriter->Serialize((void*)It.Value().GetData(), It.Value().Num());
		}
	}
