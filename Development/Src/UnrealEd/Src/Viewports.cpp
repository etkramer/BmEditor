...
void FViewportConfig_Data::SaveToINI()
{
	...
	GConfig->SetString( SectionName, TEXT("DefaultEditor_ShowFlagsBinaryString"), *SHOW_DefaultEditor.ToString(), GEditorUserSettingsIni );
	...
	for( INT x = 0 ; x < 4 ; ++x )
	{
		...
		if( viewport->bEnabled )
		{
			...
			GConfig->SetString( SectionName, *(Key+TEXT("_ShowFlagsBinaryString")), *viewport->ShowFlags.ToString(), GEditorUserSettingsIni );
...
UBOOL FViewportConfig_Data::LoadFromINI()
{
	...
	EShowFlags SavedDefaultEditor = SHOW_DefaultEditor & ~(SHOW_DepthOfField | SHOW_ImageReflections | SHOW_SubsurfaceScattering | SHOW_LightFunctions | SHOW_Tessellation);
	{
		...
		if( GConfig->GetString( SectionName, TEXT("DefaultEditor_ShowFlagsBinaryString"), ShowFlagsStr, GEditorUserSettingsIni ) )
		{
	...
	for( INT x = 0 ; x < 4 ; ++x )
	{
		...
		if( bFoundViewport && bIsEnabled )
		{
			...
			FString ShowFlagsStr;
			if( GConfig->GetString( SectionName, *(Key+TEXT("_ShowFlagsBinaryString")), ShowFlagsStr, GEditorUserSettingsIni ) )
			{
			...
			else
			{
				...
				GConfig->GetInt( SectionName, *(Key+TEXT("_ShowFlagsLower")), LowerFlags, GEditorUserSettingsIni );
				GConfig->GetInt( SectionName, *(Key+TEXT("_ShowFlagsUpper")), UpperFlags, GEditorUserSettingsIni );
					
				viewport->ShowFlags = 0;
				QWORD OldFormatShowFlags = ((QWORD)LowerFlags) | (((QWORD)UpperFlags) << 32);
				for( INT Index = 0; Index < 64; ++Index )
				{
					if( OldFormatShowFlags & ((QWORD)1 << Index) )
					{
						viewport->ShowFlags(Index) = TRUE;
					}
				}
			}
