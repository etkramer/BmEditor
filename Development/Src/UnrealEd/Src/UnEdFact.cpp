...
)
{
	...
	for( FActorIterator It; It; ++It )
	{
		...
		if( PropText )
		{
			if (Actor->ShouldImport(PropText))
			{
...
)
{
	if(	appStricmp( FileType, TEXT( "WAV" ) ) == 0 )
	{
		...
		if( ExistingSound && !bSuppressImportOverwriteDialog )
		{
		...
		if( WaveInfo.ReadWaveInfo( RawWaveData.GetTypedData(), RawWaveData.Num() ) )
		{
			...
			{
				Warn->Logf( NAME_Error, TEXT( "Currently, only mono or stereo WAV files are supported (%s)." ), *Name.ToString() );
				return NULL;
			}
		}
		else
		...
		if( bAutoCreateCue )
		{
			CreateSoundCue( Sound, InParent, Flags, bIncludeAttenuationNode, bIncludeModulatorNode, bIncludeLoopingNode, CueVolume );
		}
...
UBOOL UReimportSoundFactory::Reimport( UObject* Obj )
{
	...
	// See if the mesh is up to date based on time-stamps, if it's not, perform the import
	...
	{
		GWarn->Log( TEXT("-- Sound Node Wave is already up to date."));
	}
...
void UAITreeFactoryNew::StaticConstructor()
{
	new(GetClass()->HideCategories) FName(NAME_Object);
}
 ...
 * is initialized against its archetype, but before any objects of this class are created.
 */
void UAITreeFactoryNew::InitializeIntrinsicPropertyValues()
{
	SupportedClass		= UAITree::StaticClass();
	bCreateNew			= TRUE;
	bEditAfterNew		= TRUE;
	Description			= SupportedClass->GetName();
}

UObject* UAITreeFactoryNew::FactoryCreateNew(UClass* Class,UObject* InParent,FName Name,EObjectFlags Flags,UObject* Context,FFeedbackContext* Warn)
{
	return StaticConstructObject(Class,InParent,Name,Flags);
}

IMPLEMENT_CLASS(UAITreeFactoryNew);
...
)
{
	...
	else if( Length >= sizeof(FTGAFileHeader) && (TGA->ImageTypeCode == 2 || TGA->ImageTypeCode == 10 || (TGA->ColorMapType == 1 && TGA->ImageTypeCode == 1 && TGA->BitsPerPixel == 8)) )
	{
		// Check the resolution of the imported texture to ensure validity
		...
		else if(TGA->ColorMapType == 1 && TGA->ImageTypeCode == 1 && TGA->BitsPerPixel == 8)
		{
			...
			Texture = CreateTexture( Class, InParent, Name, Flags );
			Texture->Init(TGA->Width,TGA->Height,PF_G8);
			...
			BYTE*  const TextureData = (BYTE*) Texture->Mips(0).Data.Lock(LOCK_READ_WRITE);

			INT RevY = 0;
			for (INT Y = TGA->Height-1; Y >= 0; --Y)
			{
				const BYTE* const ImageCol = ImageData + (Y * TGA->Width); 
				BYTE* const TextureCol = TextureData + (RevY++ * TGA->Width);
...
UBOOL UReimportStaticMeshFactory::Reimport( UObject* Obj )
{
	...
	{
		GWarn->Log( TEXT("-- Static Mesh is already up to date."));
	}
...
UBOOL UReimportSkeletalMeshFactory::Reimport( UObject* Obj )
{
	...
	{
		GWarn->Log( TEXT("-- Skeletal Mesh is already up to date."));
	}
...
)
{
			...
			if ( Dialog.ShowModal() == wxID_OK )
			{
				...
				else
				{
					for ( INT NameIndex = 0 ; NameIndex < NamesToImport.Num() ; ++NameIndex )
					{
						...
						Warn->StatusUpdatef( NameIndex, NamesToImport.Num(), *FString::Printf(LocalizeSecure(LocalizeUnrealEd("Importingf"), NameIndex, NamesToImport.Num())) );
	...
	Warn->EndSlowTask();
#else
	appMsgf( AMT_OK, TEXT( "COLLADA support must be enabled to import Collada meshes.  See the WITH_COLLADA build definition." ) );
#endif
				...
				case AAF_DESTRUCTIBLE_BINARY:
					{
						...
						UApexDestructibleAsset* DestructibleAsset = CastChecked<UApexDestructibleAsset>(StaticConstructObject(sclass,InParent,Name,Flags));
						FString NameString = Name.ToString();
						DestructibleAsset->Import(dataBuffer,dataLen,NameString, TRUE );
				...
				case AAF_CLOTHING_BINARY:
					{
						...
						UApexClothingAsset* ClothingAsset = CastChecked<UApexClothingAsset>(StaticConstructObject(sclass,InParent,Name,Flags));
						FString NameString = Name.ToString();
						ClothingAsset->Import(dataBuffer,dataLen, NameString, TRUE );
