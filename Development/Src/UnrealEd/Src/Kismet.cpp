...
BEGIN_EVENT_TABLE( WxKismet, WxLinkedObjEd )
	...
	EVT_BUTTON(IDM_KISMET_DOCLASSSEARCH, WxKismet::OnDoClassSearch )
	EVT_BUTTON(IDM_KISMET_CREATEOBJECT, WxKismet::OnSequenceClassSelected )
	EVT_TEXT_ENTER(IDM_KISMET_SEARCHENTRY, WxKismet::OnDoSearch )
	EVT_TEXT_ENTER(IDM_KISMET_CLASSSEARCHENTRY, WxKismet::OnDoClassSearch )
	EVT_LISTBOX(IDM_KISMET_SEARCHRESULT, WxKismet::OnSearchResultChanged )
	EVT_LISTBOX_DCLICK(IDM_KISMET_CLASSSEARCHRESULT, WxKismet::OnSequenceClassSelected )
...
{
	USequence *Seq = GetRootSequence();
	if (Seq != NULL)
	{
		// Go through all sequence objects and add them to the selection
		for(INT Idx = 0; Idx < Seq->SequenceObjects.Num(); Idx++)
		{
			USequenceObject *Obj = Seq->SequenceObjects(Idx);
			if (Obj == NULL)
			{
				continue;
			}
...
void WxKismet::OnDoSearch(wxCommandEvent &In)
{
	...
	TArray<USequenceObject*> Results;
	DoSilentSearch(Results, SearchWindow->GetSearchString(), (EKismetSearchType)SearchWindow->GetSearchType(), Scope, SearchWindow->GetSearchObjectClass());
 ...
 */
void WxKismet::OnDoClassSearch(wxCommandEvent &In)
{
	// Should not be able to call this if the Search dialog is not up!
	check(ClassSearchWindow);

	// Save search settings to the editor user settings ini.
	TArray<UClass*> Results;
	DoClassSearch(Results, ClassSearchWindow->GetSearchString());

	// Update the list box with search results.
	ClassSearchWindow->ClearResultsList();

	for( INT Idx = 0; Idx < Results.Num(); ++Idx )
	{
		UClass *SeqClass = Results(Idx);
		if( SeqClass == NULL || SeqClass->HasAnyClassFlags(CLASS_Hidden | CLASS_Deprecated) )
		{
			continue;
		}
		...
		ClassSearchWindow->AppendResult( *TempDefaultObject->ObjName, SeqClass );
	}
}

void WxKismet::UpdateSearchWindowResults(TArray<USequenceObject*> &NewResults)
{
	// Update the list box with search results.
	SearchWindow->ClearResultsList();
	...
	for(INT Idx = 0; Idx < NewResults.Num(); Idx++)
	{
		...
		if ((EKismetSearchType)SearchWindow->GetSearchScope() == KSS_AllLevels)
		{
			ULevel *Level = Cast<ULevel>(Obj->GetRootSequence()->GetOuter());
			SearchWindow->AppendResult( *FString::Printf(TEXT("%s: %s"),*Level->GetOuter()->GetOuter()->GetName(),*ResultName), Obj );
		}
		else
		{
			SearchWindow->AppendResult( *ResultName, Obj );
		}
	}
}
 ...
 */
void WxKismet::OnSequenceClassSelected(wxCommandEvent &In)
{
	UClass* SeqObjClass = ClassSearchWindow->GetSelectedResult();
	if ( SeqObjClass )
	{
		INT NewPosX = (LinkedObjVC->NewX - LinkedObjVC->Origin2D.X)/LinkedObjVC->Zoom2D;
		INT NewPosY = (LinkedObjVC->NewY - LinkedObjVC->Origin2D.Y)/LinkedObjVC->Zoom2D;

		USequenceObject* SeqObj = NewSequenceObject( SeqObjClass, NewPosX, NewPosY );
		if ( SeqObj )
		{
			CenterViewOnSeqObj( SeqObj );
		}
...
void WxKismet::DoSearch(const TCHAR* InText, EKismetSearchType Type, UBOOL bJumpToFirstResult, EKismetSearchScope Scope /*= KSS_CurrentSequence*/, UClass* SearchClass /*= NULL*/)
{
	...
	UBOOL bUpdateSearchWindow = !bJumpToFirstResult || Results.Num() > 1;
	...
	if( SearchString.Len() > 0 )
	{
		...
		for( TObjectIterator<UClass> It; It; ++It )
		{
			if( It->IsChildOf(USequenceObject::StaticClass()) )
			{
				...
				if( TempDefaultObject != NULL )
				{
					if( TempDefaultObject->ObjName.ToUpper().InStr(*SearchString.ToUpper()) != -1 )
					{
						Results.AddItem(*It);
					}
				}
 ...
 */
void WxKismet::SetBookmark( INT InIndex )
{
	if ( GWorld )
	{
		...
		if ( WorldInfo && InIndex >= 0 && InIndex <= 9 )
		{
 ...
 */
void WxKismet::JumpToBookmark( INT InIndex )
{
	if ( GWorld )
	{
		AWorldInfo* WorldInfo = GWorld->GetWorldInfo();
		...
		if ( WorldInfo && InIndex >= 0 && InIndex <= 9 && WorldInfo->KismetBookMarks[InIndex] )
		{
