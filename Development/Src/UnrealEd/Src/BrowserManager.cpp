...
void UBrowserManager::CreateBrowserPanes(wxWindow* Parent)
{
	...
	// Check to see if the Generic Browser should be enabled
	UBOOL bShouldInitGenericBrowser = FALSE;
	{
		UBOOL bUseGenericBrowserFromIni;
		const UBOOL bReadSucceeded = GConfig->GetBool(TEXT("BrowserWindows"), TEXT("UseGenericBrowser"), bUseGenericBrowserFromIni, GEditorUserSettingsIni);
		if( bReadSucceeded )
		{
			bShouldInitGenericBrowser = bUseGenericBrowserFromIni;
		}

		if( ParseParam( appCmdLine(), TEXT("GenericBrowser") ) )
		{
			bShouldInitGenericBrowser = TRUE;
		}
	}

	// If we don't have a content browser, then always enable the generic browser
	if( !bShouldInitContentBrowser )
	{
		bShouldInitGenericBrowser = TRUE;
	}
	...
	for (INT Index = 0; Index < BrowserPanes.Num(); Index++)
	{
		...
		const FString GenericBrowserName( TEXT("WxGenericBrowser") );
		...
		const UBOOL bIsGenericBrowserPane = GenericBrowserName == (*BrowserPanes(Index).WxWindowClassName);
		...
		if ( (bShouldInitGenericBrowser && bIsGenericBrowserPane) ||
			 (bShouldInitContentBrowser && bIsContentBrowserPane) ||
			 (bShouldInitGameStatsVisualizer && bIsGameStatsPane) ||
			 (bShouldInitTaskBrowser && bIsTaskBrowserPane) ||
			 (!bIsGenericBrowserPane && !bIsContentBrowserPane && !bIsGameStatsPane && !bIsTaskBrowserPane) )
		{
