...
void CookSimpleWave( USoundNodeWave* SoundNodeWave, FConsoleSoundCooker* SoundCooker, FByteBulkData& DestinationData )
{
	...
	if( SoundNodeWave->RawData.GetBulkDataSize() > 0 )
	{
		...
		{
			warnf( TEXT( "Only mono or stereo 16 bit waves allowed: %s" ), *SoundNodeWave->GetFullName() );
		}
		else
		{
	...
	else if( SoundNodeWave->CompressedPCData.GetBulkDataSize() > 0 )
	{
...
UBOOL CookSoundNodeWave( USoundNodeWave* SoundNodeWave, UE3::EPlatformType Platform )
{
	...
	if( Platform == UE3::PLATFORM_Unknown || Platform == UE3::PLATFORM_Windows || Platform == UE3::PLATFORM_WindowsConsole)
	{
