...
void FCategoryPropertyNode::InitChildNodes(void)
{
	...
	const UBOOL bShowDMCOnlyProperties = TopPropertyWindow->HasFlags(EPropertyWindowFlags::ShowDMCOnlyProperties);

	// Get a list of properties that are in the same category
	for( TFieldIterator<UProperty> It(itemobj->GetObjectBaseClass()); It; ++It )
	{
		UBOOL bDMCOnly = It->GetBoolMetaData(TEXT("DMCOnly"));
		UBOOL bDMCOnlyTest = !bDMCOnly || bShowDMCOnlyProperties; // Show the property if it is either NOT 'DMC only', OR we are showing 'DMC only' properties
		...
		if (bMetaDataAllowVisible)
		{
			...
			if ( (bShouldShowNonEditable && CategoryName == NAME_None && It->Category == NAME_None) ||
				(It->Category == CategoryName && (bShouldShowNonEditable || ((It->PropertyFlags & CPF_Edit) && bDMCOnlyTest))) )
			{
