	...
	else if (ParseCommand(&Str, TEXT("UpdateAddLandscapeComponents")))
	{
		if (GWorld)
		{
			for (FActorIterator It; It; ++It)
			{
				...
				if (Landscape)
				{
					Landscape->UpdateAllAddCollisions();
				}
			}
		}
	...
	if ( bTimeToAutosave )
	{
		...
		if( bCanAutosave )
		{
			...
			if ( !IsUserInteracting() )
			{
				...
				{
					bAssetsSaved = FEditorFileUtils::AutosaveContentPackages( AbsoluteAutoSaveDir, NewAutoSaveIndex );
				}
...
static void MirrorActors(const FVector& MirrorScale)
{
	...
	for ( FSelectionIterator It( GEditor->GetSelectedActorIterator() ) ; It ; ++It )
	{
		...
		{
			Actor->Modify();
			Actor->EditorApplyMirror( MirrorScale, GEditorModeTools().PivotLocation );
		}
...
UBOOL UUnrealEdEngine::Exec_Actor( const TCHAR* Str, FOutputDevice& Ar )
{
	...
	{
		GApp->EditorFrame->SyncMaterialToGenericBrowser();
		return TRUE;
	}
	...
	else if(ParseCommand(&Str, TEXT("EXPORT")))
	{
		...
		if(ParseCommand(&Str, TEXT("FBX")))
		{
			...
			if( ExportFileDialog.ShowModal() == wxID_OK )
			{
				...
				UnFbx::FbxExporter* Exporter = UnFbx::FbxExporter::GetInstance();
				...
				for ( FSelectionIterator It( GetSelectedActorIterator() ) ; It ; ++It )
				{
					...
					if (Actor->IsA(AActor::StaticClass()))
					{
						...
						{
							Exporter->ExportBrush((ABrush*)Actor, TRUE);
						}
					}
				}
