...
/*------------------------------------------------------------------------------
    Base class.
------------------------------------------------------------------------------*/
extern const FLOAT MOUSE_CLICK_DRAG_DELTA	= 4.0f;
...
UBOOL FEdModeCoverEdit::InputDelta(FEditorLevelViewportClient* InViewportClient,FViewport* InViewport,FVector& InDrag,FRotator& InRot,FVector& InScale)
{
	...
	for( INT LinkIdx = 0; LinkIdx < SelectedLinks.Num(); LinkIdx++ )
	{
		...
		for( INT SlotIdx = 0; SlotIdx < Link->Slots.Num(); SlotIdx++ )
		{
			...
			// If TAB is being held down, always move the coverlink, regardless if any slots are selected or not.
			if( bTabDown )
			{
	...
	if (!ViewportClient->IsOrtho())
	{
		...
		for (INT LinkIdx = 0; LinkIdx < SelectedLinks.Num(); LinkIdx++)
		{
			...
			for (INT SlotIdx = 0; SlotIdx < Link->Slots.Num(); SlotIdx++)
			{
				...
				{
					DrawStringCentered(Canvas,DrawLocation.X,DrawLocation.Y,*FString::Printf(TEXT("%d"),SlotIdx),GEngine->GetMediumFont(),FColor(200,200,200));
				}
			}
		}
	}
...
void FEditorModeTools::SetBookMark( INT InIndex, FEditorLevelViewportClient* InViewportClient )
{
	...
	Info->BookMarks[ InIndex ]->HiddenLevels.Empty();
	for ( INT LevelIndex = 0 ; LevelIndex < Info->StreamingLevels.Num() ; ++LevelIndex )
	{
		ULevelStreaming* StreamingLevel = Info->StreamingLevels(LevelIndex);
		if ( StreamingLevel )
		{
			if( !StreamingLevel->bShouldBeVisibleInEditor )
			{
				Info->BookMarks[ InIndex ]->HiddenLevels.AddItem( StreamingLevel->GetFullName() );
			}
		}
	}
}
...
void FEditorModeTools::JumpToBookMark( INT InIndex, UBOOL bShouldRestoreLevelVisibility )
{
	...
	if( bShouldRestoreLevelVisibility )
	{
		for ( INT LevelIndex = 0 ; LevelIndex < Info->StreamingLevels.Num() ; ++LevelIndex )
		{
			...
			if ( StreamingLevel )
			{
				StreamingLevel->bShouldBeVisibleInEditor = !Info->BookMarks[ InIndex ]->HiddenLevels.ContainsItem( StreamingLevel->GetFullName() );
