...
void FLightmassExporter::WriteVisibilityData( INT Channel )
{
	...
	{
		GWarn->LightingBuild_Add(MCTYPE_CRITICALERROR, NULL, 
			*Localize(TEXT("Lightmass"), TEXT("LightmassError_MissingPrecomputedVisibilityVolume"), TEXT("UnrealEd")));
	}
		...
		if (Material)
		{
			...
			if ( ErrorCode != NSwarm::SWARM_SUCCESS )
			{
				if( ErrorCode == NSwarm::SWARM_ERROR_FILE_FOUND_NOT )
				{
					...
					UMaterial* BaseMaterial = Material->GetMaterial(MSP_BASE);
					...
					const UBOOL bWantNormals = LevelSettings.bUseNormalMapsForSimpleLightMaps;
				...
				if (SMLightingMesh->LODIndex < StaticMesh->LODModels.Num())
				{
					...
					for (INT ElementIndex = 0; ElementIndex < LODRenderData.Elements.Num(); ElementIndex++)
					{
						...
						if (FALSE)
						{
							...
							{
								GWarn->LightingBuild_Add(MCTYPE_WARNING, NULL, 
									*Localize(TEXT("Lightmass"), TEXT("LightmassError_EmissiveFracturedMesh"), TEXT("UnrealEd")));
...
void FLightmassExporter::WriteLandscapeInstances( INT Channel )
{
	...
	for (INT LandscapeIdx = 0; LandscapeIdx < LandscapeLightingMeshes.Num(); LandscapeIdx++)
	{
		...
		if (LandscapeComp)
		{
			...
			Lightmass::FMaterialElementData NewElementData;
			Copy(Material->LightingGuid, NewElementData.MaterialId);
			NewElementData.bUseTwoSidedLighting = LandscapeComp->GetLandscapeActor()->LightmassSettings.bUseTwoSidedLighting;
			NewElementData.bShadowIndirectOnly = LandscapeComp->GetLandscapeActor()->LightmassSettings.bShadowIndirectOnly;
			NewElementData.bUseEmissiveForStaticLighting = LandscapeComp->GetLandscapeActor()->LightmassSettings.bUseEmissiveForStaticLighting;
			// Combine primitive and level boost settings so we don't have to send the level settings over to Lightmass  
			Copy(LandscapeComp->GetLandscapeActor()->LightmassSettings.EmissiveLightFalloffExponent, NewElementData.EmissiveLightFalloffExponent);
			Copy(LandscapeComp->GetLandscapeActor()->LightmassSettings.EmissiveLightExplicitInfluenceRadius, NewElementData.EmissiveLightExplicitInfluenceRadius);
			...
			Copy(LandscapeComp->GetLandscapeActor()->LightmassSettings.FullyOccludedSamplesFraction, NewElementData.FullyOccludedSamplesFraction);
			MaterialElementData.AddItem(NewElementData);
		}
		...
		LandscapeInstanceMeshData.StaticLightingResolution = LandscapeComp->GetLandscapeActor()->StaticLightingResolution;
		INT DesiredSize = 1;
		LandscapeInstanceMeshData.LightMapRatio = 
			::GetTerrainExpandPatchCount(LandscapeComp->GetLandscapeActor()->StaticLightingResolution, LandscapeInstanceMeshData.ExpandQuadsX, LandscapeInstanceMeshData.ExpandQuadsY, LandscapeComp->ComponentSizeQuads, DesiredSize);
...
void FLightmassExporter::WriteSceneSettings( Lightmass::FSceneFileHeader& Scene )
{
	...
	}
	{
		...
		Scene.SceneConstants.bSimpleLightmapsStoredInLinearSpace = GWorld->GetWorldInfo()->bSimpleLightmapsStoredInLinearSpace;
		Scene.SceneConstants.bUseFixedScaleForSimpleLightmaps = TRUE;
	...
	}
	{
		...
		Scene.MaterialSettings.bUseNormalMapsForSimpleLightMaps = LevelSettings.bUseNormalMapsForSimpleLightMaps;
	}
	{
 ...
 */
FLightmassProcessor::FLightmassProcessor(const FStaticLightingSystem& InSystem, UBOOL bInDumpBinaryResults, UBOOL bInOnlyBuildVisibility)
:	Exporter(NULL)
,	Importer(NULL)
,	System(InSystem)
,	Swarm( NSwarm::FSwarmInterface::Get() )
...
void FLightmassProcessor::ImportVolumeSamples()
{
	if (VolumeSampleTaskCompleted > 0)
	{
		...
		if (Channel >= 0)
		{
			...
			// With all of the latest volumes computed and assigned, reset the warning
			// which detects DLEs outside of these volumes
			GWorld->GetWorldInfo()->bMapHasDLEsOutsideOfImportanceVolume = FALSE;
		...
		if (DominantLightComponent)
		{
			...
			if (Channel >= 0)
			{
				...
				if (DirectionalComponent)
				{
					...
					GWorld->DominantDirectionalLight->Initialize(DominantLightShadowInfo, DominantLightShadowMap);
				}
				else
				{
					UDominantSpotLightComponent* SpotComponent = Cast<UDominantSpotLightComponent>(DominantLightComponent);
					check(SpotComponent);
					SpotComponent->Initialize(DominantLightShadowInfo, DominantLightShadowMap);
				}
			}
			else
			{
	...
	if (Channel >= 0)
	{
		...
		while (MappingsImported != NumMappings)
		{
			...
			else
			{
				...
				GWarn->LightingBuild_Add(MCTYPE_CRITICALERROR, Object, TEXT("Lighting build encountered an error, lighting will likely be incorrect and a rebuild required"));
...
UBOOL FLightmassProcessor::ImportVertexMapping(INT Channel, FVertexMappingImportHelper& VMImport)
{
	...
	for (INT i = 0; i < NumLights; i++)
	{
		...
		Swarm.ReadChannel(Channel, &LightGuids(NewLightIndex), sizeof(FGuid));
	}
