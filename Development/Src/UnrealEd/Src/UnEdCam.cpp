...
void UUnrealEdEngine::SetPivot( FVector NewPivot, UBOOL bSnapPivotToGrid, UBOOL bIgnoreAxis, UBOOL bAssignPivot/*=FALSE*/ )
{
	...
	if( bAssignPivot && LastSelectedActor && GEditor->bGroupingActive ) 
	{
		...
		if(ActorGroupRoot)
		{
			ActorGroupRoot->SetLocation( GEditor->ClickLocation );
		}
	}
...
void UUnrealEdEngine::NoteSelectionChange()
{
	...
	{
		// set pivot point to the actor's top most selected group pivot
		AGroupActor* ActorGroupRoot = AGroupActor::GetRootForActor(SingleActor, FALSE, TRUE);
		FVector PivotPoint = SingleActor->Location;
		if(ActorGroupRoot && ActorGroupRoot->IsLocked() && GEditor->bGroupingActive)
		{
...
/*
 * @param	InGroupActor		Group to Select
 * @param	bForceSelection	Flag to select the group even if it's not currently locked. Defaults to false
 */
void UUnrealEdEngine::SelectGroup(AGroupActor* InGroupActor, UBOOL bForceSelection/*=FALSE*/)
{
	...
	{	
		...
		{                  
			SelectActor(GroupActors(ActorIndex), TRUE, NULL, TRUE );
		}
		...
		{
			SelectGroup(SubGroups(GroupIndex), bForceSelection);
		}
	}
}
...
void UUnrealEdEngine::SelectActor(AActor* Actor, UBOOL bInSelected, FViewportClient* InViewportClient, UBOOL bNotify, UBOOL bSelectEvenIfHidden)
{
	...
	if( !bSelectionHandled )
	{
		...
		if(bInSelected && GEditor->bGroupingActive && !bIteratingOverGroups)
		{
			// if this actor is a group, do a group select
			bIteratingOverGroups = TRUE;
			if( SelectedGroupActor )
			{
				SelectGroup(SelectedGroupActor , TRUE );
			}
			else
			{
				...
				{
					SelectGroup(ActorLockedRootGroup);
				}
			}
...
void UUnrealEdEngine::SelectNone(UBOOL bNoteSelectionChange, UBOOL bDeselectBSPSurfs)
{
	...
	INT NumActors = ActorsToDeselect.Num();
	if( NumActors >= EditorActorSelectionDefs::MaxActorsToSelectBeforeWarning )
	{
		FString ConfirmText = FString::Printf( LocalizeSecure( LocalizeUnrealEd( "Warning_ManyActorsForDeselect" ), NumActors ) );
		if( wxNO == wxMessageBox( wxString( *ConfirmText ), wxString( *LocalizeUnrealEd( "Warning_ManyActors" ) ), wxYES_NO ) )
		{
			return;
		}
	}
