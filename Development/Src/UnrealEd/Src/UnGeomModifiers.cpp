...
void UGeomModifier_Pen::Render(const FSceneView* View,FViewport* Viewport,FPrimitiveDrawInterface* PDI)
{
	...
	GWorld->LineBatcher->DrawBox( FBox::BuildAABB( MouseWorldSpacePos, FVector(BoxSz,BoxSz,BoxSz) ), FMatrix::Identity, FLinearColor(1,1,1), SDPG_UnrealEdForeground );
}
...
void UGeomModifier_Clip::Render(const FSceneView* View,FViewport* Viewport,FPrimitiveDrawInterface* PDI)
{
	...
	INT BoxSz = Max( GEditor->Constraints.GetGridSize() / 2, 1.f );
	GWorld->LineBatcher->DrawBox( FBox::BuildAABB( SnappedMouseWorldSpacePos, FVector(BoxSz,BoxSz,BoxSz) ), FMatrix::Identity, FLinearColor(1,1,1), SDPG_UnrealEdForeground );
}
