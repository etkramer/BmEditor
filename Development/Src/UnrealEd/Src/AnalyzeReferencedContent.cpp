...
{
	return TEXT("ResourceType,ResourceName,NumInstances,NumTriangles,NumSections,NumSectionsWithCollision,UsesSimpleRigidBodyCollision,ConvexCollisionPrims,NumMapsUsedIn,ResourceSize,ScalesUsed,NonUniformScale,ShouldConvertBoxColl,bIsReferencedByParticles") LINE_TERMINATOR;
...
FString FAnalyzeReferencedContentStat::FStaticMeshStats::ToSummaryCSV(const FString& LevelName, const TMap<FString,FStaticMeshStats>& StatsData)
{
	...
	return FString::Printf(TEXT("%s,%i,%i,%0.2f,%0.2f,%0.2f,%0.2f,%i,%0.2f,%0.2f,%0.2f,%i,%i,%i,%i%s"), 
		*LevelName, (INT)TotalCount, (INT)ResultNumInstances, AvgNumTriangles, ResultNumTriangles, ResultNumSections, ResultNumElementsWithCollision, (INT)(ResultUsesSimpleRigidBodyCollision*100), 
		...
		(INT)(ResultIsReferencedByParticles*100), LINE_TERMINATOR);
}
	...
	if( SpeedTreeComponent->SpeedTree )
	{
		...
		if ( SRH )
		{
			if( SpeedTreeComponent->bUseBranches && SRH->bHasBranches )
			{
				...
				{
					NumSections++;
					NumTriangles += SRH->Branch1Elements(0).NumPrimitives;	
				}
				if (SRH->Branch2Elements.Num() > 0)
				{
					NumSections++;
					NumTriangles += SRH->Branch2Elements(0).NumPrimitives;	
				}
			}
			if( SpeedTreeComponent->bUseFronds && SRH->bHasFronds )
			{
				NumSections++;
				NumTriangles += SRH->FrondElements(0).NumPrimitives;	
			}
			if( SpeedTreeComponent->bUseLeafCards && SRH->bHasLeafCards && SRH->LeafCardElements.Num() )
			{
				NumTriangles += SRH->LeafCardElements(0).NumPrimitives;	
				NumSections++;
			}
			if( SpeedTreeComponent->bUseLeafMeshes && SRH->bHasLeafMeshes && SRH->LeafMeshElements.Num() )
			{
				NumTriangles += SRH->LeafMeshElements(0).NumPrimitives;	
				NumSections++;
			}
		}
	}
...
,	BlendMode(EBlendMode(Material->BlendMode))
{
	...
	Material->MaterialResources[MSP_SM3]->GetRepresentativeInstructionCounts(ShaderDescriptions, InstructionCounts);
