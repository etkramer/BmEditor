...
public:
	...
	virtual UBOOL IsChildWindowVisible(const INT InWindowIndex) const
	{
		...
		if( TestLevel != NULL )
		{
			...
			{
				FString TestString = FilterStrings(i);
				//has a checked key word filter
				if (StreamingLevel->Keywords.ContainsItem(TestString))
				{
					return TRUE;
				}
				//the user requested "none" to be included and this doesn't have any keywords
				if ((TestString == TEXT("None")) && (StreamingLevel->Keywords.Num()==0))
				{
					return TRUE;
				}
			}
...
void WxLevelBrowser::Send(ECallbackEventType Event)
{
	...
	else if ( Event == CALLBACK_LevelDirtied )
	{
 ...
 */
void WxLevelBrowser::MergeVisibleLevels( UBOOL bDiscardHiddenLevels )
{
...
void WxLevelBrowser::ShiftSingleSelectedLevel(UBOOL bUp)
{
	...
	{
		// Copy the level browser item since it may be deleted during the shift/update
		FLevelBrowserItem LevelItemToReselect = *SingleSelectedLevel;
		...
		if ( bShiftOccurred )
		{
			...
			SelectLevelItem( LevelItemToReselect );
		}
	}
}
	...
	if( bShouldRemoveLevels)
	{
		...
		for( INT CurLevelIndex = 0; CurLevelIndex < LevelsToRemove.Num(); ++CurLevelIndex )
		{
			...
			if( !FLevelUtils::IsLevelLocked(CurLevel) )
			{
				...
				{
					GEditor->SelectActor( CurLevel->Actors( ActorIdx ), FALSE, NULL, FALSE );
				}
			}
...
void WxLevelBrowser::OnToggleSelfContainedLighting(wxCommandEvent& In)
{
	...
	if ( SingleSelectedLevel != NULL && SingleSelectedLevel->IsLevel() )
	{
		...
		GWorld->ModifyLevel( SingleSelectedLevel->GetLevel() );
		SingleSelectedLevel->GetLevel()->MarkPackageDirty();

		const UBOOL bHasSelfContainedLighting = FLevelUtils::HasSelfContainedLighting( SingleSelectedLevel->GetLevel() );
		FLevelUtils::SetSelfContainedLighting( SingleSelectedLevel->GetLevel(), !bHasSelfContainedLighting );
		SelectSingleLevelItem( *SingleSelectedLevel );
		if( FLevelUtils::IsLevelVisible( SingleSelectedLevel->GetLevel() ) )
		{
			SingleSelectedLevel->GetLevel()->UpdateComponents();
		}
	}
}
