...
BEGIN_EVENT_TABLE( WxActorBrowser, WxBrowser )
	...
	EVT_MENU( IDMN_ActorBrowser_CreateDMC, WxActorBrowser::OnCreateDMC )
	EVT_MENU( IDMN_ActorBrowser_OpenDMC,  WxActorBrowser::OnOpenDMCTool )
	...
	:	bUpdateOnActivated( FALSE )
{
	...
	GCallbackEvent->Register(CALLBACK_DMCLoaded,this);

	RightClickedClass = NULL;
	RightClickedDMC = NULL;
}
...
void WxActorBrowser::Send(ECallbackEventType InType)
{
	...
	else if(InType == CALLBACK_DMCLoaded)
	{
		debugf(TEXT("Need to pass argument to CALLBACK_DMCLoaded callback!"));
	}
}
...
void WxActorBrowser::Send(ECallbackEventType InType, UObject* InObject)
{
	...
	{
		UDMC_Prototype* DMC = CastChecked<UDMC_Prototype>(InObject);
		// Add to tree view
		GEditor->EditorClassHierarchy->AddDMCProto(DMC);
		RepopulateClassTree();
	}
}
...
void WxActorBrowser::OnItemRightClicked( wxCommandEvent& In )
{
	...
	{	
		...
		if( TreeItemData )
		{
			...
			RightClickedDMC = EditorClassHierarchy->GetDMCProto(ClassIndex);
			...
			const UBOOL bHasDMC = (RightClickedDMC != NULL);
			const UBOOL bIsDMCBase = (RightClickedClass == ADMC_Base::StaticClass());

			class WxMBActorBrowserContext : public wxMenu
			{
			public:
				WxMBActorBrowserContext(UBOOL bEnableCreateArchetypeOption, UBOOL bHasDMC, UBOOL bIsDMCBase)
				{
					if ( bEnableCreateArchetypeOption )
					{
						Append(IDMN_ActorBrowser_CreateArchetype,*LocalizeUnrealEd("Archetype_Create"),TEXT(""));

						if(GEngine->HACK_EnableDMC)
						{
							if(bIsDMCBase)
							{
								Append(IDMN_ActorBrowser_CreateDMC,TEXT("Create DMC..."),TEXT(""));
							}

							// If this is a DMC proto, allow you to open the tool
							if(bHasDMC)
							{
								Append(IDMN_ActorBrowser_OpenDMC,TEXT("Open DMCTool"),TEXT(""));
							}
						}
					}
				};
			} Menu( bCanCreateArchetype, bHasDMC, bIsDMCBase);
...
/** Create DMC */
void WxActorBrowser::OnCreateDMC( wxCommandEvent& In )
{
	if( RightClickedClass == ADMC_Base::StaticClass() )
	{
		...
		if( PackageDlg.ShowModal(EmptyName, EmptyName, EmptyName) == wxID_OK )
		{
			...
			FString ClassName = PackageDlg.GetObjectName();

			// Create new DMC_Prototype object
			UDMC_Prototype* NewProto = ConstructObject<UDMC_Prototype>(UDMC_Prototype::StaticClass(), Package, *ClassName, RF_Public|RF_Standalone|RF_Transactional);
			NewProto->ParentClass = RightClickedClass;

			// Create initial UClass
			UClass* NewClass = WxDMC::GenerateClassFromDMCProto( NewProto,  FString(TEXT("")) );
			if(NewClass != NULL)
			{
				// rename the new one to '..._C', as this is now the 'newest' version of this class
				FString GoodName = FString::Printf(TEXT("%s_C"), *NewProto->GetName());
				NewClass->Rename(*GoodName, NewProto->GetOutermost());
				NewProto->GeneratedClass = NewClass;
			}

			GEditor->EditorClassHierarchy->AddDMCProto(NewProto);
			RepopulateClassTree();

			// tell the browser
			GCallbackEvent->Send(FCallbackEventParameters(NULL, CALLBACK_RefreshContentBrowser, CBR_ObjectCreated, NewProto));
		}
	}
}

void WxActorBrowser::OnOpenDMCTool( wxCommandEvent& In )
{
	if( RightClickedDMC )
	{
		// Open the AnimSet viewer with the new AnimSet and the selected mesh
		WxDMC* DMCTool = new WxDMC( GApp->EditorFrame, -1, RightClickedDMC );
		DMCTool->Show();
	}
}
