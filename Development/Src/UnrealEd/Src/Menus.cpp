	...
	if (FConsoleSupportContainer::GetConsoleSupportContainer()->GetNumConsoleSupports() > 0)
	{
		...
		for (FConsoleSupportIterator It; It && ConsoleIndex < 20; ++It, ConsoleIndex++)
		{
			...
			if( ( ConsoleName == CONSOLESUPPORT_NAME_ANDROID ||
				ConsoleName == CONSOLESUPPORT_NAME_IPHONE ) && !GEngine->bEmulateMobileRendering )
			{
...
WxMainContextMenu::WxMainContextMenu()
{
...
#if WITH_FBX
	// Add the export option if a static mesh actor, interp actor or brush is selected
	if( bHaveStaticMesh || bHaveDynamicStaticMesh || bHaveBrush )
	{
		AppendSeparatorIfNeeded();
	...
	if(NumSelectedActors > 0)
	{
		...
		if( NumSelectedActors == 1 )
		{
			...
			{
				LODMenu->AppendSeparator();
				LODMenu->Append( IDMENU_ActorPopup_SimplifyMesh, *LocalizeUnrealEd( "ActorProperties_SimplifyMesh" ), TEXT( "" ) );
			}
			else if( bHaveSimplifiedStaticMesh )
			{
				LODMenu->AppendSeparator();
				LODMenu->Append( IDMENU_ActorPopup_SimplifyMesh, *LocalizeUnrealEd( "ActorProperties_ResimplifyMesh" ), TEXT( "" ) );
			}
		}
...
WxMainContextSurfaceMenu::WxMainContextSurfaceMenu()
{
	...
	// Find
	{
		...
		Append( ID_SyncGenericBrowser, *LocalizeUnrealEd("LevelViewportContext_SyncGenericBrowser"), TEXT(""), 0 );
	}
