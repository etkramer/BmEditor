...
struct EditorClassEntry
{
	...
	/** Pointer to DMC_Prototype object, if this entry is a DMC, instead of a normal .uc-file generate class */
	UDMC_Prototype* DMCProto;
};
...
/** Add a new DMC_Prototype to the actor browser */
void FEditorClassHierarchy::AddDMCProto(UDMC_Prototype* InProto)
{
	// Needs a paret class to add
	if(InProto->ParentClass == NULL)
	{
		debugf(TEXT("FEditorClassHierarchy::AddDMCProto : No ParentClass"));
		return;
	}

	// Create new entry struct
	EditorClassEntry ClassEntry;
	appMemzero(&ClassEntry, sizeof(EditorClassEntry));

	// Fill in class and package name
	ClassEntry.ClassName = InProto->GetName(); // we show the DMC name, rather than the actual class name
	ClassEntry.PackageName = InProto->GetOutermost()->GetName();

	FString ParentClassName = InProto->ParentClass->GetName();
	ClassEntry.ParentClassIndex = Find(ParentClassName);
	if(ClassEntry.ParentClassIndex != -1)
	{
		ClassEntry.NumChildren = 0;
		ClassEntry.NumPlaceableChildren = 0;

		ClassEntry.bIsFactory = FALSE;
		ClassEntry.bIsHidden = FALSE;
		ClassEntry.bIsPlaceable = TRUE;
		ClassEntry.bIsAbstract = FALSE;

		// Do not cache class pointer directly
		ClassEntry.DMCProto = InProto;
		ClassEntry.Class = NULL;
...
/** Get the DMC_Prototype object that corresponds to the index in the class hierarchy  */
UDMC_Prototype* FEditorClassHierarchy::GetDMCProto(INT InClassIndex)
{
	check(IsWithin(InClassIndex, 0, EditorClassArray.Num()));
	return EditorClassArray(InClassIndex).DMCProto;
}
...
UClass* FEditorClassHierarchy::GetClass(const INT InClassIndex)
{
	...
	{
		// handle DMC case first - look up class
		if(EditorClassArray(InClassIndex).DMCProto != NULL)
		{
			return EditorClassArray(InClassIndex).DMCProto->GeneratedClass;
		}
