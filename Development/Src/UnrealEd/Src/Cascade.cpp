...
void WxCascade::OnRemoveDuplicateModules( wxCommandEvent& In )
{
	if (PartSys != NULL)
	{
		...
		PartSys->RemoveAllDuplicateModules(FALSE);
...
void WxCascade::SetSelectedModule( UParticleEmitter* NewSelectedEmitter, UParticleModule* NewSelectedModule )
{
	...
	PropertyWindow->SetObject(PropObj, EPropertyWindowFlags::ShouldShowCategories | EPropertyWindowFlags::Sorted);
	PropertyWindow->SetFlags(EPropertyWindowFlags::ReadOnly, bReadOnly);
