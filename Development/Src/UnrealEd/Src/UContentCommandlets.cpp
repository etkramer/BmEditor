...
INT UConformCommandlet::Main( const FString& Params )
{
	...
	FString NameOfPackageToBeConformed, NameOfPackageToConformAgainst;
	if( !ParseToken(Parms,NameOfPackageToBeConformed,0) )
	{
		warnf(NAME_Error, TEXT("Source file not specified"));
		return 1;
	}
	if( !ParseToken(Parms,NameOfPackageToConformAgainst,0) )
	{
		warnf(NAME_Error, (TEXT("Original file not specified")));
		return 1;
	}

	// get the full pathname of the package we're conforming
	FString SrcFilename;
	GPackageFileCache->FindPackageFile(*NameOfPackageToBeConformed, NULL, SrcFilename);

	// if it's read only, don't bother
	if (GFileManager->IsReadOnly(*SrcFilename))
	{
		warnf(NAME_Warning, TEXT("Package '%s' is read only"), *SrcFilename);
		return 0;
	}

	GWarn->Log( TEXT("Loading...") );
	BeginLoad();
	FFilename OldFilename = FFilename(NameOfPackageToConformAgainst);
	UPackage* OldPackage = CreatePackage( NULL, *(OldFilename.GetBaseFilename() + TEXT("_OLD")) );
	ULinkerLoad* OldLinker = UObject::GetPackageLinker( OldPackage, *NameOfPackageToConformAgainst, LOAD_NoWarn|LOAD_NoVerify, NULL, NULL );
	EndLoad();

	UPackage* NewPackage = LoadPackage( NULL, *NameOfPackageToBeConformed, LOAD_None );
	
	if( !OldLinker )
	{
		warnf(NAME_Error, TEXT("Failed to load original file '%s'"), *NameOfPackageToConformAgainst);
		return 1;
	}
	if( !NewPackage )
	{
		warnf(NAME_Error, TEXT("Failed to load file being conformed '%s'"), *NameOfPackageToBeConformed);
		return 1;
	}

	GWarn->Log( TEXT("Saving...") );
	if ( SavePackageHelper(NewPackage, SrcFilename, GWarn, OldLinker) )
	{
		warnf( TEXT("File %s successfully conformed to %s..."), *NameOfPackageToBeConformed, *NameOfPackageToConformAgainst );
	}
	else
	{
		warnf(TEXT("File %s could not be conformed to %s.  Please see the log for details."), *NameOfPackageToBeConformed, *NameOfPackageToConformAgainst);
	}
...
INT UAnalyzeKismetCommandlet::Main( const FString& Params )
{
	...
	for( INT SwitchIndex = 0; SwitchIndex < Switches.Num(); ++SwitchIndex )
	{
		...
		else if( Switches(SwitchIndex).Left(16).ToLower() == TEXT("showunreferenced") )
		{
			...
			appGetEngineScriptPackageNames(AllScriptPackageNames, !GUseSeekFreeLoading);
			appGetGameNativeScriptPackageNames(AllScriptPackageNames, !GUseSeekFreeLoading);
...
UBOOL UTagCookedReferencedAssetsCommandlet::Initialize(const FString& Params)
{
	...
	if (appGetCookedContentPath(Platform, CookedDataPath) == FALSE)
	{
...
void UTagCookedReferencedAssetsCommandlet::UpdateCollections()
{
	...
	if (AssetsToAdd.Num() > 0)
	{
		...
		{
			// Just log them out
			debugf(TEXT("Found %d assets to add:"), AssetsToAdd.Num());
...
INT UTagReferencedAssetsCommandlet::Main( const FString & Params )
{
	...
	for( INT FileIndex = 0; FileIndex < FilesInPath.Num(); FileIndex++ )
	{
		...
		{
			warnf( NAME_Log, TEXT( "Loading %s" ), *Filename );
			LoadPackage( NULL, *Filename, LOAD_NoVerify );
			for( TObjectIterator<UObject> It; It; ++It )
			{
				ReferencedAssets.Add( It->GetFullName() );
			}
			UObject::CollectGarbage( RF_Native );
		}
	}
	...
	else
	{
		...
		if (bShouldLoadAllMaps)
		{
			...
			for (INT PackageIndex = 0; PackageIndex < AllPackages.Num(); PackageIndex++)
			{
				...
				if (PackageName.GetExtension() == FURL::DefaultMapExt)
				{
...
INT UListLoopingEmittersCommandlet::Main(const FString& Params)
{
	...
	TArray<FString> PackageNames;
	appGetGameScriptPackageNames( PackageNames, FALSE );
	...
	else
	{
		...
		for( TObjectIterator<UClass> It; It; ++It )
		{
			...
			for( TFieldIterator<UProperty> It(Class,FALSE); It; ++It)
			{
				...
				if ( Property->HasAnyPropertyFlags( CPF_Edit ) && !Class->HideCategories.ContainsItem( Property->Category ))
				{
					...
					//Keep track of all of the unique members of any structs that we have in the class
					//so that we can process them later
					const UStructProperty* StructProp = ConstCast<UStructProperty>(Property);
					if (StructProp)
					{
						for( TFieldIterator<UProperty> It(StructProp->Struct); It; ++It )
						{
							UProperty* StructMember = *It;
							StructMembers.AddUniqueItem( StructMember);
						}
					}
				}
			}
		...
		for( INT x = 0 ; x < StructMembers.Num() ; ++x )
		{
			UProperty* StructMember = StructMembers(x);
			DumpLine = FString::Printf( TEXT("[%s]%s"), *(StructMember->GetFullGroupName(TRUE)), LINE_TERMINATOR);
			DumpFile->Serialize(const_cast<TCHAR*>(*DumpLine), DumpLine.Len()*sizeof(TCHAR));
