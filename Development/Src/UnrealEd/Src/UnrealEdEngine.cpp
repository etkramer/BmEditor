...
void UUnrealEdEngine::Init()
{
	...
	if ( !GIsUCCMake && FContentBrowser::ShouldUseContentBrowser() )
	{
		...
		if ( !GConfig->GetString(TEXT("Caption"), TEXT("GenericBrowser"), GBString, *LocFilenames(0)) )
		{
...
void UUnrealEdEngine::Tick(FLOAT DeltaSeconds)
{
	...
	{
		LevelBrowser->ProcessUpdatesToDeferredActors();
	}
	...
	case CALLBACK_PackageModified:
		{
			...
			else
			{
				...
				if( !bIsAutoSaving && 
					!bIsLoadingMap && // Don't ask if the package was modified as a result of a load
