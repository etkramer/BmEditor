...
UBOOL ConditionalCacheATITCTextures(UTexture2D* Texture, UBOOL bUseFastCompression, UBOOL bForceCompression, TArray<FColor>* SourceArtOverride)
{
	...
	if (!bSourceArtIsValid)
	{
		...
		// black out texture on an error
		if (ReturnCode != 0)
		{
			warnf(TEXT(">>>>>>>>>>>>> %s failed to convert from DDS to RGBA8!"), *Texture->GetFullName());

			appMemzero(RawData.GetData(), DDSHeader.dwWidth * DDSHeader.dwHeight * 4);
		}
		else
		{
			TArray<BYTE> Data;
	...
	TGA.ImageTypeCode = 2;
	TGA.BitsPerPixel = 32;
	TGA.Height[0] = FirstMipSizeSquare & 0xFF;
	TGA.Height[1] = FirstMipSizeSquare >> 8;
	TGA.Width[0] = FirstMipSizeSquare & 0xFF;
	TGA.Width[1] = FirstMipSizeSquare >> 8;
	...
	FArchive* TGAFile = GFileManager->CreateFileWriter(*InputFilePath);
	...
	for (UINT Y=0; Y < FirstMipSizeSquare;Y++)
	{
		// If we aren't skipping alpha channels we can serialize each line
		TGAFile->Serialize(&RawData((FirstMipSizeSquare - Y - 1) * FirstMipSizeSquare), FirstMipSizeSquare * 4);
	}
