...
void WxInterpEd::DeleteSelectedTracks()
{
	...
	for( FSelectedTrackIterator TrackIt(GetSelectedTrackIterator()); TrackIt; ++TrackIt )
	{
		...
		if( ActiveTrack->GetOuter()->IsA( UInterpGroup::StaticClass() ) )
		{
			...
			AActor* GroupActor = Interp->FindFirstGroupInst(Group)->GetGroupActor();

			if(GroupActor)
			{
				DisableCameraPostProcessFlags(ActiveTrack, GroupActor);
			}
...
void WxInterpEd::AddTrackToCurveEd( FString GroupName, FColor GroupColor, UInterpTrack* InTrack )
{
	...
	if( IData->CurveEdSetup->ShowingCurve(InTrack) )
	{
...
void WxInterpEd::UpdateLevelViewport(AActor* InActor, FEditorLevelViewportClient* InViewportClient, const FRenderingPerformanceOverrides& RenderingOverrides, const FLOAT InFadeAmount, const FVector& InColorScale, const UBOOL bInEnableColorScaling )
{
	...
	if(Cam)
	{
		...
		if ((InViewportClient != GetRecordingViewport()) || (Interp && ( Interp->bIsPlaying || (TrackWindow && TrackWindow->InterpEdVC->bGrabbingHandle ) ) ))
		{
	...
	if(Opt->bAdjustingKeyframe)
	{
        ...
        if(bTrackActorModified)
        {
			...
			if( Parent )
			{
				...
				UInterpTrackInst* TrInst = GrInst->TrackInst( SelKey.Group->InterpTracks.FindItemIndex( Parent ) );
				Parent->UpdateChildKeyframe( SelKey.Track, SelKey.KeyIndex, TrInst );
			}
			else
			{
				// This track is a normal track parented to a group
				UInterpTrackInst* TrInst = GrInst->TrackInst( SelKey.Group->InterpTracks.FindItemIndex( SelKey.Track ) );
 ...
 *
 * @param	Track		The interp track to check to see if it references a post process setting.
 * @param	GroupActor	The group actor for the interp track.
 */
void WxInterpEd::DisableCameraPostProcessFlags( UInterpTrack* Track, AActor* GroupActor )
{
	check( Track && GroupActor );

	if( GroupActor->IsA(ACameraActor::StaticClass()) )
	{
		FString PropertyString;
		UBOOL bFoundPropertyName = FALSE;

		if( Track->IsA(UInterpTrackFloatProp::StaticClass()) )
		{
			PropertyString = CastChecked<UInterpTrackFloatProp>(Track)->PropertyName.ToString();
			bFoundPropertyName = TRUE;
		}
		else if( Track->IsA(UInterpTrackVectorProp::StaticClass()) )
		{
			PropertyString = CastChecked<UInterpTrackVectorProp>(Track)->PropertyName.ToString();
			bFoundPropertyName = TRUE;
		}

		if( bFoundPropertyName )
		{
			...
			CastChecked<ACameraActor>(GroupActor)->CamOverridePostProcess.DisableOverrideSetting(FName(*PropertyString));
		}
	}
}
			...
			if( dlg.ShowModal( TEXT("ChooseProperty"), TEXT("PropertyName"), PropStrings, 0, TRUE ) == wxID_OK )
			{
				...
				if ( TrackAddPropName != NAME_None )
				{
					...
					for(INT i=0; i<Group->InterpTracks.Num(); i++)
					{
						...
						{
							appMsgf(AMT_OK, *LocalizeUnrealEd("Error_BoolPropertyAlreadyControlled"));
							return FALSE;
						}
