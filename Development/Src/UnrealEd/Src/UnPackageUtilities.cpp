...
*/
UBOOL SavePackageHelper(UPackage* Package, FString Filename, FOutputDevice* ErrorDevice, ULinkerLoad* LinkerToConformAgainst)
{
...
struct FixAdditiveReferencesFunctor
{
	...
	void DoIt( UCommandlet* Commandlet, UPackage* Package, TArray<FString>& Tokens, TArray<FString>& Switches )
	{
		...
		for( TObjectIterator<OBJECTYPE> It; It; ++It )
		{
			...
			// All animations are non-additive now; skip everything.
			continue;
		}
...
UBOOL UFindAssetReferencersCommandlet::GeneratePackageList()
{
	...
	if (bNonNativeScript == TRUE)
	{
		...
		appGetGameNativeScriptPackageNames(NativeScriptPackages, FALSE);
		TArray<FString> NonNativeScriptPackages;
		appGetGameScriptPackageNames(NonNativeScriptPackages, FALSE);
		// Remove the game-name package as that will be native
		for (INT RemoveIdx = 0; RemoveIdx < NativeScriptPackages.Num(); RemoveIdx++)
		{
			NonNativeScriptPackages.RemoveItem(NativeScriptPackages(RemoveIdx));
		}
...
void UBaseObjectTagGeneratorCommandlet::ProcessPackageList()
{
	...
	for (INT PackageIdx = 0; PackageIdx < PackageList.Num(); PackageIdx++)
	{
		...
		for (TObjectIterator<UObject> It; It; ++It)
		{
			UObject* Object = *It;
			UClass* ObjClass = Object->GetClass();
			if (ObjectClassesOfInterest.Find(ObjClass) != NULL)
			{
 ...
 *	@param	InPackage	The package being processed
 */
void UBaseObjectTagGeneratorCommandlet::ProcessObject(UObject* InObject, UPackage* InPackage)
{
 ...
 *	@param	InPackage	The package being processed
 */
void UFindPSysWithBadAutoActivateSettingCommandlet::ProcessObject(UObject* InObject, UPackage* InPackage)
{
 ...
 *	@param	InPackage	The package being processed
 */
void UFindPSysWithZOrientTowardsCameraCommandlet::ProcessObject(UObject* InObject, UPackage* InPackage)
{
 ...
 */
void UFindStaticMeshCleanIssuesCommandlet::ValidateMapList(const TArray<FString>& InMapList, TArray<FString>& OutValidatedList)
{
...
void UFindStaticMeshCleanIssuesCommandlet::ProcessPackageList()
{
	...
	for (INT PackageIdx = 0; PackageIdx < PackageList.Num(); PackageIdx++)
	{
		...
		{
			ValidateMapList(*SubLevelList, InnerPackageList);
		}
 ...
 *	@param	InPackage	The package being processed
 */
void UFindStaticMeshCleanIssuesCommandlet::ProcessObject(UObject* InObject, UPackage* InPackage)
{
}
