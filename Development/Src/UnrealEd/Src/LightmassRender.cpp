...
public:
		...
		, PropertyToCompile(InPropertyToCompile)
	{
		...
		if (MaterialInstance)
		{
			...
			UMaterial* ParentMaterial = MaterialInstance->GetMaterial(MSP_BASE);
			...
			CacheShaders(&StaticParamSet, GRHIShaderPlatform, TRUE);
	...
	virtual INT CompileProperty(EMaterialShaderPlatform MatPlatform,EMaterialProperty Property,FMaterialCompiler* Compiler) const
	{
		...
		if( Property == MP_EmissiveColor )
		{
			...
			case MP_Opacity:
				...
				else if (Material->BlendMode == BLEND_Modulate)
				{
...
public:
	 ...
	 */
	INT CompileTerrainMaterial(EMaterialShaderPlatform MatPlatform,EMaterialProperty Property,FMaterialCompiler* Compiler,UTerrainMaterial* TerrainMaterial,ATerrain* Terrain) const
	{
			...
			else
			{
				...
				if (GLightmassDebugOptions.bDebugMaterials == TRUE)
				{
					...
					if (RenderTarget->GetRenderTargetResource()->ReadPixels(OutputBuffer))
					{
						...
						FString TempPath = GSys->ScreenShotPath;
