 ...
 *
 * @param	Actor					The static mesh actor that was placed.
...
void OnPlaceStaticMeshActor( AActor* Actor, UBOOL bUseSurfaceOrientation )
{
	...
	if( Actor->IsA(AStaticMeshActor::StaticClass()) && (bUseSurfaceOrientation || GEditorModeTools().IsModeActive(EM_StaticMesh)) )
	{
		AStaticMeshActor* MeshActor = CastChecked<AStaticMeshActor>(Actor);

		if( bUseSurfaceOrientation )
		{
			MeshActor->Rotation = GEditor->ClickPlane.SafeNormal().Rotation();
			...
			MeshActor->Rotation.Pitch -= 16384;
		}
		...
		if( GEditorModeTools().IsModeActive(EM_StaticMesh) )
		{
			...
			StaticMeshTool->ApplySettings(MeshActor);
		}

		MeshActor->ForceUpdateComponents();
	}
}
...
namespace
{
	...
	static UBOOL ClickActor(FEditorLevelViewportClient* ViewportClient,AActor* Actor,const FViewportClick& Click,UBOOL bAllowSelectionChange)
	{
		...
		else if( Click.GetKey() != KEY_RightMouseButton )
		{
			...
			if ( Click.GetKey() == KEY_LeftMouseButton && ViewportClient->Viewport->KeyState(KEY_T) )
			{
	...
	if (GEditor->bEnableLODLocking && !bWasControlledByOtherViewport)
	{
		for (INT ViewportIndex = 0; ViewportIndex < GEditor->ViewportClients.Num(); ViewportIndex++)
		{
			...
			if (ViewportClient && ViewportClient->Viewport && ViewportClient != this)
			{
				...
				else if (ViewportType == LVT_Perspective && ViewportClient->ViewportType != LVT_Perspective)
				{
					// don't let this other viewport update itself in its own CalcSceneView
					ViewportClient->bWasControlledByOtherViewport = TRUE;
				}
...
void FEditorLevelViewportClient::ProcessClick(FSceneView* View,HHitProxy* HitProxy,FName Key,EInputEvent Event,UINT HitX,UINT HitY)
{
	...
	if (!GEditorModeTools().HandleClick(HitProxy,Click))
	{
		...
		{
			const FScopedTransaction Transaction(TEXT("Level Socket Clicked"));
			// Assign to selected actors the socket that was clicked.
			...
			for ( FSelectionIterator It( GEditor->GetSelectedActorIterator() ) ; It ; ++It )
			{
				...
				FMatrix SocketTM;
				if( Socket->GetSocketMatrix( SocketTM, SocketProxy->SkelMeshComponent ) )
				{
					AActor* Actor = static_cast<AActor*>( *It );
					checkSlow( Actor->IsA(AActor::StaticClass()) );

					// Don't allow an actor to snap to his own socket!
					if (Actor != SocketProxy->SkelMeshComponent->GetOwner())
					{
						Actor->Modify();
						...
						Actor->Base = SocketProxy->Actor;
						Actor->BaseSkelComponent = SocketProxy->SkelMeshComponent;
						Actor->BaseBoneName = Socket->BoneName;
...
static FLOAT GPerspFrustumStartDist=NEAR_CLIPPING_PLANE;
...
void FEditorLevelViewportClient::Tick(FLOAT DeltaTime)
{
	...
	if ( ViewState->IsViewParent() || bLevelStreamingVolumePrevis )
	{
		...
		GPerspFrustumEndDist=( bVariableFarPlane && GEditor->FarClippingPlane > NEAR_CLIPPING_PLANE )
...
// 				}
	...
	else if( Event == IE_Pressed || Event == IE_Repeat )
	{
		...
		else
		{
			if( Key == KEY_G )
			{
				// CTRL+G selects will do a regroup on all selected actors, creating a new group 
				// regardless of previously existing groups in the selection
				if ( ControlDown && !AltDown && !ShiftDown )
				{
					GUnrealEd->edactRegroupFromSelected();					
				}
			}
			else if ( Key == KEY_L && ControlDown && !AltDown && !ShiftDown )
			{
		...
		if( !bInGameViewMode && CachedShowFlags == 0 )
		{
			...
			if( bAnyLights )
			{
				...
				if( bForcedUnlitShowFlags )
				{
					...
					if( ( ShowFlags & SHOW_ViewMode_Unlit ) == SHOW_ViewMode_Unlit )
					{
			...
			else
			{
				...
				if( !bForcedUnlitShowFlags )
				{
					...
					if( ShowFlags & SHOW_Lighting )
					{
						...
						const EShowFlags ShowFlagsWithoutLighting = ( ShowFlags & ~SHOW_Lighting );
													...
													const FVector& InScale)
{
	...
	for ( FSelectionIterator It( GEditor->GetSelectedActorIterator() ) ; It ; ++It )
	{
		...
		if ( !Actor->bLockLocation )
		{
			...
			AGroupActor* ParentGroup = AGroupActor::GetRootForActor(Actor, FALSE, TRUE);
			if(ParentGroup && ParentGroup->IsSelected() && GEditor->bGroupingActive )
			{
	...
	{
		AGroupActor* ActorGroup = ActorGroups(ActorGroupsIndex);
		ApplyDeltaToActor(ActorGroup, InDrag, InRot, ModifiedScale);
	}
...
EMouseCursor FEditorLevelViewportClient::GetCursor(FViewport* Viewport,INT X,INT Y)
{
	...
	else if(!bWidgetAxisControlledByDrag)
	{
...
void FEditorLevelViewportClient::UpdateAudioListener( const FSceneView& View )
{
	...
	if( AudioDevice )
	{
		...
		AudioDevice->SetListener( 0, 1, ViewLocation, ProjUp, ProjRight, ProjFront );
...
void FEditorLevelViewportClient::Draw(FViewport* Viewport,FCanvas* Canvas)
{
	...
	if ( GIsTiledScreenshot )
	{
		ViewFamily.ShowFlags = (SHOW_DefaultGame & ~(SHOW_ViewMode_Mask|SHOW_Fog)) | SHOW_ViewMode_Lit;
...
void FEditorLevelViewportClient::DrawActorScreenSpaceBoundingBox( FCanvas* InCanvas, const FSceneView* InView, FViewport* InViewport, AActor* InActor, const FLinearColor& InColor, const UBOOL bInDrawBracket, const FString& InLabelText )
{
	...
	// Check to see if we're dealing with a sprite
	USpriteComponent* Sprite = NULL;
	{
		for( INT CurComponentIndex = 0; CurComponentIndex < InActor->Components.Num(); ++CurComponentIndex )
		{
			Sprite = Cast<USpriteComponent>( InActor->Components( CurComponentIndex ) );
			if( Sprite != NULL )
			{
				break;
			}
	...
	if( ProjBoxCenter.W > 0.f )
	{
		...
		{
			DrawStringCentered(InCanvas,ScreenBoxMin.X + ((ScreenBoxMax.X - ScreenBoxMin.X) * 0.5f),ScreenBoxMin.Y,*InLabelText,GEngine->GetMediumFont(),InColor);
		}
	}
}
