 ...
 * TODO - put into a full preferences dialog
 */
...
public:
	...
	WxPreferencesMenu()
	{
		...
		AppendCheckItem( IDM_REPLACE_RESPECTS_CANVAS, *LocalizeUnrealEd("MainMenu_ReplaceRespectsScale"), *LocalizeUnrealEd("MainMenu_ReplaceRespectsScale_ToolTip"));
		...
		AppendCheckItem( IDM_MainMenu_UseStrictBoxSelection, *LocalizeUnrealEd( "MainMenu_UseStrictBoxSelection" ), *LocalizeUnrealEd( "MainMenu_UseStrictBoxSelection_Desc") );
#if HAVE_SCC
		...
		if (bSupportsD3D11RHI)
		{
			AppendCheckItem( IDM_MainMenu_EnableD3D11, *LocalizeUnrealEd( "MainMenu_EnableD3D11" ), *LocalizeUnrealEd( "MainMenu_EnableD3D11_ToolTip" ) );
		}
#endif
	...
	/**
	 * Append a sub-menu to the preferences menu full of all of the various language options to switch the
	 * editor to
	 */
...
WxMainMenu::WxMainMenu()
{
	...
	// File menu
	{
		...
		// These directories may be a project.
		TArray<FString> DirectoryNames;
		FString SearchPath = appRootDir()*FString(TEXT("*"));
		GFileManager->FindFiles( DirectoryNames, *SearchPath, FALSE, TRUE );

		// Look through all the found directories to see if they are a valid project
		INT ProjectIndex = 0;
		for( INT DirIndex = 0; DirIndex < DirectoryNames.Num(); ++DirIndex )
		{
			const FString& DirName = DirectoryNames(DirIndex);

			// Index into the string where the word "game" starts
			const INT GameStringIndex = DirName.InStr(TEXT("Game"));

			// If this is a Game directory and its not our current game see if its a valid project to switch to
			if( GameStringIndex != INDEX_NONE && DirName.InStr( GGameName ) == INDEX_NONE )
			{
				// Check for a script directory.
				TArray<FString> Filenames;
				SearchPath = appRootDir()*DirName*FString(TEXT("Script"))*FString(TEXT("*.u"));
				GFileManager->FindFiles( Filenames, *SearchPath, TRUE, FALSE );

				// If the script directory exists check to see if the core script file is there
				// as a test to see if this project has a valid setup.
				if( Filenames.ContainsItem( FString(TEXT("Core.u")) ) )
				{
					FString ProjectName = DirName.Left( GameStringIndex );
					ProjectMenu->Append( IDM_SWITCH_PROJECT_Start+ProjectIndex, *ProjectName );
					GApp->EditorFrame->ProjectNames.AddItem( ProjectName );
					++ProjectIndex;
				}
			}
		}

		FileMenu->Append( IDM_SWITCH_PROJECT_MENU, *LocalizeUnrealEd("SwitchProjectMenu"), ProjectMenu );

		if( GApp->EditorFrame->ProjectNames.Num() == 0 )
		{
			// If there are no projects to swtich to, disable the switch project menu
			FileMenu->Enable( IDM_SWITCH_PROJECT_MENU, FALSE );
		}
#endif
		FileMenu->AppendSeparator();
	...
	// View menu
	{
		...
		ViewMenu->AppendSeparator();
		ViewMenu->Append( IDM_PREFERENCES, *LocalizeUnrealEd("MainMenu_Preferences"), GApp->EditorFrame->GetPreferencesMenu() );
	...
	// Play Menu
	{
		...
		if (FConsoleSupportContainer::GetConsoleSupportContainer()->GetNumConsoleSupports() > 0)
		{
			...
			for (FConsoleSupportIterator It; It && ConsoleIndex < 20; ++It, ConsoleIndex++)
			{
				const wchar_t* ConsoleName = It->GetConsoleName();
				if( (appStricmp( ConsoleName, CONSOLESUPPORT_NAME_ANDROID ) == 0) ||
					(appStricmp( ConsoleName, CONSOLESUPPORT_NAME_IPHONE ) == 0 && !GEngine->bEmulateMobileRendering) )
				{
					// No "Play On" support for these platforms (yet!)
				}
				else
				{
					PlayMenu->AppendSeparator();

					FString ConsoleName = It->GetConsoleName();
					...
					if( ConsoleName == CONSOLESUPPORT_NAME_IPHONE )
					{
						...
						PlayMenu->Append(
							...
							*FString::Printf(LocalizeSecure(LocalizeUnrealEd("ToolTip_InstallOnIOSDevice"), It->GetConsoleName())), 
					...
					// If we're emulating mobile rendering, we'll also add a 'play using mobile preview' command
					if( GEngine->bEmulateMobileRendering && ConsoleName == CONSOLESUPPORT_NAME_IPHONE )
					{
						PlayMenu->Append(
	...
	// Help Menu
	{
		...
		HelpMenu->Append( IDMENU_GOTOFORUMS, *LocalizeUnrealEd("EditorFrame_GoToForums"), *LocalizeUnrealEd("ToolTip_GoToForums") );
		HelpMenu->Append( IDMENU_GOTOSWARMSETUP, *LocalizeUnrealEd("EditorFrame_GoToSwarmSetup"), *LocalizeUnrealEd("ToolTip_GoToSwarmSetup") );
		HelpMenu->AppendSeparator();
#if WITH_MANAGED_CODE
		HelpMenu->Append( IDMENU_ShowWelcomeScreen, *LocalizeUnrealEd("MainMenu_ShowWelcomeScreen"), *LocalizeUnrealEd("MainMenu_ShowWelcomeScreen_ToolTip") );
#endif // #if WITH_MANAGED_CODE
		HelpMenu->Append( IDMENU_ShowStartupTip, *LocalizeUnrealEd("MainMenu_OpenStartupTip"), *LocalizeUnrealEd("MainMenu_OpenStartupTip_Desc") );
		HelpMenu->AppendSeparator();
		...
		//@todo remove GDC hack
		if (GameName==TEXT("Nano"))
		{
			GameName = TEXT("Demo");
		}
		const FString AppName = FString::Printf( LocalizeSecure( LocalizeUnrealEd( "UnrealEdTitle_F" ), *GameName ) );
#endif
		HelpMenu->Append( IDMENU_ABOUTBOX, *FString::Printf(LocalizeSecure(LocalizeUnrealEd("AboutUnrealEdE"), *AppName)), *FString::Printf(LocalizeSecure(LocalizeUnrealEd("AboutUnrealEdE_Desc"), *AppName)));
...
public:
	WxDlgAbout(WxEditorFrame* Parent) : wxDialog(Parent, -1, TEXT(""), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxSIMPLE_BORDER | wxSYSTEM_MENU | wxCLOSE_BOX)
	{
		...
		{
			SetLabel(*FString::Printf(LocalizeSecure(WindowLabel, *AppName)));
		}

		VersionStatic = new wxStaticText(this, -1, *FString::Printf(LocalizeSecure(LocalizeUnrealEd("UnrealEdVersion"), *AppName, GEngineVersion, GBuiltFromChangeList)));
...
BEGIN_EVENT_TABLE( WxEditorFrame, wxFrame )
	...
	EVT_MENU_RANGE( IDM_SWITCH_PROJECT_Start, IDM_SWITCH_PROJECT_End, WxEditorFrame::MenuSwitchProject )
	...
	EVT_MENU( IDM_REPLACE_RESPECTS_CANVAS, WxEditorFrame::MenuReplaceRespectsScale)
	...
	EVT_MENU( IDM_MainMenu_EnableD3D11, WxEditorFrame::MenuToggleEnableD3D11 )
	...
	EVT_MENU( IDMENU_GOTOSWARMSETUP, WxEditorFrame::MenuGoToSwarmSetup)
	EVT_MENU( IDMENU_ShowStartupTip, WxEditorFrame::MenuStartupTip )
	EVT_MENU( IDMENU_ShowWelcomeScreen, WxEditorFrame::MenuShowWelcomeScreen )
	...
	EVT_UPDATE_UI( IDM_REPLACE_RESPECTS_CANVAS, WxEditorFrame::UI_MenuReplaceRespectsScale )
	...
	EVT_UPDATE_UI( IDM_MainMenu_EnableD3D11, WxEditorFrame::UI_MenuToggleEnableD3D11 )
...
void WxEditorFrame::SetUp()
{
	...
	MainMenuBar->MRUFavoritesList->ReadFromINI();
...
void WxEditorFrame::OnClose( wxCloseEvent& InEvent )
{
	...
	else
	{
		if( PendingProjectExe.Len() > 0 )
		{
			...
			void* Handle = appCreateProc( *PendingProjectExe, TEXT("editor") );
			if( !Handle )
			{
...
void WxEditorFrame::MenuFileMRU( wxCommandEvent& In )
{
	...
	const FFilename NewFilename = MainMenuBar->MRUFavoritesList->GetMRUItem( In.GetId() - IDM_MRU_START );
	
	if( MainMenuBar->MRUFavoritesList->VerifyMRUFile( In.GetId() - IDM_MRU_START ) )
	{
...
void WxEditorFrame::MenuSwitchProject( wxCommandEvent& In )
{
	// Get the project name to switch to
	const INT ProjectIndex = In.GetId() - IDM_SWITCH_PROJECT_Start;
	const FString& ProjectName = ProjectNames( ProjectIndex );

	// Warn the user that this will restart the editor.  Make sure they want to continue
	INT Result = appMsgf( AMT_YesNo, *FString::Printf( LocalizeSecure(LocalizeUnrealEd("SwitchProjectWarning"), *ProjectName) ) );
...
void WxEditorFrame::MenuToggleGroupsActive( wxCommandEvent& In )
{
	...
	// If grouping has been activated, re-select all currently selected actors with locked parent groups to select the group correctly
	if(GUnrealEd->bGroupingActive)
	{
		for ( FSelectionIterator It( GEditor->GetSelectedActorIterator() ) ; It ; ++It )
		{
			...
			if(GroupActor)
			{
				GEditor->SelectGroup(GroupActor);
			}
		}
	}
...
/**
 * Called when the user selects "Enable D3D11" from the View->Preferences menu
 *
 * @param	In	Event generated by wxWidgets when the menu item is selected
 */
void WxEditorFrame::MenuToggleEnableD3D11( wxCommandEvent& In )
{
	GConfig->SetBool( TEXT("EditorFrame"), TEXT("EditorUsesD3D11"), In.IsChecked(), GEditorUserSettingsIni );
 ...
 */
#if GB_SUPPORTED
void WxEditorFrame::SyncBrowserToObjects( TArray<UObject*>& ObjectSet, UBOOL bAlwaysSyncGenericBrowser/*=TRUE*/ )
#else
void WxEditorFrame::SyncBrowserToObjects( TArray<UObject*>& ObjectSet )
#endif
{
	UBOOL bHasShownWindow = FALSE;
...
//@todo cb: GB conversion
	...
	if ( GenericBrowser )
	{
		...
		// Sync.
		if ( ObjectSet.Num() > 0 && (!bHasShownWindow || bAlwaysSyncGenericBrowser) )
		{
			GenericBrowser->SyncToObjects( ObjectSet );
		}
	}
#endif
}
...
void WxEditorFrame::MenuBuildPlayUsingMobilePreview( wxCommandEvent& In )
{
	...
	for( INT CurConsoleIndex = 0; CurConsoleIndex < ConsoleCount; ++CurConsoleIndex )
	{
		...
		if( Console != NULL && appStricmp( Console->GetConsoleName(), CONSOLESUPPORT_NAME_PC ) == 0 )
		{
...
/**
 * Called by wxWidgets to update the editor UI for the 'Enable D3D11' option
 *
 *	@param	In	Event generated by wxWidgets to update the UI
 */
void WxEditorFrame::UI_MenuToggleEnableD3D11( wxUpdateUIEvent& In )
{
	...
	In.Check( bEditorUsesD3D11? TRUE: FALSE );
}
...
/**
 * Navigates to UDN Forums
 */
void WxEditorFrame::MenuGoToForums( wxCommandEvent& In )
{
	wxLaunchDefaultBrowser(TEXT("http://www.udk.com/forums"));
}

/**
 * Navigates to Swarm Setup URL
 */
void WxEditorFrame::MenuGoToSwarmSetup( wxCommandEvent& In )
{
	wxLaunchDefaultBrowser(TEXT("http://udn.epicgames.com/Three/Swarm.html"));
}


/** Shows the startup tip dialog. */
void WxEditorFrame::MenuStartupTip(wxCommandEvent& In )
{
 ...
 */
void WxEditorFrame::MenuShowWelcomeScreen( wxCommandEvent& In )
{
#if WITH_MANAGED_CODE
...
void WxEditorFrame::SyncMaterialToGenericBrowser( )
{
	...
	for ( USelection::TObjectIterator Itor = GEditor->GetSelectedActors()->ObjectItor() ; Itor ; ++Itor )
	{
		...
		if ( Actor )
		{
			UMaterialInterface* MaterialInterface = GetMaterialInterface( Actor, 0, 0 );
			ObjectsToSyncTo.AddItem( MaterialInterface->GetMaterial( MSP_BASE ) );
		}
	}
...
void WxEditorFrame::MenuSyncMaterialInterface( wxCommandEvent &In )
{
	INT ComponentIdx = -1;
	INT MaterialIdx = -1;
	UBOOL bSyncToBaseMaterial = FALSE;
	...
	{
		bSyncToBaseMaterial = TRUE;
	}
	else 
	{
		verify(GetComponentAndElementIndex(In.GetId(), IDM_SYNC_GENERICBROWSER_TO_MATERIALINTERFACE_START, IDM_SYNC_GENERICBROWSER_TO_MATERIALINTERFACE_END, ComponentIdx, MaterialIdx));
	}
	...
	{
		if (bSyncToBaseMaterial)
		{
			ObjectsToSyncTo.AddItem(SelectedMaterialInterface->GetMaterial(MSP_BASE));
		}
		else
		{
			ObjectsToSyncTo.AddItem(SelectedMaterialInterface);
		}
	}
...
void WxEditorFrame::MenuSyncTexture( wxCommandEvent& In )
{
	...
	if( SelectedMaterialInterface )
	{
		SelectedMaterialInterface->GetUsedTextures( Textures );
		UTexture* Texture = Textures( TextureIndex );
		if( Texture )
		{
			// Sync the content browser to the texture
...
void WxEditorFrame::MenuEditMaterialInterface( wxCommandEvent &In )
{
	...
	UMaterialInterface* MaterialInterfaceToEdit = GetSelectedMaterialInterface(ComponentIdx, MaterialIdx);
	if (MaterialInterfaceToEdit != NULL)
	{
		UBOOL bWasEditorOpened = FALSE;
#if WITH_MANAGED_CODE
		if( FContentBrowser::IsInitialized() )
		{
			GCallbackEvent->Send( FCallbackEventParameters( NULL, CALLBACK_RefreshContentBrowser, CBR_ActivateObject, MaterialInterfaceToEdit ) );
...
//@todo cb: GB conversion
		...
		{
			WxGenericBrowser* GenericBrowser = GUnrealEd->GetBrowser<WxGenericBrowser>( TEXT("GenericBrowser") );
			// Search through the generic browser types until we find one that supports the selected material interface
			for (INT ResourceType = 0; ResourceType < GenericBrowser->LeftContainer->ResourceTypes.Num(); ResourceType++)
			{
				UGenericBrowserType* BrowserType = GenericBrowser->LeftContainer->ResourceTypes(ResourceType);
				if (BrowserType && BrowserType->Supports(MaterialInterfaceToEdit))
				{
					if (BrowserType->ShowObjectEditor(MaterialInterfaceToEdit))
					{
						bWasEditorOpened = TRUE;
						break;
					}
...
void WxEditorFrame::MenuCreateMaterialInstance( wxCommandEvent &In )
{
	...
	UMaterialInterface* SelectedMaterial = GetSelectedMaterialInterface(ComponentIdx, MaterialIdx);

	USelection* SelectedActors = GEditor->GetSelectedActors();
	AActor* FirstActor = SelectedActors->GetTop<AActor>();

	if (SelectedActors->Num() == 1 && SelectedMaterial)
	{
		AActor* FirstActor = SelectedActors->GetTop<AActor>();
	...
	if (SelectedActors->Num() == 1)
	{
		...
		if(FirstActor)
		{
			...
			if (MaterialSelection && MaterialSelection->Num() == 1)
			{
				...
				SetMaterial(FirstActor, InstanceToAssign, ComponentIdx, MaterialIdx);
			}
		}
	}
}
