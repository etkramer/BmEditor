...
IMPLEMENT_CLASS(UGenericBrowserType_AITree)
...
IMPLEMENT_CLASS(UGenericBrowserType_DMC)
...
public:
	 ...
	 */
	bool Show( bool InShow, TArray<USoundCue*>& SoundCueList );
private:
	...
	/** The window with sound nodes' properties */
...
{
	PropertyWindow = NULL;
	SoundNodesPropWindow = NULL;

	const bool bSuccess = wxXmlResource::Get()->LoadDialog( this, GApp->EditorFrame, TEXT("ID_DLG_SOUND_CUE_PROPERTIES") );
	check( bSuccess );

	FWindowUtil::LoadPosSize( TEXT("DlgSoundProperties"), this );
	FLocalizeWindow( this );

	// Property window for editing the sound cue
	wxWindow* win = FindWindow( XRCID( "ID_PROPERTY_WINDOW" ) );
	check( win != NULL );
	PropertyWindow = new WxPropertyWindowHost;
	PropertyWindow->Create(win, this, 1, FALSE);
	wxRect rc = win->GetRect();
	PropertyWindow->SetSize( 0, 0, rc.GetWidth(), rc.GetHeight() );
	win->Show();

	// Property window for sound nodes
	win = FindWindow( XRCID( "ID_SOUND_NODES_WINDOW" ) );
	check( win != NULL );
	SoundNodesPropWindow = new WxPropertyWindowHost;
	SoundNodesPropWindow->Create(win, this, 1, FALSE);
	rc = win->GetRect();
	SoundNodesPropWindow->SetSize( 0, 0, rc.GetWidth(), rc.GetHeight() );
	win->Show();
}
 ...
 *			already in the requested state
 */
bool WxDlgSoundCueProperties::Show( bool InShow, TArray<USoundCue*>& SoundCueList )
{
	// View the properties of the sound cues
	PropertyWindow->SetObjectArray( SoundCueList, EPropertyWindowFlags::Sorted );

	// Gather a list of sound nodes and display their properties
	...
	// Set the label of the node window to reflect how many nodes are selected
	wxWindow* winNodes = FindWindow( XRCID( "ID_SOUND_NODES_PARENT_PANE" ) );
	const wxString& PaneName =  ( NodeList.Num() > 1 )
		? *FString::Printf( LocalizeSecure(LocalizeUnrealEd("SoundCueNodeMultipleCaption"), NodeList.Num()) )
		: *LocalizeUnrealEd("SoundCueNodeCaption");
	winNodes->SetLabel( PaneName );

	// Populate the node properties
	if ( NodeList.Num() > 0 )
	{
		SoundNodesPropWindow->SetObjectArray( NodeList, EPropertyWindowFlags::Sorted | EPropertyWindowFlags::ShouldShowCategories | EPropertyWindowFlags::UseTypeAsTitle );
	}
	...
	if( SoundCueList.Num() == 1 )
	{
		...
		SetTitle( *FString::Printf( LocalizeSecure(LocalizeUnrealEd("SoundCuePropertiesCaption"), *(FirstSoundCue->GetPathName())) ) );
	}
	else
	{// Multiple sound cues selected
		check( SoundCueList.Num() > 1 );
		SetTitle( *FString::Printf( LocalizeSecure(LocalizeUnrealEd("SoundCueMultiplePropertiesCaption"), SoundCueList.Num()) ) );
	}

	return WxTrackableDialog::Show(InShow);
}
...
void WxDlgSoundCueProperties::OnOK( wxCommandEvent& In )
{
	...
	SoundNodesPropWindow->FlushLastFocused();
	Destroy();
}
...
private:
	 ...
	 */
	ESimpleElementBlendMode GetBlendMode(UBOOL r, UBOOL g, UBOOL b, UBOOL a, UBOOL d );
	...
	CheckerboardTexture( NULL )
{
	...
	ColourChannelBlendMode = GetBlendMode( RedColorChannelEnabled, GreenColorChannelEnabled, BlueColorChannelEnabled, AlphaColorChannelEnabled, DesaturationEnabled);
 ...
 * @param	d	Desaturation	
 */
ESimpleElementBlendMode WxTexturePropertiesFrame::GetBlendMode(UBOOL r, UBOOL g, UBOOL b, UBOOL a, UBOOL d)
{
...
{
	for( INT Index = 0; Index < SupportInfo.Num(); Index++ )
	{
		delete SupportInfo( Index ).ContextMenu;
	}
	SupportInfo.Empty();
}
...
WxMBGenericBrowserContextBase* UGenericBrowserType::GetContextMenu( USelection* Selection )
{
	check( Selection );

	// Find the first type that supports all selected objects and has a context menu.
	for( INT InfoIndex = 0 ; InfoIndex < SupportInfo.Num() ; ++InfoIndex )
	{
		UBOOL bSelectionMatchesThisType = TRUE;
		for ( USelection::TObjectIterator It( Selection->ObjectItor() ) ; It ; ++It )
		{
			UObject* Object = *It;
			if ( Object )
			{
				if( !SupportInfo(InfoIndex).Supports(Object) )
				{
					bSelectionMatchesThisType = FALSE;
				}
			}
		}

		if(bSelectionMatchesThisType && SupportInfo(InfoIndex).ContextMenu != NULL)
		{
			return SupportInfo(InfoIndex).ContextMenu;
		}
	}
...
void UGenericBrowserType::QueryStandardSupportedCommands( USelection* InObjects, TArray< FObjectSupportedCommandType >& OutCommands )
{
...

	{
		...
		if( NormalRenamingCount == NumObjects )
		{
			bSupportsNormalRename = TRUE;
		}
		if( LocRenamingCount == NumObjects )
		{
...
void UGenericBrowserType_Material::Init()
{
	SupportInfo.AddItem( FGenericBrowserTypeInfo( 
		...
		FColor(0,255,0), 
		new WxMBGenericBrowserContext_Material, 
		0, 
		this,
...
{
	SupportInfo.AddItem( FGenericBrowserTypeInfo( UDecalMaterial::StaticClass(), FColor(192,255,192), new WxMBGenericBrowserContext_Material, 0, this ) );
}
...
{
	SupportInfo.AddItem( FGenericBrowserTypeInfo( UPhysXParticleSystem::StaticClass(), FColor(200,192,128), new WxMBGenericBrowserContext_PhysXParticleSystem, 0, this ) );
}
...
{
#if WITH_APEX
	SupportInfo.AddItem( FGenericBrowserTypeInfo( UApexDestructibleDamageParameters::StaticClass(), FColor(200,192,128), new WxMBGenericBrowserContext_ApexDestructibleDamageParameters, 0, this ) );
#endif
}
...
void UGenericBrowserType_Sounds::Init( void )
{
	...
	SupportInfo.AddItem( FGenericBrowserTypeInfo( USoundClass::StaticClass(), FColor( 255, 175, 0 ), new WxMBGenericBrowserContext_SoundClass, 0, this ) );
	SupportInfo.AddItem( FGenericBrowserTypeInfo( USoundMode::StaticClass(), FColor( 175, 0, 255 ), new WxMBGenericBrowserContext_SoundMode, 0, this ) );
}
...
void UGenericBrowserType_SoundCue::InvokeCustomCommand( INT InCommand, TArray<UObject*>& InObjects )
{
	for (INT ObjIndex = 0; ObjIndex < InObjects.Num(); ++ObjIndex)
	{
		if( InCommand == IDMN_ObjectContext_EditNodes )
		{
			...
			{
				WxDlgSoundCueProperties* dlg = new WxDlgSoundCueProperties();
				dlg->Show( 1, SoundCueList );
			}
			break;
		}
	}
...
{
	SupportInfo.AddItem( FGenericBrowserTypeInfo( UFracturedStaticMesh::StaticClass(), FColor(96,200,255), new WxMBGenericBrowserContext_FracturedStaticMesh, 0, this ) );
...
{
	WxStaticMeshEditor* StaticMeshEditor = new WxStaticMeshEditor( GApp->EditorFrame, -1, CastChecked<UStaticMesh>(InObject), FALSE );
	...
	for (INT ObjIndex = 0; ObjIndex < InObjects.Num(); ++ObjIndex)
	{
		...
		if( InCommand == IDMN_ObjectContext_ResliceFracturedMeshes )
		{
			...
			if(FracturedStaticMesh)
			{
				...
				WxStaticMeshEditor* StaticMeshEditor = new WxStaticMeshEditor( GApp->EditorFrame, -1, FracturedStaticMesh, FALSE );
...
{
	SupportInfo.AddItem( FGenericBrowserTypeInfo( UTextureMovie::StaticClass(), FColor(0,255,0), NULL, 0, this ) );
}
...
void UGenericBrowserType_Texture::InvokeCustomCommand( INT InCommand, TArray<UObject*>& InObjects )
{
	...
	else if( InCommand == IDMN_TextureContext_FindAllMaterialsUsingThis )
	{
		...
		{
			GWarn->BeginSlowTask( TEXT("Searching materials"), TRUE );
			...
			if( MaterialsUsingTexture.Num() > 0 )
			{
				...
				ActiveInstance.AddAssetsToLocalCollection( NewCollectionName, MaterialsUsingTexture );
...
{
	SupportInfo.AddItem( FGenericBrowserTypeInfo( UTextureCube::StaticClass(), FColor(255,0,0), NULL, 0, this ) );
}
...
{
	SupportInfo.AddItem( FGenericBrowserTypeInfo( UObject::StaticClass(), FColor(255,0,0), new WxMBGenericBrowserContext_Archetype, RF_ArchetypeObject, this, UGenericBrowserType_Archetype::IsArchetypeSupported) );
}
...
{
	SupportInfo.AddItem( FGenericBrowserTypeInfo( UProcBuildingRuleset::StaticClass(), FColor(222,255,135), NULL, 0, this ) );
}
...
{
	SupportInfo.AddItem( FGenericBrowserTypeInfo( UApexDestructibleAsset::StaticClass(), FColor(255,192,128), 0, 0, this ) );
}


void UGenericBrowserType_ApexClothingAsset::Init()
{
	SupportInfo.AddItem( FGenericBrowserTypeInfo( UApexClothingAsset::StaticClass(), FColor(255,192,128), 0, 0, this ) );
}

void UGenericBrowserType_ApexGenericAsset::Init()
{
#if WITH_APEX_PARTICLES
	SupportInfo.AddItem( FGenericBrowserTypeInfo( UApexGenericAsset::StaticClass(), FColor(192,128,255), new WxMBGenericBrowserContext_ApexParticles , 0, this) );
#endif
}
...
/*------------------------------------------------------------------------------
UGenericBrowserType_DMC
------------------------------------------------------------------------------*/

void UGenericBrowserType_DMC::Init()
{
	SupportInfo.AddItem( FGenericBrowserTypeInfo( 
		UDMC_Prototype::StaticClass(), 
		FColor(255,0,0), 
		new WxMBGenericBrowserContext_DMC, 
		0, 
		this) );	
}

UBOOL UGenericBrowserType_DMC::ShowObjectEditor(UObject* InObject)
{
	// Cast to the font so we have access to the texture data
	UDMC_Prototype* DMC = Cast<UDMC_Prototype>(InObject);
	if (DMC != NULL)
	{
		WxDMC* DMCTool = new WxDMC( GApp->EditorFrame, -1, DMC );
		DMCTool->Show();
	}
	return 1;
}

void UGenericBrowserType_DMC::QuerySupportedCommands( class USelection* InObjects, TArray< FObjectSupportedCommandType >& OutCommands ) const
{
	OutCommands.AddItem( FObjectSupportedCommandType( IDMN_ObjectContext_Editor, *LocalizeUnrealEd( "ObjectContext_EditUsingDMCTool" ) ) );
}

INT UGenericBrowserType_DMC::QueryDefaultCommand( TArray<UObject*>& InObjects ) const
{
	return IDMN_ObjectContext_Editor;
}

/*------------------------------------------------------------------------------
UGenericBrowserType_AITree
------------------------------------------------------------------------------*/

void UGenericBrowserType_AITree::Init( void )
{
	SupportInfo.AddItem( FGenericBrowserTypeInfo( 
		UAITree::StaticClass(), 
		FColor( 255, 80, 80 ), 
		new WxMBGenericBrowserContext_AITree, 
		0, 
		this ) );
}

UBOOL UGenericBrowserType_AITree::ShowObjectEditor( UObject* InObject )
{
	UAITree* AITree = Cast<UAITree>(InObject);
	if(AITree)
	{
		// @todo: AI Tree tool isn't ready for use yet so we've disabled it for now
		if( 1 )
		{
			appMsgf( AMT_OK, *LocalizeUnrealEd( TEXT( "AITreeEditor_NotSupportedYetWarning" ) ) );
		}
		else
		{
			WxAITreeEditor* AITreeEditor = new WxAITreeEditor( (wxWindow*)GApp->EditorFrame, -1, AITree );
			AITreeEditor->Show(1);
		}
	}
...
* @param	OutCommands		The list of custom commands to support
*/
void UGenericBrowserType_AITree::QuerySupportedCommands( USelection* InObjects, TArray< FObjectSupportedCommandType >& OutCommands ) const
{
	const UBOOL bAnythingCooked = AnyObjectsAreCooked( InObjects );

	OutCommands.AddItem( FObjectSupportedCommandType( IDMN_ObjectContext_Editor, *LocalizeUnrealEd( "ObjectContext_EditUsingAITreeEditor" ) ) );
}
