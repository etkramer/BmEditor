...
/** Simple enumeration representing the states of the PIE button */
enum EPIEButtons
{
	PIE_Play,
	PIE_Stop
	...
	: WxToolBar( InParent, InID, wxDefaultPosition, wxDefaultSize, wxTB_HORIZONTAL | wxNO_BORDER | wxTB_FLAT | wxCLIP_CHILDREN | wxTB_NO_TOOLTIPS )
{
...
#endif
#if GB_SUPPORTED
	if( GUnrealEd->GetBrowserManager()->GetBrowserPane( TEXT("GenericBrowser") ) != NULL )
	{
		// Generic Browser
		AddTool( IDM_BROWSER_START+1, TEXT(""), GenericB, *LocalizeUnrealEd("ToolTip_41") );
	}
#endif
	AddSeparator();
	...
	for (FConsoleSupportIterator It; It && ConsoleIndex < 20; ++It, ConsoleIndex++)
	{
		...
		{
			// select console icon
			FString TooltipStr = *FString::Printf(LocalizeSecure(LocalizeUnrealEd("ToolTip_50_F"), It->GetConsoleName()));
			WxBitmap* PlayConsoleB = &PlayOnB[B_PC];
			if( appStricmp( It->GetConsoleName(), CONSOLESUPPORT_NAME_PS3 ) == 0 )
			{
				PlayConsoleB = &PlayOnB[B_PS3];
			}
			else if( appStricmp( It->GetConsoleName(), CONSOLESUPPORT_NAME_360 ) == 0 )
			{
				PlayConsoleB = &PlayOnB[B_XBox360];
			}
			else if( appStricmp( It->GetConsoleName(), CONSOLESUPPORT_NAME_IPHONE ) == 0 )
			{
...
#if WITH_ES2_RHI
			// If we're emulating mobile rendering on the PC platform, we'll also add a 'play using mobile preview' command
			if( !bHasAddedPreviewButton &&
				GEngine->bEmulateMobileRendering && 
				(appStricmp( It->GetConsoleName(), CONSOLESUPPORT_NAME_IPHONE ) == 0 ||
				 appStricmp( It->GetConsoleName(), CONSOLESUPPORT_NAME_NGP ) == 0))
			{
				...
				AddTool(
					...
					TEXT(""),
					*PlayConsoleB,
					*FString::Printf(LocalizeSecure(LocalizeUnrealEd("ToolTip_50_F"), *LocalizeUnrealEd(TEXT("MobilePreview"))))
					);
	...
	PlayInEditorButton->AddState( PIE_Stop, &PlayInEditorStopB );
...
{
	if ( In.GetId() == IDM_BuildPlayInEditorButton && PlayInEditorButton->GetCurrentState()->ID == PIE_Play )
	{
		// Bring up the edit url dialog
		GUnrealEd->EditPlayWorldURL();
	}
...
void WxMainToolBar::OnPlayInEditorButtonClick( wxCommandEvent& In )
{
	...
	if ( PlayInEditorButton->GetCurrentState()->ID == PIE_Play )
	{
		GUnrealEd->PlayMap();
	}
...
void WxMainToolBar::UI_PlayInEditor( wxUpdateUIEvent& In )
{
	...
	if ( PlayInEditorButton->GetCurrentState()->ID != CurState )
	{
		...
		FString NewToolTip = ( CurState == PIE_Play ) ? LocalizeUnrealEd("MainToolBar_PlayInEditorPlay_ToolTip") : LocalizeUnrealEd("MainToolBar_PlayInEditorStop_ToolTip");
