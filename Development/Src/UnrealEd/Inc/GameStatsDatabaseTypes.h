...
public:
    ...
    */
	...
	/** 
	 * Add the game session metadata to the database, caching the ID of the database entry
	 * @param	GameSessionInfo		Information related to the session recorded
	 * @return TRUE if successful, FALSE otherwise
	 */
	UBOOL AddGameSessionInfo(const struct FGameSessionInformation& GameSessionInfo);
	/** 
	 * Add the Player metadata to the database, caching the IDs of the database entries for each player
	 * @param	PlayerMetadata		Player information related to the session recorded
	 * @return TRUE if successful, FALSE otherwise
	 */
	UBOOL AddPlayerMetadata(const TArray<struct FPlayerInformation>& PlayerMetadata);
	/** 
	 * Add the Team metadata to the database, caching the IDs of the database entries for each team
	 * @param	TeamMetadata		Team information related to the session recorded
	 * @return TRUE if successful, FALSE otherwise
	 */
	UBOOL AddTeamMetadata(const TArray<struct FTeamInformation>& TeamMetadata);
	 ...
	 * @return TRUE if successful, FALSE otherwise
	 */
	UBOOL AddGameIntEvent(INT EventID, FLOAT EventTime, INT IntValue);
	UBOOL AddGameStringEvent(INT EventID, FLOAT EventTime, const FString& StringEvent);
	UBOOL AddGameFloatEvent(INT EventID, FLOAT EventTime, FLOAT FloatValue);
	UBOOL AddGamePositionEvent(INT EventID, FLOAT EventTime, const FVector& Position, FLOAT IntValue);
	 ...
	 * @return TRUE if successful, FALSE otherwise
	 */
	UBOOL AddTeamIntEvent(INT EventID, FLOAT EventTime, INT TeamID, INT IntValue);
	UBOOL AddTeamFloatEvent(INT EventID, FLOAT EventTime, INT TeamID, FLOAT FloatValue);
	UBOOL AddTeamStringEvent(INT EventID, FLOAT EventTime, INT TeamID, const FString& StringValue);

	/** 
	 * Adds a player event to track to the DB.
	 *
	 * @param	EventID				ID of the event to track
	 * @param	EventTime			Time in seconds when the event occurred 
	 * @param	PlayerID			ID of the player who caused the event
	 * @param	PlayerPos			Position of the player at the time of the event
	 * @param	PlayerRot			Rotation of the player at the time of the event
	 * @return TRUE if successful, FALSE otherwise
	 */
	UBOOL AddPlayerIntEvent(INT EventID, FLOAT EventTime, INT PlayerID, const FVector& PlayerPos, const FRotator& PlayerRot, INT IntValue);
	UBOOL AddPlayerFloatEvent(INT EventID, FLOAT EventTime, INT PlayerID, const FVector& PlayerPos, const FRotator& PlayerRot, FLOAT FloatValue);
	UBOOL AddPlayerStringEvent(INT EventID, FLOAT EventTime, INT PlayerID, const FVector& PlayerPos, const FRotator& PlayerRot, const FString& StringValue);
	UBOOL AddPlayerLoginEvent(INT EventID, FLOAT EventTime, INT PlayerID, const FVector& PlayerPos, const FRotator& PlayerRot);
	UBOOL AddPlayerSpawnEvent(INT EventID, FLOAT EventTime, INT PlayerID, const FVector& PlayerPos, const FRotator& PlayerRot, const FString& PawnClassName, INT TeamID);
	UBOOL AddPlayerPlayerEvent(INT EventID, FLOAT EventTime, INT PlayerID, const FVector& PlayerPos, const FRotator& PlayerRot, INT TargetID, const FVector& TargetPos, const FRotator& TargetRot);
	UBOOL AddPlayerKillDeathEvent(INT EventID, FLOAT EventTime, INT PlayerID, const FVector& PlayerPos, const FRotator& PlayerRot, INT TargetID, const FVector& TargetPos, const FRotator& TargetRot, INT KillType, const FString& DamageClassName);

	UBOOL AddWeaponIntEvent(INT EventID, FLOAT EventTime, INT PlayerID, const FVector& PlayerPos, const FRotator& PlayerRot, const FString& WeaponClassName, INT IntValue);
	UBOOL AddProjectileIntEvent(INT EventID, FLOAT EventTime, INT PlayerID, const FVector& PlayerPos, const FRotator& PlayerRot, const FString& ProjectileClassName, INT IntValue);
	UBOOL AddDamageIntEvent(INT EventID, FLOAT EventTime, INT PlayerID, const FVector& PlayerPos, const FRotator& PlayerRot, INT TargetID, const FVector& TargetPos, const FRotator& TargetRot, const FString& DamageClassName, INT IntValue);
