...
// Vertex state.
void FD3D11DynamicRHI::SetStreamSource(UINT StreamIndex,FVertexBufferRHIParamRef VertexBufferRHI,UINT Stride,UBOOL bUseInstanceIndex,UINT NumVerticesPerInstance,UINT NumInstances)
{
	DYNAMIC_CAST_D3D11RESOURCE(VertexBuffer,VertexBuffer);

	const UINT Offsets = 0;
	ID3D11Buffer* D3DBuffer = VertexBuffer;
	Direct3DDeviceIMContext->IASetVertexBuffers(StreamIndex,1,&D3DBuffer,&Stride,&Offsets);
...
void FD3D11DynamicRHI::SetTextureParameter(FPixelShaderRHIParamRef PixelShaderRHI,UINT TextureIndex,FTextureRHIParamRef NewTextureRHI)
{
	...
	ID3D11ShaderResourceView* SRV = NewTexture->View;
...
INT FD3D11DynamicRHI::GetMobileUniformSlotIndexByName(FName ParamName)
{
...
void FD3D11DynamicRHI::SetViewParametersWithOverrides( const FSceneView& View, const FMatrix& ViewProjectionMatrix, const FVector4& DiffuseOverride, const FVector4& SpecularOverride )
{
	...
	HSCBContents.GlobalTesselationFactor = GSystemSettings.TessellationFactorMultiplier;
	...
	DSCBContents.TemporalAAParameters = View.TemporalAAParameters.GetVector();
 ...
 * Not used on PC
 */
void FD3D11DynamicRHI::SetViewPixelParameters(const FSceneView* View,FPixelShaderRHIParamRef PixelShader,const class FShaderParameter* SceneDepthCalcParameter,const class FShaderParameter* ScreenPositionScaleBiasParameter)
{
}
...
void FD3D11DynamicRHI::CommitVertexAndPixelShaderConstants()
{
...
void FD3D11DynamicRHI::DrawPrimitive(UINT PrimitiveType,UINT BaseVertexIndex,UINT NumPrimitives)
{
	...
	if(bUsingTessellation && PrimitiveType != PT_TriangleList)
	{
		debugf(NAME_Warning,TEXT("Attempt to render non PT_TriangleList primitive with Tessellation shaders enabled!! Skipping this primitive..."));
		return;
	}
...
void FD3D11DynamicRHI::DrawIndexedPrimitive(FIndexBufferRHIParamRef IndexBufferRHI,UINT PrimitiveType,INT BaseVertexIndex,UINT MinIndex,UINT NumVertices,UINT StartIndex,UINT NumPrimitives)
{
	...
	if(bUsingTessellation && PrimitiveType != PT_TriangleList)
	{
		debugf(NAME_Warning,TEXT("Attempt to render non PT_TriangleList indexed primitive with Tessellation shaders enabled!! Skipping this primitive..."));
		return;
	}
 ...
 */
void FD3D11DynamicRHI::DrawSpriteParticles( const FMeshElement& Mesh)
{
 ...
 */
void FD3D11DynamicRHI::DrawSubUVParticles( const FMeshElement& Mesh)
{
	checkSlow(Mesh.DynamicVertexData);
 ...
 * @param Mesh The mesh element containing the data for rendering the sprite subuv particles
 */
void FD3D11DynamicRHI::DrawPointSpriteParticles(const FMeshElement& Mesh) 
{
	// Not implemented yet!
}
...
void FD3D11DynamicRHI::MSAAInitPrepass()
{
}
void FD3D11DynamicRHI::MSAAFixViewport()
{
}
void FD3D11DynamicRHI::MSAABeginRendering(UBOOL bRequiresClear)
{
}
void FD3D11DynamicRHI::MSAAEndRendering(FTexture2DRHIParamRef DepthTexture,FTexture2DRHIParamRef ColorTexture,UINT MSAAEndRendering)
{
}
void FD3D11DynamicRHI::RestoreColorDepth(FTexture2DRHIParamRef ColorTexture, FTexture2DRHIParamRef DepthTexture)
{
}
void FD3D11DynamicRHI::GetMSAAOffsets(UINT *HalfScreenY, UINT *ResolveOffset)
{
	check(0);
}
