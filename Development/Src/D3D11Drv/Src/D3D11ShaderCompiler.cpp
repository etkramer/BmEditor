...
#define COMPILER_NAME "COMPILER_HLSL"
	...
	else
	{
		...
		//  This is needed because of a compiler bug in MS Feb10 D3D11Compile call whereby adding a
		//	 tex2Dlod to a hull shader will result in an internal error in the compiler when optimizations are turned on.
		//  once the compiler has been bug fixed should be able to reenable optimizations on hull shaders
		if(Target.Frequency == SF_Hull)
		{
			CompileFlags |= D3D10_SHADER_SKIP_OPTIMIZATION;
		}
		else
