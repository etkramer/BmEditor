...
void AGameCrowdAgentSkeletal::TickSpecial(FLOAT DeltaTime)
{
	...
	UBOOL bResult = ShouldPerformCrowdSimulation(DeltaTime) && (WorldInfo->TimeSeconds - LastRenderTime < 0.2f);
	if( bResult )
	{
...
{
	eventUpdateSpawner( CloudSpawnInfo, DeltaTime );
	for( INT Idx = 0; Idx < ScriptedSpawnInfo.Num(); Idx++ )
	{
		eventUpdateSpawner( ScriptedSpawnInfo(Idx), DeltaTime );
	}
}
...
void AGameCrowdPopulationManager::TickSpawnInfo( FCrowdSpawnInfoItem& Item, FLOAT DeltaTime )
{
	if( Item.NumAgentsToTickPerFrame >= 0 )
	{
		...
		for( INT TickIdx = 0; TickIdx < NumToTick; TickIdx++ )
		{
			INT AgentIdx = (Item.LastAgentTickedIndex + 1) % Item.ActiveAgents.Num();
			AGameCrowdAgent* Agent = Item.ActiveAgents(AgentIdx);
			if( Agent == NULL )
			{
				Item.ActiveAgents.Remove(AgentIdx,1);
				AgentIdx--;
				continue;
			}
			...
			{
				Item.LastAgentTickedIndex = AgentIdx;
...
void AGameCrowdPopulationManager::WarmupByIdx( INT SpawnerIdx, INT WarmupNum )
{
	FCrowdSpawnInfoItem& Item = (SpawnerIdx < 0) ? CloudSpawnInfo : ScriptedSpawnInfo(SpawnerIdx);
