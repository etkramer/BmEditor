...
//		GWorld->GetWorldInfo()->AddOnScreenDebugMessage((QWORD)((PTRINT)this) + 2, 5.0f, FColor(255,255,255), FString::Printf(TEXT("DesiredCamLoc: %f %f %f"), DesiredCamLoc.X, DesiredCamLoc.Y, DesiredCamLoc.Z));
		...
		if (P != NULL && P->Mesh != NULL)
		{
			...
			GWorld->GetWorldInfo()->AddOnScreenDebugMessage((QWORD)((PTRINT)this) + 4, 5.0f, FColor(255,255,255), FString::Printf(TEXT("Offset from root bone: %f %f %f"), RootBoneOffset.X, RootBoneOffset.Y, RootBoneOffset.Z));
		}
...
FVector UGameThirdPersonCameraMode::GetViewOffset(class APawn* ViewedPawn, FLOAT DeltaTime, const FVector& ViewOrigin, const FRotator& ViewRotation)
{
	...
	if (bSmoothViewOffsetPitchChanges)
	{
		...
		{
			GWorld->GetWorldInfo()->AddOnScreenDebugMessage((QWORD)((PTRINT)this), 5.0f, FColor(255,255,255), FString::Printf(TEXT("%s out_Offset: %f %f %f, Pitch: %f, MidOffset: %f %f %f, LowOffset: %f %f %f,"), *GetName(), out_Offset.X, out_Offset.Y, out_Offset.Z, Pitch, MidOffset.X, MidOffset.Y, MidOffset.Z, LowOffset.X, LowOffset.Y, LowOffset.Z));
		}
#endif
	}
	else
	{
		...
		if( Pitch >= 0.f )
		{
			...
			if (ThirdPersonCam->bDrawDebug)
			{
				GWorld->GetWorldInfo()->AddOnScreenDebugMessage((QWORD)((PTRINT)this), 5.0f, FColor(255,255,255), FString::Printf(TEXT("%s out_Offset: %f %f %f, Pct: %f, MidOffset: %f %f %f, LowOffset: %f %f %f,"), *GetName(), out_Offset.X, out_Offset.Y, out_Offset.Z, Pct, MidOffset.X, MidOffset.Y, MidOffset.Z, LowOffset.X, LowOffset.Y, LowOffset.Z));
			}
#endif
		}
		else
