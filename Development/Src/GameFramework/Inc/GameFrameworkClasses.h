...
#define FOREACH_ENUM_EZONETOUCHEVENT(op) \
    op(ZoneEvent_Touch) \
    op(ZoneEvent_Update) \
    op(ZoneEvent_Stationary) \
    op(ZoneEvent_UnTouch) \
    op(ZoneEvent_Cancelled) 
enum EZoneState
{
...
public:
    ...
    class UAITree* AITree;
    struct FAITreeHandle AITreeHandle;
    BITFIELD bUseAITree:1;
...
public:
    ...
    virtual void WarmupByIdx(INT SpawnerIdx,INT WarmupNum);
    ...
    DECLARE_FUNCTION(execWarmupByIdx)
    {
        P_GET_INT(SpawnerIdx);
        P_GET_INT(WarmupNum);
        P_FINISH;
        this->WarmupByIdx(SpawnerIdx,WarmupNum);
    }
    ...
    void eventUpdateSpawner(struct FCrowdSpawnInfoItem& Item,FLOAT DeltaTime)
    {
...
public:
    ...
    NO_DEFAULT_CONSTRUCTOR(AGameExplosionActor)
};
...
struct MobilePlayerInput_eventOnMobileMotion_Parms
{
    class UPlayerInput* PlayerInput;
    FVector CurrentAttitude;
    FVector CurrentRotationRate;
    FVector CurrentGravity;
    FVector CurrentAcceleration;
    MobilePlayerInput_eventOnMobileMotion_Parms(EEventParm)
    {
    }
};
...
public:
    ...
    BITFIELD bSupportsAccelerometer:1;
    BITFIELD bAllowTouchesInCinematic:1;
    BITFIELD bDisableTouchInput:1;
    BITFIELD bFakeMobileTouches:1;
    BITFIELD bDeviceHasGyroscope:1;
    BITFIELD bDeviceHasAccelerometer:1;
    FLOAT MobilePitch;
    ...
    FVector DeviceMotionAttitude;
    FVector DeviceMotionRotationRate;
    FVector DeviceMotionGravity;
    FVector DeviceMotionAcceleration;
    ...
    FVector DeviceGyroRawData;
    FVector DeviceAccelerometerRawData;
    FScriptDelegate __OnMobileMotion__Delegate;
    ...
    virtual void NativeInitializeInputZones();
    ...
    {
        P_FINISH;
        this->NativeInitializeInputZones();
    }
    ...
    void delegateOnMobileMotion(class UPlayerInput* PlayerInput,FVector CurrentAttitude,FVector CurrentRotationRate,FVector CurrentGravity,FVector CurrentAcceleration)
    {
        MobilePlayerInput_eventOnMobileMotion_Parms Parms(EC_EventParm);
        Parms.PlayerInput=PlayerInput;
        Parms.CurrentAttitude=CurrentAttitude;
        Parms.CurrentRotationRate=CurrentRotationRate;
        Parms.CurrentGravity=CurrentGravity;
        Parms.CurrentAcceleration=CurrentAcceleration;
        ProcessDelegate(GAMEFRAMEWORK_OnMobileMotion,&__OnMobileMotion__Delegate,&Parms);
    }
...
public:
    ...
    UBOOL eventOnSceneTouch(BYTE EventType,FLOAT TouchX,FLOAT TouchY)
    {
    ...
    void eventOnTouch(class UMobileMenuObject* Sender,FLOAT TouchX,FLOAT TouchY,UBOOL bCancel)
    {
        MobileMenuScene_eventOnTouch_Parms Parms(EC_EventParm);
        Parms.Sender=Sender;
        Parms.TouchX=TouchX;
        Parms.TouchY=TouchY;
        Parms.bCancel=bCancel ? FIRST_BITFIELD : FALSE;
...
AUTOGENERATE_FUNCTION(AGameCrowdPopulationManager,-1,execWarmupByIdx);
...
{ 
	MAP_NATIVE(AGameCrowdPopulationManager, execWarmupByIdx)
...
VERIFY_CLASS_OFFSET_NODIE(AGameAIController,GameAIController,AITree)
...
VERIFY_CLASS_OFFSET_NODIE(UMobileMenuObject,MobileMenuObject,OwnerScene)
...
VERIFY_CLASS_OFFSET_NODIE(USeqAct_GameCrowdPopulationManagerToggle,SeqAct_GameCrowdPopulationManagerToggle,NumAgentsToTickPerFrame)
