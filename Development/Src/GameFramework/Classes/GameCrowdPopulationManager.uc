...
function OnGameCrowdPopulationManagerToggle( SeqAct_GameCrowdPopulationManagerToggle inAction)
{
	...
	if( inAction.InputLinks[0].bHasImpulse || inAction.InputLinks[2].bHasImpulse )
	{
		...
		{
			inAction.FillCrowdSpawnInfoItem( CloudSpawnInfo, self );
			CloudSpawnInfo.SeqSpawner = inAction;
			CloudSpawnInfo.bSpawningActive  = TRUE;
		}
	}

	if( inAction.InputLinks[1].bHasImpulse )
	{
		CloudSpawnInfo.bSpawningActive = FALSE;
	}
	if( inAction.InputLinks[2].bHasImpulse ) // WARMUP
	{
 		if( inAction.WarmupPopulationPct > 0.0 )
 		{
			WarmupNum = FMin(inAction.WarmupPopulationPct, 1.f) * CloudSpawnInfo.SpawnNum;
			WarmupByIdx( -1, WarmupNum );
 		}
	}
	if( inAction.InputLinks[3].bHasImpulse || inAction.InputLinks[4].bHasImpulse ) // KILL
	{
...
function OnGameCrowdSpawner( SeqAct_GameCrowdSpawner inAction )
{
	local int Idx;
	local CrowdSpawnInfoItem Item;
	local int WarmupNum;

	Idx = ScriptedSpawnInfo.Find( 'SeqSpawner', inAction );
	if( inAction.InputLinks[0].bHasImpulse || inAction.InputLinks[2].bHasImpulse  ) // START
	{
		if( Idx < 0 )
		{
		...
		if( !ScriptedSpawnInfo[Idx].bSpawningActive )
		{
			inAction.FillCrowdSpawnInfoItem( Item, self );
			ScriptedSpawnInfo[Idx] = Item;
			ScriptedSpawnInfo[Idx].SeqSpawner      = inAction;
			ScriptedSpawnInfo[Idx].bSpawningActive = TRUE;
		}
	}
	...
	if( Idx >= 0 )
	{
		...
		if( inAction.InputLinks[1].bHasImpulse ) // STOP
		{
			ScriptedSpawnInfo[Idx].bSpawningActive = FALSE;
		}
		if( inAction.InputLinks[2].bHasImpulse ) // WARMUP
		{
			if( inAction.WarmupPopulationPct > 0.0 )
			{
				WarmupNum = FMin(inAction.WarmupPopulationPct, 1.f) * ScriptedSpawnInfo[Idx].SpawnNum;
				WarmupByIdx( Idx, WarmupNum );
			}
		}
		if( inAction.InputLinks[3].bHasImpulse || inAction.InputLinks[4].bHasImpulse ) // KILL
		{
...
function FlushAgents( CrowdSpawnInfoItem Item )
{
	local int AgentIdx;
...
native function WarmupByIdx( int SpawnerIdx, int WarmupNum );
