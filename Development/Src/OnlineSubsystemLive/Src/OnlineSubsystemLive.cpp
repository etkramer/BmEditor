...
UBOOL FLiveAsyncTaskReadContent::ProcessAsyncResults(UOnlineSubsystemLive* LiveSubsystem)
{
	...
	else
	{
		...
		// check for corrupted DLC. Note that the API seems to return 0x80070570, which 
		// doesn't have a #define for it, but 0x0570 (=1392) is the standard "file is corrupt"
		// windows error
		else if (Result == ERROR_FILE_CORRUPT || Result == ERROR_DISK_CORRUPT || Result == 0x80070570)
		{
 ...
 */
#define WORKAROUND_XDK_7978_CRASH 0

// The call to XOnlineGetNatType() can crash on some machines in some configurations due to uninitialised memory.
// Microsoft are aware of the issue, and we are working with them for a fix.

BYTE UOnlineSubsystemLive::GetNATType(void)
{
#if !WORKAROUND_XDK_7978_CRASH
...
DWORD UOnlineSubsystemLive::DestroyLiveGame(FNamedSession* Session)
{
	...
	{
		debugf(NAME_DevOnline,
			TEXT("XSessionDelete() skipped since nobody is logged in."));
	}
	if (Return == ERROR_SUCCESS || Return == ERROR_IO_PENDING)
	{
	...
	{
		// Manually clean up
		XCloseHandle(SessionInfo->Handle);
		// Don't leak the task
		delete AsyncTask;
	}
	return Return;
	...
	for (INT Index = 0; Index < SourceProps.Num(); Index++)
	{
		...
		switch (Prop.Data.Type)
		{
			...
			case SDT_DateTime:
			{
				...
				DestProps[Index].value.ftData.dwHighDateTime = (DWORD)(PTRINT)Prop.Data.Value2;
...
#if CONSOLE
				XContentClose(TCHAR_TO_ANSI(*LocalReadData->GetSaveFileInfo().LogicalPath), NULL);
#endif			
			}
		}
		else
			...
			if (PlayerStorage != NULL)
			{
				...
				if (UserLoginStatus > LS_NotLoggedIn && !IsGuestLogin(LocalUserNum))
				{
					...
					else
					{
						...
						delete AsyncTask;
						PlayerStorageCacheLocal[LocalUserNum].PlayerStorage = NULL;
						PlayerStorage->AsyncState = OPAS_Finished;
					}
				}
				else
...
#if CONSOLE
				XContentClose(TCHAR_TO_ANSI(*LocalWriteData->GetSaveFileInfo().LogicalPath), NULL);
#endif			
			}
		}
		else
