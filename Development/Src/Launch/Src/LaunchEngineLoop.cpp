...
#elif GAMENAME == BMGAME
	extern void AutoCheckNativeClassSizesBmGame(UBOOL & Mismatch);
	AutoCheckNativeClassSizesBmGame(Mismatch);
...
void CreateConsoleVariables()
{
	...
	GConsoleManager->RegisterConsoleVariable(TEXT("MotionBlurTimeScaleMin"),
		...
		TEXT("Value is in seconds (default 0.001)"),
		ECVF_Cheat);
	...
	GConsoleManager->RegisterConsoleVariable(TEXT("MotionBlurTimeScaleMax"),
		...
		TEXT("Value is in seconds (default 1.0)"),
		ECVF_Cheat);
	...
	GConsoleManager->RegisterConsoleVariable(TEXT("BloomSize"),
		-1.0f,
		TEXT("Allows to override the bloom size grain post process setting.\n")
		TEXT("<0: use post process settings (default: -1)\n")
		TEXT(">=0: override post process settings by the given value"),
		ECVF_Cheat);
...
/** Triggers a full texture streaming update next tick. */
void TriggerTextureStreaming()
{
	if ( GEngine )
	{
		GEngine->bTriggerTextureStreaming = TRUE;
	}
}
...
INT FEngineLoop::PreInit( const TCHAR* CmdLine )
{
	...
	GTriggerTextureStreaming = &TriggerTextureStreaming;
...
#if UDK && !CONSOLE && !PLATFORM_MACOSX
/**
