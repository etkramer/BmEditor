                                        ...
                                        const FName& Name, UObject* InOuter, EObjectFlags Flags, FFeedbackContext* Warn)
{
	...
	// Refresh the Generic Browser window so that our new package gets listed.
	GCallbackEvent->Send( CALLBACK_RefreshEditor_GenericBrowser );
/*
	// And now make sure that the package is selected, and displayed.

	WxGenericBrowser* GenericBrowser = GUnrealEd->GetBrowser<WxGenericBrowser>( TEXT("GenericBrowser") );
	if ( GenericBrowser )
	{
		GenericBrowser->SelectPackage(newPackage);
