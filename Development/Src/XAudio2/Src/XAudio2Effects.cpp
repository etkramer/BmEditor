...
/*-----------------------------------------------------------------------------
	FChebyshevWaveshaper
-----------------------------------------------------------------------------*/

class FChebyshevWaveshaper
{
public:

	inline static FLOAT Process( FLOAT Sample )
	{
		const FLOAT SampleCubed = Sample * Sample * Sample;
		return( ( 16.0f * appPow( Sample, 5.0f ) ) - ( 20.0f * SampleCubed ) + ( 5.0f * Sample ) );
	}
};
		...
		if( IsEnabled && GVolumeMultiplier != 0.0f )
		{
			...
			if( pInputProcessParameters[0].BufferFlags == XAPO_BUFFER_VALID )
			{
				...
				for( UINT SampleIndex = 0; SampleIndex < SampleSize; ++SampleIndex )
				{
					...
					// Waveshape it
					Sample = FChebyshevWaveshaper::Process( Sample );
