...
UBOOL FXAudio2SoundSource::ReadProceduralData( const INT BufferIndex )
{
	...
	appMemcpy( ( void* )XAudio2Buffers[BufferIndex].pAudioData, &PCMBuffer( 0 ), Min<INT>( Generated, MaxSamples ) * sizeof( SWORD ) );
...
UBOOL FXAudio2SoundSource::CreateSource( void )
{
	...
	{
		if( AudioDevice->WorkAroundXDKRegression )
		{
			Destinations[NumSends].Flags = XAUDIO2_SEND_USEFILTER;
		}
	...
	case 1:
		{
			...
			else if( WaveInstance->bCenterChannelOnly )
			{
				...
				FLOAT ClampedCenterChannelVolume = Clamp<FLOAT>( WaveInstance->VoiceCenterChannelVolume * AttenuatedVolume, 0.0f, 1.0f );
...
void FXAudio2SoundSource::Update( void )
{
	...
	if( AudioDevice->WorkAroundXDKRegression )
	{
		if( IsEQFilterApplied() )
		{
			AudioDevice->ValidateAPICall( TEXT( "SetOutputFilterParameters" ), 
				Source->SetOutputFilterParameters( Effects->EQPremasterVoice, &LPFParameters ) );
		}

		if( bReverbApplied )
		{
			AudioDevice->ValidateAPICall( TEXT( "SetOutputFilterParameters" ), 
				Source->SetOutputFilterParameters( Effects->ReverbEffectVoice, &LPFParameters ) );
		}

		if( WaveInstance->bApplyRadioFilter )
		{
			AudioDevice->ValidateAPICall( TEXT( "SetOutputFilterParameters" ), 
				Source->SetOutputFilterParameters( Effects->RadioEffectVoice, &LPFParameters ) );
		}
	}
	else
	{
...
void FXAudio2SoundSource::Stop( void )
{
	...
	{	
		...
		if( Source )
		{
			...
			{
				// This should also be XAUDIO2_PLAY_TAILS, but that causes crashes
				AudioDevice->ValidateAPICall( TEXT( "Stop" ), 
					Source->Stop( 0 ) );
			}
