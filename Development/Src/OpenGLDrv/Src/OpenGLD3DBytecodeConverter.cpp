...
static ANSICHAR *GenerateConstFloatArrayName(ConversionState& State, INT RegNum, DWORD &Adjusted)
{
...
static void Convert_rep( ConversionState* State )
{
	...
	*State->OutputBuffer += FString::Printf(TEXT("for( LoopTemp%d = 0; LoopTemp%d < %s; LoopTemp%d++ )\n{\n"),
		State->Shader->CurrentLoopDepth, State->Shader->CurrentLoopDepth, ANSI_TO_TCHAR(Source.SourceExpression), State->Shader->CurrentLoopDepth );
...
void D3DShaderData::GenerateGLSLDeclarations( FString &TargetBuffer )
{
	...
	if (bHasGlobalConsts)
	{
		TargetBuffer += FString::Printf(TEXT("bindable uniform vec4 %cConstGlobal[7];\n"), Prefix);
	}
