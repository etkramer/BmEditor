...
static GLuint ReadBufferSet = GL_BACK;
static GLuint DrawBufferSet = GL_BACK;
...
void CachedSetReadBuffer( GLuint Buffer )
{
	if( ReadBufferSet != Buffer )
	{
		glReadBuffer( Buffer );
		ReadBufferSet = Buffer;
	}
}

void CachedSetDrawBuffer( GLuint Buffer )
{
	if( DrawBufferSet != Buffer )
	{
		glDrawBuffer( Buffer );
		DrawBufferSet = Buffer;
	}
}
