...
// Vertex state.
void FOpenGLDynamicRHI::SetStreamSource(UINT StreamIndex,FVertexBufferRHIParamRef VertexBufferRHI,UINT Stride,UBOOL bUseInstanceIndex,UINT NumVerticesPerInstance,UINT NumInstances)
{
	...
	if (Stride == 0)
	{
		PendingStreams[StreamIndex].Frequency = 1;
	}
	else if (bUseInstanceIndex || NumInstances > 1)
	{
		PendingStreams[StreamIndex].Frequency = bUseInstanceIndex ? 1 : NumInstances;
	}
	else
	{
...
void FOpenGLDynamicRHI::SetSamplerState(FPixelShaderRHIParamRef PixelShaderRHI,UINT TextureIndex,UINT SamplerIndex,FSamplerStateRHIParamRef NewStateRHI,FTextureRHIParamRef NewTextureRHI,FLOAT MipBias,FLOAT LargestMip, FLOAT /*SmallestMip*/,UBOOL /*bForceLinearMinFilter*/)
{
	...
	UBOOL NeedsToChangeSamplerState = NewTexture->NeedsToChangeSamplerState(NewState, MinFilter, ResultMip);

	INT NewMipBias = NewState->MipMapLODBias ? NewState->MipMapLODBias : MipBias;
	UBOOL NeedsToChangeLODBias = (CachedState.Textures[SamplerIndex].MipMapLODBias != NewMipBias);

	UBOOL NeedsToChangeTexture = ((CachedState.Textures[SamplerIndex].Target != NewTexture->Target) || (CachedState.Textures[SamplerIndex].Resource != NewTexture->Resource));

	if( NeedsToChangeSamplerState || NeedsToChangeLODBias || NeedsToChangeTexture)
	{
		CachedSetActiveTexture(GL_TEXTURE0 + SamplerIndex);

		if (NeedsToChangeSamplerState || NeedsToChangeTexture)
		{
...
void FOpenGLDynamicRHI::SetRenderTarget( FSurfaceRHIParamRef NewRenderTargetRHI, FSurfaceRHIParamRef NewDepthStencilTargetRHI)
{
	...
	InternalSetViewport( 0, 0, CachedState.MinZ, CachedState.RenderTargetWidth, CachedState.RenderTargetHeight, CachedState.MinZ, TRUE );
}
...
void FOpenGLDynamicRHI::SetupVertexArrays( UINT BaseVertexIndex )
{
	...
	for (INT ElementIndex = 0; ElementIndex < VertexDeclaration->VertexElements.Num(); ElementIndex++)
	{
		...
		// @todo opengl - enable glVertexAttribDivisorARB on Windows when NVidia fixes the driver
#if _WINDOWS
		if (Frequency == 0)
		{
#endif
			EnableVertexElement( VertexElement, Stride, (void *)(BaseVertexIndex * Stride + VertexElement.Offset), Stream->VertexBuffer->Resource );
		...
		else if( CachedVertexAttrs[VertexElement.Usage].Pointer != FOpenGLCachedAttr_SingleVertex )
		{
			glVertexAttrib4f(VertexElement.Usage, 1.0f, 1.0f, 1.0f, 1.0f);
			CachedVertexAttrs[VertexElement.Usage].Pointer = FOpenGLCachedAttr_SingleVertex;
		}
#endif
	}
...
void FOpenGLDynamicRHI::CommitVertexAndPixelShaderConstants()
{
...
//	glDrawElementsInstanced(DrawMode, NumElements, IndexType, (void *)StartIndex, PendingNumInstances);
	CheckOpenGLErrors();
}
 ...
 */
void FOpenGLDynamicRHI::DrawSpriteParticles( const FMeshElement& Mesh)
{
	...
	if (OutVertexData && OutIndexData)
	{
		...
		SpriteData->GetVertexAndIndexData(Vertices, OutIndexData, (FParticleOrder*)(Mesh.DynamicIndexData));
 ...
 */
void FOpenGLDynamicRHI::DrawSubUVParticles( const FMeshElement& Mesh)
{
	checkSlow(Mesh.DynamicVertexData);
	...
	if (OutVertexData && OutIndexData)
	{
		...
		SubUVData->GetVertexAndIndexData(Vertices, OutIndexData, (FParticleOrder*)(Mesh.DynamicIndexData));
		// End the draw, which will submit the data for rendering
		RHIEndDrawIndexedPrimitiveUP();
	}
}
 ...
 * @param Mesh The mesh element containing the data for rendering the sprite subuv particles
 */
void FOpenGLDynamicRHI::DrawPointSpriteParticles(const FMeshElement& Mesh) 
{
	// Not implemented yet!
}
...
// not used on PC
void FOpenGLDynamicRHI::SetViewPixelParameters(const FSceneView* View,FPixelShaderRHIParamRef PixelShader,const class FShaderParameter* SceneDepthCalcParameter,const class FShaderParameter* ScreenPositionScaleBiasParameter){}
...
void FOpenGLDynamicRHI::MSAAInitPrepass(){}
void FOpenGLDynamicRHI::MSAAFixViewport(){}
void FOpenGLDynamicRHI::MSAABeginRendering(UBOOL bRequiresClear){}
void FOpenGLDynamicRHI::MSAAEndRendering(FTexture2DRHIParamRef DepthTexture,FTexture2DRHIParamRef ColorTexture,UINT MSAAEndRendering){}
void FOpenGLDynamicRHI::RestoreColorDepth(FTexture2DRHIParamRef ColorTexture, FTexture2DRHIParamRef DepthTexture){}
void FOpenGLDynamicRHI::GetMSAAOffsets(UINT *HalfScreenY, UINT *ResolveOffset){	check(0);}
...
INT FOpenGLDynamicRHI::GetMobileUniformSlotIndexByName(FName ParamName){return -1;}
...
{ 
	...
	return CreateBoundShaderState(VertexDeclaration, StreamStrides, VertexShader, PixelShader);
}
#endif
