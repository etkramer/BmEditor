    ...
    abstract class Utils
    {
        ...
        /*public static void SetEnvironmentVariablesFromBatchFile(string BatchFileName)
		{
			if( File.Exists( BatchFileName ) )
			{
				...
				// Regex that matches @SET VARIABLE=VALUE
				Regex EnvironmentRegex = new Regex(@"\s*@?set\s+(?<Variable>[^\=]*)=(?<Value>.+)", RegexOptions.IgnoreCase);

				// Read lines from the batch file.
				string[] FileContents = File.ReadAllLines(BatchFileName);

				// Iterate over all lines, looking for matches and setting the environment variable after resolving it.
				foreach (string Line in FileContents)
				{
					Match M = EnvironmentRegex.Match(Line);
					if (M.Success)
					{
						String EnvVar = M.Groups["Variable"].Captures[0].Value;
						String Value = Utils.ResolveEnvironmentVariable(M.Groups["Value"].Captures[0].Value);
        ...
        public static void SetEnvironmentVariablesFromBatchFile(string BatchFileName)
        {
            if (File.Exists(BatchFileName))
            {
                ...
                var EnvReaderBatchFileName = Path.GetTempFileName() + ".bat";
                if (BuildConfiguration.bPrintDebugInfo)
                {
