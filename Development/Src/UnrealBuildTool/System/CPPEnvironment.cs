	...
	partial class CPPEnvironment
	{
		...
		public CPPOutput CompileFiles(List<FileItem> CPPFiles)
		{
			...
			else if( TargetPlatform == CPPTargetPlatform.PS3_PPU )
			{
				...
				{
					return SNCToolChain.CompileCPPFiles( this, CPPFiles );
				}
				else
				{
			...
			{
				return SPUToolChain.CompileCPPFiles(this, CPPFiles);
			}
            ...
            {
                return NGPToolChain.CompileCPPFiles(this, CPPFiles);
            }
            else
			{
		...
		public bool ShouldUsePCHs()
		{
			...
			if( TargetPlatform == CPPTargetPlatform.Android || TargetPlatform == CPPTargetPlatform.PS3_SPU )
			{
				bUsePCHFiles = false;
			}
			// PS3 PCH support only works for gcc w/o XGE
			if( TargetPlatform == CPPTargetPlatform.PS3_PPU )
			{
				// PCHs are not set up with SNC and have some issues with XGE.
				if( BuildConfiguration.bUseSNCCompiler || BuildConfiguration.bAllowXGE )
				{
					bUsePCHFiles = false;
				}
			}
