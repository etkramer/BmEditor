		...
		static int Main(string[] Arguments)
		{
			...
			using (Mutex SingleInstanceMutex = new Mutex(true, "Global\\UnrealBuildTool_Mutex", out bCreatedMutex))
			{
				...
				try
                {
					...
					if (NumExecutedActions > 0)
					{
						foreach( var SyncTarget in Targets )
						{
							// If the platform requires it, perform any pre-build syncing to remote machines, etc.
							if (SyncTarget.GetPlatform() == UnrealTargetPlatform.IPhoneDevice || SyncTarget.GetPlatform() == UnrealTargetPlatform.IPhoneSimulator)
							{
			...
			{
				Console.WriteLine( "UBT execution time: {0:0.00} seconds, {1:0.00} seconds linking", BuildAndSqlDuration, TotalLinkTime );
			}
			else if( ExecutorName == "XGE" )
			{
				Console.WriteLine( "XGE execution time: {0:0.00} seconds", BuildAndSqlDuration );
			}
