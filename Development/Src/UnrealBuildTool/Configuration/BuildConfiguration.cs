...
namespace UnrealBuildTool
{
	class BuildConfiguration
	{
		...
		/** Whether to use the SNC compiler for the PS3 build */
		public static bool bUseSNCCompiler = Utils.GetEnvironmentVariable("ue3.bUseSNCCompiler", true);

		/** Whether to generate a dSYM file or not. */
		public static bool bGeneratedSYMFile = Utils.GetEnvironmentVariable("ue3.bGeneratedSYMFile", false);
		...
		public static void ValidateConfiguration( CPPTargetConfiguration Configuration, CPPTargetPlatform Platform )
		{
			...
			{
				// verify that SNC is installed
				if (bUseSNCCompiler)
				{
					string CompilerPath = Path.Combine(UE3BuildTarget.GetSCEPS3Root(), "host-win32/sn/bin/ps3ppusnc.exe");
					if (!File.Exists(CompilerPath))
					{
						bUseSNCCompiler = false;
					}
				}
			}
			// iPhone
			if( Platform == CPPTargetPlatform.IPhoneSimulator || Platform == CPPTargetPlatform.IPhoneDevice )
			{
				...
				ProcessorCountMultiplier = IPhoneToolChain.GetAdjustedProcessorCountMultiplier( CPPTargetPlatform.IPhoneDevice );
			}
		}
